...
is
   ...
   function Valid (Value : State; Capacity : Positive) return Boolean is
     (Value.Head <= Capacity and then Value.Count <= Capacity);

   --  Report whether a valid state has no unused slot.
   --  @param Value Valid state to inspect
   --  @param Capacity Ring capacity governing Value
   --  @return True when Count equals Capacity
   function Is_Full (Value : State; Capacity : Positive) return Boolean is
   ...
   function Can_Append
     (Value               : State;
      Capacity            : Positive;
      Overwrite_When_Full : Boolean) return Boolean
   ...
   function Insertion_Index
     ...
     with Pre  => Valid (Value, Capacity),
          Post => Insertion_Index'Result <= Capacity;
   ...
   procedure Append
     ...
     with Pre  => Valid (Value, Capacity),
          ...
          Contract_Cases =>
            (Value.Count < Capacity =>
               (not Overwrote
                and then Value.Head = Value.Head'Old
                and then Value.Count = Value.Count'Old + 1),
             Value.Count = Capacity =>
               (Overwrote
                and then Value.Count = Value.Count'Old
                and then
                  Value.Head =
                    (if Value.Head'Old = Capacity
                     then 1
                     else Value.Head'Old + 1)));
   ...
   function Saturating_Increment
     ...
     with Contract_Cases =>
       (Value = Interfaces.Unsigned_64'Last =>
          Saturating_Increment'Result = Value,
        Value < Interfaces.Unsigned_64'Last =>
          Saturating_Increment'Result = Value + 1);
