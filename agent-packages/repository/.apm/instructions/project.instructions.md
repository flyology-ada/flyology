---
description: Preserve Flyology's project-specific architecture, terminology, and verification rules.
---
...
- Current scoped providers cover descriptor readiness, timers, raw stream and
  ...
  intrusive and address-bucketed per generic instance; the public channel
  wrapper exposes no separate arm hook. Its private protected state claims a
  wake in the transition cut, and a caller-owned guard writes after unlocking.
  Unlink drains any in-flight borrowed descriptor claim before set reclamation.
  Mutating wrapper calls require no enclosing protected action. A pending send
  owns its copied element, while a successful receive retains its element until
  typed `Finish`.
