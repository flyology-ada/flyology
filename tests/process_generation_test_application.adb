...
package body Process_Generation_Test_Application is
   procedure Run (Version : Ada.Streams.Stream_Element) is
      ...
      begin
         Connection.Receive_Exactly (Request, Timeout => 30.0, Token => Cancellation);
         Connection.Send_All ([1 => Context.Image_Version], Timeout => 5.0, Token => Cancellation);
      ...
      private
         ...
         Last    : Generations.Upgrade_Handle := (Coordinator => 1, Upgrade => 1, Candidate => 1);
         Value   : Generations.Compensation_Result := Generations.Not_Required;
      ...
      type Application_Context is limited record
         Handler   : aliased Handler_Context := (Image_Version => Version);
      ...
      procedure Prepare (Context : in out Application_Context; Data : Messages.Provisioning_Data) is
      begin
         if Data.Application_Signature /= 16#F1# or else Data.Topology_Schema /= 1 then
      ...
      begin
         ...
         if Context.Provision.Digest (0) = 16#FB# then
            ...
            begin
               Structured.Serve (Context.Server, Listener, Context.Handler, Drain_Timeout => 2.0);
            end;
         else
            Structured.Serve (Context.Server, Listener, Context.Handler, Drain_Timeout => 2.0);
         end if;
      ...
      function Ready (Context : Application_Context; Data : Messages.Provisioning_Data) return Boolean is
         Current : constant Structured.Snapshot := Structured.Current (Context.Server);
      begin
         return
           Data.Topology_Epoch = Context.Provision.Topology_Epoch
      ...
      procedure Promoted (Context : in out Application_Context; Authority : Generations.Upgrade_Handle) is
      begin
         pragma Unreferenced (Context, Authority);
      end Promoted;

      function Compensate
        (Context : in out Application_Context; Authority : Generations.Upgrade_Handle)
         return Generations.Compensation_Result
      is
         Desired : constant Generations.Compensation_Result :=
           (if Context.Provision.Digest (0) = 16#FE#
            then Generations.Compensation_Failed
