...
static _Atomic int run_claim_barrier_armed;
static _Atomic int run_claim_barrier_reached;
static _Atomic int run_claim_barrier_released;
...
void flyology_test_worker_pool_reset(void)
{
   ...
   atomic_store_explicit(&run_claim_barrier_armed, 0, memory_order_seq_cst);
   atomic_store_explicit(&run_claim_barrier_reached, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&run_claim_barrier_released, 1,
                         memory_order_seq_cst);
   ...
   atomic_store_explicit(&native_executor_idle_barrier_armed, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_idle_barrier_reached, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_idle_barrier_released, 1,
                         memory_order_seq_cst);
...
void flyology_test_worker_run_claim_barrier_arm(void)
{
   atomic_store_explicit(&run_claim_barrier_reached, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&run_claim_barrier_released, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&run_claim_barrier_armed, 1,
                         memory_order_seq_cst);
}

/*  Only the first claimer that arrives is held, so an unrelated Run in the
 *  same program is never delayed by an armed barrier.  */
int flyology_test_worker_run_claim_barrier_arrive(void)
{
   if (atomic_exchange_explicit(&run_claim_barrier_armed, 0,
                                memory_order_seq_cst) == 0)
      return 0;
   atomic_store_explicit(&run_claim_barrier_reached, 1,
                         memory_order_seq_cst);
   return 1;
}

int flyology_test_worker_run_claim_barrier_reached(void)
{
   return atomic_load_explicit(&run_claim_barrier_reached,
                               memory_order_seq_cst);
}

int flyology_test_worker_run_claim_barrier_released(void)
{
   return atomic_load_explicit(&run_claim_barrier_released,
                               memory_order_seq_cst);
}

void flyology_test_worker_run_claim_barrier_release(void)
{
   atomic_store_explicit(&run_claim_barrier_released, 1,
                         memory_order_seq_cst);
   atomic_store_explicit(&run_claim_barrier_armed, 0,
                         memory_order_seq_cst);
}
...
void flyology_test_worker_native_executor_dispatch_barrier_arm(void)
{
   atomic_store_explicit(&native_executor_dispatch_barrier_reached, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_dispatch_barrier_released, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_dispatch_barrier_armed, 1,
                         memory_order_seq_cst);
}

int flyology_test_worker_native_executor_dispatch_barrier_arrive(void)
{
   if (atomic_exchange_explicit(&native_executor_dispatch_barrier_armed, 0,
                                memory_order_seq_cst) == 0)
      return 0;
   atomic_store_explicit(&native_executor_dispatch_barrier_reached, 1,
                         memory_order_seq_cst);
   return 1;
}

int flyology_test_worker_native_executor_dispatch_barrier_reached(void)
{
   return atomic_load_explicit(&native_executor_dispatch_barrier_reached,
                               memory_order_seq_cst);
}

int flyology_test_worker_native_executor_dispatch_barrier_released(void)
{
   return atomic_load_explicit(&native_executor_dispatch_barrier_released,
                               memory_order_seq_cst);
}

void flyology_test_worker_native_executor_dispatch_barrier_release(void)
{
   atomic_store_explicit(&native_executor_dispatch_barrier_released, 1,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_dispatch_barrier_armed, 0,
...
void flyology_test_worker_native_executor_idle_barrier_arm(void)
{
   atomic_store_explicit(&native_executor_idle_barrier_reached, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_idle_barrier_released, 0,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_idle_barrier_armed, 1,
                         memory_order_seq_cst);
}

int flyology_test_worker_native_executor_idle_barrier_arrive(void)
{
   if (atomic_exchange_explicit(&native_executor_idle_barrier_armed, 0,
                                memory_order_seq_cst) == 0)
      return 0;
   atomic_store_explicit(&native_executor_idle_barrier_reached, 1,
                         memory_order_seq_cst);
   return 1;
}

int flyology_test_worker_native_executor_idle_barrier_reached(void)
{
   return atomic_load_explicit(&native_executor_idle_barrier_reached,
                               memory_order_seq_cst);
}

int flyology_test_worker_native_executor_idle_barrier_released(void)
{
   return atomic_load_explicit(&native_executor_idle_barrier_released,
                               memory_order_seq_cst);
}

void flyology_test_worker_native_executor_idle_barrier_release(void)
{
   atomic_store_explicit(&native_executor_idle_barrier_released, 1,
                         memory_order_seq_cst);
   atomic_store_explicit(&native_executor_idle_barrier_armed, 0,
