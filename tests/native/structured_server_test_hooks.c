...
void flyology_test_structured_server_barrier_reset(void)
{
...
int flyology_test_structured_server_barrier_arrive(int point)
{
...
int flyology_test_structured_server_barrier_reached(int point)
{
   return valid_point(point)
     ? atomic_load_explicit(&reached[point], memory_order_seq_cst) : 0;
}

int flyology_test_structured_server_barrier_released(int point)
{
   return valid_point(point)
     ? atomic_load_explicit(&released[point], memory_order_seq_cst) : 1;
}

void flyology_test_structured_server_barrier_release(int point)
{
   if (!valid_point(point)) return;
   atomic_store_explicit(&released[point], 1, memory_order_seq_cst);
   atomic_store_explicit(&armed[point], 0, memory_order_seq_cst);
