...
procedure Process_Generation_Coordinator_Smoke is
   ...
   V1_Path : constant String := Ada.Directories.Compose
     (Bin_Directory, "process_generation_agent_v1");
   V2_Path : constant String := Ada.Directories.Compose
     (Bin_Directory, "process_generation_agent_v2");

   function Provision
     (Epoch : Messages.Nonzero_U64;
      Mode  : Ada.Streams.Stream_Element := 0)
      return Messages.Provisioning_Data
   is
      Digest : Messages.Topology_Digest := (others => 0);
   begin
      Digest (0) := Protocol.Octet (Mode);
      Digest (1) := Protocol.Octet
        (Interfaces.Unsigned_64 (Epoch) mod 256);
      return
   ...
   function Command (Path : String) return Subprocesses.Command is
     (Subprocesses.To_Command (Path));

   function Query
     (Address : Sockets.Endpoint) return Ada.Streams.Stream_Element
   is
...
begin
   ...
   Coordinators.Start_Initial
     (Manager, Command (V1_Path), Provision (1), First, Timeout => 5.0);
   Assert
     (Coordinators.Snapshot (Manager).Phase = Generations.Completed,
      "initial image did not commit");
      ...
      begin
         begin
            Coordinators.Start_Upgrade
              (Manager, Command (V2_Path), Provision (2, 16#FC#), Ignored,
   ...
   begin
      ...
      Assert (Cancelled and then not Unexpected,
              "in-flight provisioning ignored cancellation");
      Assert
        (Coordinators.Snapshot (Manager).Phase = Generations.Cancelled and then
         not Coordinators.Snapshot (Manager).Has_Candidate,
         "cancelled provisioning retained a candidate");
      Assert (Query (Address) = Character'Pos ('1'),
              "startup cancellation disturbed the active image");
   end;

   Coordinators.Start_Upgrade
     (Manager, Command (V2_Path), Provision (2, 16#F9#), Candidate,
      Timeout => 5.0);
   declare
      Stop : aliased Flyology.Cancellation.Token;
      Cancelled : Boolean := False;
      ...
      begin
         begin
            Coordinators.Begin_Canary
              (Manager, Candidate, Timeout => 5.0, Token => Stop'Access);
            Unexpected := True;
         exception
            when Flyology.Cancellation.Operation_Cancelled =>
               Cancelled := True;
   ...
   begin
      ...
      Assert (Cancelled and then not Unexpected,
              "in-flight canary readiness ignored cancellation");
      declare
         Sample : constant Coordinators.Coordinator_Snapshot :=
           Coordinators.Snapshot (Manager);
      begin
         Assert
           (Sample.Phase = Generations.Cancelled and then
            not Sample.Has_Candidate and then
            not Sample.Candidate_Admitted and then
            Sample.Compensation = Generations.Compensated,
            "readiness cancellation did not compensate the canary");
      end;
      Assert (Query (Address) = Character'Pos ('1'),
              "readiness cancellation disturbed the active image");
   end;
   ...
   begin
      ...
      begin
         Coordinators.Start_Upgrade
           (Manager, Command (V2_Path), Bad, Candidate, Timeout => 5.0);
      exception
         when Coordinators.Upgrade_Error => Failed := True;
      end;
      Assert (Failed, "incompatible candidate was accepted");
      Assert (Query (Address) = Character'Pos ('1'),
              "failed startup disturbed the active image");
   end;

   Coordinators.Start_Upgrade
     (Manager, Command (V2_Path), Provision (3), Candidate, Timeout => 5.0);
   declare
      Stale : Generations.Upgrade_Handle := Candidate;
      Rejected : Boolean := False;
      Before : constant Coordinators.Coordinator_Snapshot :=
        Coordinators.Snapshot (Manager);
   begin
      ...
      exception
         when Coordinators.Stale_Authority => Rejected := True;
      end;
      Assert (Rejected, "stale canary authority was accepted");
      Assert
        (Coordinators.Snapshot (Manager).Phase = Before.Phase and then
         Coordinators.Snapshot (Manager).Has_Candidate,
         "stale authority mutated coordinator state");
   end;
   Coordinators.Begin_Canary (Manager, Candidate, Timeout => 5.0);
   Coordinators.Cancel
     (Manager, Candidate, Compensation, Timeout => 5.0);
   Assert (Compensation = Generations.Compensated,
           "canary compensation was not retained");
   Assert (Query (Address) = Character'Pos ('1'),
           "canary cancellation disturbed v1");

   Coordinators.Start_Upgrade
     (Manager, Command (V2_Path), Provision (4, 16#FE#), Candidate,
      Timeout => 5.0);
   Coordinators.Begin_Canary (Manager, Candidate, Timeout => 5.0);
   Coordinators.Cancel
     (Manager, Candidate, Compensation, Timeout => 5.0);
   Assert (Compensation = Generations.Compensation_Failed,
           "compensation failure was not retained");
   Assert (Query (Address) = Character'Pos ('1'),
           "failed compensation disturbed v1 admission");

   Coordinators.Start_Upgrade
     (Manager, Command (V2_Path), Provision (5), Candidate, Timeout => 5.0);
   Coordinators.Begin_Canary (Manager, Candidate, Timeout => 5.0);
   Sockets.Send_All (Held, [1 => Character'Pos ('R')], Timeout => 2.0);
   Sockets.Receive_Exactly (Held, Held_Reply, Timeout => 2.0);
   Assert (Held_Reply (1) = Character'Pos ('1'),
           "v1 connection did not survive candidate overlap");
   Coordinators.Promote (Manager, Candidate, Timeout => 5.0);
   Assert (Query (Address) = Character'Pos ('2'), "v2 did not become active");

   Coordinators.Rollback_To_Previous
     (Manager, Reversal, Timeout => 5.0);
   Assert
     (Reversal.Candidate /= First.Candidate,
      "rollback reused the original image generation");
   Assert
     (Query (Address) = Character'Pos ('1'),
      "rollback did not construct a fresh v1 image");

   Coordinators.Start_Upgrade
     (Manager, Command (V2_Path), Provision (6), Candidate, Timeout => 5.0);
   Coordinators.Begin_Canary (Manager, Candidate, Timeout => 5.0);
   Coordinators.Promote (Manager, Candidate, Timeout => 5.0);
   Assert (Query (Address) = Character'Pos ('2'), "v2 did not recommit");

   Coordinators.Start_Upgrade
     (Manager, Command (V1_Path), Provision (7, 16#FB#), Candidate,
      Timeout => 5.0);
   Coordinators.Begin_Canary (Manager, Candidate, Timeout => 5.0);
   declare
      ...
      loop
         ...
         if Ada.Real_Time.To_Duration (Ada.Real_Time.Clock - Began) >= 3.0 then
            raise Program_Error with
              "post-readiness server exit was not reconciled";
         end if;
         delay 0.010;
      end loop;
      Assert
        (not Sample.Has_Candidate and then
         not Sample.Candidate_Ready and then
         not Sample.Candidate_Admitted and then
         Sample.Compensation = Generations.Compensation_Pending,
         "server exit retained candidate admission state");
   end;
   Assert (Query (Address) = Character'Pos ('2'),
           "candidate server exit disturbed the active image");

   Coordinators.Start_Upgrade
     (Manager, Command (V1_Path), Provision (8, 16#FF#), Candidate,
      Timeout => 5.0);
   declare
      Failed : Boolean := False;
   begin
