...
procedure Managed_Connection_Connect_Smoke is
   ...
   type Test_Result is
     (Pending, Succeeded, Timed_Out, Cancelled, Admission_Stopped, Transfer_Rejected, Failed);
   ...
   procedure Await_Result (Box : in out Result_Box; Expected : Test_Result; Label : String) is
      Actual : Test_Result;
   begin
      select
         Box.Wait (Actual);
   ...
   procedure Run_Composition (Model : Flyology.Execution_Model; Secure : Boolean; Scoped : Boolean) is
   ...
   begin
      ...
      declare
         ...
         begin
            begin
               Sockets.Accept_Connection (Listener, Peer, Remote, Timeout => 1.0);
               if Secure then
                  Sockets.Receive (Peer, Data, Last, Timeout => 1.0);
                  if Last >= Data'First then
                     raise Program_Error with "TLS composition peer did not observe scope close";
                  end if;
               else
                  Sockets.Receive_Exactly (Peer, Data, Timeout => 1.0);
                  if Data /= [1 => 41] then
                     raise Program_Error with "managed connection driver received wrong request";
                  end if;
                  Sockets.Send_All (Peer, [1 => 42], Timeout => 1.0);
                  Sockets.Receive (Peer, Data, Last, Timeout => 1.0);
                  if Last >= Data'First then
                     raise Program_Error with "managed connection peer did not observe scope close";
                  end if;
               end if;
               Close_If_Open (Peer);
         ...
         begin
            begin
               declare
                  ...
                  procedure Wait_For (IO : in out Drivers.Capability; Required : Drivers.Step_Result) is
                  ...
                  begin
                     ...
                     if Ready /= Drivers.Transport_Ready then
                        raise Program_Error with "managed connection driver woke for wrong source";
                     end if;
                  end Wait_For;
                  ...
                  begin
                     if Secure then
                        if not Drivers.Is_Acquired (IO) then
                           raise Program_Error with "TLS connection capability did not acquire";
                        end if;
                        return;
                     end if;
                     ...
                     loop
                        ...
                        if Step not in Drivers.Need_Read | Drivers.Need_Write then
                           raise Program_Error with "managed connection driver send failed";
                        end if;
                        Wait_For (IO, Step);
                     end loop;
                     if Last /= 1 then
                        raise Program_Error with "managed connection driver sent wrong length";
                     end if;
                     ...
                     loop
                        ...
                        exit when Step = Drivers.Made_Progress;
                        if Step not in Drivers.Need_Read | Drivers.Need_Write then
                           raise Program_Error with "managed connection driver receive failed";
                        end if;
                        Wait_For (IO, Step);
                     end loop;
                     if Last /= Data'Last or else Data /= [1 => 42] then
                        raise Program_Error with "managed connection driver received wrong reply";
                     end if;
                  end Pump;
               begin
                  if Scoped then
                     declare
                        ...
                        Attempt : Connections.Connect_Operation :=
                          Connections.Connect (Set'Access, Manager'Access, Address, Timeout => 1.0);
                     begin
                        Operations.Wait_All (Set);
                        Connections.Finish (Attempt, Item);
                     end;
                  else
                     Connections.Connect (Manager, Address, Item, Timeout => 1.0);
                  end if;
                  if not Connections.Is_Open (Item) or else Manager.Active /= 1 then
                     raise Program_Error with "managed connect did not publish sole ownership";
                  end if;
                  if Secure then
                     Connection_TLS.Upgrade (Item, Backend, TLS.Client, "localhost", Timeout => 1.0);
                  end if;
                  Drivers.Run (Item, Pump'Access, Timeout => 1.0);
               end;
               if Manager.Active /= 0 then
                  raise Program_Error with "connection scope exit did not release admission";
               end if;
      ...
      begin
         Await_Result
           (Client_Result,
            Succeeded,
            "managed composition client/" & Model'Image & "/scoped=" & Scoped'Image);
         Await_Result
           (Server_Result,
            Succeeded,
            "managed composition server/" & Model'Image & "/scoped=" & Scoped'Image);
      end;
      ...
      if Secure then
         ...
         if State.Sessions_Created /= 1 or else State.Sessions_Finalized /= 1 or else State.Sessions_Live /= 0
         then
            raise Program_Error with "managed TLS composition leaked provider state";
         end if;
      end if;
   ...
   type Admission_Trigger is (Deadline, Token_Request, Manager_Shutdown);

   procedure Run_Admission_Wait
     (Model : Flyology.Execution_Model; Trigger : Admission_Trigger; Scoped : Boolean)
   is
      Manager                : aliased Connections.Server (Capacity => 1);
   ...
   begin
      ...
      begin
         ...
         Await_Result
           (Result,
            Expected,
            "managed admission " & Trigger'Image & "/" & Model'Image & "/scoped=" & Scoped'Image);
      exception
         when others =>
            Token.Request;
            Testing.Release (Testing.Admission_Wait_Armed);
   ...
   procedure Run_Connected_Interruption
     (Model : Flyology.Execution_Model; Trigger : Connected_Trigger; Scoped : Boolean)
   is
      Manager       : aliased Connections.Server (Capacity => 1);
      Token         : aliased Connections.Cancellation_Token;
      Listener      : Sockets.Socket_Type;
   ...
   begin
      ...
      declare
         ...
         begin
            begin
               ...
               Sockets.Receive (Peer, Data, Last, Timeout => 1.0);
               Close_If_Open (Peer);
               Server_Result.Set (if Last < Data'First then Succeeded else Failed);
            exception
               when others =>
                  Close_If_Open (Peer);
                  Server_Result.Set (Failed);
            ...
            begin
               if Scoped then
                  declare
                     ...
                     Attempt : Connections.Connect_Operation :=
                       Connections.Connect
                         (Set'Access, Manager'Access, Address, Timeout => 1.0, Token => Token'Access);
                  begin
                     Operations.Wait_All (Set);
                     Connections.Finish (Attempt, Item);
                  end;
               else
                  Connections.Connect (Manager, Address, Item, Timeout => 1.0, Token => Token'Access);
               end if;
               Client_Result.Set (Succeeded);
            exception
               when Connections.Operation_Cancelled =>
      ...
      begin
         ...
         if Manager.Active /= 1 then
            raise Program_Error with "connected interruption did not retain admission";
         end if;
         case Trigger is
            when Token_Request    =>
               Token.Request;
         ...
         Await_Result
           (Client_Result,
            Cancelled,
            "managed connected interruption/"
            & Trigger'Image
            & "/"
            & Model'Image
            & "/scoped="
         ...
         Await_Result
           (Server_Result,
            Succeeded,
            "managed connected cleanup/" & Trigger'Image & "/" & Model'Image & "/scoped=" & Scoped'Image);
      exception
         when others =>
            Token.Request;
            Testing.Release (Testing.Managed_Connect_Connected);
      ...
      if Manager.Active /= 0 then
         raise Program_Error with "connected interruption leaked admission";
      end if;
      Close_If_Open (Listener);
      Testing.Reset_Barriers;
   exception
   ...
   procedure Run_Transfer_Failure (Model : Flyology.Execution_Model; Scoped : Boolean) is
   ...
   begin
                     ...
                     exception
                        when Program_Error =>
                           if not Operations.Is_Terminal (Attempt) then
                              raise Program_Error with "invalid Finish consumed managed ownership";
                           end if;
                           raise;
                     end;
                  end;
      ...
      begin
         ...
         if Manager.Active /= 2 or else not Connections.Is_Open (Item) then
            raise Program_Error with "transfer failure setup did not own both permits";
         end if;
         Testing.Release (Testing.Managed_Connect_Connected);
         Await_Result
           (Client_Result,
            Transfer_Rejected,
            "managed transfer failure/" & Model'Image & "/scoped=" & Scoped'Image);
         Await_Result
           (Server_Result, Succeeded, "managed transfer cleanup/" & Model'Image & "/scoped=" & Scoped'Image);
      exception
         when others =>
            Testing.Release (Testing.Managed_Connect_Connected);
            raise;
      ...
      if Manager.Active /= 1 or else not Connections.Is_Open (Item) then
         raise Program_Error with "failed transfer disturbed the existing connection";
      end if;
      ...
      Testing.Reset_Barriers;
      if Manager.Active /= 0 then
         raise Program_Error with "managed transfer failure leaked ownership";
      end if;
   exception
      when others =>
         Testing.Release (Testing.Managed_Connect_Connected);
   ...
   procedure Run_Scoped_Abandonment (Model : Flyology.Execution_Model) is
      Manager       : aliased Connections.Server (Capacity => 1);
      Listener      : Sockets.Socket_Type;
      Address       : Sockets.Endpoint;
      Client_Result : Result_Box;
   ...
   begin
      ...
      declare
         ...
         begin
            begin
               ...
               begin
                  Operations.Wait_All (Set);
                  if Operations.Outcome (Attempt) /= Operations.Succeeded or else Manager.Active /= 1 then
                     raise Program_Error with "successful managed operation retained wrong state";
                  end if;
               --  Deliberately omit Finish. Controlled finalization must
               --  close the socket and release the permit.
               end;
               Client_Result.Set (if Manager.Active = 0 then Succeeded else Failed);
            exception
               when others =>
                  Client_Result.Set (Failed);
            end;
      ...
      begin
         Await_Result (Client_Result, Succeeded, "scoped managed abandonment/" & Model'Image);
         Await_Result (Server_Result, Succeeded, "scoped managed abandonment peer/" & Model'Image);
      end;
      Close_If_Open (Listener);
      if Manager.Active /= 0 then
         raise Program_Error with "abandoned managed operation leaked admission";
      end if;
   exception
      when others =>
         Close_If_Open (Listener);
   ...
   procedure Run_Scoped_Child_Capacity (Model : Flyology.Execution_Model) is
      Manager : aliased Connections.Server (Capacity => 1);
      Address : constant Sockets.Endpoint := Sockets.Network_Endpoint (Sockets.Loopback_IPv4, 1);
      Result  : Result_Box;
   begin
      declare
         task Worker is
         ...
         begin
            begin
               ...
               Result.Set (if Failed_As_Bounded and then Manager.Active = 0 then Succeeded else Failed);
            exception
               when others =>
                  Result.Set (Failed);
            end;
      ...
      begin
         Await_Result (Result, Succeeded, "scoped managed child capacity/" & Model'Image);
      end;
   end Run_Scoped_Child_Capacity;

   procedure Run_Scoped_Selective_Wait (Model : Flyology.Execution_Model) is
      Manager       : aliased Connections.Server (Capacity => 1);
      Listener      : Sockets.Socket_Type;
      Address       : Sockets.Endpoint;
      Client_Result : Result_Box;
   ...
   begin
            ...
            begin
               declare
                  ...
                  Attempt : aliased Connections.Connect_Operation :=
                    Connections.Connect (Set'Access, Manager'Access, Address, Timeout => 1.0);
                  Alarm   : aliased Timers.Timer_Operation := Timers.Sleep_For (Set'Access, 1.0);
                  First   : Operations.Gate_Operation :=
                    Operations.Wait_For_Success
                      (Set'Access, [Operations.Reference (Attempt), Operations.Reference (Alarm)]);
               ...
               begin
                  ...
                  if Matches.Count /= 1
                    ...
                    or else Operations.Outcome (Attempt) /= Operations.Succeeded
                  then
                     raise Program_Error with "managed connect did not win selective wait";
                  end if;

                  Connections.Finish (Attempt, Item);
                  if not Connections.Is_Open (Item) or else Manager.Active /= 1 then
                     raise Program_Error with "selective wait did not transfer managed ownership";
                  end if;
      ...
      begin
         Await_Result (Client_Result, Succeeded, "scoped managed selective wait/" & Model'Image);
         Await_Result (Server_Result, Succeeded, "scoped managed selective wait peer/" & Model'Image);
      end;
      Close_If_Open (Listener);
      if Manager.Active /= 0 then
         raise Program_Error with "selective managed wait leaked admission";
      end if;
   exception
      when others =>
         Close_If_Open (Listener);
   ...
   procedure Run_Scoped_Abort_Boundary (Model : Flyology.Execution_Model; Point : Testing.Barrier_Point) is
      Manager       : aliased Connections.Server (Capacity => 1);
      Listener      : Sockets.Socket_Type;
      Address       : Sockets.Endpoint;
      Server_Result : Result_Box;
   begin
      ...
      begin
         ...
         if Manager.Active /= 1 then
            raise Program_Error with "managed abort boundary did not retain admission";
         end if;
         abort Client;
         Testing.Release (Point);
         Await_Result
           (Server_Result, Succeeded, "managed abort boundary cleanup/" & Point'Image & "/" & Model'Image);
      exception
         when others =>
            abort Client;
            Testing.Release (Point);
      ...
      if Manager.Active /= 0 then
         raise Program_Error with "managed abort boundary leaked admission at " & Point'Image;
      end if;
      Close_If_Open (Listener);
      Testing.Reset_Barriers;
   exception
   ...
   procedure Run_Raw_Scoped_Interruption (Model : Flyology.Execution_Model) is
      Token         : aliased Connections.Cancellation_Token;
      Listener      : Sockets.Socket_Type;
      Address       : Sockets.Endpoint;
      Client_Result : Result_Box;
   begin
      ...
      declare
         ...
         begin
            begin
               ...
               if Requested then
                  raise Program_Error with "raw scoped interrupt was requested before initiation";
               end if;
               Sockets.Create_Socket (Socket, Family => Address.Family, Mode => Sockets.Socket_Stream);
               declare
                  Set     : aliased Operations.Completion_Set (1);
                  Attempt : Sockets.Connect_Operation :=
                    Sockets.Connect
                      (Set'Access, Socket'Access, Address, Timeout => 1.0, Interrupts => (1 => Interrupt));
               begin
                  Operations.Wait_All (Set);
      ...
      begin
         ...
         Await_Result (Client_Result, Cancelled, "raw scoped connect interruption/" & Model'Image);
      exception
         when others =>
            Token.Request;
            Testing.Release (Testing.Raw_Scoped_Connect_Armed);
   ...
   begin
      ...
      Run_Scoped_Abort_Boundary (Model, Testing.Managed_Connect_Child_Started);
      Run_Scoped_Abort_Boundary (Model, Testing.Managed_Connect_Child_Detached);
