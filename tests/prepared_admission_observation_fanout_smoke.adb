...
begin
   ...
   declare
      First_Claim    : Prepared.Start_Claim := Prepared.Vacant_Start_Claim (Item'Access);
      Second_Claim   : Prepared.Start_Claim := Prepared.Vacant_Start_Claim (Item'Access);
      First          : Prepared.Started_Admission := Prepared.Vacant_Started_Admission (Item'Access);
      Second         : Prepared.Started_Admission := Prepared.Vacant_Started_Admission (Item'Access);
      First_Monitor  : Prepared.Prepared_Observation_Claim := Prepared.Vacant_Observation_Claim (Item'Access);
      Second_Monitor : Prepared.Prepared_Observation_Claim := Prepared.Vacant_Observation_Claim (Item'Access);
      ...
      First_Wait     : Prepared.Observation_Operation (Set'Access, Item'Access);
      Second_Wait    : Prepared.Observation_Operation (Set'Access, Item'Access);
   ...
   begin
      ...
      Prepared.Release_To_Run (First, R_Result'Access, Completed'Access);
      ...
      Completed := False;
      Prepared.Release_To_Run (Second, R_Result'Access, Completed'Access);
      if R_Result /= Prepared.Admission_Released or else not Completed then
         raise Program_Error with "second fan-out admission did not release";
      end if;

      Prepared.Activate_Exact (First_Monitor, First_Handle, -1.0, First_Wait);
      Prepared.Activate_Exact (Second_Monitor, Second_Handle, -1.0, Second_Wait);
      ...
      then
         raise Program_Error with "shared wake did not isolate the first prepared observation";
      end if;
      ...
      Prepared.Reserve_Observation (First, First_Monitor, O_Result);
      if O_Result /= Prepared.Observation_Reserved or else not Prepared.Is_Active (Second_Monitor) then
         raise Program_Error with "one released monitor slot was not independently reusable";
      end if;
      Prepared.Cancel_And_Join (First);
      Prepared.Release_Observation_Claim (First_Monitor);
