   ...
   begin
      ...
      begin
         ...
         Prepared.Reserve_Observation
           (Admission, Observation_Claim, O_Result);
            ...
            begin
               ...
               begin
                  ...
                  if not Cancelled then
                     raise Program_Error
                       with "capacity witness observation did not cancel";
                  end if;
               end;
               ...
               if Prove_Pending then
                  ...
                  if not Flyology.Operations.Is_Active (Wait) then
                     raise Program_Error
                       with "replacement rearm did not remain pending";
                  end if;
                  ...
                  if Expect_Replacement then
                     ...
                     loop
                        ...
                        if Ada.Real_Time.Clock >= Deadline then
                           raise Program_Error
                             with "replacement did not publish before dormant rearm";
                        end if;
                        delay 0.001;
                     end loop;
                     ...
                     if Prove_Current_Boundary then
                        ...
                        if Observation.Status /= Observation_Timed_Out then
                           raise Program_Error
                             with "current replacement boundary inherited an older fact";
                        end if;
                        Prepared.Activate_Exact
                          (Observation_Claim,
                           Retained_Current,
                        ...
                        begin
                           ...
                           if not Cancelled then
                              raise Program_Error
                                with "cancelled current probe did not report cancellation";
                           end if;
                        end;
                        declare
                           task Canceller is
                        ...
                        begin
                           ...
                           if Observation.Status /= Generation_Terminated
                             or else Observation.Snapshot.Generation
                                     /= Current_Generation (Retained_Current)
                           then
                              raise Program_Error
                                with "current probe did not receive its terminal boundary";
                           end if;
                        end;
                        ...
                        loop
                           ...
                           if Ada.Real_Time.Clock >= Deadline then
                              raise Program_Error
                                with "newer replacement did not start before join";
                           end if;
                           delay 0.001;
                        end loop;
                        Prepared.Cancel_And_Join (Admission);
                        ...
                        then
                           raise Program_Error
                             with "newer terminal probe returned "
                        ...
                        if Observation.Status /= Generation_Replaced
                          or else Observation.Snapshot.Generation
                                  /= Current_Generation (Retained_Latest)
                          or else Observation.Snapshot.State /= Joined
                        then
                           raise Program_Error
                             with "reused slot replaced retained newer terminal evidence";
                        end if;
               ...
               if Expect_Replacement then
                  if Observation.Status /= Generation_Replaced then
                     raise Program_Error
                       with "replacement rearm did not report replacement";
                  elsif Observation.Snapshot.Generation
                          <= Current_Generation (First)
                  then
                     raise Program_Error
                       with "replacement rearm did not advance generation";
                  elsif not Ended_Before_Consumption
                    and then Observation.Snapshot.State /= Starting
                  then
                     raise Program_Error
                       with "replacement rearm did not retain starting state";
                  elsif not Ended_Before_Consumption
                    and then not Observation.Snapshot.Live
                  then
                     raise Program_Error
                       with "replacement rearm did not retain live state";
                  elsif Ended_Before_Consumption
                    and then (Observation.Snapshot.State /= Joined
                              or else Observation.Snapshot.Live)
                  then
                     raise Program_Error
                       with "retained replacement did not carry the terminal boundary";
                  end if;
                  declare
                  ...
                  begin
                     ...
                     if Prove_Newer_Terminal then
                        declare
                           Terminal_Current : constant Child_Handle :=
                             Retained_Latest;
                        begin
                           Reused := Early_Reused;
                           ...
                           if Observation.Status /= Generation_Terminated
                             or else Observation.Snapshot.Generation
                                     /= Current_Generation (Terminal_Current)
                           then
                              raise Program_Error
                                with "newer terminal marker was lost before slot reuse";
                           end if;
                        end;
                     end if;
                     if Prove_Current_Boundary or else Prove_Newer_Terminal then
                        ...
                        begin
                           ...
                           if not Stale then
                              raise Program_Error
                                with "ended observation claim crossed slot reuse";
                           end if;
                        end;
                     end if;
                     if Ended_Before_Consumption then
                        ...
                        begin
                           ...
                           if not Stale then
                              raise Program_Error
                                with "ended observation claim crossed admission slot reuse";
                           end if;
                        end;
                     end if;
