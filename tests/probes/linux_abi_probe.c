...
long flyology_linux_io_submit(unsigned long context, long count, void *controls);
long flyology_linux_io_cancel(unsigned long context, void *control, void *result);
long flyology_linux_io_getevents(unsigned long context, long minimum, long count,
...
static int check_epoll_translation(void) {
    struct flyology_epoll_event events[3] = {
    ...
    if (flyology_linux_epoll_ctl(epoll_fd, EPOLL_CTL_ADD, first,
                                EPOLLIN | EPOLLONESHOT) != 0 ||
        flyology_linux_epoll_ctl(epoll_fd, EPOLL_CTL_ADD, second,
                                EPOLLIN | EPOLLONESHOT) != 0 ||
    ...
    count = flyology_linux_epoll_wait(epoll_fd, events, 2, 1000);
    if (count != 2) {
...
#define EXPECT_FAILURE(call)                                                    \
    ...
    EXPECT_FAILURE(flyology_linux_io_setup(1, NULL));
    EXPECT_FAILURE(flyology_linux_io_destroy(~0UL));
    EXPECT_FAILURE(flyology_linux_io_submit(~0UL, 1, NULL));
    EXPECT_FAILURE(flyology_linux_io_cancel(~0UL, NULL, NULL));
    EXPECT_FAILURE(flyology_linux_io_getevents(~0UL, 0, 1, NULL, NULL));
    EXPECT_FAILURE(flyology_linux_io_uring_setup(0, NULL));
    EXPECT_FAILURE(flyology_linux_io_uring_enter(-1, 0, 0, 0, NULL, 0));
    EXPECT_FAILURE(flyology_linux_io_uring_register(-1, 0, NULL, 0));
