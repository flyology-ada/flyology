...
procedure Waiter_Mixed_Work_Smoke is
   ...
   protected body Progress is
      ...
      function Passed return Boolean
      is (Ready_Count = Scale
          and then Interrupted_Count = 1
   ...
   begin
      ...
      Outcome :=
        Flyology.IO.Wait_Interruptibly
          (Sockets.Native_Descriptor (Shared_Server),
   ...
   procedure Fill_Send_Buffer is
      Data        : constant Ada.Streams.Stream_Element_Array (1 .. 4_096) :=
        (others => 1);
...
begin
   ...
   pragma Assert (Target_Wake_Work.Unlink_Scans = 0);
   pragma
     Assert
       (Target_Wake_Work.Retention_Scans = Interfaces.Unsigned_64 (Scale));
   ...
   pragma
     Assert (Shared_Wake_Work.Delivery_Scans = Interfaces.Unsigned_64 (Scale));
