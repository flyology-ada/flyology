...
procedure Socket_Preparation_Smoke is
   ...
   begin
      Sockets.Send_Datagram
        (Sender, Payload, Last, Destination, Timeout => 1.0);
      pragma Assert (Last = Payload'Last);
      Sockets.Receive_Datagram
        (Receiver, Incoming, Last, Metadata, Timeout => 1.0);
...
begin
   Sockets.Create_Socket
     (Server, Sockets.IPv4, Sockets.Socket_Datagram);
   Sockets.Bind_Socket
     (Server,
   ...
   begin
      ...
      Sockets.Accept_Connection
        (Listener, Accepted, Peer_Address, Timeout => 1.0);
      ...
      Sockets.Receive_Exactly
        (Accepted_Moved, Incoming, Timeout => 1.0);
      pragma Assert (Incoming = Payload);
      pragma Assert (Nonblocking_Setup_Count = 3);
