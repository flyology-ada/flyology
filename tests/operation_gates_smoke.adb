...
procedure Operation_Gates_Smoke is
   ...
   procedure Finish_Cancelled
     (Item : in out Flyology.IO.Timers.Timer_Operation)
   is
   begin
   ...
   begin
      ...
      begin
         ...
         Passed := Passed
           ...
           and then Flyology.Operations.Outcome (Downstream) =
             Flyology.Operations.Failed;
         begin
            ...
            declare
               Invalid : Flyology.Operations.Gate_Operation :=
                 Flyology.Operations.Wait_Some
                   (Set'Access, [Ref (Child)], Required => 2);
      ...
      begin
         begin
            declare
               Invalid : Flyology.Operations.Gate_Operation :=
                 Flyology.Operations.Wait_All
                   (Set'Access, [Ref (Child), Ref (Child)]);
               pragma Unreferenced (Invalid);
            begin
               null;
            end;
      ...
      declare
         Set : aliased Flyology.Operations.Completion_Set
           (Flyology.Operations.Max_Operations);
         type Timer_Access is access
           Flyology.IO.Timers.Timer_Operation;
         type Timer_Access_Array is
           array (Positive range <>) of Timer_Access;
         procedure Free is new Ada.Unchecked_Deallocation
           (Flyology.IO.Timers.Timer_Operation, Timer_Access);
      ...
      begin
         for Position in Items'Range loop
            Items (Position) :=
              new Flyology.IO.Timers.Timer_Operation'
                (Flyology.IO.Timers.Sleep_For (Set'Access, 1.0));
         ...
         begin
            Passed := Passed
              and then Flyology.Operations.Id (Joined) =
                Flyology.Operations.Max_Operations;
      ...
      begin
         ...
         begin
            ...
            Passed := Passed
              and then Rejected
              and then Flyology.Operations.Is_Active (Joined);
      ...
      declare
         ...
         begin
            ...
            return Flyology.Operations.Wait_All
              (Set'Access, [Ref (Fast), Ref (Slow)]);
         end Observe_Local;
      ...
      begin
         Passed := Passed
           and then Flyology.Operations.Id (Replacement) /= Fast_Id;
         Flyology.Operations.Cancel (Slow);
         Flyology.Operations.Finish (Joined, Matches);
         Passed := Passed
           and then Matches.Count = 2
      ...
      declare
         ...
         Joined : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_All
             (Set'Access, [Ref (Observer_1), Ref (Observer_2)]);
         Batch : Flyology.Operations.Completion_Batch (Set.Capacity);
         Matches : Flyology.Operations.Completion_Batch (Set.Capacity);
      begin
         Flyology.Operations.Wait_Some (Set, Batch);
         ...
         for Position in 1 .. Batch.Count loop
            Passed := Passed
              and then Natural (Batch.Ids (Position)) = Position;
         end loop;
         Flyology.Operations.Finish (Joined, Matches);
      ...
      declare
         Set : aliased Flyology.Operations.Completion_Set (3);
         Only : Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_For (Set'Access, 0.0);
         Batch : Flyology.Operations.Completion_Batch (Set.Capacity);
      begin
         Flyology.Operations.Wait_Some
           (Set, Required => 3, Completed => Batch);
         Passed := Passed
           and then Batch.Count = 1
           and then Contains (Batch, Flyology.Operations.Id (Only));
      ...
      declare
         Set : aliased Flyology.Operations.Completion_Set (2);
         Success : Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_For (Set'Access, 0.0);
         Cancelled : Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_For (Set'Access, 1.0);
         Batch : Flyology.Operations.Completion_Batch (Set.Capacity);
      begin
      ...
      declare
         Set : aliased Flyology.Operations.Completion_Set (2);
         Left : Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_For (Set'Access, 1.0);
         Right : Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_For (Set'Access, 1.0);
         Batch : Flyology.Operations.Completion_Batch (Set.Capacity);
      begin
         Flyology.Operations.Cancel (Left);
