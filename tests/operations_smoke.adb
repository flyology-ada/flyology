   ...
   begin
      ...
      declare
         ...
         Ready_1      : aliased Flyology.IO.Readiness_Operation :=
           Flyology.IO.Wait
             (Set'Access, Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Read);
         Ready_2      : aliased Flyology.IO.Readiness_Operation :=
           Flyology.IO.Wait
             (Set'Access, Flyology.IO.Sockets.Native_Descriptor (Left_2), Flyology.IO.For_Read);
         ...
         Ready_Gate   : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_Some
             (Set'Access, [Ref (Ready_1), Ref (Ready_2), Ref (Alarm)], Required => 2);
         Batch        : Flyology.Operations.Completion_Batch (Set.Capacity);
         Gate_Matches : Flyology.Operations.Completion_Batch (Set.Capacity);
      begin
         ...
         Flyology.Operations.Wait_Some (Set, Batch);
         Passed := Passed and then Batch.Count = 1 and then Contains (Batch, Flyology.Operations.Id (Alarm));
         Flyology.IO.Timers.Finish (Alarm);

         Flyology.IO.Wait (Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Read, Ready_1);
         Flyology.IO.Timers.Sleep_For (0.0, Alarm);
         Flyology.Operations.Cancel (Ready_1);
         Flyology.Operations.Wait_All (Set);
         Passed := Passed and then Flyology.Operations.Terminal_Count (Set) = 2;
         Flyology.IO.Timers.Finish (Alarm);
         declare
         ...
         Flyology.IO.Wait (Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Read, Ready_1);
         Flyology.IO.Wait (Flyology.IO.Sockets.Native_Descriptor (Left_2), Flyology.IO.For_Read, Ready_2);
         Flyology.IO.Timers.Sleep_Until (Ada.Real_Time.Clock + Ada.Real_Time.Seconds (1), Alarm);
      ...
      declare
         ...
         Writable : aliased Flyology.IO.Readiness_Operation :=
           Flyology.IO.Wait
             (Set'Access, Flyology.IO.Sockets.Native_Descriptor (Right_1), Flyology.IO.For_Write);
      ...
      declare
         ...
         Both          : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_All (Set'Access, [Ref (Ready_1), Ref (Ready_2)]);
         Batch         : Flyology.Operations.Completion_Batch (Set.Capacity);
         Matches       : Flyology.Operations.Completion_Batch (Set.Capacity);
      ...
      begin
         ...
         Flyology.IO.Finish (Ready_1);
         Flyology.IO.Finish (Ready_2);
         Flyology.IO.Sockets.Receive_Socket (Left_1, Consumed, Consumed_Last);
         Passed := Passed and then Consumed_Last = Consumed'Last and then Consumed (Consumed'First) = 42;
      end;
      ...
      begin
         begin
            declare
               Invalid : Flyology.IO.Readiness_Operation :=
                 Flyology.IO.Wait (Set'Access, Flyology.IO.Invalid_Descriptor, Flyology.IO.For_Read);
         ...
         begin
            ...
            Passed := Passed and then Rejected and then Matches.Count = 1;
         end;
      end;
      ...
      begin
         ...
         begin
            Flyology.IO.Rearm (Flyology.IO.Invalid_Descriptor, Flyology.IO.For_Read, Ready);
         exception
            when Flyology.Operations.Operation_Error =>
               Rejected := True;
         end;
         ...
         Flyology.IO.Rearm (Flyology.IO.Sockets.Native_Descriptor (Right_1), Flyology.IO.For_Write, Ready);
         Flyology.Operations.Wait_Some (Set, Batch);
         Flyology.IO.Finish (Ready);
      end;
      ...
      declare
         ...
         Fast_2          : aliased Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_Until (Set'Access, Ada.Real_Time.Clock);
         ...
         Two_Ready       : aliased Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_Some
             (Set'Access, [Ref (Fast_1), Ref (Fast_2), Ref (Slow)], Required => 2);
         First_Success   : aliased Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_For_Success (Set'Access, [Ref (Two_Ready), Ref (Slow)]);
         Joined          : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_All (Set'Access, [Ref (Two_Ready), Ref (First_Success)]);
         Batch           : Flyology.Operations.Completion_Batch (Set.Capacity);
         Some_Matches    : Flyology.Operations.Completion_Batch (Set.Capacity);
         Success_Matches : Flyology.Operations.Completion_Batch (Set.Capacity);
      ...
      begin
         ...
         Passed :=
           Passed
           and then Flyology.Operations.Outcome (Two_Ready) = Flyology.Operations.Succeeded
           and then Flyology.Operations.Outcome (First_Success) = Flyology.Operations.Succeeded
           and then Flyology.Operations.Outcome (Joined) = Flyology.Operations.Succeeded;

         Flyology.Operations.Finish (Joined, Join_Matches);
         Flyology.Operations.Finish (First_Success, Success_Matches);
         Flyology.Operations.Finish (Two_Ready, Some_Matches);
         Passed :=
           Passed
           and then Join_Matches.Count = 2
           and then Contains (Join_Matches, Flyology.Operations.Id (Two_Ready))
           and then Contains (Join_Matches, Flyology.Operations.Id (First_Success))
           and then Success_Matches.Count = 1
           and then Contains (Success_Matches, Flyology.Operations.Id (Two_Ready))
           and then Some_Matches.Count = 2
           and then Contains (Some_Matches, Flyology.Operations.Id (Fast_1))
           and then Contains (Some_Matches, Flyology.Operations.Id (Fast_2));
      ...
      declare
         ...
         Alarm   : aliased Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_Until
             (Set'Access, Ada.Real_Time.Clock + Ada.Real_Time.To_Time_Span (0.005));
         Done    : Flyology.Operations.Gate_Operation :=
      ...
      declare
         ...
         Quorum      : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_For_Successes
             (Set'Access, [Ref (Candidate_1), Ref (Candidate_2)], Required => 2);
         Matches     : Flyology.Operations.Completion_Batch (Set.Capacity);
      begin
         ...
         Passed :=
           ...
           and then Flyology.Operations.Outcome (Quorum) = Flyology.Operations.Failed;
         Flyology.Operations.Finish (Quorum, Matches);
         Passed :=
           Passed
           and then Matches.Count = 1
           and then Contains (Matches, Flyology.Operations.Id (Candidate_1))
      ...
      declare
         ...
         Receive_1      : aliased Flyology.IO.Sockets.Receive_Operation :=
           Flyology.IO.Sockets.Receive (Set'Access, Left_1'Access, Input_1'Access, 1.0);
         Receive_2      : aliased Flyology.IO.Sockets.Receive_Operation :=
           Flyology.IO.Sockets.Receive (Set'Access, Left_2'Access, Input_2'Access, 1.0);
         Alarm          : Flyology.IO.Timers.Timer_Operation :=
           Flyology.IO.Timers.Sleep_For (Set'Access, 1.0);
         Receives_Done  : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_For_Successes (Set'Access, [Ref (Receive_1), Ref (Receive_2)], 2);
         Batch          : Flyology.Operations.Completion_Batch (Set.Capacity);
         Gate_Matches   : Flyology.Operations.Completion_Batch (Set.Capacity);
      ...
      begin
         ...
         Passed :=
           ...
           and then Contains (Gate_Matches, Flyology.Operations.Id (Receive_1))
           and then Contains (Gate_Matches, Flyology.Operations.Id (Receive_2));
      ...
      declare
         ...
         Receive_All   : aliased Flyology.IO.Sockets.Receive_Exactly_Operation :=
           Flyology.IO.Sockets.Receive_Exactly (Set'Access, Left_1'Access, Input'Access, 1.0);
         Send_All      : aliased Flyology.IO.Sockets.Send_All_Operation :=
           Flyology.IO.Sockets.Send_All (Set'Access, Right_1'Access, Output'Access, 1.0);
         Transfer_Done : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_All (Set'Access, [Ref (Receive_All), Ref (Send_All)]);
         Batch         : Flyology.Operations.Completion_Batch (Set.Capacity);
         Gate_Matches  : Flyology.Operations.Completion_Batch (Set.Capacity);
      begin
         ...
         Passed := Passed and then Gate_Matches.Count = 2 and then Input = Output;
      end;
      Check (Passed, "socket exact/send-all gate failed");

      declare
         Storage        : aliased Flyology.Buffers.Pool (Block_Size => 16, Capacity => 2);
      ...
      begin
         ...
         declare
            Receive_Buffer : aliased Flyology.IO.Sockets.Buffer_Receive_Operation :=
              Flyology.IO.Sockets.Receive (Set'Access, Left_2'Access, Incoming'Access, 1.0);
            Send_Buffer    : aliased Flyology.IO.Sockets.Buffer_Send_Operation :=
              Flyology.IO.Sockets.Send (Set'Access, Right_2'Access, Outgoing'Access, 1.0);
            Buffers_Done   : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_All (Set'Access, [Ref (Receive_Buffer), Ref (Send_Buffer)]);
            Gate_Matches   : Flyology.Operations.Completion_Batch (Set.Capacity);
         begin
      ...
      declare
         Set                      : aliased Flyology.Operations.Completion_Set (2);
         Output                   : aliased Ada.Streams.Stream_Element_Array := [1 => 21, 2 => 22];
         Input                    : Ada.Streams.Stream_Element_Array := [1 => 0, 2 => 0];
         Send_One                 : aliased Flyology.IO.Sockets.Send_Operation :=
           Flyology.IO.Sockets.Send (Set'Access, Right_1'Access, Output'Access, 1.0);
         Sent                     : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_For_Success (Set'Access, [Ref (Send_One)]);
         Batch                    : Flyology.Operations.Completion_Batch (Set.Capacity);
         Matches                  : Flyology.Operations.Completion_Batch (Set.Capacity);
      ...
      begin
         ...
         Flyology.IO.Sockets.Receive_Socket (Left_1, Input, Received_Last);
         Passed :=
           Passed
           and then Matches.Count = 1
      ...
      begin
         ...
         declare
            Send_All_Buffer : aliased Flyology.IO.Sockets.Buffer_Send_All_Operation :=
              Flyology.IO.Sockets.Send_All (Set'Access, Right_2'Access, Outgoing'Access, 1.0);
            Sent            : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_All (Set'Access, [Ref (Send_All_Buffer)]);
         begin
            while not Flyology.Operations.Is_Terminal (Sent) loop
               Flyology.Operations.Wait_Some (Set, Batch);
            end loop;
      ...
      declare
         ...
         Receive_One   : aliased Flyology.IO.Sockets.Receive_Operation :=
           Flyology.IO.Sockets.Receive (Set'Access, Left_1'Access, Input'Access, 1.0);
         Done          : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_All (Set'Access, [Ref (Receive_One)]);
         Matches       : Flyology.Operations.Completion_Batch (Set.Capacity);
         Cancelled     : Boolean := False;
         Received_Last : Ada.Streams.Stream_Element_Offset;
      begin
         ...
         Passed := Passed and then Cancelled and then Matches.Count = 1;
      end;
      ...
      begin
         ...
         declare
            Receive_Buffer : aliased Flyology.IO.Sockets.Buffer_Receive_Operation :=
              Flyology.IO.Sockets.Receive (Set'Access, Left_2'Access, Incoming'Access, 1.0);
            Done           : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_All (Set'Access, [Ref (Receive_Buffer)]);
            Matches        : Flyology.Operations.Completion_Batch (Set.Capacity);
         begin
      ...
      declare
         ...
         Input         : aliased Ada.Streams.Stream_Element_Array := [1 => 0];
         Receive_One   : aliased Flyology.IO.Sockets.Receive_Operation :=
           Flyology.IO.Sockets.Receive (Set'Access, Left_1'Access, Input'Access, 0.01);
         Succeeded     : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_For_Success (Set'Access, [Ref (Receive_One)]);
         Batch         : Flyology.Operations.Completion_Batch (Set.Capacity);
         Matches       : Flyology.Operations.Completion_Batch (Set.Capacity);
      ...
      begin
         ...
         Passed :=
           Passed
           and then Flyology.Operations.Outcome (Receive_One) = Flyology.Operations.Failed
           and then Flyology.Operations.Outcome (Succeeded) = Flyology.Operations.Failed;
         ...
         Passed := Passed and then Timed_Out and then Matches.Count = 1;
      end;
      ...
      begin
         ...
         declare
            ...
            Partial : aliased Flyology.IO.Sockets.Receive_Operation :=
              Flyology.IO.Sockets.Receive (Set'Access, Peer_Left'Access, Input'Access, 1.0);
            Done    : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_All (Set'Access, [Ref (Partial)]);
            Batch   : Flyology.Operations.Completion_Batch (Set.Capacity);
            Matches : Flyology.Operations.Completion_Batch (Set.Capacity);
         begin
            while not Flyology.Operations.Is_Terminal (Done) loop
            ...
            Passed := Passed and then Partial_Last = Input'First - 1 and then Matches.Count = 1;
         end;
         declare
            Set       : aliased Flyology.Operations.Completion_Set (2);
            Exact     : aliased Flyology.IO.Sockets.Receive_Exactly_Operation :=
              Flyology.IO.Sockets.Receive_Exactly (Set'Access, Peer_Left'Access, Input'Access, 1.0);
            Succeeded : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_For_Success (Set'Access, [Ref (Exact)]);
            Batch     : Flyology.Operations.Completion_Batch (Set.Capacity);
            Matches   : Flyology.Operations.Completion_Batch (Set.Capacity);
         ...
         begin
            ...
            Passed := Passed and then Flyology.Operations.Outcome (Succeeded) = Flyology.Operations.Failed;
            Flyology.Operations.Finish (Succeeded, Matches);
            begin
      ...
      declare
         ...
         Response                   : aliased Ada.Streams.Stream_Element_Array := [1 => 16#41#, 2 => 16#42#];
         Incoming_Request           : aliased Ada.Streams.Stream_Element_Array := [1 .. 3 => 0];
         Incoming_Response          : aliased Ada.Streams.Stream_Element_Array := [1 .. 2 => 0];
         Request_Metadata           : Flyology.IO.Sockets.Datagram_Metadata;
      begin
         Flyology.IO.Sockets.Create_Socket
           (Server, Flyology.IO.Sockets.IPv4, Flyology.IO.Sockets.Socket_Datagram);
         ...
         Destination :=
           Flyology.IO.Sockets.Network_Endpoint (Flyology.IO.Sockets.Loopback_IPv4, Server_Bound.Port);

         Flyology.IO.Sockets.Create_Socket
           (Client, Flyology.IO.Sockets.IPv4, Flyology.IO.Sockets.Socket_Datagram);
         Flyology.IO.Sockets.Bind_Socket
           (Client,
            Flyology.IO.Sockets.Network_Endpoint
              (Flyology.IO.Sockets.Loopback_IPv4, Flyology.IO.Sockets.Any_Port));
         ...
         declare
            Set                      : aliased Flyology.Operations.Completion_Set (3);
            Receive                  : aliased Flyology.IO.Sockets.Receive_Datagram_Operation :=
              Flyology.IO.Sockets.Receive_Datagram (Set'Access, Server'Access, Incoming_Request'Access, 1.0);
            Send                     : aliased Flyology.IO.Sockets.Send_Datagram_Operation :=
              Flyology.IO.Sockets.Send_Datagram (Set'Access, Client'Access, Request'Access, Destination, 1.0);
            Both                     : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_For_Successes (Set'Access, [Ref (Receive), Ref (Send)], 2);
            Batch                    : Flyology.Operations.Completion_Batch (Set.Capacity);
            Matches                  : Flyology.Operations.Completion_Batch (Set.Capacity);
            Sent_Last, Received_Last : Ada.Streams.Stream_Element_Offset;
         begin
            ...
            Flyology.IO.Sockets.Finish (Receive, Received_Last, Request_Metadata);
            Passed :=
              Passed
         ...
         begin
            Flyology.IO.Sockets.Receive_Datagram (Client'Access, Incoming_Response'Access, 1.0, Receive);
         ...
         declare
            Empty_Output             : aliased Ada.Streams.Stream_Element_Array := [1 .. 0 => 0];
            Empty_Input              : aliased Ada.Streams.Stream_Element_Array := [1 .. 0 => 0];
            Set                      : aliased Flyology.Operations.Completion_Set (2);
            Receive                  : Flyology.IO.Sockets.Receive_Datagram_Operation :=
              Flyology.IO.Sockets.Receive_Datagram (Set'Access, Server'Access, Empty_Input'Access, 1.0);
            Send                     : Flyology.IO.Sockets.Send_Datagram_Operation :=
              Flyology.IO.Sockets.Send_Datagram
                (Set'Access, Client'Access, Empty_Output'Access, Destination, 1.0);
            Sent_Last, Received_Last : Ada.Streams.Stream_Element_Offset;
            Metadata                 : Flyology.IO.Sockets.Datagram_Metadata;
         begin
            Flyology.Operations.Wait_All (Set);
         ...
         declare
            ...
            Input     : aliased Ada.Streams.Stream_Element_Array := [1 => 0];
            Receive   : Flyology.IO.Sockets.Receive_Datagram_Operation :=
              Flyology.IO.Sockets.Receive_Datagram (Set'Access, Server'Access, Input'Access, 0.01);
            Last      : Ada.Streams.Stream_Element_Offset;
            Metadata  : Flyology.IO.Sockets.Datagram_Metadata;
            Timed_Out : Boolean := False;
         begin
         ...
         declare
            ...
            Input     : aliased Ada.Streams.Stream_Element_Array := [1 => 0];
            Receive   : Flyology.IO.Sockets.Receive_Datagram_Operation :=
              Flyology.IO.Sockets.Receive_Datagram (Set'Access, Server'Access, Input'Access, 1.0);
            Last      : Ada.Streams.Stream_Element_Offset;
            Metadata  : Flyology.IO.Sockets.Datagram_Metadata;
            Cancelled : Boolean := False;
         begin
      ...
      declare
         ...
         First_Client, Second_Client, Third_Client : aliased Flyology.IO.Sockets.Socket_Type;
         Refused_Client                            : aliased Flyology.IO.Sockets.Socket_Type;
         First_Peer, Second_Peer, Third_Peer       : Flyology.IO.Sockets.Socket_Type;
      ...
      begin
         ...
         Flyology.IO.Sockets.Set_Socket_Option
           (Listener, (Name => Flyology.IO.Sockets.Reuse_Address, Enabled => True));
         Flyology.IO.Sockets.Bind_Socket
           (Listener,
            Flyology.IO.Sockets.Network_Endpoint
              (Flyology.IO.Sockets.Loopback_IPv4, Flyology.IO.Sockets.Any_Port));
         ...
         Destination := Flyology.IO.Sockets.Network_Endpoint (Flyology.IO.Sockets.Loopback_IPv4, Bound.Port);
         ...
         declare
            ...
            First_Accept                  : aliased Flyology.IO.Sockets.Accept_Operation :=
              Flyology.IO.Sockets.Accept_Connection (Set'Access, Listener'Access, 1.0);
            Second_Accept                 : aliased Flyology.IO.Sockets.Accept_Operation :=
              Flyology.IO.Sockets.Accept_Connection (Set'Access, Listener'Access, 1.0);
            First                         : aliased Flyology.IO.Sockets.Connect_Operation :=
              Flyology.IO.Sockets.Connect (Set'Access, First_Client'Access, Destination, 1.0);
            Second                        : aliased Flyology.IO.Sockets.Connect_Operation :=
              Flyology.IO.Sockets.Connect (Set'Access, Second_Client'Access, Destination, 1.0);
            Both                          : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_For_Successes
                (Set'Access, [Ref (First), Ref (Second), Ref (First_Accept), Ref (Second_Accept)], 4);
            Batch                         : Flyology.Operations.Completion_Batch (Set.Capacity);
            Matches                       : Flyology.Operations.Completion_Batch (Set.Capacity);
         ...
         begin
            ...
            Flyology.IO.Sockets.Finish (First_Accept, First_Peer, First_Address);
            Flyology.IO.Sockets.Finish (Second_Accept, Second_Peer, Second_Address);
            Passed :=
              Passed
              ...
              and then Flyology.IO.Sockets.Get_Peer_Name (First_Client).Port = Destination.Port
              and then Flyology.IO.Sockets.Get_Peer_Name (Second_Client).Port = Destination.Port;
         end;
         ...
         begin
            Flyology.IO.Sockets.Connect (Third_Client'Access, Destination, 1.0, Connection);
            Flyology.IO.Sockets.Accept_Connection (Listener'Access, 1.0, Acceptance);
            Flyology.Operations.Wait_All (Set);
            Flyology.IO.Sockets.Finish (Connection);
            Flyology.IO.Sockets.Finish (Acceptance, Third_Peer, Address);
            Passed := Passed and then Address.Family = Flyology.IO.Sockets.IPv4;
         end;
         ...
         begin
            ...
            begin
               ...
               Passed :=
                 Passed
                 and then Flyology.Operations.Outcome (Acceptance) = Flyology.Operations.Succeeded
         ...
         declare
            ...
            Timed                   : Flyology.IO.Sockets.Accept_Operation :=
              Flyology.IO.Sockets.Accept_Connection (Set'Access, Listener'Access, 0.001);
            Cancelled               : Flyology.IO.Sockets.Accept_Operation :=
              Flyology.IO.Sockets.Accept_Connection (Set'Access, Listener'Access);
         ...
         declare
            ...
            Connection       : Flyology.IO.Sockets.Connect_Operation :=
              Flyology.IO.Sockets.Connect (Set'Access, Refused_Client'Access, Destination, 1.0);
            Failed_At_Finish : Boolean := False;
         begin
            Flyology.Operations.Wait_All (Set);
            Passed := Passed and then Flyology.Operations.Outcome (Connection) = Flyology.Operations.Failed;
            begin
               Flyology.IO.Sockets.Finish (Connection);
            exception
      ...
      begin
         ...
         declare
            Set     : aliased Flyology.Operations.Completion_Set (3);
            Change  : aliased Watches.Next_Operation := Watches.Next (Set'Access, Watcher'Access, 1.0);
            Alarm   : Flyology.IO.Timers.Timer_Operation := Flyology.IO.Timers.Sleep_For (Set'Access, 1.0);
            Ready   : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_For_Success (Set'Access, [Ref (Change)]);
            Batch   : Flyology.Operations.Completion_Batch (Set.Capacity);
            Matches : Flyology.Operations.Completion_Batch (Set.Capacity);
         ...
         begin
            declare
               Abandoned : Watches.Next_Operation :=
                 Watches.Next (Set'Access, Watcher'Access, Flyology.IO.Infinite);
               pragma Unreferenced (Abandoned);
            begin
               null;
            end;
            declare
               Replacement : Watches.Next_Operation := Watches.Next (Set'Access, Watcher'Access, 0.0);
            begin
               Flyology.Operations.Wait_All (Set);
         ...
         declare
            Set     : aliased Flyology.Operations.Completion_Set (1);
            Failed  : Watches.Next_Operation := Watches.Next (Set'Access, Watcher'Access, 0.0);
            Event   : Watches.File_Event;
            Outcome : Flyology.IO.Wait_Outcome;
            Raised  : Boolean := False;
         begin
            Flyology.Operations.Wait_All (Set);
            Passed := Passed and then Flyology.Operations.Outcome (Failed) = Flyology.Operations.Failed;
            begin
      ...
      declare
         ...
         Set         : aliased Flyology.Operations.Completion_Set (3);
         Done        : aliased Results.Wait_Operation := Results.Wait (Set'Access, Target'Identity, 1.0);
         Alarm       : Flyology.IO.Timers.Timer_Operation := Flyology.IO.Timers.Sleep_For (Set'Access, 1.0);
         Successful  : Flyology.Operations.Gate_Operation :=
           Flyology.Operations.Wait_For_Success (Set'Access, [Ref (Done)]);
         Batch       : Flyology.Operations.Completion_Batch (Set.Capacity);
         Matches     : Flyology.Operations.Completion_Batch (Set.Capacity);
      ...
      begin
         ...
         declare
            First   : aliased Results.Wait_Operation := Results.Wait (Set'Access, Monitor, 1.0);
            Second  : aliased Results.Wait_Operation := Results.Wait (Set'Access, Monitor, 1.0);
            Both    : Flyology.Operations.Gate_Operation :=
              Flyology.Operations.Wait_All (Set'Access, [Ref (First), Ref (Second)]);
            Matches : Flyology.Operations.Completion_Batch (Set.Capacity);
         begin
            ...
            Passed := Passed and then Matches.Count = 2 and then Observation.Status = Results.Terminal;
         end;
      end;
      ...
      begin
         declare
            Abandoned : Results.Wait_Operation := Results.Wait (Set'Access, Target'Identity, 1.0);
            pragma Unreferenced (Abandoned);
         begin
            null;
         end;
         declare
            Replacement : Results.Wait_Operation := Results.Wait (Set'Access, Target'Identity, 1.0);
         begin
            Flyology.Operations.Wait_All (Set);
      ...
      begin
         begin
            declare
               Invalid : Results.Wait_Operation := Results.Wait (Set'Access, Detached, 1.0);
               pragma Unreferenced (Invalid);
            begin
               null;
            end;
      ...
      if Model = Flyology.Lightweight_Task then
         declare
            ...
            File       : Flyology.IO.Files.File_Descriptor :=
              Flyology.IO.Files.Open
                (Path, Mode => Flyology.IO.Files.Read_Write, Create => True, Truncate => True);
            Left_Data  : aliased Ada.Streams.Stream_Element_Array := [1, 2, 3, 4];
            Right_Data : aliased Ada.Streams.Stream_Element_Array := [5, 6, 7, 8];
            Read_Left  : aliased Ada.Streams.Stream_Element_Array := [0, 0, 0, 0];
            Read_Right : aliased Ada.Streams.Stream_Element_Array := [0, 0, 0, 0];
         begin
            declare
               Set                   : aliased Flyology.Operations.Completion_Set (4);
               Write_Left            : aliased Flyology.IO.Files.Write_Operation :=
                 Flyology.IO.Files.Write_At (Set'Access, File, 0, Left_Data'Access, 1.0);
               Write_Right           : aliased Flyology.IO.Files.Write_Operation :=
                 Flyology.IO.Files.Write_At (Set'Access, File, 4, Right_Data'Access, 1.0);
               Alarm                 : Flyology.IO.Timers.Timer_Operation :=
                 Flyology.IO.Timers.Sleep_For (Set'Access, 1.0);
               Writes_Done           : Flyology.Operations.Gate_Operation :=
                 Flyology.Operations.Wait_For_Successes
                   (Set'Access, [Ref (Write_Left), Ref (Write_Right)], Required => 2);
               Batch                 : Flyology.Operations.Completion_Batch (Set.Capacity);
               Gate_Matches          : Flyology.Operations.Completion_Batch (Set.Capacity);
               Last_Left, Last_Right : Ada.Streams.Stream_Element_Offset;
            begin
               ...
               Passed :=
                 Passed
                 and then Gate_Matches.Count = 2
                 and then Contains (Gate_Matches, Flyology.Operations.Id (Write_Left))
                 and then Contains (Gate_Matches, Flyology.Operations.Id (Write_Right));
               Flyology.IO.Files.Finish (Write_Left, Last_Left);
               Flyology.IO.Files.Finish (Write_Right, Last_Right);
               Passed := Passed and then Last_Left = Left_Data'Last and then Last_Right = Right_Data'Last;
               Flyology.Operations.Cancel (Alarm);
               begin
                  Flyology.IO.Timers.Finish (Alarm);
               exception
            ...
            declare
               ...
               Get_Left              : aliased Flyology.IO.Files.Read_Operation :=
                 Flyology.IO.Files.Read_At (Set'Access, File, 0, Read_Left'Access, 1.0);
               Get_Right             : aliased Flyology.IO.Files.Read_Operation :=
                 Flyology.IO.Files.Read_At (Set'Access, File, 4, Read_Right'Access, 1.0);
               Reads_Done            : Flyology.Operations.Gate_Operation :=
                 Flyology.Operations.Wait_All (Set'Access, [Ref (Get_Left), Ref (Get_Right)]);
               Batch                 : Flyology.Operations.Completion_Batch (Set.Capacity);
               Gate_Matches          : Flyology.Operations.Completion_Batch (Set.Capacity);
               Last_Left, Last_Right : Ada.Streams.Stream_Element_Offset;
            begin
            ...
            declare
               Storage                                           :
                 aliased Flyology.Buffers.Pool (Block_Size => 8, Capacity => 3);
               Other_Storage                                     :
                 aliased Flyology.Buffers.Pool (Block_Size => 8, Capacity => 1);
               Input                                             :
                 Flyology.Buffers.Unique_Buffer (Storage'Access);
               Output                                            :
                 Flyology.Buffers.Unique_Buffer (Storage'Access);
               Occupied                                          :
                 Flyology.Buffers.Unique_Buffer (Storage'Access);
               ...
               Set                                               :
                 aliased Flyology.Operations.Completion_Set (3);
               Matches                                           :
                 Flyology.Operations.Completion_Batch (Set.Capacity);
               Read, Written                                     : Natural;
               Rejected_Scalar, Rejected_Occupied, Rejected_Pool : Boolean := False;
            ...
            begin
               ...
               declare
                  Get  : aliased Flyology.IO.Files.Read_Operation :=
                    Flyology.IO.Files.Read_At (Set'Access, File, 0, Input, 1.0);
                  Put  : aliased Flyology.IO.Files.Write_Operation :=
                    Flyology.IO.Files.Write_At (Set'Access, File, 16, Output, 1.0);
                  Both : Flyology.Operations.Gate_Operation :=
                    Flyology.Operations.Wait_For_Successes (Set'Access, [Ref (Get), Ref (Put)], 2);
               begin
                  Passed :=
                    Passed
                  ...
                  begin
                     Flyology.IO.Files.Finish (Put, Occupied, Written);
                  exception
                     when Program_Error =>
                        Rejected_Occupied := True;
                  end;
                  begin
                     Flyology.IO.Files.Finish (Put, Wrong_Pool, Written);
                  exception
                     when Program_Error =>
                        Rejected_Pool := True;
                  end;
            ...
            declare
               Storage : aliased Flyology.Buffers.Pool (Block_Size => 4, Capacity => 1);
            ...
            begin
               ...
               Flyology.IO.Files.Write_At (File, 28, Item, 1.0, Put);
            ...
            begin
               ...
               declare
                  Put : Flyology.IO.Files.Write_Operation :=
                    Flyology.IO.Files.Write_At (Set'Access, File, 24, Item, 1.0);
                  pragma Unreferenced (Put);
               begin
                  Passed := Passed and then not Flyology.Buffers.Has_Buffer (Item);
               end;
               Flyology.Buffers.Try_Acquire (Item, Acquired);
               Passed := Passed and then Acquired and then Flyology.Buffers.Has_Buffer (Item);
            end;
            ...
            declare
               ...
               Procedure_Write         : Flyology.IO.Files.Write_Operation (Set'Access);
               Procedure_Input         : aliased Ada.Streams.Stream_Element_Array := [0, 0, 0, 0];
               Procedure_Output        : aliased Ada.Streams.Stream_Element_Array := [9, 10, 11, 12];
               Last_Read, Last_Written : Ada.Streams.Stream_Element_Offset;
            begin
               Flyology.IO.Files.Read_At (File, 0, Procedure_Input'Access, 1.0, Procedure_Read);
               Flyology.IO.Files.Write_At (File, 8, Procedure_Output'Access, 1.0, Procedure_Write);
               Flyology.Operations.Wait_All (Set);
            ...
            declare
               Immediate_Data : aliased Ada.Streams.Stream_Element_Array := [0, 0, 0, 0];
               Set            : aliased Flyology.Operations.Completion_Set (1);
               Immediate_Read : Flyology.IO.Files.Read_Operation :=
                 Flyology.IO.Files.Read_At (Set'Access, File, 0, Immediate_Data'Access, 0.0);
               Last_Immediate : Ada.Streams.Stream_Element_Offset;
            begin
               Flyology.Operations.Wait_All (Set);
               Flyology.IO.Files.Finish (Immediate_Read, Last_Immediate);
               Passed :=
                 Passed and then Last_Immediate = Immediate_Data'Last and then Immediate_Data = Left_Data;
            end;

            declare
               Set                   : aliased Flyology.Operations.Completion_Set (1);
               Cancelled_Write       : Flyology.IO.Files.Write_Operation :=
                 Flyology.IO.Files.Write_At (Set'Access, File, 8, Left_Data'Access, 1.0);
            ...
            begin
               ...
               Flyology.Operations.Wait_All (Set);
               begin
                  Flyology.IO.Files.Finish (Cancelled_Write, Last_Cancelled);
               exception
                  when Flyology.Operations.Operation_Cancelled =>
            ...
            declare
               ...
               Abandoned_Write : Flyology.IO.Files.Write_Operation :=
                 Flyology.IO.Files.Write_At (Set'Access, File, 12, Right_Data'Access, 1.0);
            ...
            declare
               ...
               Read                    : aliased Flyology.IO.Files.Read_Operation :=
                 Flyology.IO.Files.Read_At (Set'Access, File, 0, Empty_Input'Access, 1.0);
               Write                   : aliased Flyology.IO.Files.Write_Operation :=
                 Flyology.IO.Files.Write_At (Set'Access, File, 0, Empty_Output'Access, 1.0);
               Both                    : Flyology.Operations.Gate_Operation :=
                 Flyology.Operations.Wait_For_Successes (Set'Access, [Ref (Read), Ref (Write)], 2);
               Matches                 : Flyology.Operations.Completion_Batch (Set.Capacity);
               Last_Read, Last_Written : Ada.Streams.Stream_Element_Offset;
            begin
               Passed :=
                 ...
                 and then Flyology.Operations.Outcome (Both) = Flyology.Operations.Succeeded;
               Flyology.Operations.Finish (Both, Matches);
         ...
         declare
            Path   : constant String := "/tmp/flyology_operations_native_smoke.dat";
            File   : Flyology.IO.Files.File_Descriptor :=
              Flyology.IO.Files.Open
                (Path, Mode => Flyology.IO.Files.Read_Write, Create => True, Truncate => True);
            Input  : aliased Ada.Streams.Stream_Element_Array := [1 => 0];
         ...
         begin
            declare
               Set                       : aliased Flyology.Operations.Completion_Set (3);
               Read                      : aliased Flyology.IO.Files.Read_Operation (Set'Access);
               Write                     : aliased Flyology.IO.Files.Write_Operation (Set'Access);
               Matches                   : Flyology.Operations.Completion_Batch (Set.Capacity);
               Last_Read, Last_Written   : Ada.Streams.Stream_Element_Offset;
               Read_Failed, Write_Failed : Boolean := False;
            begin
               Flyology.IO.Files.Read_At (File, 0, Input'Access, 1.0, Read);
               Flyology.IO.Files.Write_At (File, 0, Output'Access, 1.0, Write);
               declare
                  Both_Terminal : Flyology.Operations.Gate_Operation :=
                    Flyology.Operations.Wait_All (Set'Access, [Ref (Read), Ref (Write)]);
               begin
                  Passed :=
                    Passed
                    and then Flyology.Operations.Is_Terminal (Both_Terminal)
                    and then Flyology.Operations.Outcome (Both_Terminal) = Flyology.Operations.Succeeded;
                  ...
                  Passed := Passed and then Matches.Count = 2 and then Read_Failed and then Write_Failed;
               end;
            end;
            ...
            declare
               Storage                   : aliased Flyology.Buffers.Pool (Block_Size => 4, Capacity => 2);
            ...
            begin
               ...
               declare
                  Get : Flyology.IO.Files.Read_Operation :=
                    Flyology.IO.Files.Read_At (Set'Access, File, 0, Input, 1.0);
                  Put : Flyology.IO.Files.Write_Operation :=
                    Flyology.IO.Files.Write_At (Set'Access, File, 0, Output, 1.0);
               begin
                  Flyology.Operations.Wait_All (Set);
                  begin
                     Flyology.IO.Files.Finish (Get, Input, Read);
