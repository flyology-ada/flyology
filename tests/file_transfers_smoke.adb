...
procedure File_Transfers_Smoke is
   ...
   Path : constant String :=
     Ada.Environment_Variables.Value ("FLYOLOGY_TEST_TEMP_ROOT") & "/file-transfers-smoke.data";
   ...
   procedure Run (Kind : Flyology.Execution_Model; Lane : String) is
      ...
      task body Sender is
         Storage : aliased Flyology.Buffers.Pool (Block_Size => 64 * 1_024, Capacity => 1);
      ...
      begin
         ...
         Transfers.Send_Chunk (File, Sender_Socket, Offset, 64, Scratch, Sent, Timeout => 5.0);
         if Sent /= 0 then
            raise Program_Error with "file transfer passed EOF";
         end if;
         Transfers.Send_Chunk (File, Sender_Socket, 0, 0, Scratch, Sent, Timeout => 5.0);
         if Sent /= 0 then
         ...
         begin
            ...
            begin
               Transfers.Send_Chunk
                 (File, Sender_Socket, 0, 64, Scratch, Sent, Timeout => 5.0, Token => Stop'Access);
            exception
               when Files.Operation_Cancelled =>
                  Caught := True;
            end;
            if not Caught then
               raise Program_Error with "pre-cancelled transfer was not cancelled";
            end if;
         end;
         Outcome := 1;
      exception
   ...
   begin
      ...
      Sockets.Accept_Connection (Listener, Receiver_Socket, Peer, Timeout => 5.0);
      Sockets.Close_Socket (Listener);
      declare
         Worker : Sender;
      begin
         Sockets.Receive_Exactly (Receiver_Socket, Received, Timeout => 5.0);
      end;
   ...
   procedure Run_Edge (Kind : Flyology.Execution_Model; Lane : String; Mode : Edge_Mode) is
      Sender_Socket   : Sockets.Socket_Type;
      Receiver_Socket : Sockets.Socket_Type;
      Listener        : Sockets.Socket_Type;
      Server          : Sockets.Endpoint;
      ...
      task body Sender is
         ...
         procedure Touch (Buffer : in out Stream_Element_Array; Length : in out Natural) is
         begin
            ...
            begin
               Transfers.Send_Chunk
                 (File,
                  ...
                  Scratch,
                  Sent,
                  Timeout => (if Mode = Backpressure_Timeout then 0.05 else 5.0),
                  Token   => (if Mode = Post_Submit_Cancel then Stop'Access else null));
               if Sent = 0 then
                  raise Program_Error with "edge transfer made no progress before EOF";
               end if;
               Made_Progress := True;
            exception
               when Files.Operation_Cancelled =>
   ...
   begin
      ...
      Sockets.Close_Socket (Sender_Socket);
      if Outcome /= 1 then
         raise Program_Error with Lane & " file-transfer " & Mode'Image & " edge failed";
      end if;
   end Run_Edge;
...
begin
   ...
   for Mode in Edge_Mode loop
      Ada.Text_IO.Put_Line ("file transfer smoke: BEGIN native " & Mode'Image);
      Run_Edge (Flyology.Native_Task, "native", Mode);
      Ada.Text_IO.Put_Line ("file transfer smoke: PASS native " & Mode'Image);
      Ada.Text_IO.Put_Line ("file transfer smoke: BEGIN lightweight " & Mode'Image);
      Run_Edge (Flyology.Lightweight_Task, "lightweight", Mode);
      Ada.Text_IO.Put_Line ("file transfer smoke: PASS lightweight " & Mode'Image);
   end loop;
   Files.Close (File);
   Remove_Test_File;
