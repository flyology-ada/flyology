...
procedure Flyology.TLS_OpenSSL_Policy.Smoke is
   ...
   Server : aliased Wire_Bytes :=
     [2, H, Character'Pos ('2'),
      8, H, T, T, P, Slash, One, Dot, One];
   Offered : aliased Wire_Bytes :=
     [8, H, T, T, P, Slash, One, Dot, One,
      2, H, Character'Pos ('2')];
   No_Overlap : aliased Wire_Bytes :=
     [8, H, T, T, P, Slash, One, Dot, Character'Pos ('0')];
   Empty : aliased Wire_Bytes (1 .. 0);
   Zero_Length : aliased Wire_Bytes := [0];
   Truncated : aliased Wire_Bytes := [2, H];
   Matching_Then_Malformed : aliased Wire_Bytes :=
     [2, H, Character'Pos ('2'), 0];
...
begin
   ...
   pragma Assert
     (Versions_Match (1, 16#3000_0000#, 16#3000_0000#,
                      16#3000_0000#) = 1);
   pragma Assert
     (Versions_Match (0, 16#3000_0000#, 16#3000_0000#,
                      16#3000_0000#) = 0);
   pragma Assert
     (Versions_Match (1, 16#3000_0001#, 16#3000_0002#,
                      16#3000_0002#) = 0);
   pragma Assert
     (Versions_Match (1, 16#2000_0000#, 16#2000_0000#,
                      16#2000_0000#) = 0);
   pragma Assert
     (Versions_Match (1, 16#4000_0000#, 16#4000_0000#,
                      16#4000_0000#) = 0);
   ...
   pragma Assert
     (not Select_ALPN (Server, Matching_Then_Malformed).Found);
   ...
   pragma Assert
     (Raw.Valid_ALPN_List (Server'Address, C.unsigned (Server'Length)) = 1);
   pragma Assert
     (Raw.Valid_ALPN_List
        (Zero_Length'Address, C.unsigned (Zero_Length'Length)) = 0);
   pragma Assert
     (Raw.Valid_ALPN_List
        (Truncated'Address, C.unsigned (Truncated'Length)) = 0);
   ...
   Raw_Selection :=
     Raw.Select_ALPN
       (Server'Address,
        ...
        Offered'Address,
        C.unsigned (Offered'Length));
   pragma Assert
     (Raw_Selection =
        (Interfaces.Shift_Left (Interfaces.Unsigned_64'(10), 8) or 2));
   pragma Assert
     (Raw.Select_ALPN
        (Server'Address,
         C.unsigned (Server'Length),
         No_Overlap'Address,
         C.unsigned (No_Overlap'Length)) = 0);
   pragma Assert
     (Raw.Select_ALPN
        (Server'Address,
         C.unsigned (Server'Length),
         Matching_Then_Malformed'Address,
         C.unsigned (Matching_Then_Malformed'Length)) = 0);
