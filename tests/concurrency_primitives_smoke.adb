...
procedure Concurrency_Primitives_Smoke is
   ...
   overriding
   procedure Adjust (Item : in out Fragile_Result);

   overriding
   procedure Adjust (Item : in out Fragile_Result) is
   ...
   procedure Exercise_Lightweight_Waits is new
   ...
   begin
      ...
      declare
         Item : aliased Local_Executors.Executor (Workers => 3, Capacity => 1);
      begin
         begin
   ...
   begin
      ...
      Native_Executors.Shutdown (Item);
      pragma Assert (Worker_Pool_Test_Control.Outstanding_Cleanup_Tokens = 0);
      Native_Executors.Await (Item, Handle, Result);
   ...
   procedure Exercise_Capacity_Timed_Acquire_Abort is
      ...
      overriding
      procedure Finalize (Guard : in out Permit_Guard);

      overriding
      procedure Finalize (Guard : in out Permit_Guard) is
      begin
         if Guard.Armed then
   ...
   procedure Exercise_Worker_Pool is
      ...
      procedure Exercise_Success is
         Item    : aliased Pools.Pool (Worker_Count => 3, Queue_Capacity => 2);
      ...
      begin
         ...
         pragma Assert (Pools.First_Failure_Information (Item)'Length > 0);
   ...
   procedure Exercise_Lightweight_Pool is new
