...
procedure DNS_Parser_Matrix is
   ...
   Base : constant Packet (1 .. 43) :=
     [16#12#, 16#34#, 16#81#, 16#80#,
      0, 1, 0, 1, 0, 0, 0, 0,
      4, Character'Pos ('f'), Character'Pos ('u'), Character'Pos ('z'),
      Character'Pos ('z'), 4, Character'Pos ('t'), Character'Pos ('e'),
      Character'Pos ('s'), Character'Pos ('t'), 0, 0, 1, 0, 1,
      16#C0#, 12, 0, 1, 0, 1, 0, 0, 0, 60, 0, 4,
      192, 0, 2, 1];
   ...
   function Observe
     (Value         : Packet;
      ...
      For_IPv6      : Boolean := False) return Expected_Result
   is
   begin
      Testing.Validate_Response
        (Value, Expected_ID, Expected_Name, For_IPv6);
      return Accepted;
   exception
      ...
      when Error : others =>
         raise Program_Error with
           "DNS parser leaked an exception for " & Context & ": "
   ...
   begin
      if Observe (Base, Context & " sentinel") /= Accepted then
         raise Program_Error with
           "DNS parser retained state after " & Context;
      end if;
   end Assert_Sentinel;
   ...
   is
      Actual : constant Expected_Result :=
        Observe (Value, Context, Expected_ID, Expected_Name, For_IPv6);
   begin
      if Actual /= Expected then
         raise Program_Error with
           "DNS parser category mismatch for " & Context
           & ": expected " & Expected'Image & ", got " & Actual'Image;
      end if;
      case Expected is
         when Accepted  => Accepted_Cases := Accepted_Cases + 1;
         when Malformed => Malformed_Cases := Malformed_Cases + 1;
      end case;
   ...
   procedure Check_Safe (Value : Packet; Case_Number : Positive) is
      Context : constant String :=
        "seed" & Interfaces.Unsigned_32'Image (Seed)
        & " case" & Case_Number'Image;
   ...
   begin
      Put
        (Buffer, Last,
         Natural (Interfaces.Shift_Right (Value, 24) and 16#FF#));
      Put
        (Buffer, Last,
   ...
   function Value_Of
     (Buffer : Wire_Buffer; Last : Natural) return Packet
   is
   ...
   begin
      ...
      for Last in Base'First .. 12 loop
         Check
           (Base (Base'First .. Last), Malformed,
            "header truncation" & Last'Image);
      end loop;
      for Last in 13 .. Base'Last - 1 loop
         Check
           (Base (Base'First .. Last), Malformed,
            "body truncation" & Last'Image);
      end loop;
      ...
      Check
        (Base, Malformed, "expected transaction ID zero", Expected_ID => 0);
      Check
        (Base, Malformed, "expected transaction ID maximum",
         Expected_ID => Natural'Last);
      ...
      for Count of Packet'[0, 2, 255] loop
         ...
         Check
           (Mutant, Malformed,
            "question count" & Natural'Image (Natural (Count)));
      end loop;
      ...
      Check
        (Base, Accepted, "case-insensitive expected question",
         Expected_Name => "FUZZ.TEST.");
      ...
      for Field_First of Offset_Array'[7, 9, 11] loop
         ...
         Check
           (Mutant, Malformed,
            "record count field" & Offset'Image (Field_First));
      end loop;
   ...
   begin
      ...
      Start_Response
        (Buffer, Last, Answers => 1, Kind => 28);
      Put_Record_Header (Buffer, Last, Kind => 28, Data_Length => 16);
      for Index in 0 .. 15 loop
         Put (Buffer, Last, Index);
      end loop;
      Check
        (Value_Of (Buffer, Last), Accepted, "valid AAAA response",
         For_IPv6 => True);
      Check
        (Value_Of (Buffer, Last), Malformed, "AAAA question in A mode");
      ...
      Check
        (Value_Of (Buffer, Last), Malformed,
         "CNAME decoding exceeds RDLENGTH");
   ...
   begin
      ...
      Mutant (45) := 12;
      Check
        (Mutant (1 .. 45), Accepted, "two-hop compression chain");
      ...
      Start_Response
        (Buffer, Last, Answers => 0, Name => Long_253);
      Check
        (Value_Of (Buffer, Last), Accepted, "maximum decoded name",
         Expected_Name => Long_253);
      Start_Response
        (Buffer, Last, Answers => 0, Name => Long_254);
      Check
        (Value_Of (Buffer, Last), Malformed, "decoded name over maximum");
   ...
   begin
      Start_Response
        (Buffer, Last, Answers => 17, Name => "fuzz.test");
      ...
      Check
        (Value_Of (Buffer, Last), Malformed,
         "CNAME chain exceeds depth limit");

      Start_Response
        (Buffer, Last, Answers => 2, Name => "fuzz.test");
      Put_CNAME_Record ("fuzz.test", "loop.test");
      Put_CNAME_Record ("loop.test", "fuzz.test");
      Check
        (Value_Of (Buffer, Last), Malformed, "CNAME alias cycle");
   end Check_CNAME_Depth;

   procedure Check_Deterministic_Perturbations is
      Replacements : constant Packet :=
        [0, 1, 16#3F#, 16#40#, 16#BF#, 16#C0#, 16#FF#];
      Mutant : Packet (Base'Range);
   ...
   begin
      ...
      for Iteration in 1 .. 4_096 loop
         declare
            ...
            Value : Packet (1 .. Last);
            Mutation_Count : constant Positive :=
              Positive (Natural (Next_Random mod 8) + 1);
         begin
                  ...
                  declare
                     Position : constant Offset :=
                       ...
                       + Offset
                         (Natural
                            (Next_Random
                             mod Interfaces.Unsigned_32 (Value'Length)));
                  begin
...
begin
   ...
   Ada.Text_IO.Put_Line
     ("DNS parser matrix: accepted=" & Accepted_Cases'Image
      & " malformed=" & Malformed_Cases'Image
      & " safety=" & Safety_Cases'Image
      & " seed=" & Interfaces.Unsigned_32'Image (Seed));
