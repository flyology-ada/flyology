...
package TLS_ALPN_Test_Provider is
   ...
   type Offer_Kind is
     (No_Offer, H2_Only, H2_Then_HTTP_1_1, Other_Offer);

   type Provider is new Flyology.IO.TLS.ALPN.Provider with private;

   procedure Set_Selection
     (Item : in out Provider; Selection : Selection_Kind);
   procedure Set_Handshake_Status
     (Item : in out Provider; Status : Flyology.IO.TLS.Step_Status);
...
private
   type Provider is new Flyology.IO.TLS.ALPN.Provider with record
      ...
      Handshake_Status : Flyology.IO.TLS.Step_Status :=
        Flyology.IO.TLS.Complete;
   end record;
