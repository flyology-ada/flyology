...
begin
   ...
   Affected_Children (4, 1, Public.Restart_Dependents, Cohort, Dependencies, Affected);
   pragma Assert (Affected (1) and then Affected (2) and then Affected (3) and then Affected (4));

   Cohort (3) := True;
   Affected_Children (4, 2, Public.Restart_Cohort, Cohort, Dependencies, Affected);
   pragma Assert (not Affected (1) and then Affected (2) and then Affected (3) and then not Affected (4));
   ...
   pragma
     Assert (Recorded_Transition_Allowed (Public.Restart_Admitted, Public.Terminated, Public.Backing_Off));
   pragma
     Assert (not Recorded_Transition_Allowed (Public.Lifecycle_Changed, Public.Running, Public.Starting));
   pragma Assert (Recorded_Transition_Allowed (Public.Supervisor_Stopped, Public.Running, Public.Running));

   pragma Assert (not Should_Restart (Public.Never, Public.Unhandled_Exception));
   pragma Assert (Should_Restart (Public.On_Failure, Public.Unhandled_Exception));
   pragma Assert (Should_Restart (Public.On_Failure, Public.Unhealthy));
   pragma Assert (Should_Restart (Public.On_Failure, Public.Restart_Requested));
   pragma Assert (not Should_Restart (Public.On_Failure, Public.Normal_Return));
   pragma Assert (not Should_Restart (Public.On_Failure, Public.Stuck));
   pragma Assert (not Should_Restart (Public.On_Failure, Public.Policy_Exhaustion));
   pragma Assert (not Should_Restart (Public.Always, Public.Stuck));
   pragma Assert (not Should_Restart (Public.Always, Public.Supervisor_Shutdown));
   ...
   pragma Assert (Was_New and then Parent_Observation.Count = 1 and then Incident.Attempt = 1);
   ...
   pragma Assert (Generation_Matches (High_Id, Generation, High_Id, Generation));
   pragma Assert (not Generation_Matches (High_Id, Generation, High_Id, Public.Generation'First));
   pragma Assert (Authority_Matches (7, High_Id, Generation, 7, High_Id, Generation));
   pragma Assert (not Authority_Matches (7, High_Id, Generation, 8, High_Id, Generation));
   pragma Assert (Generation_Start_Allowed (High_Id, Generation, High_Id, Generation, False));
   pragma
     Assert (Generation_Start_Allowed (High_Id, Generation, High_Id, Next_Generation (Generation), True));
   pragma
     Assert
       (not Generation_Start_Allowed (High_Id, Generation, High_Id, Next_Generation (Generation), False));
   pragma
     Assert
       (not Generation_Start_Allowed (High_Id, Generation, High_Id + 1, Next_Generation (Generation), True));
   ...
   pragma
     Assert (Family_Stop_Command_Allowed (Current => True, Queued => True, Managed => False, Live => False));
   pragma
     Assert (Family_Stop_Command_Allowed (Current => True, Queued => False, Managed => True, Live => True));
   pragma
     Assert
       (not Family_Stop_Command_Allowed (Current => True, Queued => False, Managed => True, Live => False));
   pragma
     Assert
       (not Family_Stop_Command_Allowed (Current => False, Queued => True, Managed => False, Live => False));
