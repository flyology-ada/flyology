...
package TLS_Test_Provider is
   type Operation_Kind is (Handshake_Operation, Receive_Operation, Send_Operation, Shutdown_Operation);

   type Output_Effect is (Preserve_Output, Advance_Output, Before_First, After_Last);
   ...
   type Receive_Behavior is
     (Return_Data, Orderly_EOF, Invalid_Lower, Invalid_Upper, Complete_Without_Receive_Progress);
   ...
   type Peer_Close_Point is (No_Peer_Close, Handshake_Peer_Close, Send_Peer_Close, Shutdown_Peer_Close);
   ...
   procedure Set_Create_Behavior (Item : in out Provider; Behavior : Create_Behavior);
   procedure Set_Create_Delay (Item : in out Provider; Delay_For : Duration)
   ...
   procedure Set_Receive_Behavior (Item : in out Provider; Behavior : Receive_Behavior);
   procedure Set_Send_Behavior (Item : in out Provider; Behavior : Send_Behavior);
   procedure Set_Peer_Close (Item : in out Provider; Point : Peer_Close_Point);
   procedure Set_Script (Item : in out Provider; Operation : Operation_Kind; Steps : Step_Script);
   procedure Reset_Telemetry;
   procedure Get_Telemetry (Want_Results : out Natural; Partial_Progress : out Natural);
...
private
   ...
   type Stored_Steps is array (Positive range 1 .. Maximum_Script_Length) of Script_Step;
