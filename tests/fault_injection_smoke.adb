...
procedure Fault_Injection_Smoke is
   ...
   procedure Free_Probe is new Ada.Unchecked_Deallocation (Probe, Probe_Access);
   ...
   procedure Test_Watch_Error is
      ...
      Failed_As_Device_Error : Boolean := False
      with Atomic;
      ...
      begin
         begin
            if IO.Wait (Flyology.IO.Sockets.Native_Descriptor (Reader), IO.For_Read, Timeout => 0.1) then
               null;
            end if;
         exception
      ...
      procedure Free_Waiter is new Ada.Unchecked_Deallocation (Waiter, Waiter_Access);
      Item : Waiter_Access;
   begin
   ...
   procedure Test_Multi_Watch_Rollback is
      ...
      procedure Free is new Ada.Unchecked_Deallocation (Failed_Waiter, Failed_Waiter_Access);
   ...
   begin
      ...
      begin
         while Item /= null and then not Item.all'Terminated loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "later-watch fault waiter did not terminate";
            end if;
            delay 0.001;
         end loop;
      end;
      ...
      declare
         ...
         task body Retry is
         begin
            Retry_Ready := IO.Wait (Reused_FD, IO.For_Read, Timeout => 0.1);
         end Retry;
      begin
   ...
   procedure Test_File_Saturation is
      ...
      task body Writer is
         ...
         Last : Ada.Streams.Stream_Element_Offset;
      begin
         Files.Write_At (File, Files.File_Offset (Index - 1), Data, Last);
      ...
      procedure Free_Writer is new Ada.Unchecked_Deallocation (Writer, Writer_Access);
   ...
   procedure Test_Scoped_File_Saturation is
      ...
      task body Writer is
         Data : aliased Ada.Streams.Stream_Element_Array := [1 => 1, 2 => 2, 3 => 3, 4 => 4];
         Last : Ada.Streams.Stream_Element_Offset;
      begin
         ...
         declare
            Set       : aliased Operations.Completion_Set (1);
            Write     : Files.Write_Operation := Files.Write_At (Set'Access, File, 4, Data'Access, 1.0);
         ...
         declare
            Set       : aliased Operations.Completion_Set (1);
            Input     : aliased Ada.Streams.Stream_Element_Array := [1 .. 4 => 0];
            Read      : Files.Read_Operation := Files.Read_At (Set'Access, File, 0, Input'Access, 1.0);
            Cancelled : Boolean := False;
         begin
         ...
         declare
            Set       : aliased Operations.Completion_Set (1);
            Write     : Files.Write_Operation := Files.Write_At (Set'Access, File, 4, Data'Access, 0.01);
            Timed_Out : Boolean := False;
         begin
            Operations.Wait_All (Set);
            Passed := Passed and then Operations.Outcome (Write) = Operations.Failed;
            begin
               Files.Finish (Write, Last);
            exception
         ...
         begin
            declare
               Abandoned : Files.Read_Operation := Files.Read_At (Set'Access, File, 0, Input'Access, 1.0);
            ...
            Fault_Control.Reset;
            declare
               Replacement : Files.Read_Operation := Files.Read_At (Set'Access, File, 0, Input'Access, 1.0);
            begin
               Operations.Wait_All (Set);
               Files.Finish (Replacement, Last);
               Passed := Passed and then Last = Input'Last and then Input = Data;
            end;
         end;
         ...
         begin
            ...
            declare
               Abandoned : Files.Write_Operation := Files.Write_At (Set'Access, File, 8, Item, 1.0);
               pragma Unreferenced (Abandoned);
            begin
         ...
         declare
            ...
            Failed_Write : Files.Write_Operation :=
              Files.Write_At (Set'Access, Files.Invalid_File, 0, Data'Access, 1.0);
            Failed       : Boolean := False;
         begin
            Operations.Wait_All (Set);
            Passed := Passed and then Operations.Outcome (Failed_Write) = Operations.Failed;
            begin
         ...
         declare
            ...
            Short_Data           : aliased Ada.Streams.Stream_Element_Array := [3 .. 8 => 0];
            EOF_Data             : aliased Ada.Streams.Stream_Element_Array := [5 .. 8 => 0];
         ...
         begin
            for Round in 1 .. 16 loop
               One_Byte (1) := Ada.Streams.Stream_Element (Round mod 251);
               for Index in Writes'Range loop
                  Files.Write_At
                    (File, Files.File_Offset (128 + Index - 1), One_Byte'Access, 1.0, Writes (Index));
               end loop;
               Operations.Wait_All (Set);
               for Index in Writes'Range loop
   ...
   begin
      ...
      if not Passed then
         raise Program_Error with "scoped file operation did not survive submission pressure";
      end if;
      Fault_Control.Reset;
      Files.Close (File);
      Ada.Directories.Delete_File (Path);
   ...
   begin
      ...
      File := Files.Open (Path, Mode => Files.Read_Write, Create => True, Truncate => True);
      Fault_Control.Reset;
      Fault_Control.Arm (Fault_Control.File_Submission_Full, Count => 1_000_000_000);
      Item := new Writer;
      ...
      then
         raise Program_Error with "file wait was treated as a dormancy candidate";
      end if;
   ...
   begin
      ...
      if Selected_Linux_Backend /= 1 then
         Ada.Text_IO.Put_Line ("io_uring CQ test skipped: backend is not io_uring");
         return;
      end if;
      ...
      declare
         ...
         Data    : Ada.Streams.Stream_Element_Array (1 .. Ada.Streams.Stream_Element_Offset (Writer_Count));
         Last    : Ada.Streams.Stream_Element_Offset;
      begin
         if Capacity = 0 then
         ...
         File := Files.Open (Path, Mode => Files.Read_Write, Create => True, Truncate => True);
         Fault_Control.Reset;
         Fault_Control.Arm (Fault_Control.File_Uring_Drain_Pause, Count => 1_000_000);
         Fault_Control.Arm (Fault_Control.File_Uring_Submit_EBUSY);
         Fault_Control.Arm (Fault_Control.File_Uring_Overflow_Flush, Count => 2);
         ...
         if not Progress.Passed then
            ...
            raise Program_Error with "io_uring overflow GETEVENTS flush was not exercised";
         elsif Fault_Control.Calls (Fault_Control.File_Uring_Flush_EBUSY) = 0 then
            raise Program_Error with "io_uring overflow EBUSY retry was not exercised";
         end if;
         ...
         for Index in Writers'Range loop
            if Data (Ada.Streams.Stream_Element_Offset (Index)) /= Ada.Streams.Stream_Element (Index mod 251)
            then
               raise Program_Error with "io_uring capacity result was corrupted";
            end if;
         end loop;
         for Item of Writers loop
            while not Item.all'Terminated loop
   ...
   procedure Test_Uring_Initialization_Fallback (At_Point : Fault_Control.Point) is
      Path       : constant String := "/tmp/flyology-uring-initialization-fallback.data";
      File       : Files.File_Descriptor := Files.Invalid_File;
   ...
   begin
      if At_Point
         not in Fault_Control.File_Uring_Probe_Unsupported | Fault_Control.File_Uring_Post_Setup_Failure
      then
      ...
      if Selected_Linux_Backend = 0 then
         ...
         Files.Close (File);
         Ada.Directories.Delete_File (Path);
         Ada.Text_IO.Put_Line ("io_uring initialization fallback skipped on non-Linux host");
         return;
      elsif Fault_Control.Calls (At_Point) = 0 then
         raise Program_Error with "io_uring initialization fault was not reached";
      elsif Selected_Linux_Backend /= 2 then
         raise Program_Error with "io_uring initialization did not fall back to native AIO";
      elsif not Wrote then
         raise Program_Error with "native-AIO fallback did not complete file I/O";
      end if;

      After_FDs := Open_FD_Count;
      if After_FDs /= Before_FDs + 2 then
         raise Program_Error with "io_uring fallback retained a ring descriptor";
      end if;
      Fault_Control.Reset;
      Files.Close (File);
      Ada.Directories.Delete_File (Path);
   ...
   procedure Test_Cross_Domain_Cancellation is
      ...
      task body Connection_Waiter is
         ...
         Last : Ada.Streams.Stream_Element_Offset;
      begin
         Connections.Receive (Connection, Data, Last, Timeout => IO.Infinite, Token => Token'Access);
         Progress.Done (False);
      exception
      ...
      procedure Free_File_Waiter is new Ada.Unchecked_Deallocation (File_Waiter, File_Waiter_Access);
      procedure Free_Connection_Waiter is new
        Ada.Unchecked_Deallocation (Connection_Waiter, Connection_Waiter_Access);
   ...
   begin
      ...
      Fault_Control.Reset;
      Fault_Control.Arm (Fault_Control.File_Submission_Full, Count => 100_000);
      File_Task := new File_Waiter;
      Connection_Task := new Connection_Waiter;
      declare
         Limit : constant Ada.Real_Time.Time := Ada.Real_Time.Clock + Ada.Real_Time.Seconds (2);
      begin
         while Fault_Control.Calls (Fault_Control.File_Submission_Full) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "file cancellation waiter never entered pending queue";
            end if;
            delay 0.001;
         end loop;
      end;
      ...
      Progress.Wait;
      if not Progress.Passed then
         raise Program_Error with "shared token did not cancel both I/O domains";
      end if;

      while not File_Task.all'Terminated or else not Connection_Task.all'Terminated loop
         delay 0.001;
      end loop;
      ...
      begin
         ...
         begin
            Connections.Receive (Connection, Data, Last, Timeout => 0.01, Token => Token'Access);
         exception
            when Files.Operation_Cancelled =>
               Connection_Caught := True;
         end;
         if not File_Caught or else not Connection_Caught then
            raise Program_Error with "shared token did not preserve serial cancellation identity";
         end if;
      end;
   ...
   begin
      ...
      begin
         while Fault_Control.Calls (Fault_Control.File_Submission_Full) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "aborted file task never entered pending queue";
            end if;
            delay 0.001;
         end loop;
         abort Item.all;
         while not Item.all'Terminated loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "aborted file task retained its caller buffer";
            end if;
            delay 0.001;
         end loop;
      end;
      ...
      begin
         Files.Write_At (File, 0, Data, Last);
         if Last /= Data'Last then
            raise Program_Error with "descriptor/request reuse failed after task abort";
         end if;
      end;
      Files.Close (File);
      Ada.Directories.Delete_File (Path);
   ...
   procedure Test_File_Pre_Park_Abort is
      ...
      procedure Free_Aborter is new Ada.Unchecked_Deallocation (Aborter, Aborter_Access);
   ...
   begin
      ...
      File := Files.Open (Path, Mode => Files.Read_Write, Create => True, Truncate => True);
      Fault_Control.Reset;
      Fault_Control.Arm (Fault_Control.File_Pre_Park, Count => 1_000_000_000);
      Item := new Writer;
      declare
         Limit : constant Ada.Real_Time.Time := Ada.Real_Time.Clock + Ada.Real_Time.Seconds (2);
      begin
         while Fault_Control.Calls (Fault_Control.File_Pre_Park) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "file operation did not reach the pre-park gate";
            end if;
            delay 0.001;
         end loop;
         ...
         while not Item.all'Terminated or else not Killer.all'Terminated loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "pre-park abort was lost before file suspension";
            end if;
            delay 0.001;
         end loop;
      end;
   ...
   procedure Test_File_Backend_Cancel is
      ...
      task body Writer is
         ...
         Data : constant Data_Access := new Ada.Streams.Stream_Element_Array'(1 .. 1_048_576 => 19);
         Last : Ada.Streams.Stream_Element_Offset;
      begin
      ...
      is (Fault_Control.File_Cancel_Count (Backend, Fault_Control.Submitted, False)
          + Fault_Control.File_Cancel_Count (Backend, Fault_Control.Submitted, True)
          + Fault_Control.File_Cancel_Count (Backend, Fault_Control.Already_Completing, False)
          + Fault_Control.File_Cancel_Count (Backend, Fault_Control.Not_Cancelable, False)
          + Fault_Control.File_Cancel_Count (Backend, Fault_Control.Failed, False));
   begin
      if Ada.Directories.Exists (Path) then
         Ada.Directories.Delete_File (Path);
      end if;
      File := Files.Open (Path, Mode => Files.Read_Write, Create => True, Truncate => True);
      Fault_Control.Reset;
      Fault_Control.Arm (Fault_Control.Poller_EINTR, Count => 1_000_000_000);
      Item := new Writer;
      ...
      begin
         while Fault_Control.Calls (Fault_Control.File_Submission_Full) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "backend cancellation test did not submit file I/O";
            end if;
            delay 0.001;
         end loop;
         abort Item.all;
         while Total = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "owning loop did not invoke the file cancel backend";
            end if;
            delay 0.001;
         end loop;
         ...
         while not Item.all'Terminated loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "backend cancellation retained the file buffer";
            end if;
            delay 0.001;
         end loop;
      end;
      ...
      if Backend = Fault_Control.Linux_IO_Uring
        and then Fault_Control.File_Cancel_Count (Backend, Fault_Control.Submitted, False) = 0
      then
         raise Program_Error with "io_uring asynchronous cancellation was not submitted";
      end if;
      ...
      if Backend = Fault_Control.Linux_Native_AIO then
         if Fault_Control.File_Cancel_Count (Backend, Fault_Control.Failed, False) /= 0 then
            raise Program_Error with "native AIO cancellation reported a transport failure";
         end if;
         if Fault_Control.File_Cancel_Count (Backend, Fault_Control.Not_Cancelable, False) = 0 then
            raise Program_Error with "native AIO cancellation never reported not-cancelable";
         end if;
      end if;
   ...
   procedure Test_File_Cancel_Fallback (Disposition : Fault_Control.Point) is
      ...
      task body Writer is
         Data : constant Ada.Streams.Stream_Element_Array (1 .. 64 * 1_024) := (others => 12);
         Last : Ada.Streams.Stream_Element_Offset;
      begin
         Files.Write_At (File, 0, Data, Last);
      end Writer;
   ...
   begin
      if Disposition
         not in Fault_Control.File_Cancel_Not_Cancelable | Fault_Control.File_Cancel_Already_Completing
      then
      ...
      begin
         while Fault_Control.Calls (Fault_Control.File_Submission_Full) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "fallback test did not submit a kernel file request";
            end if;
            delay 0.001;
         end loop;
         abort Item.all;
         while Fault_Control.Calls (Disposition) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "owning loop did not exercise cancellation disposition";
            end if;
            delay 0.001;
         end loop;
         ...
         while not Item.all'Terminated loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "fallback cancellation resumed before terminal completion";
            end if;
            delay 0.001;
         end loop;
         if Fault_Control.Calls (Disposition) /= 1 then
            raise Program_Error with "cancellation disposition was attempted more than once";
         end if;
      end;
      Free (Item);
      Fault_Control.Reset;
   ...
   procedure Test_Uring_Request_Identity is
      ...
      task body Writer is
         type Data_Access is access Ada.Streams.Stream_Element_Array;
         First_Data  : constant Data_Access := new Ada.Streams.Stream_Element_Array'(1 .. 1_048_576 => 31);
         Second_Data : constant Ada.Streams.Stream_Element_Array := [1 .. 4_096 => 47];
         Last        : Ada.Streams.Stream_Element_Offset;
      begin
         begin
   ...
   begin
      ...
      Fault_Control.Reset;
      Fault_Control.Arm (Fault_Control.Poller_EINTR, Count => 1_000_000_000);
      Fault_Control.Arm (Fault_Control.File_Cancel_Admin_Delay, Count => 1_000_000_000);
      Item := new Writer;
      declare
         Limit : constant Ada.Real_Time.Time := Ada.Real_Time.Clock + Ada.Real_Time.Seconds (5);
      begin
         ...
         Token.Request;
         while Fault_Control.File_Cancel_Count (Fault_Control.Linux_IO_Uring, Fault_Control.Submitted, False)
           = 0
         loop
            if Ada.Real_Time.Clock >= Limit then
         ...
         while Fault_Control.Uring_Identity_Count (False) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "io_uring follow-up did not overlap delayed cancellation";
            end if;
            delay 0.001;
         end loop;
         ...
         while not Item.all'Terminated loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "same-fiber io_uring follow-up did not terminate";
            end if;
            delay 0.001;
         end loop;
      ...
      if Stage /= 2
        or else Fault_Control.Calls (Fault_Control.File_Cancel_Admin_Delay) = 0
        or else Fault_Control.Uring_Identity_Count (False) = 0
        or else Fault_Control.Uring_Identity_Count (True) /= 0
      then
         raise Program_Error with "io_uring reused an operation identity before cancel CQE terminality";
      end if;
      Free (Item);
      Fault_Control.Reset;
      Files.Close (File);
   ...
   procedure Test_Uring_Last_Fiber_Admin is
      ...
      task body Writer is
         type Data_Access is access Ada.Streams.Stream_Element_Array;
         Data : constant Data_Access := new Ada.Streams.Stream_Element_Array'(1 .. 1_048_576 => 61);
         Last : Ada.Streams.Stream_Element_Offset;
      begin
         begin
   ...
   begin
      ...
      Fault_Control.Arm (Fault_Control.File_Cancel_Admin_Delay, Count => 2);
      Item := new Writer;
      declare
         Limit : constant Ada.Real_Time.Time := Ada.Real_Time.Clock + Ada.Real_Time.Seconds (5);
      begin
         ...
         while Fault_Control.Uring_Admin_Complete_Count = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "idle loop did not drain late io_uring cancellation";
            end if;
            delay 0.001;
         end loop;
      end;
      if Stage /= 1 or else Fault_Control.Calls (Fault_Control.File_Cancel_Admin_Delay) < 3 then
         raise Program_Error with "late io_uring cancellation did not cross last-fiber quiescence";
      end if;
      Free (Item);
      Fault_Control.Reset;
      Files.Close (File);
   ...
   procedure Test_Darwin_Cancel_Cleanup (Delete_Fault : Fault_Control.Point) is
      ...
      task body Writer is
         type Data_Access is access Ada.Streams.Stream_Element_Array;
         Data : constant Data_Access := new Ada.Streams.Stream_Element_Array'(1 .. 1_048_576 => 53);
         Last : Ada.Streams.Stream_Element_Offset;
      begin
         Files.Write_At (File, 0, Data.all, Last);
      end Writer;
   ...
   begin
      if Selected_Linux_Backend /= 0 then
         return;
            ...
            not in Fault_Control.File_Cancel_Delete_EINTR | Fault_Control.File_Cancel_Delete_Failure
      then
      ...
      begin
         ...
         while Fault_Control.Calls (Delete_Fault) = 0 loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "Darwin terminal cleanup fault was not exercised";
            end if;
            delay 0.001;
         end loop;
         Fault_Control.Disarm (Fault_Control.Poller_EINTR);
         while not Item.all'Terminated loop
            if Ada.Real_Time.Clock >= Limit then
               raise Program_Error with "Darwin delete failure prevented terminal AIO reap";
            end if;
            delay 0.001;
         end loop;
         if Delete_Fault = Fault_Control.File_Cancel_Delete_Failure then
            while Fault_Control.Calls (Fault_Control.File_Cancel_Stale_Event) = 0 loop
               if Ada.Real_Time.Clock >= Limit then
                  raise Program_Error with "Darwin stale AIO event was not consumed";
               end if;
               delay 0.001;
            end loop;
         end if;
...
begin
   if not Fault_Control.Enabled then
      raise Program_Error with "fault test requires FLYOLOGY_TEST_FAULTS=1 runtime";
   end if;
   ...
   if Case_Name = "fiber-allocation" then
      ...
      Test_Uring_Initialization_Fallback (Fault_Control.File_Uring_Probe_Unsupported);
   elsif Case_Name = "file-uring-post-setup-fallback" then
      Test_Uring_Initialization_Fallback (Fault_Control.File_Uring_Post_Setup_Failure);
      ...
      Test_File_Cancel_Fallback (Fault_Control.File_Cancel_Not_Cancelable);
      Test_File_Cancel_Fallback (Fault_Control.File_Cancel_Already_Completing);
      ...
      Test_Darwin_Cancel_Cleanup (Fault_Control.File_Cancel_Delete_EINTR);
      Test_Darwin_Cancel_Cleanup (Fault_Control.File_Cancel_Delete_Failure);
