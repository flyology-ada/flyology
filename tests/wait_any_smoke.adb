   ...
   begin
      ...
      declare
         Requests : constant Flyology.IO.Wait_Request_Array (7 .. 8) :=
           [7 => (Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Read),
            8 => (Flyology.IO.Sockets.Native_Descriptor (Left_2), Flyology.IO.For_Read)];
      begin
      ...
      begin
         Passed :=
           ...
           and then Flyology.IO.Wait_Interruptibly
                      (Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Read, 1.0, Interrupts)
                    = Flyology.IO.Interrupted;
      end;
      ...
      declare
         Requests : constant Flyology.IO.Wait_Request_Array (4 .. 5) :=
           [others => (Flyology.IO.Sockets.Native_Descriptor (Left_2), Flyology.IO.For_Read)];
      begin
         Passed := Passed and then Flyology.IO.Wait_Any (Requests, 0.0) = 4;
      end;

      declare
         Requests : Flyology.IO.Wait_Request_Array (1 .. Flyology.IO.Max_Wait_Requests);
         Selected : Natural;
      begin
         Requests := [others => (Flyology.IO.Sockets.Native_Descriptor (Left_2), Flyology.IO.For_Read)];
         ...
         if Selected /= 1 then
            raise Program_Error with "full readiness set selected" & Natural'Image (Selected);
         end if;
      end;
      ...
      declare
         Requests : constant Flyology.IO.Wait_Request_Array :=
           [1 => (Flyology.IO.Invalid_Descriptor, Flyology.IO.For_Read),
            2 => (Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Write)];
      ...
      begin
         Flyology.IO.Sockets.Create_Socket_Pair (Closed_Left, Closed_Right);
         declare
         ...
         begin
            ...
            begin
               declare
                  Ignored : constant Natural := Flyology.IO.Wait_Any (Requests, 0.1);
                  pragma Unreferenced (Ignored);
               begin
                  null;
               end;
      ...
      declare
         Requests : constant Flyology.IO.Wait_Request_Array (1 .. 2) :=
           [1 => (Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Read),
            2 => (Flyology.IO.Sockets.Native_Descriptor (Left_1), Flyology.IO.For_Write)];
      begin
...
begin
   ...
   declare
      ...
      begin
         for Index in Left'Range loop
            Flyology.IO.Sockets.Create_Socket_Pair (Left (Index), Right (Index));
            Flyology.IO.Sockets.Prepare (Left (Index));
            Requests (Index) := (Flyology.IO.Sockets.Native_Descriptor (Left (Index)), Flyology.IO.For_Read);
         end loop;

         OK := Flyology.IO.Wait_Any (Requests, 0.005) = 0;
         Flyology.IO.Sockets.Send_Socket (Right (8), Data, Last);
         OK := OK and then Last = Data'Last and then Flyology.IO.Wait_Any (Requests, 1.0) = 8;
   ...
   declare
      ...
      task body Abortable_Waiter is
         Requests : constant Flyology.IO.Wait_Request_Array := [1 => (FD, Flyology.IO.For_Read)];
         Ignored  : Natural;
         pragma Unreferenced (Ignored);
      begin
         Started.Set;
   ...
   begin
      ...
      declare
         FD     : constant Flyology.IO.Descriptor := Flyology.IO.Sockets.Native_Descriptor (Left);
