...
procedure Stack_Size_Limits_Smoke is
   ...
   function Run_Child (Program : Strings.chars_ptr; Scenario : Strings.chars_ptr) return C.int;
   pragma Import (C, Run_Child, "flyology_test_run_stack_size_child");

   Program : Strings.chars_ptr :=
     Strings.New_String
       (Ada.Directories.Compose
          (Ada.Directories.Containing_Directory (Ada.Command_Line.Command_Name), "stack_size_limits_child"));
      ...
      if Status /= 0 then
         Ada.Text_IO.Put_Line
           (Ada.Text_IO.Standard_Error,
            "stack-size scenario " & Scenario & " reported status " & Status'Image);
         Strings.Free (Program);
         raise Program_Error with "unrepresentable stack size was not refused cleanly";
      end if;
   end Run;
begin
   Run ("round-up-wrap");
