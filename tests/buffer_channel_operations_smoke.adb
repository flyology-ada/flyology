...
procedure Buffer_Channel_Operations_Smoke is
   ...
   task body Runner is
      ...
      procedure Acquire_Tagged
        (Item : in out Buffers.Unique_Buffer;
         Tag  : Interfaces.Unsigned_64) is
      begin
   ...
   begin
      ...
      begin
         ...
         Check (not Buffers.Has_Buffer (Outgoing),
                "synchronous send retained ownership");
         loop
      ...
      declare
         ...
         Left : aliased Channels.Receive_Operation :=
           Channels.Receive_Move
             (Set'Access, Queue'Unchecked_Access, 1.0);
         ...
         Batch : Flyology.Operations.Completion_Batch (Set.Capacity);
         Matches : Flyology.Operations.Completion_Batch (Set.Capacity);
         First, Second, Left_Value, Right_Value :
           Buffers.Unique_Buffer (Storage'Access);
      ...
      begin
         ...
         declare
            Put : Channels.Send_Operation :=
              Channels.Send_Move
                (Set'Access, Queue'Unchecked_Access, Outgoing, 22, 1.0);
         begin
            Check (not Buffers.Has_Buffer (Outgoing),
                   "pending send did not move ownership");
            Channels.Try_Receive_Move (Queue, Drained, Result, Metadata);
            Check (Result = Channels.Item_Received
                     and then Buffers.Tag (Drained) = 201,
                   "capacity release missed filler");
            Buffers.Release (Drained);
            Flyology.Operations.Wait_All (Set);
            Channels.Finish (Put, Outgoing);
            Check (not Buffers.Has_Buffer (Outgoing),
                   "successful send Finish restored transferred ownership");
         end;
         Channels.Try_Receive_Move (Queue, Drained, Result, Metadata);
         Passed := Passed
      ...
      declare
         ...
         Filler, Timed_Value, Cancelled_Value, Closed_Value, Drained :
           Buffers.Unique_Buffer (Storage'Access);
         Wrong_Pool : Buffers.Unique_Buffer (Other_Storage'Access);
         Result : Channels.Try_Receive_Result;
      ...
      begin
         ...
         declare
            Put : Channels.Send_Operation :=
              Channels.Send_Move
                (Set'Access, Queue'Unchecked_Access,
                 Timed_Value, Timeout => 0.005);
         begin
            Flyology.Operations.Wait_All (Set);
            begin
               Channels.Finish (Put, Wrong_Pool);
            exception
               when Program_Error => Pool_Mismatch := True;
            end;
            Check (Flyology.Operations.Is_Terminal (Put),
                   "wrong-pool Finish consumed the send operation");
            begin
         ...
         declare
            Put : Channels.Send_Operation :=
              Channels.Send_Move
                (Set'Access, Queue'Unchecked_Access, Cancelled_Value);
         begin
            Flyology.Operations.Cancel (Put);
            begin
               Channels.Finish (Put, Cancelled_Value);
            exception
               when Channels.Operation_Cancelled => Was_Cancelled := True;
            end;
         end;
         ...
         declare
            Put : Channels.Send_Operation :=
              Channels.Send_Move
                (Set'Access, Queue'Unchecked_Access, Closed_Value);
         begin
            ...
            exception
               when Channels.Channel_Closed => Was_Closed := True;
            end;
         end;
         Passed := Passed
           and then Timed_Out
         ...
         Check (Result = Channels.Item_Received,
                "buffer channel filler disappeared");
         Buffers.Release (Drained);
      end;
      ...
      begin
         for Pass in 1 .. 2 loop
            ...
            Channels.Send_Move
              (Queue'Unchecked_Access, Value,
               Channels.Transfer_Metadata (60 + Pass), 0.0, Put);
            Flyology.Operations.Wait_All (Set);
            Channels.Finish (Put, Value);
            Check (not Buffers.Has_Buffer (Value),
                   "restarted send restored successful ownership");

            Channels.Receive_Move
              (Queue'Unchecked_Access, 0.0, Get);
            Flyology.Operations.Wait_All (Set);
            Channels.Finish (Get, Target, Metadata);
            Passed := Passed
              and then Buffers.Tag (Target) =
                Interfaces.Unsigned_64 (500 + Pass)
      ...
      declare
         ...
         Timed : Channels.Receive_Operation :=
           Channels.Receive_Move
             (Set'Access, Queue'Unchecked_Access, 0.005);
      ...
      begin
         ...
         Passed := Passed
           and then Timed_Out and then Was_Closed and then Was_Cancelled
      ...
      begin
         ...
         declare
            Abandoned : Channels.Send_Operation :=
              Channels.Send_Move
                (Set'Access, Queue'Unchecked_Access, Value);
            pragma Unreferenced (Abandoned);
         begin
            null;
         end;
         Check (Buffers.Current (Storage).Outstanding = 1,
                "abandoned send leaked its owned token");
         Channels.Try_Receive_Move (Queue, Drained, Result);
         Check (Result = Channels.Item_Received,
                "abandoned-send filler disappeared");
         Buffers.Release (Drained);
         ...
         Passed := Passed
           and then Buffers.Current (Storage).Outstanding = 0;
      end;
