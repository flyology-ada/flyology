...
procedure TLS_Operations_Smoke is
   ...
   type Synthetic_Phase is
     (Handshake_Phase, Receive_Phase, Send_Phase, Shutdown_Phase);

   type Synthetic_Receive_Operation
     (Set  : not null access Flyology.Operations.Completion_Set'Class;
      Item : not null access TLS.Connection'Class) is
   ...
   begin
      if Event = Flyology.Operations.Deadline_Reached then
         ...
         TLS_Drivers.Start
           (Item.IO, Item.Item, Acquired, Timeout => 1.0);
      ...
      case Item.Phase is
         ...
         when Receive_Phase =>
            TLS_Drivers.Receive
              (Item.IO, Item.Data.all, Item.Last, Step);
         when Send_Phase =>
            TLS_Drivers.Send
              (Item.IO, Item.Data.all, Item.Last, Step);
         when Shutdown_Phase =>
   ...
   begin
      ...
      declare
         ...
         Output  : aliased constant Stream_Element_Array :=
           [1 => 7, 2 => 8];
      begin
         ...
         declare
            ...
            Receive : aliased TLS.Receive_Operation :=
              TLS.Receive
                (Set'Access, Item'Access, One'Access, Timeout => 1.0);
            Receive_All : aliased TLS.Receive_Exactly_Operation :=
              TLS.Receive_Exactly
                (Set'Access, Item'Access, Exact'Access, Timeout => 1.0);
            Send : aliased TLS.Send_All_Operation :=
              TLS.Send_All
                (Set'Access, Item'Access, Output'Access, Timeout => 1.0);
         ...
         begin
            ...
            Check
              (All_Outcome = Flyology.Operations.Succeeded
                 and then Matches.Count = 5,
            ...
            exception
               when Error : others =>
                  raise Program_Error with
                    "scoped TLS send failed: " &
                      Ada.Exceptions.Exception_Information (Error);
            end;
            TLS.Finish (Shutdown);
            Check
              (Last = One'Last and then One = [42]
                 and then Exact = [42, 42],
            ...
            TLS.Handshake
              (Item'Access, Timeout => 1.0, Operation => Handshake);
            TLS.Receive
              (Item'Access, One'Access, Timeout => 1.0,
               Operation => Receive);
            TLS.Receive_Exactly
              (Item'Access, Exact'Access, Timeout => 1.0,
               Operation => Receive_All);
            TLS.Send_All
              (Item'Access, Output'Access, Timeout => 1.0,
               Operation => Send);
            TLS.Shutdown
              (Item'Access, Timeout => 1.0, Operation => Shutdown);
      ...
      declare
         ...
         Peer    : Sockets.Socket_Type;
         Item    : aliased TLS.Connection;
         Empty_In : aliased Stream_Element_Array :=
           Stream_Element_Array'(1 .. 0 => 0);
         Empty_Out : aliased constant Stream_Element_Array :=
           Stream_Element_Array'(1 .. 0 => 0);
      begin
         Provider.Set_Script
         ...
         declare
            ...
            Handshake : TLS.Handshake_Operation :=
              TLS.Handshake (Set'Access, Item'Access, Timeout => 0.0);
            Receive : TLS.Receive_Operation :=
              TLS.Receive
                (Set'Access, Item'Access, Empty_In'Access, Timeout => 0.0);
            Receive_All : TLS.Receive_Exactly_Operation :=
              TLS.Receive_Exactly
                (Set'Access, Item'Access, Empty_In'Access, Timeout => 0.0);
            Send : TLS.Send_All_Operation :=
              TLS.Send_All
                (Set'Access, Item'Access, Empty_Out'Access, Timeout => 0.0);
            Shutdown : TLS.Shutdown_Operation :=
              TLS.Shutdown (Set'Access, Item'Access, Timeout => 0.0);
            Last : Stream_Element_Offset;
         begin
            Flyology.Operations.Wait_All (Set);
            Check
            ...
            TLS.Finish (Shutdown);
            Check
              (Last = Empty_In'First - 1,
               "empty TLS receive returned the wrong bound");
         end;
         TLS.Close (Item);
         Close_If_Open (Peer);
      end;
      ...
      begin
         ...
         begin
            Flyology.Operations.Wait_All (Set);
            Check
              (Flyology.Operations.Outcome (Handshake) =
                 Flyology.Operations.Succeeded,
      ...
      begin
         ...
         begin
            ...
            Check
              (Timed_Out,
               "repeated ready TLS WANT did not retain its timeout");
         end;
         TLS.Close (Item);
         Close_If_Open (Peer);
      end;
      ...
      begin
         ...
         declare
            ...
            Get : TLS.Receive_Operation :=
              TLS.Receive
                (Set'Access, Item'Access, Data'Access, Timeout => 0.010);
            Last : Stream_Element_Offset;
         begin
            Flyology.Operations.Wait_All (Set);
            Check
              (Flyology.Operations.Outcome (Get) =
                 Flyology.Operations.Failed,
      ...
      begin
         ...
         declare
            ...
            Get : TLS.Receive_Operation :=
              TLS.Receive
                (Set'Access, Item'Access, Data'Access,
                 Token => Token'Access);
            Last : Stream_Element_Offset;
         begin
            Flyology.Operations.Wait_All (Set);
            ...
            TLS.Receive
              (Item'Access, Data'Access, Operation => Get);
         ...
         Check
           (Raised and then Data = [77],
            "TLS cancellation did not preserve the pending buffer");
         declare
            Set : aliased Flyology.Operations.Completion_Set (1);
            Abandoned : TLS.Receive_Operation :=
              TLS.Receive (Set'Access, Item'Access, Data'Access);
      ...
      begin
         ...
         declare
            Set : aliased Flyology.Operations.Completion_Set (1);
            Failed : TLS.Handshake_Operation :=
              TLS.Handshake (Set'Access, Item'Access, Timeout => 1.0);
            Batch : Flyology.Operations.Completion_Batch (Set.Capacity);
         begin
      ...
      begin
         ...
         TLS.Take (Backend, Socket, TLS.Server, "", Item);
         TLS_Drivers.Start
           (Holder, Item'Access, Held, Timeout => 1.0);
         ...
         Check
           (Rejected
              and then TLS_Drivers.Is_Engaged (Holder)
              and then TLS_Drivers.Is_Acquired (Holder),
            "rejected TLS driver Start destroyed its existing lease");
         declare
            Set : aliased Flyology.Operations.Completion_Set (1);
            Queued : TLS.Receive_Operation :=
              TLS.Receive (Set'Access, Item'Access, Data'Access);
         ...
         TLS.Close (Item);
         Check
           (not TLS.Is_Open (Item),
            "finalized queued TLS operation leaked its registration");
         Close_If_Open (Peer);
      end;
      ...
      begin
         ...
         TLS.Take (Backend, Socket, TLS.Server, "", Item);
         TLS_Drivers.Start
           (Holder, Item'Access, Result, Timeout => 1.0);
         Check (Result = TLS_Drivers.Acquired, "TLS holder lease unavailable");
         TLS_Drivers.Start
           (Waiter, Item'Access, Result, Timeout => 1.0);
         Check
           (Result = TLS_Drivers.Need_Acquire_Readiness,
            "second TLS capability did not queue");
         declare
            task Closer is
               pragma Task_Info (Model);
            end Closer;
      ...
      begin
         ...
         begin
            ...
            Check
              (TLS_Drivers.Is_Engaged (Get.IO)
                 and then not TLS_Drivers.Is_Acquired (Get.IO),
