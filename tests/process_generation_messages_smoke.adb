...
procedure Process_Generation_Messages_Smoke is
   ...
   Original : constant Messages.Provisioning_Data :=
     (Application_Signature => 16#0102_0304_0506_0708#,
      ...
      Topology_Epoch        => 11,
      Digest                =>
        (0 => 16#AA#, 1 => 16#BB#, 31 => 16#CC#, others => 0),
...
begin
   ...
   Messages.Decode_Provision
     (Payload, Messages.Provision_Length, Copy, Result);
   pragma Assert (Result = Messages.Decoded);
   pragma Assert
     (Copy.Application_Signature = Original.Application_Signature and then
      Copy.Topology_Schema = Original.Topology_Schema and then
      Copy.Topology_Epoch = Original.Topology_Epoch and then
      Copy.Digest = Original.Digest and then
      Copy.Role = Original.Role);

   Messages.Decode_Provision
     (Payload, Messages.Provision_Length - 1, Copy, Result);
   ...
   Messages.Encode_Topology_Proof
     ((Epoch => Original.Topology_Epoch, Digest => Original.Digest), Payload);
   Messages.Decode_Topology_Proof
     (Payload, Messages.Topology_Proof_Length, Proof, Result);
   pragma Assert
     (Result = Messages.Decoded and then
      Proof.Epoch = Original.Topology_Epoch and then
      Proof.Digest = Original.Digest);

   Messages.Encode_Compensation
     (Generations.Irreversible_Effects, Payload);
   Messages.Decode_Compensation
     (Payload, Messages.Compensation_Length, Compensation, Result);
   pragma Assert
     (Result = Messages.Decoded and then
      Compensation = Generations.Irreversible_Effects);
   Payload (0) := 0;
   Messages.Decode_Compensation
     (Payload, Messages.Compensation_Length, Compensation, Result);
   pragma Assert (Result = Messages.Invalid_Value);
