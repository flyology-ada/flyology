...
package body TLS_ALPN_Test_Provider is
   ...
   overriding
   function Selected_Protocol (Item : Test_Session) return String;
   ...
   begin
      if ALPN.Count (Protocols) = 0 then
         return No_Offer;
      elsif ALPN.Count (Protocols) = 1 and then ALPN.Identifier (Protocols, 1) = "h2" then
         return H2_Only;
   ...
   begin
      return
        new Test_Session'
          (TLS.Session with Selection => Item.Selection, Handshake_Status => Item.Handshake_Status);
   end New_Session;

   procedure Set_Selection (Item : in out Provider; Selection : Selection_Kind) is
   begin
      Item.Selection := Selection;
   end Set_Selection;

   procedure Set_Handshake_Status (Item : in out Provider; Status : TLS.Step_Status) is
   begin
   ...
   overriding
   function Selected_Protocol (Item : Test_Session) return String is
   begin
