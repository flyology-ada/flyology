...
procedure File_Transfer_Benchmark is
   ...
   Path          : constant String := Ada.Environment_Variables.Value ("FLYOLOGY_FILE_TRANSFER_BENCH_PATH");
   ...
   function C_Clock_Gettime (Clock_ID : Interfaces.C.int; Value : access C_Timespec) return Interfaces.C.int;
   pragma Import (C, C_Clock_Gettime, "clock_gettime");

   Thread_Clock_ID : constant Interfaces.C.int := (if Flyology_Config.Alire_Host_OS = "linux" then 3 else 16);
   ...
   begin
      ...
      return Duration (Value.Seconds) + Duration (Value.Nanoseconds) / 1_000_000_000;
   end Thread_CPU_Time;
   ...
   pragma Import (C, Observed_Send_ZC, "flyology_transfer_send_zc_observed");

   function Send_ZC_Copy_Fallback return Interfaces.C.int;
   pragma Import (C, Send_ZC_Copy_Fallback, "flyology_send_zc_copy_fallback");
   ...
   procedure Populate is
      Block  : Stream_Element_Array (1 .. Stream_Element_Offset (IO_Block_Size));
   ...
   begin
      ...
      while Offset < Files.File_Offset (Max_Size) loop
         ...
         if Last < Block'First then
            raise Program_Error with "benchmark fixture write made no progress";
         end if;
   ...
   procedure Read_Into
     (Offset : Files.File_Offset;
      ...
      Read   : out Natural)
   is
      procedure Borrow (Data : in out Stream_Element_Array; Length : in out Natural) is
         Last : Stream_Element_Offset;
      begin
         Files.Read_At
           (File,
            Offset,
            Data (Data'First .. Data'First + Stream_Element_Offset (Limit) - 1),
            Last,
            Timeout => 10.0);
   ...
   is
      ...
      Sender_OK       : Boolean := False
      with Atomic;
      Receiver_OK     : Boolean := False
      with Atomic;
      ...
      task body Receiver is
         Buffer    : Stream_Element_Array (1 .. Stream_Element_Offset (IO_Block_Size));
      ...
      begin
         while Remaining > 0 loop
            ...
            Sockets.Receive
              (Receiver_Socket, Buffer (1 .. Stream_Element_Offset (Limit)), Last, Timeout => 10.0);
            ...
            if Validate then
               for Index in Buffer'First .. Last loop
                  Absolute := Bytes - Remaining + Natural (Index - Buffer'First);
                  if Buffer (Index) /= Stream_Element (((Absolute mod IO_Block_Size + 1) * 17 + 11) mod 251)
                  then
                     raise Program_Error with "benchmark receiver payload mismatch";
                  end if;
               end loop;
            end if;
      ...
      task body Sender is
         Storage     : aliased Flyology.Buffers.Pool (Block_Size => Scratch_Size, Capacity => 1);
      ...
      begin
         ...
         while Offset < Files.File_Offset (Bytes) loop
            if Method = Read_Then_Send then
               Read_Into (Offset, Positive'Min (Bytes - Natural (Offset), Scratch_Size), Scratch, Read);
               if Read = 0 then
               ...
               Transfers.Send_Chunk
                 (File,
                  ...
                  Timeout => 10.0);
               if Sent = 0 then
                  raise Program_Error with "benchmark transfer made no progress";
               end if;
   ...
   begin
      ...
      Sockets.Accept_Connection (Listener, Receiver_Socket, Peer, Timeout => 10.0);
   ...
   function Median (Values : Duration_Samples; Count : Positive) return Duration is
   ...
   function Median (Values : Ratio_Samples; Count : Positive) return Long_Float is
   ...
   function Paired_Measurements
     (Kind : Flyology.Execution_Model; Bytes : Positive; Runs : Positive) return Paired_Summary
   is
   ...
   begin
      ...
      Warmup_Baseline := Measure (Kind, Read_Then_Send, Bytes, Validate => True);
      Warmup_Optimized := Measure (Kind, Reduced_Copy, Bytes, Validate => True);
      for Run in 1 .. Runs loop
         ...
         Wall_Ratios (Run) := Long_Float (Baseline.Wall) / Long_Float (Optimized.Wall);
         CPU_Ratios (Run) := Long_Float (Baseline.CPU) / Long_Float (Optimized.CPU);
      end loop;
      if Warmup_Baseline.Wall <= 0.0 or else Warmup_Optimized.Wall <= 0.0 then
         raise Program_Error with "benchmark warmup did not run";
      end if;
      return
        (Baseline     => (Wall => Median (Baseline_Wall, Runs), CPU => Median (Baseline_CPU, Runs)),
         Optimized    => (Wall => Median (Optimized_Wall, Runs), CPU => Median (Optimized_CPU, Runs)),
         Wall_Speedup => Median (Wall_Ratios, Runs),
         CPU_Speedup  => Median (CPU_Ratios, Runs));
   end Paired_Measurements;

   function Rate (Bytes : Positive; Elapsed : Duration) return Long_Float
   is (Long_Float (Bytes) / (1_024.0 * 1_024.0) / Long_Float (Elapsed));

   procedure Run_Lane (Kind : Flyology.Execution_Model; Name : String) is
   ...
   begin
      Put_Line (Name & " task");
      Put_Line ("  size     read+send MiB/s   Send_Chunk MiB/s   wall    CPU");
      for Row of Rows loop
         ...
         Put_Line
           ("  "
            & Integer'Image (Row.Bytes / (1_024 * 1_024))
            & " MiB"
            & "    "
            & Showcase_Support.Fixed_Image (Rate (Row.Bytes, Baseline.Wall), 1)
            & "             "
            & Showcase_Support.Fixed_Image (Rate (Row.Bytes, Optimized.Wall), 1)
            & "             "
            & Showcase_Support.Fixed_Image (Speedup, 2)
            & "x  "
            & Showcase_Support.Fixed_Image (CPU_Speedup, 2)
            & "x");
      end loop;
      if Kind = Flyology.Lightweight_Task then
         case Selected_Linux_Backend is
            when 1      =>
               Put_Line ("  lightweight file backend: io_uring");

            when 2      =>
               Put_Line ("  lightweight file backend: native AIO fallback");

            when others =>
               Put_Line ("  lightweight file backend: Darwin AIO");
         end case;
         if Observed_Send_ZC /= 0 then
            if Send_ZC_Copy_Fallback = 0 then
               Put_Line ("  reduced-copy socket path: SEND_ZC zero-copy observed");
            else
               Put_Line ("  socket path: SEND_ZC kernel copy fallback observed");
            end if;
         else
      ...
      if Best >= 1.05
        and then (Kind = Flyology.Native_Task
                  or else (Observed_Send_ZC /= 0 and then Send_ZC_Copy_Fallback = 0))
      then
         Put_Line
         ...
         Put_Line ("  measured Send_Chunk crossover, but reduced-copy fallback" & " was active");
      else
      ...
      then
         Put_Line
           ("  measured CPU-efficiency crossover: " & Showcase_Support.Fixed_Image (CPU_Best, 2) & "x");
      end if;
   end Run_Lane;

begin
   Remove_File;
   File := Files.Open (Path, Files.Read_Write, Create => True, Truncate => True);
   Populate;
   Put_Line ("regular-file to socket throughput (paired median samples)");
   Put_Line ("  alternating method order; validated warmups; cached file data");
