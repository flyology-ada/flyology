...
procedure Evented_Vs_Threads is
   use Ada.Real_Time;
   use Ada.Text_IO;
   ...
   procedure Report
     (Evented_Elapsed : Duration;
      Native_Elapsed  : Duration)
   is
      Ratio : constant Long_Float :=
        Long_Float (Native_Elapsed) / Long_Float (Evented_Elapsed);
   begin
      Put_Line
        ("  event loop: " & Evented_Elapsed'Image
         & " s (one OS thread)");
      Put_Line
...
begin
   ...
   declare
      ...
      Workers : array (1 .. Yield_Workers) of Evented_Worker;
      pragma Unreferenced (Workers);
   begin
      Completion.Wait;
   end;
   Evented_Elapsed := To_Duration (Clock - Started);
   ...
   Report (Evented_Elapsed, Native_Elapsed);
   ...
   declare
      ...
      Workers : array (1 .. Wait_Workers) of Evented_Waiter;
      pragma Unreferenced (Workers);
   begin
      Completion.Wait;
   end;
