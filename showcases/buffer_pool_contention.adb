...
procedure Buffer_Pool_Contention is
   ...
   function Policy_Name (Value : Pool_Policy) return String
   is (case Value is
         when Shared      => "shared",
         when Partitioned => "partitioned");

   function Workload_Name (Value : Workload_Kind) return String
   is (case Value is
         when Churn => "churn",
         when Touch => "touch");
   ...
   begin
      ...
      raise Constraint_Error with "POLICY must be shared or partitioned";
   end Parse_Policy;
   ...
   protected body Run_Control is
      ...
      entry Await_Finished (Succeeded : out Boolean) when Finished_Count = Worker_Count is
      begin
         Succeeded := Is_Valid;
      end Await_Finished;
   end Run_Control;
   ...
   task body Producer_Task is
      ...
      procedure Touch_Buffer (Data : in out Ada.Streams.Stream_Element_Array; Length : in out Natural) is
      begin
         Data (Data'First) := 42;
         Length := 1;
      end Touch_Buffer;
   ...
   task body Consumer_Task is
      ...
      procedure Check_Buffer (Data : Ada.Streams.Stream_Element_Array) is
      begin
   ...
   procedure Free_Pool is new Ada.Unchecked_Deallocation (Buffers.Pool, Pool_Access);
   procedure Free_Channel is new Ada.Unchecked_Deallocation (Buffer_Channels.Channel, Channel_Access);
   procedure Free_Control is new Ada.Unchecked_Deallocation (Run_Control, Run_Control_Access);
   ...
   procedure Report
     (Policy      : Pool_Policy;
      ...
      Elapsed     : Duration)
   is
      Total : constant Long_Long_Integer := Long_Long_Integer (Group_Count) * Long_Long_Integer (Iterations);
      Rate  : constant Long_Float := Long_Float (Total) / Long_Float (Elapsed);
   begin
      Ada.Text_IO.Put
        (Policy_Name (Policy)
         & ","
   ...
   procedure Run
     (Policy : Pool_Policy; Workload : Workload_Kind; Group_Count : Positive; Iterations : Positive)
   is
      Block_Size       : constant Positive := (if Workload = Churn then 64 else 4_096);
      Pool_Count       : constant Positive := (if Policy = Shared then 1 else Group_Count);
      Pool_Capacity    : constant Positive :=
        (if Policy = Shared then Pair_Capacity * Group_Count else Pair_Capacity);
      ...
      Control          : Run_Control_Access := new Run_Control (Worker_Count => 2 * Group_Count);
      Started, Stopped : RT.Time;
      Succeeded        : Boolean;
   begin
      for Index in Pools'Range loop
         Pools (Index) := new Buffers.Pool (Block_Size => Block_Size, Capacity => Pool_Capacity);
      end loop;
      for Index in Queues'Range loop
         declare
            Pool_Index : constant Positive := (if Policy = Shared then 1 else Index);
         begin
            Queues (Index) := new Buffer_Channels.Channel (Pools (Pool_Index), Queue_Capacity);
            Producers (Index) :=
              new Producer_Task
      ...
      for Index in Producers'Range loop
         while not Producers (Index).all'Terminated or else not Consumers (Index).all'Terminated loop
            delay 0.0;
         end loop;
      ...
      Report (Policy, Workload, Group_Count, Block_Size, Iterations, RT.To_Duration (Stopped - Started));
   end Run;

begin
   if CLI.Argument_Count /= 4 then
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error, "usage: buffer_pool_contention POLICY GROUPS ITERATIONS WORKLOAD");
   ...
   declare
      ...
      Workload    : constant Workload_Kind := Parse_Workload (CLI.Argument (4));
   begin
      if Group_Count > Positive (Groups.Configured_Pool_Size) then
         raise Constraint_Error with "GROUPS exceeds the configured execution-group pool";
      end if;
...
exception
   when Error : others =>
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error, "buffer_pool_contention: " & Ada.Exceptions.Exception_Message (Error));
      CLI.Set_Exit_Status (CLI.Failure);
