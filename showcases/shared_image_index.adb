...
procedure Shared_Image_Index is
   ...
   pragma Import (C, Socketpair, "flyology_showcase_image_socketpair");
   ...
   pragma Import (C, Spawn_Worker, "flyology_showcase_spawn_image_worker");
   function Poll_Worker (PID : C.int) return C.int;
   pragma Import (C, Poll_Worker, "flyology_showcase_poll_image_worker");
   ...
   function Image (Value : DS.Byte_Count) return String
   is (Trimmed (DS.Byte_Count'Image (Value)));
   function Image (Value : Model.U64) return String
   is (Trimmed (Model.U64'Image (Value)));
   ...
   function Wide (Value : String) return Wide_Wide_String
   is (Ada.Characters.Conversions.To_Wide_Wide_String (Value));
   function Batch_Token (Batch : Model.U64; Worker_Limit : Natural) return Ada.Streams.Stream_Element_Array is
      Result : Ada.Streams.Stream_Element_Array (1 .. 16);
      Limit  : constant Model.U64 := Model.U64 (Worker_Limit);
   begin
      for Byte_Index in 0 .. 7 loop
         Result (Result'First + Ada.Streams.Stream_Element_Offset (Byte_Index)) :=
           Ada.Streams.Stream_Element (Interfaces.Shift_Right (Batch, Byte_Index * 8) and 16#FF#);
         Result (Result'First + Ada.Streams.Stream_Element_Offset (Byte_Index + 8)) :=
           Ada.Streams.Stream_Element (Interfaces.Shift_Right (Limit, Byte_Index * 8) and 16#FF#);
      end loop;
      return Result;
   end Batch_Token;
   ...
   is ((Value + 63) / 64 * 64);
   function Align_Mapping (Value : DS.Byte_Count) return DS.Byte_Count
   is ((Value + 65_535) / 65_536 * 65_536);
   function Segment_Length (Index_Capacity : Positive) return DS.Byte_Count is
      Total : DS.Byte_Count := Segments.Required_Registry_Storage (Segment_Config);
   begin
      Total := Align_64 (Total) + Align_64 (Job_Rings.Required_Storage (Model.Job_Ring_Capacity));
      Total := Align_64 (Total) + Align_64 (Result_Rings.Required_Storage (Model.Result_Ring_Capacity));
      Total := Align_64 (Total) + Align_64 (Image_Maps.Required_Storage (Index_Capacity));
      Total := Align_64 (Total) + Align_64 (Strings.Required_Storage (Model.Gate_Capacity));
      Total := Align_64 (Total) + Align_64 (Strings.Required_Storage (Model.Race_Capacity));
   ...
   function Interactive_Terminal return Boolean
   is (Is_A_Terminal (0) = 1
   ...
   procedure Begin_Dashboard (Terminal_Width, Terminal_Height : out Natural) is
   ...
   begin
      ...
      if Interactive_Terminal then
         ...
         TUI_POSIX.Current_Size (Dashboard_Terminal, Terminal_Width, Terminal_Height, Available);
   ...
   Muted_Style        : constant TUI_Styles.Style :=
     TUI_Styles.With_Foreground (TUI_Styles.Default, TUI_Colors.Basic (TUI_Colors.Bright_Black));
   Border_Style       : constant TUI_Styles.Style := Muted_Style;
   Title_Style        : constant TUI_Styles.Style :=
     TUI_Styles.Emphasized
       (TUI_Styles.With_Foreground (TUI_Styles.Default, TUI_Colors.True_Color (90, 86, 224)));
   Movement_Style     : constant TUI_Styles.Style :=
     TUI_Styles.With_Foreground (TUI_Styles.Default, TUI_Colors.True_Color (117, 113, 249));
   Completion_Style   : constant TUI_Styles.Style :=
     TUI_Styles.Emphasized
       (TUI_Styles.With_Foreground (TUI_Styles.Default, TUI_Colors.True_Color (0, 170, 118)));
   Warning_Style      : constant TUI_Styles.Style :=
     TUI_Styles.Emphasized
       (TUI_Styles.With_Foreground (TUI_Styles.Default, TUI_Colors.True_Color (214, 139, 44)));
   Error_Style        : constant TUI_Styles.Style :=
     TUI_Styles.Emphasized
       (TUI_Styles.With_Foreground (TUI_Styles.Default, TUI_Colors.True_Color (198, 40, 80)));
   ...
   type Extent_Id is (Pending_Jobs, Completed_Results, Image_Index, Start_Gate, Startup_Race);
   ...
   function Extent_Name (Id : Extent_Id) return String
   is (case Id is
   ...
   function Extent_Kind (Id : Extent_Id) return String
   is (case Id is
         when Pending_Jobs | Completed_Results => "MPMC ring",
         when Image_Index                      => "hash map",
         when Start_Gate | Startup_Race        => "byte string");

   type Segment_Column is (Name_Column, Offset_Column, Length_Column, Kind_Column, Activity_Column);
   ...
   function Segment_Row_Id (Item : Segment_Row) return Extent_Id
   is (Item.Id);

   function Segment_Cell (Item : Segment_Row; Column : Segment_Column) return Wide_Wide_String
   is (case Column is
         when Name_Column     => Wide (Extent_Name (Item.Id)),
         when Offset_Column   => Wide ("@" & Image (DS.Byte_Count (Item.Location))),
         when Length_Column   => Wide ("+" & Image (Item.Length)),
         when Kind_Column     => Wide (Extent_Kind (Item.Id)),
         when Activity_Column => Wide_Text.To_Wide_Wide_String (Item.Activity));

   function Segment_Less (Left, Right : Segment_Row; Column : Segment_Column) return Boolean is
      pragma Unreferenced (Column);
   begin
      return Left.Id < Right.Id;
   end Segment_Less;

   package Segment_Tables is new
   ...
   function Framed_Panel
     (Title : String; Contents : TUI_Surfaces.Surface; Width, Height : Natural) return TUI_Surfaces.Surface
   is
      Inner_Width  : constant Natural := (if Width > 4 then Width - 4 else 0);
      Inner_Height : constant Natural := (if Height > 4 then Height - 4 else 0);
      Content      : TUI_Surfaces.Surface := TUI_Surfaces.Create (Inner_Width, Inner_Height);
   ...
   begin
      ...
      if Inner_Height > 1 then
         Content.Overlay (Indicators.Divider (Inner_Width, Theme => Dashboard_Theme), 0, 1);
      end if;
   ...
   is
      ...
      Valid  : constant TUI_Surfaces.Surface :=
        Indicators.Badge ("VALID", Indicators.Success_Tone, Dashboard_Theme);

      function Activity_For (Id : Extent_Id) return String
      is (case Id is
            when Pending_Jobs      => "push " & Image (Queued) & " / pressure " & Image (Queue_Pressure),
            when Completed_Results => "drain " & Image (Completed) & "/" & Image (Total),
            when Image_Index       => "publish " & Image (Completed) & " / retries " & Image (Index_Retries),
            when Start_Gate        =>
              "attached "
              & Image (Workers_Ready)
              & "/"
              ...
              & " / done "
              & Image (Workers_Done),
            when Startup_Race      => "creator 1 / retries " & Image (Registry_Retries));

      function Make_Row (Id : Extent_Id) return Segment_Row
      is (Id       => Id,
          Location => Layout (Id).Location,
          Length   => Layout (Id).Length,
          Activity => Wide_Text.To_Unbounded_Wide_Wide_String (Wide (Activity_For (Id))));
      ...
      Table_Y          : constant Natural := (if Height >= 8 then 2 elsif Height >= 7 then 1 else 0);
      Available_Height : constant Natural := Height - Table_Y;
      Visible_Rows     : constant Natural :=
        (if Available_Height > 1 then Natural'Min (5, Available_Height - 1) else 0);
      Column_Space     : constant Natural := (if Width > 6 then Width - 6 else 0);
      ...
      Columns          : constant Segment_Tables.Column_Definitions :=
        [Name_Column     =>
           (Heading       => Wide_Text.To_Unbounded_Wide_Wide_String ("Name"),
            Width         => Name_Width,
            Minimum_Width => 0,
            Align         => Segment_Tables.Align_Left,
            Sortable      => False),
         Offset_Column   =>
           (Heading       => Wide_Text.To_Unbounded_Wide_Wide_String ("Offset"),
            Width         => Offset_Width,
            Minimum_Width => 0,
            Align         => Segment_Tables.Align_Right,
            Sortable      => False),
         Length_Column   =>
           (Heading       => Wide_Text.To_Unbounded_Wide_Wide_String ("Length"),
            Width         => Length_Width,
            Minimum_Width => 0,
            Align         => Segment_Tables.Align_Right,
            Sortable      => False),
         Kind_Column     =>
           (Heading       => Wide_Text.To_Unbounded_Wide_Wide_String ("Kind"),
            Width         => Kind_Width,
            Minimum_Width => 0,
            Align         => Segment_Tables.Align_Left,
            Sortable      => False),
         Activity_Column =>
           (Heading       => Wide_Text.To_Unbounded_Wide_Wide_String ("Activity"),
            Width         => Activity_Width,
            Minimum_Width => 0,
            Align         => Segment_Tables.Align_Left,
            Sortable      => False)];
      Table            : Segment_Tables.Model :=
        Segment_Tables.Create (Rows, Columns, Viewport_Rows => Visible_Rows, Enabled => True);
   ...
   begin
      ...
      if Table_Y > 0 and then Width > 9 then
         Result.Write
           (9, 0, Wide ("SHOWIMG/1  " & Image (Mapped_Length) & " bytes  64-byte alignment"), Muted_Style);
      end if;
      if Table_Y > 1 then
         Result.Write (0, 1, "registry: fixed capacity, exact names, stored offsets", Muted_Style);
      end if;
      Segment_Tables.Select_Id (Table, Image_Index);
      Result.Overlay (Segment_Tables.Render (Table, Table_Look), 0, Table_Y);
      return Result;
   end Segment_Body;
   procedure Put_Stored_Layout (Layout : Segment_Layout; Mapped_Length : DS.Byte_Count; File : IO.File_Type)
   is
   begin
      IO.Put_Line (File, "  stored segment layout: " & Image (Mapped_Length) & " mapped bytes");
   ...
   is
      ...
      Rate    : constant Long_Float :=
        (if Elapsed <= 0.0 then 0.0 else Long_Float (Completed) / Long_Float (Elapsed));
      MiB     : constant Long_Float :=
        Long_Float (Completed)
        * Long_Float (Width)
        * Long_Float (Height)
        * 3.0
        * Long_Float (Passes)
        / 1_048_576.0;
      function Workload_Body (Content_Width, Content_Height : Natural) return TUI_Surfaces.Surface is
         Result      : TUI_Surfaces.Surface := TUI_Surfaces.Create (Content_Width, Content_Height);
         State_Label : constant Wide_Wide_String :=
           (if Stop_Pending
            then "STOPPING"
            elsif Generator_Paused
            then "BACKOFF"
            elsif Source_Waiting
            then "SOURCE GAP"
            else "FLOW");
         ...
         Gauge_Width : constant Natural := (if Content_Width > 7 then Content_Width - 7 else Content_Width);
         Progress    : constant Indicators.Ratio :=
           Indicators.Ratio (Long_Float (Completed) / Long_Float (Total));
         procedure Put_Value (Row : Natural; Key, Value : String) is
         begin
            if Row < Content_Height then
               Result.Overlay
                 (Indicators.Key_Value (Wide (Key), Wide (Value), Content_Width, Dashboard_Theme), 0, Row);
            end if;
         end Put_Value;
      begin
         ...
         if Content_Height > 1 then
            Result.Write
              (0,
               1,
               (if Stop_Pending
                then "generation stopped; admitted work is draining"
                elsif Generator_Paused
                then "producer paused at saturation; extra workers are draining"
                elsif Source_Waiting
                then "irregular upstream gap; workers continue consuming"
                else "generation and indexing share one bounded pipeline"),
               Muted_Style);
         end if;
         if Content_Height > 2 then
            Result.Overlay (Indicators.Gauge (Progress, Gauge_Width, Dashboard_Theme), 0, 2);
            if Content_Width > Gauge_Width then
               Result.Write (Gauge_Width + 1, 2, Wide (Pad (Image (Percent) & "%", 6)), Title_Style);
            end if;
         end if;
         ...
         Put_Value
           (7,
            "throughput",
            Showcase_Support.Fixed_Image (Rate, 1)
            & " images/s  "
            & Showcase_Support.Fixed_Image ((if Elapsed <= 0.0 then 0.0 else MiB / Long_Float (Elapsed)), 1)
            & " MiB/s");
         Put_Value
           (8, "flow", Image (Producer_Backoffs) & " backoffs  " & Image (Source_Gaps) & " source gaps");
         Put_Value
           (9,
            "contention",
            ...
            & Image (Index_Retries)
            & " index  "
            & Image (Registry_Retries)
            & " registry");
         return Result;
      end Workload_Body;
      function Workers_Body (Content_Width, Content_Height : Natural) return TUI_Surfaces.Surface is
         Result         : TUI_Surfaces.Surface := TUI_Surfaces.Create (Content_Width, Content_Height);
         Available_Rows : constant Natural := (if Content_Height > 2 then Content_Height - 2 else 0);
         Visible_Rows   : constant Natural := Natural'Min (Maximum_Worker_Count, Available_Rows);
         ...
         Expected       : constant Positive :=
           Natural'Max (1, (Total + Initial_Worker_Count - 1) / Initial_Worker_Count);
         Gauge_Width    : constant Natural := (if Content_Width > 29 then Content_Width - 29 else 0);
      begin
         if Content_Height = 0 or else Content_Width = 0 then
            return Result;
         end if;
           ...
           (0,
            ...
            Wide
              (Image (Workers_Ready)
               ...
               & Image (Active_Worker_Count)
               & " active  "
               & Image (Maximum_Worker_Count)
               & " slots"),
            Muted_Style);
         if Content_Height > 1 then
            Result.Overlay (Indicators.Divider (Content_Width, Theme => Dashboard_Theme), 0, 1);
         end if;
         if Visible_Rows > 0 then
            for Worker in 1 .. Visible_Rows loop
               declare
                  Row         : constant Natural := Worker + 1;
                  Active      : constant Boolean := Worker <= Active_Worker_Count;
                  Count       : constant Natural := Per_Worker (Worker);
                  Fraction    : constant Indicators.Ratio :=
                    Indicators.Ratio (Long_Float'Min (1.0, Long_Float (Count) / Long_Float (Expected)));
                  Count_Label : constant String :=
                    (if Active
                     then Image (Count) & " images"
                     elsif Count = 0
                     then "available"
                     else "drained " & Image (Count));
                  Count_X     : constant Natural :=
                    (if Count_Label'Length >= Content_Width then 0 else Content_Width - Count_Label'Length);
               begin
                  Result.Put
                    (0,
                     Row,
                     (if Active then "●" else "○"),
                     (if Active and then Count > 0
                      then Completion_Style
                      elsif Active
                      then Movement_Style
                      else Muted_Style));
                  if Content_Width > 2 then
                     Result.Write
                       (2,
                        Row,
                        Wide ("worker " & Pad (Image (Worker), 2)),
                  ...
                  if Gauge_Width > 0 then
                     ...
                     Result.Overlay (Indicators.Gauge (Fraction, Gauge_Width, Dashboard_Theme), 12, Row);
                  end if;
                  Result.Write
                    (Count_X, Row, Wide (Count_Label), (if Active then Muted_Style else Border_Style));
               end;
            end loop;
         end if;
         if Maximum_Worker_Count > Visible_Rows and then Content_Height > 2 then
            Result.Write
              (0,
               Content_Height - 1,
               Wide ("+" & Image (Maximum_Worker_Count - Visible_Rows) & " worker slots; resize to inspect"),
               Muted_Style);
         end if;
         return Result;
      end Workers_Body;
      function Current_View return TUI_Views.View is
         ...
         Canvas        : TUI_Surfaces.Surface := TUI_Surfaces.Create (Canvas_Width, Canvas_Height);
         Body_Y        : constant Natural := Natural'Min (2, Canvas_Height);
         Body_Height   : constant Natural := (if Canvas_Height > 3 then Canvas_Height - 3 else 0);
         ...
         Status        : constant TUI_Surfaces.Surface :=
           Indicators.Status_Line
             ([Indicators.Make_Segment (Wide ("epoch " & Image (Batch)), Indicators.Critical),
               Indicators.Make_Segment (Wide (State_Label), Indicators.Critical, State_Tone),
               Indicators.Make_Segment
                 (Wide (Image (Percent) & "% indexed"), Indicators.High, Indicators.Success_Tone),
               Indicators.Make_Segment
                 (Wide (Image (Active_Worker_Count) & "/" & Image (Maximum_Worker_Count) & " workers"),
                  Indicators.Normal),
               Indicators.Make_Segment
                 (Wide ("segment valid / 5 extents"), Indicators.Low, Indicators.Success_Tone)],
              Canvas_Width,
              Dashboard_Theme);
      ...
      begin
         ...
         if Canvas_Width > 10 then
            Canvas.Write (10, 0, "shared image index", TUI_Styles.Default);
         end if;
         ...
         if Canvas_Width < 44 or else Canvas_Height < 12 then
            if Canvas_Height > 3 then
               Canvas.Write (0, 3, "Terminal too small for the dashboard", Warning_Style);
            end if;
            ...
            declare
               Right_Width    : constant Natural := Natural'Max (34, Canvas_Width / 3);
               Left_Width     : constant Natural := Canvas_Width - Right_Width - 1;
               Work_Height    : constant Natural := Natural'Max (12, (Body_Height - 1) / 2);
               Segment_Height : constant Natural := Body_Height - Work_Height - 1;
            begin
               Canvas.Overlay
                 (Framed_Panel
                    ("PIPELINE",
                     Workload_Body (Left_Width - 4, (if Work_Height > 6 then Work_Height - 6 else 0)),
                     Left_Width,
                     Work_Height),
                  0,
                  Body_Y);
                    ...
                    ("SHARED SEGMENT",
                     Segment_Body
                       (Layout,
                        ...
                        Registry_Retries,
                        Left_Width - 4,
                        (if Segment_Height > 6 then Segment_Height - 6 else 0)),
                     Left_Width,
                     Segment_Height),
                  0,
                  Body_Y + Work_Height + 1);
               Canvas.Overlay
                 (Framed_Panel
                    ("WORKERS",
                     Workers_Body (Right_Width - 4, (if Body_Height > 6 then Body_Height - 6 else 0)),
                     Right_Width,
                     Body_Height),
                  Left_Width + 1,
                  Body_Y);
            ...
            declare
               Right_Width     : constant Natural := Natural'Max (30, Canvas_Width * 2 / 5);
               Left_Width      : constant Natural := Canvas_Width - Right_Width - 1;
               Overview_Height : constant Natural := Body_Height - 6;
               Work_Rows       : constant Natural := Natural'Min (8, Overview_Height);
            ...
            begin
               Canvas.Overlay
                 (Framed_Panel ("PIPELINE + SEGMENT", Overview, Left_Width, Body_Height), 0, Body_Y);
               Canvas.Overlay
                 (Framed_Panel
                    ("WORKERS",
                     Workers_Body (Right_Width - 4, (if Body_Height > 6 then Body_Height - 6 else 0)),
            ...
            Canvas.Overlay
              (Framed_Panel
                 ("PIPELINE",
                  Workload_Body (Canvas_Width - 4, (if Body_Height > 6 then Body_Height - 6 else 0)),
                  Canvas_Width,
                  Body_Height),
               0,
               Body_Y);
         ...
         if Canvas_Height > 0 then
            Canvas.Write
              (0,
               Canvas_Height - 1,
               (if Stop_Pending
                then "stop requested; draining admitted images"
         ...
         Result.Window_Title := Wide_Text.To_Unbounded_Wide_Wide_String ("Flyology shared image index");
         return Result;
      end Current_View;
   begin
      if Dashboard_Active then
   ...
   begin
      loop
         Segments.Try_Find_Or_Create (Segment, Name, Requested_Length, Handle, Claim, Outcome, Failure);
         exit when Outcome = Segments.Created;
         if Outcome /= Segments.Registry_Busy then
            raise Program_Error with "cannot reserve segment extent " & Name & ": " & Outcome'Image;
         end if;
         delay 0.0;
      end loop;
      Segments.Claimed_Extent (Segment, Claim, Location, Extent);
      if Extent < Requested_Length then
         raise Program_Error with "segment reservation is shorter than requested";
      end if;
   end Reserve;
   procedure Resolve_Ready_Extent
     (Segment : Segments.View; Name : String; Location : out DS.Region_Offset; Length : out DS.Byte_Count)
   is
      Handle  : Segments.Named_Handle;
   ...
   begin
      loop
         ...
         if Outcome not in Segments.Registry_Busy | Segments.Initialization_In_Progress then
            raise Program_Error with "cannot resolve ready segment extent " & Name & ": " & Outcome'Image;
         end if;
         delay 0.0;
      end loop;
   ...
   procedure Run_Coordinator is
      ...
      Index_Capacity                : constant Positive := Next_Power_Of_Two (Image_Count * 2);
      ...
      Index_Length                  : constant DS.Byte_Count := Image_Maps.Required_Storage (Index_Capacity);
      Gate_Length                   : constant DS.Byte_Count :=
        Strings.Required_Storage (Model.Gate_Capacity);
      Mapping_Size                  : constant DS.Byte_Count := Segment_Length (Index_Capacity);
      Program                       : constant String :=
        Directories.Containing_Directory (Directories.Full_Name (CLI.Command_Name))
      ...
      Batch_Done                    : Worker_Boolean_Array (1 .. Maximum_Workers) := (others => False);
            ...
            if PIDs (Worker) > 0 then
               ...
               begin
                  ...
                  if Status = 1 then
                     ...
                     raise Program_Error with "worker " & Image (Worker) & " exited unsuccessfully";
                  end if;
               end;
            end if;
         end loop;
      ...
      procedure Consume (Value : Model.Image_Result) is
         Worker : Positive;
      begin
         if Value.Worker_Id = 0 or else Natural (Value.Worker_Id) > Maximum_Workers then
         ...
         if (Value.Flags and Model.Worker_Attached) /= 0 then
            ...
            Registry_Retries := Registry_Retries + Natural (Value.Index_Retries);
            ...
            elsif Value.Image_Id = 0 or else Value.Image_Id > Model.U64 (Image_Count) then
      ...
      begin
         loop
            ...
            exit when
              ...
              or else (Target = "raced" and then Workers_Raced = Active_Workers)
              or else (Target = "topology"
                       and then Workers_Ready = Active_Workers
                       and then Workers_Raced = Active_Workers)
              or else (Target = "exited" and then Workers_Ready = 0);
            if RT.To_Duration (RT.Clock - Started) > 30.0 then
               raise Program_Error with "timed out waiting for workers to be " & Target;
            end if;
            delay 0.001;
         end loop;
      end Wait_For;
      procedure Advance_Gate (Next_Batch : Model.U64; Worker_Limit : Natural) is
      begin
         loop
            begin
               Strings.Assign (Gate, Batch_Token (Next_Batch, Worker_Limit));
               exit;
            exception
               when DS.Busy_Error =>
                  delay 0.0;
      ...
      begin
         ...
         Status :=
           Spawn_Worker
             (C.To_C (Program),
              ...
              C.To_C (Corpus),
              C.int (Worker),
              C.unsigned_long_long (Mapping_Size),
              C.int (Index_Capacity),
              C.int (Width),
              C.int (Height),
              C.int (Passes),
              C.int (Index_Rounds),
         ...
         if Status /= 0 then
            raise Program_Error with "worker spawn failed (error" & C.int'Image (Status) & ")";
         end if;
      ...
      begin
         for Worker in Worker_Sockets'Range loop
            if Worker_Sockets (Worker) >= 0 then
               declare
                  Ignored : constant C.int := Close_Descriptor (Worker_Sockets (Worker));
                  pragma Unreferenced (Ignored);
               begin
                  Worker_Sockets (Worker) := -1;
               end;
      ...
      procedure Reap_Departed (First_Worker : Positive; Last_Worker : Positive) is
         Started : constant RT.Time := RT.Clock;
      begin
         loop
            Check_Children;
            exit when (for all Worker in First_Worker .. Last_Worker => PIDs (Worker) < 0);
            if RT.To_Duration (RT.Clock - Started) > 30.0 then
               raise Program_Error with "timed out reaping dynamically departed workers";
            end if;
            delay 0.001;
         end loop;
      end Reap_Departed;
      ...
      begin
         ...
         if Race_Winners /= 1 then
            raise Program_Error
              with "registry race designated " & Image (Race_Winners) & " creators after dynamic join";
         end if;
         ...
         Dynamic_Joins := Dynamic_Joins + Target_Workers - Previous_Workers;
      end Join_Workers;
      procedure Retire_Workers (Target_Workers : Positive) is
         Previous_Workers : constant Positive := Positive (Active_Workers);
      begin
         if Target_Workers >= Previous_Workers then
            raise Program_Error with "dynamic retirement does not reduce workers";
         end if;
         ...
         Dynamic_Leaves := Dynamic_Leaves + Previous_Workers - Target_Workers;
      end Retire_Workers;
      function External_Stop_Requested return Boolean
      is (Env.Exists ("FLYOLOGY_SHOWCASE_STOP_FILE")
          and then Directories.Exists (Env.Value ("FLYOLOGY_SHOWCASE_STOP_FILE")));
      procedure Put_Summary (File : IO.File_Type) is
      begin
         IO.Put_Line (File, "shared image index session complete");
         IO.Put_Line (File, "  completed epochs:      " & Image (Batch));
         IO.Put_Line
           (File,
            "  image slots per epoch: "
            & Image (Image_Count)
            & " ("
            & Image (Width)
            & "x"
            & Image (Height)
            & " P6 PPM)");
         IO.Put_Line (File, "  final-epoch admitted: " & Image (Batch_Target) & " / " & Image (Image_Count));
         IO.Put_Line (File, "  generated images total: " & Image (Session_Images));
         IO.Put_Line (File, "  initial workers:       " & Image (Worker_Count));
         IO.Put_Line (File, "  peak workers:          " & Image (Peak_Workers));
         IO.Put_Line (File, "  final-epoch workers:   " & Image (Final_Batch_Workers));
         IO.Put_Line
           (File, "  dynamic joins/leaves:  " & Image (Dynamic_Joins) & "/" & Image (Dynamic_Leaves));
         IO.Put_Line (File, "  dynamic-worker images: " & Image (Session_Dynamic_Worker_Images));
         IO.Put_Line (File, "  pixel-analysis passes: " & Image (Passes));
         IO.Put_Line (File, "  registry race winners: " & Image (Race_Winners));
         IO.Put_Line (File, "  final-epoch queue pressure: " & Image (Queue_Pressure));
         IO.Put_Line (File, "  producer saturation backoffs: " & Image (Session_Producer_Backoffs));
         IO.Put_Line (File, "  irregular source gaps: " & Image (Session_Source_Gaps));
         IO.Put_Line (File, "  session queue pressure:    " & Image (Session_Queue_Pressure));
         IO.Put_Line (File, "  final-epoch index retries: " & Image (Index_Retries));
         IO.Put_Line (File, "  session index retries:     " & Image (Session_Index_Retries));
         IO.Put_Line
           (File,
            "  verified final map:    "
            & Image (Image_Maps.Length (Index))
            & " entries"
            & (if Index_Rounds > 1 and then Session_Image_High_Water > 0
               then " (images + contention key)"
               else ""));
         IO.Put_Line
           (File,
            "  final-epoch processing: "
            & Showcase_Support.Fixed_Image (Long_Float (Last_Batch_Elapsed), 3)
            & " s");
         IO.Put_Line
           (File,
            "  session processing:    "
            & Showcase_Support.Fixed_Image (Long_Float (Session_Work_Elapsed), 3)
            & " s");
         IO.Put_Line
           (File,
            "  session wall time:     "
            & Showcase_Support.Fixed_Image (Long_Float (RT.To_Duration (RT.Clock - Session_Started)), 3)
            & " s");
         IO.Put_Line
           (File,
            "  one segment remained live while workers joined on saturation " & "and drained after recovery");
         IO.Put_Line
           (File, "  backing descriptor closed before final worker shutdown; " & "mappings stayed live");
      ...
      begin
         if Env.Exists ("FLYOLOGY_SHOWCASE_SUMMARY_FILE") then
            declare
               Report : IO.File_Type;
            begin
               IO.Create (Report, IO.Out_File, Env.Value ("FLYOLOGY_SHOWCASE_SUMMARY_FILE"));
               Put_Summary (Report);
               IO.Close (Report);
            exception
               when others =>
   ...
   begin
      ...
      then
         raise Constraint_Error with "unbounded coordinator requires a stop-request path";
      end if;
      ...
      Reserve (Segment, Model.Job_Ring_Name, Job_Length, Claim, Layout (Pending_Jobs).Location);
      Layout (Pending_Jobs).Length := Job_Length;
      Job_Rings.Initialize (Jobs, Region, Layout (Pending_Jobs).Location, Model.Job_Ring_Capacity);
      Segments.Publish (Segment, Claim);
      Reserve (Segment, Model.Result_Ring_Name, Result_Length, Claim, Layout (Completed_Results).Location);
      Layout (Completed_Results).Length := Result_Length;
      Result_Rings.Initialize
        (Results, Region, Layout (Completed_Results).Location, Model.Result_Ring_Capacity);
      Segments.Publish (Segment, Claim);
      Reserve (Segment, Model.Index_Name, Index_Length, Claim, Layout (Image_Index).Location);
      Layout (Image_Index).Length := Index_Length;
      Image_Maps.Initialize (Index, Region, Layout (Image_Index).Location, Index_Capacity);
      Segments.Publish (Segment, Claim);
      Reserve (Segment, Model.Gate_Name, Gate_Length, Claim, Layout (Start_Gate).Location);
      Layout (Start_Gate).Length := Gate_Length;
      Strings.Initialize (Gate, Region, Layout (Start_Gate).Location, Model.Gate_Capacity);
      Segments.Publish (Segment, Claim);
      ...
      Wait_For ("raced");
      if Race_Winners /= 1 then
         raise Program_Error with "registry race designated " & Image (Race_Winners) & " creators";
      end if;
      Resolve_Ready_Extent
        (Segment, Model.Race_Name, Layout (Startup_Race).Location, Layout (Startup_Race).Length);
      Begin_Dashboard (Initial_Terminal_Width, Initial_Terminal_Height);
      declare
         ...
         begin
            if Dashboard_Active then
               loop
                  TUI_POSIX.Next_Event (Dashboard_Terminal, Event, Status);
                  exit when Status in TUI_Backends.Interrupted | TUI_Backends.End_Of_Input;
                  case Event.Kind is
         ...
         procedure Render_Current (Generated : Natural; Generator_Paused : Boolean; Source_Waiting : Boolean)
         is
      ...
      begin
         ...
         loop
            ...
            declare
               ...
               High_Water       : constant Natural := Natural'Max (8, Model.Job_Ring_Capacity / 2);
               Low_Water        : constant Natural := Natural'Max (1, High_Water / 2);
               Stop_Observed    : Boolean := False;
               procedure Observe_Stop is
               begin
                  if not Stop_Observed and then (Dashboard.Stop_Requested or else External_Stop_Requested)
                  then
                     Stop_Observed := True;
                     Batch_Target := Queued;
                     Pending_Image := 0;
            ...
            begin
               while Completed < Batch_Target or else Workers_Done < Active_Workers loop
                  Observe_Stop;
                  if Generator_Paused and then Queued - Completed <= Low_Water then
                     Generator_Paused := False;
                  ...
                  then
                     if Pending_Image = 0 then
                        ...
                        Model.Generate_Image
                          (Model.Image_Path (Corpus, Pending_Image), Width, Height, Random_State);
                     end if;
                     Observe_Stop;
                     if not Stop_Observed then
                        declare
                           Job     : constant Model.Image_Job :=
                             (Batch_Id => Model.U64 (Batch), Image_Id => Model.U64 (Pending_Image));
                        ...
                        begin
                           ...
                           case Outcome is
                              when Job_Rings.Pushed                          =>
                                 Queued := Queued + 1;
                                 Pending_Image := 0;
                                 if Queued - Completed >= High_Water and then Queued < Image_Count then
                                    Generator_Paused := True;
                                    Producer_Backoffs := Producer_Backoffs + 1;
                                 end if;
                                 Burst_Remaining := Burst_Remaining - 1;
                                 if Burst_Remaining = 0 and then Queued < Image_Count then
                                    Source_Waiting := True;
                                    Source_Gaps := Source_Gaps + 1;
                                    Source_Resume :=
                                      RT.Clock + RT.Milliseconds (20 + (Queued * 29 + Batch * 11) mod 81);
                                 end if;
                  ...
                  then
                     declare
                        Job     : constant Model.Image_Job := (Batch_Id => Model.U64 (Batch), Image_Id => 0);
                        Outcome : Job_Rings.Push_Result;
                     begin
                        Job_Rings.Try_Push (Jobs, Job, Outcome);
                        case Outcome is
                  ...
                  then
                    ...
                    and then (Stop_Observed
                              or else (not Generator_Paused
                                       and then (Queued = Image_Count
                  ...
                  if RT.To_Duration (RT.Clock - Last_Render) >= 0.075 then
                     Render_Current
                       (Queued + (if Pending_Image = 0 then 0 else 1), Generator_Paused, Source_Waiting);
            ...
            for Image_Id in 1 .. Batch_Target loop
               ...
               begin
                  loop
                     begin
                        Image_Maps.Get (Index, Model.U64 (Image_Id), Value, Found);
                        exit;
                     exception
                        when DS.Busy_Error =>
                           delay 0.0;
            ...
            Session_Image_High_Water := Natural'Max (Session_Image_High_Water, Batch_Target);
            if Image_Maps.Length (Index)
              /= Session_Image_High_Water
                 + (if Index_Rounds > 1 and then Session_Image_High_Water > 0 then 1 else 0)
            then
               raise Program_Error with "shared index contains unexpected entries";
            end if;
            Last_Batch_Elapsed := RT.To_Duration (RT.Clock - Work_Started);
            Session_Work_Elapsed := Session_Work_Elapsed + Last_Batch_Elapsed;
            ...
            if Maximum_Workers > Worker_Count then
               for Worker in Worker_Count + 1 .. Maximum_Workers loop
                  Session_Dynamic_Worker_Images :=
                    Session_Dynamic_Worker_Images + Model.U64 (Per_Worker (Worker));
               end loop;
            end if;
            Session_Queue_Pressure := Session_Queue_Pressure + Model.U64 (Queue_Pressure);
            Session_Producer_Backoffs := Session_Producer_Backoffs + Model.U64 (Producer_Backoffs);
            Session_Source_Gaps := Session_Source_Gaps + Model.U64 (Source_Gaps);
            Session_Index_Retries := Session_Index_Retries + Model.U64 (Index_Retries);
   ...
   exception
      when Error : others =>
         ...
         Stop_Children;
         IO.Put_Line
           (IO.Standard_Error, "shared image index failed: " & Ada.Exceptions.Exception_Information (Error));
         raise;
   end Run_Coordinator;
begin
