...
procedure Task_Snapshot_Contention is
   ...
   procedure Sort_Durations is new Ada.Containers.Generic_Array_Sort (Positive, Duration, Duration_Array);

   type Instance_Array is array (Positive range <>) of Observation.Task_Instance_Id;
   procedure Sort_Instances is new
     Ada.Containers.Generic_Array_Sort (Positive, Observation.Task_Instance_Id, Instance_Array);

   function Image (Value : Long_Long_Integer) return String
   is (Ada.Strings.Fixed.Trim (Long_Long_Integer'Image (Value), Ada.Strings.Both));

   function Fixed (Value : Long_Float; Decimals : Natural) return String
   is (Showcase_Support.Fixed_Image (Value, Decimals));
   ...
   is (Fixed (Long_Float (Value) * 1_000_000.0, 3));

   Operation    : constant String :=
     (if Ada.Command_Line.Argument_Count >= 1 then Ada.Command_Line.Argument (1) else "tasks");
   ...
   Capacity     : constant Positive :=
     (if Ada.Command_Line.Argument_Count >= 3 then Positive'Value (Ada.Command_Line.Argument (3)) else 32);
   Window       : constant Duration :=
     (if Ada.Command_Line.Argument_Count >= 4 then Duration'Value (Ada.Command_Line.Argument (4)) else 0.200);
   Run          : constant Positive :=
     (if Ada.Command_Line.Argument_Count >= 5 then Positive'Value (Ada.Command_Line.Argument (5)) else 1);
   Interval     : constant Duration :=
     (if Ada.Command_Line.Argument_Count >= 6 then Duration'Value (Ada.Command_Line.Argument (6)) else 0.0);
   ...
   Group_CPU : constant CPU_Range := System.Multiprocessors.Not_A_Specific_CPU;

   task type Parked_Task
     (Slot : Positive;
      CPU  : CPU_Range)
     with CPU => CPU
   is
   ...
   procedure Free_Parked is new Ada.Unchecked_Deallocation (Parked_Task, Parked_Access);
   procedure Free_Spinner is new Ada.Unchecked_Deallocation (Spinner_Task, Spinner_Access);
...
begin
   ...
   declare
      ...
      Before_Ticks, Observe_Ticks, After_Ticks, Final_Ticks : Observation.Counter;
         ...
         if Written > 1 then
            ...
            for Index in 2 .. Written loop
               if Instances (Index) = Instances (Index - 1) then
                  raise Program_Error with "task snapshot returned duplicate identity";
               end if;
            end loop;
         end if;
      ...
      begin
         if Operation = "tasks" then
            Available := Observation.Snapshot_Tasks (0, Items, Written, Total);
            if not Available
            ...
            if not Available or else Group_Sample.Members /= Observation.Counter (Member_Count) then
   ...
   begin
      ...
      while Clock < Observe_End loop
         ...
         if Interval > 0.0 then
            Next_Observation := Next_Observation + To_Time_Span (Interval);
      ...
      Baseline_Seconds := To_Duration (Before_End - Before_Start) + To_Duration (After_End - After_Start);
      Observe_Seconds := To_Duration (Observe_End - Observe_Start);
      Baseline_Rate :=
        Long_Float ((Observe_Ticks - Before_Ticks) + (Final_Ticks - After_Ticks))
        / Long_Float (Baseline_Seconds);
      Observe_Rate := Long_Float (After_Ticks - Observe_Ticks) / Long_Float (Observe_Seconds);
      Rate_Percent := (if Baseline_Rate = 0.0 then 0.0 else 100.0 * Observe_Rate / Baseline_Rate);
      Call_Throughput := Long_Float (Calls) / Long_Float (Observe_Seconds);

      Put_Line
        (Standard_Error,
         Operation
         & " snapshot contention: members="
   ...
   exception
      when Failure : others =>
         ...
         Put_Line
           (Standard_Error,
            "task snapshot benchmark failed: " & Ada.Exceptions.Exception_Information (Failure));
         raise;
   end;
