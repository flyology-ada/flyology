...
is
   ...
   procedure Consume_After_Close_Attempt (Value : in out Interfaces.C.int)
   with Global => null, Pre => Value /= Interfaces.C.int (-1), Post => Value = Interfaces.C.int (-1);
   ...
   function Begin_Allowed (Phase : Run_Phase; Serve_Started : Boolean) return Boolean
   with
     Post =>
       Begin_Allowed'Result = (Phase = Idle or else (Phase = Stop_Requested and then not Serve_Started));
   ...
   with
     Pre  => Phase in Idle | Stop_Requested,
     Post => Phase_After_Begin'Result = (if Phase = Idle then Serving else Stop_Requested);
   ...
   with Post => Phase_After_Stop'Result = (if Phase in Idle | Serving then Stop_Requested else Phase);
   ...
   with Post => Stop_Was_Requested'Result = (Phase in Stop_Requested | Finished);
   ...
   function Handler_Start_Allowed (Active : Natural; Expected : Natural) return Boolean
   ...
   function Worker_Finish_Allowed (Workers_Done : Natural; Expected : Natural) return Boolean
   ...
   function Serve_Finish_Allowed (Workers_Done : Natural; Expected : Natural; Active : Natural) return Boolean
   with Post => Serve_Finish_Allowed'Result = (Workers_Done = Expected and then Active = 0);
   ...
   function Snapshot_Running (Serve_Started : Boolean; Phase : Run_Phase) return Boolean
   with Post => Snapshot_Running'Result = (Serve_Started and then Phase in Serving | Stop_Requested);
   ...
   function Accepting_After_Worker_Start (Phase : Run_Phase) return Boolean
   ...
   function Snapshot_Accepting (Was_Marked : Boolean; Phase : Run_Phase) return Boolean
   with Post => Snapshot_Accepting'Result = (Was_Marked and then Phase = Serving);
   ...
   with Post => Snapshot_Shutdown'Result = (Phase in Stop_Requested | Finished);
