...
is
   ...
   subtype Nanosecond_Part is Interfaces.Integer_64 range 0 .. 999_999_999;
   subtype Civil_Day_Count is Interfaces.Integer_64 range -25_202 .. 157_053;
   ...
   function Days_From_Civil (Year : Integer; Month : Integer; Day : Integer) return Civil_Day_Count
   with Pre => Year in 1901 .. 2399 and then Month in 1 .. 12 and then Day in 1 .. 31;
   ...
   function Add_Nanoseconds (Value : Timestamp; Amount : Interfaces.Integer_64) return Timestamp_Result
   with Pre => Amount >= 0;
   ...
   with Post => (if Before (Left, Right) then Earlier'Result = Left else Earlier'Result = Right);
   ...
   function Difference_Nanoseconds (Later, Earlier : Timestamp) return Difference_Result;
