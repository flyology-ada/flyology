...
package body Flyology.Data_Structures.Allocation_Pools.Adaptive is
   ...
   function Chunk_Storage return Byte_Count is
     (Layouts.Checked_Add
        (Byte_Count (Chunk_Location),
         Chunk_Slabs.Required_Storage (Slots_Per_Chunk)));

   function Required_Storage return Byte_Count is
   begin
      return Layouts.Align_Up
        (Layouts.Checked_Add
           (Table_Offset,
            Layouts.Checked_Multiply
              (Byte_Count (Maximum_Chunks), Entry_Size)),
         8);
   ...
   begin
      if Index > Maximum_Chunks
        or else Relative > Entry_Size
        or else Extent > Entry_Size - Relative
      then
        ...
        (Item.Core,
         Layouts.Checked_Add
           (Table_Offset,
            Layouts.Checked_Add
              (Layouts.Checked_Multiply
                 (Byte_Count (Index - 1), Entry_Size),
               Relative)),
         Extent, Alignment);
   end Entry_Address;

   function Entry_State
     (Item : View; Index : Positive) return Interfaces.Unsigned_32 is
     (Atomic.Load_Acquire_U32
        (Entry_Address (Item, Index, State_Offset, 4, 4)));

   procedure Set_Entry_State
     (Item : View; Index : Positive; State : Interfaces.Unsigned_32) is
   begin
      Atomic.Store_Release_U32
        (Entry_Address (Item, Index, State_Offset, 4, 4), State);
   end Set_Entry_State;

   function Entry_Allocation
     (Item : View; Index : Positive)
      return Arena_Provider.Allocation_Handle is
     ((Token => Bytes.Read_U64
         (Entry_Address (Item, Index, Token_Offset, 8, 8)),
       Generation => Bytes.Read_U64
         (Entry_Address (Item, Index, Generation_Offset, 8, 8))));

   procedure Set_Entry_Allocation
     (Item  : View;
      Index : Positive;
      Value : Arena_Provider.Allocation_Handle) is
   begin
      Bytes.Write_U64
        (Entry_Address (Item, Index, Token_Offset, 8, 8), Value.Token);
      Bytes.Write_U64
        (Entry_Address (Item, Index, Generation_Offset, 8, 8),
         Value.Generation);
   ...
   begin
      ...
      for Index in 1 .. Maximum_Chunks loop
         Set_Entry_Allocation
           (Item, Index, Arena_Provider.Null_Allocation);
         Bytes.Write_U32
           (Entry_Address (Item, Index, Reserved_Offset, 4, 4), 0);
   ...
   begin
      ...
      Clear_Cache (Item, Index);
      Arena_Provider.Attach_Allocation
        (Item.Chunks (Index).Region, Arena, Allocation);
      begin
         Chunk_Slabs.Attach
           (Item.Chunks (Index).Slab, Item.Chunks (Index).Region,
            Chunk_Location, Slots_Per_Chunk);
   ...
   procedure Validate_Table
     (Item : in out View; Arena : Arena_Provider.View) is
   begin
      for Index in 1 .. Maximum_Chunks loop
         if Bytes.Read_U32
           (Entry_Address (Item, Index, Reserved_Offset, 4, 4)) /= 0
         then
            raise Layout_Error with
              "adaptive-pool chunk reserved field is corrupt";
         end if;
         case Entry_State (Item, Index) is
            when Empty_State =>
               if Entry_Allocation (Item, Index) /=
                 Arena_Provider.Null_Allocation
               then
                  raise Layout_Error with
                    "adaptive-pool empty chunk retains an allocation";
               end if;
            when Live_State =>
               Ensure_Chunk (Item, Arena, Index);
            when Initializing_State =>
               raise Layout_Error with
                 "adaptive-pool chunk initialization is incomplete";
            when others =>
               raise Layout_Error with
                 "adaptive-pool chunk state is corrupt";
         end case;
      end loop;
   end Validate_Table;
   ...
   begin
      ...
      if Header.Capacity /= Interfaces.Unsigned_32 (Maximum_Chunks)
        ...
        or else Header.Word_2 /= Interfaces.Unsigned_64
          (Arena_Metadata.Incarnation)
        or else Core.Extent /= Required_Storage
      then
         raise Layout_Error with
           "adaptive-pool creation parameters do not match";
   ...
   begin
      ...
      then
         raise Layout_Error with
           "adaptive pool is attached to another arena incarnation";
      end if;
   end Require_Arena;
   ...
   begin
      ...
      for Index in 1 .. Maximum_Chunks loop
         if Entry_State (Item, Index) = Live_State then
            Ensure_Chunk (Item, Arena, Index);
            Chunk_Slabs.Try_Allocate
              (Item.Chunks (Index).Slab, Data, Slab_Handle, Slab_Result);
            case Slab_Result is
      ...
      Result :=
        (if Saw_Contention then Allocation_Contended else Exhausted);
   end Try_Existing;
   ...
   begin
      ...
      begin
         ...
         Arena_Provider.Try_Allocate
           (Arena, Positive (Chunk_Storage), Allocation, Arena_Result);
         ...
         Arena_Provider.Attach_Allocation
           (Item.Chunks (Empty_Index).Region, Arena, Allocation);
         ...
         Chunk_Slabs.Try_Allocate
           (Item.Chunks (Empty_Index).Slab, Data,
            Slab_Handle, Slab_Result);
         if Slab_Result /= Chunk_Slabs.Allocated then
            raise Layout_Error with
              "new adaptive-pool chunk did not provide its first slot";
         end if;
   ...
   begin
      Resolve (Item, Arena, Value, Index, Slab_Handle);
      Chunk_Slabs.Replace
        (Item.Chunks (Index).Slab, Slab_Handle, Data);
   end Replace;
