...
package body Flyology.Process_Generations.Agents is
   ...
   function Expired
     ...
     (Timeout >= 0.0 and then
        Ada.Real_Time.To_Duration (Ada.Real_Time.Clock - Started) >= Timeout);
   ...
   is
      Authority    : constant Upgrade_Handle :=
        Command_Lines.Read_Authority;
      ...
      begin
         ...
         if Frame.Kind /= Kind then
            raise Agent_Error with
              "unexpected process-generation command " &
      ...
      procedure Send_Failure_Text (Message : String) is
         Length  : constant Natural :=
           Natural'Min (Message'Length, Protocol.Maximum_Payload);
      begin
         ...
         if Length > 0 then
            for Index in 0 .. Length - 1 loop
               Payload (Index) := Protocol.Octet
                 (Character'Pos (Message (Message'First + Index)));
            end loop;
         end if;
         begin
            Transport.Send
              (Control, Protocol.Failure_Message, Payload, Length,
   ...
   begin
      ...
      Transport.Send
        (Control, Protocol.Acknowledgment, Timeout => Control_Timeout);

      Expect (Protocol.Provision);
      Messages.Decode_Provision
        (Frame.Payload, Frame.Length, Provisioning, Decode);
      if Decode /= Messages.Decoded then
         raise Agent_Error with
           "invalid provisioning payload " &
           Messages.Decode_Result'Image (Decode);
      end if;
      ...
      Transport.Send
        (Control, Protocol.Capability_Ready, Timeout => Control_Timeout);
      Handoffs.Receive_Listener (Capabilities, Listener);
      Transport.Send
        (Control, Protocol.Capability_Adopted, Timeout => Control_Timeout);
      Messages.Encode_Topology_Proof
        ((Epoch => Provisioning.Topology_Epoch,
          Digest => Provisioning.Digest), Payload);
      Transport.Send
        (Control, Protocol.Prepared_Message, Payload,
         Messages.Topology_Proof_Length, Timeout => Control_Timeout);
      ...
      declare
         ...
         begin
            select
               ...
               exception
                  when others => Server_Result.Finish (True);
               end;
            or
               terminate;
            end select;
         ...
         begin
            while not Server'Terminated loop
               if Expired (Began, Drain_Timeout) then
                  Send_Failure_Text
                    ("candidate server did not drain before its deadline");
                  raise Agent_Error with
                    "candidate server did not drain before its deadline";
               end if;
               delay Poll_Interval;
         ...
         begin
            if Started then
               ...
               if not Finished or else Failed then
                  raise Agent_Error with
                    "managed server failed while draining";
               end if;
            end if;
         end Stop_Server;
         ...
         begin
            Request_Stop (Context);
            Transport.Send
              (Control, Protocol.Admission_Revoked_Message,
               Timeout => Control_Timeout);
            Await_Server_Stop;
            Transport.Send
              (Control, Protocol.Drained_Message,
               Timeout => Control_Timeout);
            begin
               Result := Compensate (Context, Authority);
            exception
               when others => Result := Compensation_Failed;
            end;
            Messages.Encode_Compensation (Result, Payload);
            Transport.Send
              (Control, Protocol.Compensation_Message, Payload,
            ...
            if Started then
               loop
                  ...
                  if Finished then
                     raise Agent_Error with
                       (if Failed then
                           "managed server failed after readiness"
                     ...
                     raise Flyology.IO.Timeout_Error with
                       "control command deadline expired";
                  end if;
                  Slice :=
                    (if Control_Timeout < 0.0 then Poll_Interval
                     else Duration'Min
                       (Poll_Interval, Remaining (Began, Control_Timeout)));
                  exit when Transport.Message_Available
                    (Control, Timeout => Slice);
               end loop;
               Transport.Receive
                 (Control, Frame,
                  Timeout => Remaining (Began, Control_Timeout));
            else
               Transport.Receive
                 (Control, Frame, Timeout => Control_Timeout);
            end if;
         end Receive_Command;
      begin
         loop
            ...
            case Frame.Kind is
               when Protocol.Start_Canary_Message =>
                  begin
                     if Started then
                        raise Agent_Error with
                          "candidate server already started";
                     end if;
                     Server.Start;
                     Started := True;
                     declare
                        Began : constant Ada.Real_Time.Time :=
                          Ada.Real_Time.Clock;
                        Finished, Failed : Boolean;
                     begin
                        loop
                           Server_Result.Snapshot (Finished, Failed);
                           if Transport.Message_Available
                             (Control, Timeout => 0.0)
                           then
                              Transport.Receive
                                (Control, Frame, Timeout => Control_Timeout);
                              if Frame.Kind = Protocol.Cancel_Message then
                                 ...
                                 raise Agent_Error with
                                   "unexpected command during readiness";
                              end if;
                           elsif Ready (Context, Provisioning) then
                              exit;
                           elsif Finished then
                              raise Agent_Error with
                                 (if Failed then "candidate server failed"
                                  else
                                    "candidate server stopped before " &
                                    "readiness");
                     ...
                     Messages.Encode_Topology_Proof
                       ((Epoch => Provisioning.Topology_Epoch,
                         Digest => Provisioning.Digest), Payload);
                     Transport.Send
                       (Control, Protocol.Ready_Message, Payload,
                  ...
                  exception
                     when Error : others =>
                        ...
                        begin
                           ...
                           Messages.Encode_Compensation (Result, Payload);
                           Transport.Send
                             (Control, Protocol.Compensation_Message,
                              Payload, Messages.Compensation_Length,
                              Timeout => Control_Timeout);
                        end;
                        Send_Failure (Error);
                        return;
               ...
               when Protocol.Cancel_Message =>
                  if Started then
                     ...
                     Transport.Send
                       (Control, Protocol.Acknowledgment,
                        Timeout => Control_Timeout);
                  end if;
                  return;
               ...
               when others =>
                  raise Agent_Error with
                    "command is invalid for a prepared image agent";
            end case;
         end loop;
      exception
         when others =>
