...
package body Flyology.Observability.Stall_Watchdogs is
   ...
   task body Monitor_Task is
      ...
      function Runnable (Item : Group_Snapshot) return Boolean
      is (Item.Ready /= 0 or else Item.Running /= 0);
   ...
   procedure Free_State is new Ada.Unchecked_Deallocation (Watchdog_State, Watchdog_State_Access);
   procedure Free_Monitor is new Ada.Unchecked_Deallocation (Monitor_Task, Monitor_Task_Access);

   procedure Start (Object : in out Watchdog; Config : Watchdog_Config := (others => <>)) is
   ...
   begin
      ...
      then
         raise Invalid_Configuration with "positive threshold must be at least the sampling interval";
   ...
   exception
      when others =>
         if Object.Monitor = null and then Allocated_State and then Object.State /= null then
   ...
   is (Object.Monitor /= null and then not Object.Monitor.all'Terminated);
   ...
   overriding
   procedure Finalize (Object : in out Watchdog) is
   begin
      Stop (Object);
      if Object.State /= null then
         Free_State (Object.State);
