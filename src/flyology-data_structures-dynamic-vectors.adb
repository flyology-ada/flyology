...
package body Flyology.Data_Structures.Dynamic.Vectors is
   ...
   function Arena_Metadata (Arena : Arenas.View) return Arenas.Metadata is
     (Arenas.Current_Metadata (Arena));
   ...
   begin
      ...
      then
         raise Constraint_Error with
           "dynamic-vector initial capacity does not fit the arena";
      end if;
   ...
   procedure Validate_Allocation
     (Item     : View;
      Arena    : Arenas.View;
      Value    : Arenas.Allocation_Handle;
      Capacity : Interfaces.Unsigned_64)
   is
      Block : Byte_Count;
      Payload : Byte_Count;
   begin
      if Value = Arenas.Null_Allocation then
         if Capacity /= 0 then
            raise Layout_Error with
              "dynamic-vector capacity has no allocation";
         end if;
         return;
      elsif Capacity = 0 then
         raise Layout_Error with
           "dynamic-vector allocation has zero capacity";
      end if;
      begin
         Block := Arenas.Block_Capacity (Arena, Value);
      exception
         when Handle_Error =>
            raise Layout_Error with
   ...
   begin
      ...
      then
         raise Layout_Error with
           "dynamic-vector creation parameters do not match";
      ...
      if Retired /= Arenas.Null_Allocation then
         if Retired = Current then
            raise Layout_Error with
              "dynamic-vector current and retired allocations alias";
         end if;
         declare
            Ignored : constant Byte_Count :=
              Arenas.Block_Capacity (Arena, Retired);
   ...
   exception
      when Handle_Error =>
         ...
         raise Layout_Error with
           "dynamic-vector deferred allocation handle is stale";
      when others =>
         if Item.Core.Attached then
            Detach (Item);
         end if;
   ...
   exception
      when Handle_Error =>
         raise Layout_Error with
           "dynamic-vector deferred allocation is stale";
   end Cleanup_Retired;
   ...
   begin
      ...
      begin
         ...
         then
            raise Layout_Error with
              "arena returned an unusable dynamic-vector block";
         end if;
