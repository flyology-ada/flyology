...
package body Flyology.Data_Structures.Slab_Pools is
   ...
   is (Byte_Count'Max (8, Byte_Count (Element.Alignment)));
   ...
   procedure Geometry
     (Capacity : Positive; Payload_Offset : out Byte_Count; Stride : out Byte_Count; Extent : out Byte_Count)
   is
   ...
   begin
      ...
      return Layouts.Address_At (Item.Core, Layouts.Checked_Add (Slot_Relative, Offset), Extent, Alignment);
   end Field_Address;

   function Generation_Address (Item : View; Slot : Interfaces.Unsigned_32) return System.Address
   is (Field_Address (Item, Slot, Generation_Offset, 4, 4));

   function State_Address (Item : View; Slot : Interfaces.Unsigned_32) return System.Address
   is (Field_Address (Item, Slot, State_Offset, 4, 4));

   function Next_Address (Item : View; Slot : Interfaces.Unsigned_32) return System.Address
   is (Field_Address (Item, Slot, Next_Offset, 4, 4));
   ...
   is
      ...
      Alignment_32   : constant Interfaces.Unsigned_32 := U32 (Element.Alignment);
   begin
      Detach (Item);
      Geometry (Capacity, Payload_Offset, Stride, Extent);
      Layouts.Begin_Initialize
        (Core,
         ...
         (Capacity     => Capacity_32,
          ...
          Word_2       => 0),
         Storage_Alignment);
      Finish_Initialize (Item, Core, Capacity_32, Element_32, Alignment_32, Payload_Offset, Stride);
   exception
      when others =>
   ...
   begin
      Detach (Item);
      Geometry (Capacity, Expected_Payload, Expected_Stride, Expected_Extent);
   ...
   procedure Check_Slot (Item : View; Slot : Handles.Slot_Index; Value : out Interfaces.Unsigned_32) is
   begin
      Layouts.Require_Ready (Item.Core);
      if Slot = 0 or else Interfaces.Unsigned_64 (Slot) > Interfaces.Unsigned_64 (Item.Capacity_Value) then
         raise Handle_Error with "null or out-of-range slab slot";
      end if;
      Value := Interfaces.Unsigned_32 (Slot);
   end Check_Slot;

   procedure Check_Handle_Shape (Item : View; Value : Handles.Handle; Slot : out Interfaces.Unsigned_32) is
   begin
   ...
   begin
      ...
      for Attempt in 1 .. Attempts loop
         ...
         if State = Poisoned_State then
            ...
            Expected := Live_State;
            if Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), Expected, Desired_State) then
               if Bytes.Read_U32 (Generation_Address (Item, Slot)) /= Interfaces.Unsigned_32 (Value.Stamp)
               then
                  Atomic.Store_Release_U32 (State_Address (Item, Slot), Live_State);
   ...
   begin
      loop
         begin
            Acquire_Live (Item, Value, Desired_State, Slot, Attempts => Timed_Contention_Limit);
            return;
         exception
            when Busy_Error =>
               Waiting.Retry (Wait);
   ...
   function Payload_Address (Item : View; Slot : Interfaces.Unsigned_32) return System.Address;

   function Binding
     (Item : View; Slot : Interfaces.Unsigned_32; Writable : Boolean) return Immutable_Storage_View;
   ...
   begin
      ...
      for Offset in Interfaces.Unsigned_32 range 0 .. Item.Capacity_Value - 1 loop
         ...
         if State = Free_State then
            ...
            if Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), State_Expected, Allocating_State) then
               Generation := Bytes.Read_U32 (Generation_Address (Item, Slot));
               if Generation = 0 then
                  State_Expected := Allocating_State;
                  if Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), State_Expected, Poisoned_State)
                  then
                     null;
                  end if;
                  ...
                  when others =>
                     ...
                     then
                        if State_Expected = Poisoned_State then
                           raise Poison_Error with "slab construction was poisoned";
                        else
                           raise Layout_Error with "slab construction state changed";
                        end if;
                     end if;
                     raise;
               end;
               State_Expected := Allocating_State;
               if not Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), State_Expected, Live_State)
               then
                  if State_Expected = Poisoned_State then
      ...
      Result := (if Saw_Free_Contention then Allocation_Contended else Exhausted);
   end Try_Allocate;
   ...
   procedure Release (Item : in out View; Value : Handles.Handle) is
      Slot       : Interfaces.Unsigned_32;
      Expected   : Interfaces.Unsigned_32 := Releasing_State;
      Generation : Interfaces.Unsigned_32;
   begin
      Acquire_Live (Item, Value, Releasing_State, Slot);
      if not Policy.Generation_Can_Advance (Interfaces.Unsigned_32 (Value.Stamp)) then
         if not Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), Expected, Poisoned_State) then
         ...
         raise Poison_Error with "slab generation is exhausted; exclusive reinitialization required";
      end if;
      Generation := Policy.Next_Generation (Interfaces.Unsigned_32 (Value.Stamp));
      Bytes.Write_U32 (Generation_Address (Item, Slot), Generation);
      if not Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), Expected, Free_State) then
         if Expected = Poisoned_State then
   ...
   procedure Release (Item : in out View; Value : Handles.Handle; Timeout : Wait_Timeout) is
      Slot       : Interfaces.Unsigned_32;
      Expected   : Interfaces.Unsigned_32 := Releasing_State;
      Generation : Interfaces.Unsigned_32;
   begin
   ...
   function Payload_Address (Item : View; Slot : Interfaces.Unsigned_32) return System.Address is
   begin
      return
        Field_Address
          (Item,
   ...
   function Binding
     (Item : View; Slot : Interfaces.Unsigned_32; Writable : Boolean) return Immutable_Storage_View
   is (Base      => Payload_Address (Item, Slot),
       Extent    => Byte_Count (Element.Size),
       Signature => Element.Signature,
       Version   => Element.Version,
       Writable  => Writable);

   procedure Read (Item : View; Value : Handles.Handle; Data : out Element.Observed) is
   ...
   begin
      ...
      if not Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), Expected, Live_State) then
         if Expected = Poisoned_State then
   ...
   exception
      when others =>
         if Acquired and then Atomic.Load_Acquire_U32 (State_Address (Item, Slot)) = Accessing_State then
            Expected := Accessing_State;
            if Atomic.Compare_Exchange_U32 (State_Address (Item, Slot), Expected, Live_State) then
               null;
            end if;
         end if;
         raise;
   ...
   procedure Replace (Item : in out View; Value : Handles.Handle; Data : Element.Source) is
      ...
      when others =>
         if Acquired and then Atomic.Load_Acquire_U32 (State_Address (Item, Slot)) = Accessing_State then
            ...
            if Atomic.Compare_Exchange_U32
                 (State_Address (Item, Slot), Expected, (if Mutated then Poisoned_State else Live_State))
            then
               null;
            end if;
         end if;
   ...
   procedure Poison_Abandoned (Item : in out View; Slot : Handles.Slot_Index) is
   ...
   begin
      ...
      for Attempt in 1 .. Contention_Limit loop
         ...
         Expected := State;
         if Atomic.Compare_Exchange_U32 (State_Address (Item, Index), Expected, Poisoned_State) then
            return;
         end if;
      end loop;
   ...
   procedure Recover_Poisoned (Item : in out View; Slot : Handles.Slot_Index) is
      Index      : Interfaces.Unsigned_32;
      Expected   : Interfaces.Unsigned_32 := Poisoned_State;
      Generation : Interfaces.Unsigned_32;
   begin
      Check_Slot (Item, Slot, Index);
      if not Atomic.Compare_Exchange_U32 (State_Address (Item, Index), Expected, Releasing_State) then
         if Expected > Poisoned_State then
            raise Layout_Error with "slab slot state is corrupt";
         else
      ...
      Bytes.Write_U32 (Generation_Address (Item, Index), Policy.Next_Generation (Generation));
      Expected := Releasing_State;
      if not Atomic.Compare_Exchange_U32 (State_Address (Item, Index), Expected, Free_State) then
         if Expected = Poisoned_State then
   ...
   begin
      Layouts.Require_Ready (Item.Core);
      for Slot in Interfaces.Unsigned_32 range 1 .. Item.Capacity_Value loop
         if Atomic.Load_Acquire_U32 (State_Address (Item, Slot)) /= Free_State then
            raise Program_Error with "cannot destroy a slab with owned or poisoned slots";
         end if;
      end loop;
      Validate_Slots (Item);
   end Validate_Empty;
