...
package body Flyology.Observability is
   ...
   function Runtime_Observe_Group
     (Group : C.int; Snapshot : access Runtime_Group_Snapshot; Snapshot_Size : C.size_t) return C.int;
   pragma Import (C, Runtime_Observe_Group, "flyology_runtime_observe_group");
   ...
   pragma Import (C, Runtime_Observe_Tasks, "flyology_runtime_observe_tasks");

   function Runtime_Current_Task_Instance return C.unsigned_long_long;
   pragma Import (C, Runtime_Current_Task_Instance, "flyology_runtime_current_task_instance");

   function Runtime_Observe_Last_Fatal return C.int;
   pragma Import (C, Runtime_Observe_Last_Fatal, "flyology_runtime_observe_last_fatal");
   ...
   function Runtime_Observe_Stack_Pool (Snapshot : System.Address; Snapshot_Size : C.size_t) return C.int;
   pragma Import (C, Runtime_Observe_Stack_Pool, "flyology_runtime_observe_stack_pool");

   function Snapshot (Group : Group_Id; Result : out Group_Snapshot) return Boolean is
      Raw    : aliased Runtime_Group_Snapshot;
      Status : C.int;
   begin
      Status := Runtime_Observe_Group (C.int (Group), Raw'Access, Runtime_Group_Snapshot'Size / 8);
      if Status = 0 then
         return False;
      ...
      Result :=
        (Group                    => Group,
         ...
         Pending_File_Submissions => Counter (Raw.Pending_File_Submissions),
         Dormancy_Candidates      => Counter (Raw.Dormancy_Candidates),
         Dormancy_Candidate_Bytes => Counter (Raw.Dormancy_Candidate_Bytes),
   ...
   is (Task_Instance_Id (Runtime_Current_Task_Instance));

   function Snapshot_Tasks
     (Group : Group_Id; Items : in out Task_Snapshot_Array; Count : out Natural; Total : out Counter)
      return Boolean
   is
      Metadata : aliased Runtime_Task_Snapshot_Metadata := (Version => 0, Written => 0, Total => 0);
      Status   : C.int;
   begin
      Count := 0;
      Total := 0;
      ...
      then
         raise Program_Error with "incompatible Flyology task snapshot ABI";
      end if;
   ...
   is (case Runtime_Observe_Last_Fatal is
         when 0      => No_Fatal,
   ...
   function Stack_Pool return Stack_Pool_Snapshot is
      ...
      Status : C.int;
   begin
      Status := Runtime_Observe_Stack_Pool (Raw'Address, Runtime_Stack_Pool_Snapshot'Size / 8);
      if Status /= 1 or else Raw.Version /= Stack_Pool_ABI_Version then
         raise Program_Error with "incompatible Flyology stack-pool observability ABI";
      end if;
      return
