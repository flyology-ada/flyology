...
package body Flyology.Data_Structures.Allocation_Algorithms.Adapter is
   ...
   procedure Translate (Occurrence : Ada.Exceptions.Exception_Occurrence) is
      Identity : constant Ada.Exceptions.Exception_Id := Ada.Exceptions.Exception_Identity (Occurrence);
   ...
   function To_Inner (Value : Allocation_Handle) return FAA.Allocation_Handle
   is ((Token => Value.Token, Generation => Value.Generation));

   function From_Inner (Value : FAA.Allocation_Handle) return Allocation_Handle
   is ((Token => Value.Token, Generation => Value.Generation));

   function Inner_Offset (Configuration : Algorithm.Configuration) return Byte_Count
   is (Layouts.Align_Up (Layouts.Header_Size, Byte_Count (Algorithm.Minimum_Block_Size (Configuration))));

   function Required_Storage (Configuration : Algorithm.Configuration) return Byte_Count is
   begin
      return
        Layouts.Checked_Add
          (Inner_Offset (Configuration), Byte_Count (Algorithm.Required_Storage (Configuration)));
   exception
   ...
   procedure Prepare_Inner
     (Item : in out View; Configuration : Algorithm.Configuration; Initialize_New : Boolean) is
   begin
      FA_Regions.Attach (Item.Inner_Region, Item.Core.Base, FA.Byte_Count (Item.Core.Extent));
      if Initialize_New then
         Algorithm.Initialize
           (Item.Inner,
            Item.Inner_Region,
            FA.Region_Offset (Item.Inner_Location),
            Configuration,
            Inner_Instance);
      else
         Algorithm.Attach
   ...
   procedure Finish_New
     (Item : in out View; Configuration : Algorithm.Configuration; Instance_ID : Interfaces.Unsigned_64) is
   begin
      Item.Instance_Value := Instance_ID;
      Item.Usable_Value := Interfaces.Unsigned_32 (Algorithm.Usable_Capacity (Configuration));
      Item.Minimum_Value := Interfaces.Unsigned_32 (Algorithm.Minimum_Block_Size (Configuration));
   ...
   is
      ...
      Usable  : constant Positive := Algorithm.Usable_Capacity (Configuration);
      Minimum : constant Positive := Algorithm.Minimum_Block_Size (Configuration);
   begin
      ...
      Layouts.Begin_Initialize
        (Item.Core,
         ...
         (Capacity     => Interfaces.Unsigned_32 (Usable),
          ...
          Auxiliary    => 0,
          Word_1       => Instance_ID,
          Word_2       => Interfaces.Unsigned_64 (Inner_Offset (Configuration))),
         Byte_Count (Minimum));
   ...
   procedure Attach
     (Item          : out View;
      ...
      Instance_ID   : Interfaces.Unsigned_64)
   is
      Expected_Extent  : constant Byte_Count := Required_Storage (Configuration);
      Expected_Usable  : constant Positive := Algorithm.Usable_Capacity (Configuration);
      Expected_Minimum : constant Positive := Algorithm.Minimum_Block_Size (Configuration);
   ...
   begin
      ...
      Clear (Item);
      Layouts.Attach (Item.Core, Header, Region, Location, Identity, Byte_Count (Expected_Minimum));
      if Header.Capacity /= Interfaces.Unsigned_32 (Expected_Usable)
        or else Header.Element_Size /= Interfaces.Unsigned_32 (Expected_Minimum)
   ...
   is (Item.Core.Attached and then Algorithm.Is_Attached (Item.Inner));
   ...
   begin
      Layouts.Require_Ready (Item.Core);
      Algorithm.Try_Allocate (Item.Inner, Requested_Size, Inner_Value, Inner_Result);
   ...
   begin
      ...
      Algorithm.Try_Allocate
        (Item.Inner, Requested_Size, FA.Wait_Timeout (Timeout), Inner_Value, Inner_Result);
      Value := From_Inner (Inner_Value);
      Result :=
        (case Inner_Result is
           when FAA.Allocated            => Allocated,
   ...
   begin
      Layouts.Require_Ready (Item.Core);
      Algorithm.Release (Item.Inner, To_Inner (Value), FA.Wait_Timeout (Timeout));
   exception
      when Occurrence : others =>
         Translate (Occurrence);
   end Release;

   function Block_Capacity (Item : View; Value : Allocation_Handle) return Byte_Count is
   begin
      Layouts.Require_Ready (Item.Core);
      return Byte_Count (Algorithm.Block_Capacity (Item.Inner, To_Inner (Value)));
   exception
      when Occurrence : others =>
         Translate (Occurrence);
         raise Program_Error;
   ...
   begin
      ...
      Layouts.Require_Ready (Item.Core);
      Algorithm.Attach_Allocation (Inner_Allocation, Item.Inner, To_Inner (Value));
   ...
   begin
      if Signature = 0 or else Version = 0 or else Alignment not in 1 | 2 | 4 | 8 | 16 | 32 | 64 then
         raise Constraint_Error with "invalid immutable arena binding contract";
      end if;
      Layouts.Require_Ready (Item.Core);
      Algorithm.Attach_Allocation (Inner_Allocation, Item.Inner, To_Inner (Value));
      Result :=
        (Base      =>
           FA_Regions.Address_At
             (Inner_Allocation, FA.Byte_Count (Offset), FA.Byte_Count (Extent), FA.Byte_Count (Alignment)),
         Extent    => Extent,
         Signature => Signature,
         Version   => Version,
         Writable  => Writable);
   ...
   begin
      Layouts.Require_Ready (Item.Core);
      Algorithm.Attach_Allocation (Allocation, Item.Inner, To_Inner (Value));
      ...
      then
         ...
         if FA.Byte_Count (Native_Length) /= Length then
            raise Constraint_Error with "arena read is not natively representable";
         end if;
         Bytes.Copy (Data'Address, FA_Regions.Address_At (Allocation, Inner_Offset, Length), Native_Length);
      end if;
   ...
   begin
      ...
      then
         ...
         Native_Length := C.size_t (Length);
         if FA.Byte_Count (Native_Length) /= Length then
            raise Constraint_Error with "arena write is not natively representable";
         end if;
         Bytes.Copy (FA_Regions.Address_At (Allocation, Inner_Offset, Length), Data'Address, Native_Length);
      end if;
      FA_Regions.Detach (Allocation);
   exception
      when Occurrence : others =>
   ...
   begin
      ...
      Algorithm.Copy
        (Item.Inner,
         To_Inner (Source),
         FA.Byte_Count (Source_Offset),
         To_Inner (Target),
         FA.Byte_Count (Target_Offset),
