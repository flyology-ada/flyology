...
is
   ...
   type Acquire_Action is (Admit_Permit, Wait_For_Permit, Reject_Closed);

   --  Distinguish admission, capacity wait, and terminal rejection.
   function Classify_Acquire (Stopping : Boolean; Active : Natural; Capacity : Positive) return Acquire_Action
   with
     Pre  => Active <= Capacity,
     Post =>
       (if Stopping
        then Classify_Acquire'Result = Reject_Closed
        elsif Active = Capacity
        then Classify_Acquire'Result = Wait_For_Permit
        else Classify_Acquire'Result = Admit_Permit);

   --  Open the protected entry for either admission or terminal rejection.
   function Acquire_Entry_Open (Stopping : Boolean; Active : Natural; Capacity : Positive) return Boolean
   with Pre => Active <= Capacity, Post => Acquire_Entry_Open'Result = (Stopping or else Active < Capacity);

   --  Increment the active count without exceeding the configured capacity.
   function Active_After_Acquire (Active : Natural; Capacity : Positive) return Positive
   with
     Pre  => Active < Capacity,
     Post => Active_After_Acquire'Result = Active + 1 and then Active_After_Acquire'Result <= Capacity;
   ...
   function Is_Drained (Stopping : Boolean; Active : Natural) return Boolean
