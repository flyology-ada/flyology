...
package body Flyology.Debug_Producer_Selection is
   ...
   pragma Import (C, Native_Thread_Key, "flyology_debug_native_thread_key");
   ...
   begin
      ...
      if Group
         in Interfaces.C.int (Flyology.Execution_Groups.Group_Id'First)
          .. Interfaces.C.int (Flyology.Execution_Groups.Group_Id'Last)
      then
         return Natural (Group) mod Producer_Count + 1;
      elsif Group /= -1 then
         raise Flyology.Execution_Groups.Group_Error
           with "invalid current execution group from Flyology runtime";
      end if;

      return Positive (Native_Thread_Key mod Interfaces.Unsigned_64 (Producer_Count) + 1);
   end Choose;
