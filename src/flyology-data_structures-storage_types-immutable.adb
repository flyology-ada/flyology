...
package body Flyology.Data_Structures.Storage_Types.Immutable is
   ...
   begin
      ...
      if Address > Addressing.Integer_Address'Last -
        Addressing.Integer_Address (Offset)
      then
         raise Region_Error with "immutable element field address overflows";
      end if;
      Address := Address + Addressing.Integer_Address (Offset);
      if Alignment > 1
        and then Address mod Addressing.Integer_Address (Alignment) /= 0
      then
   ...
   function Start return Value_Builder is
     (Data => (others => 0), Active => True);
   ...
   function Load_U8
     (Item : Const_Ref; Offset : Natural) return Interfaces.Unsigned_8 is
   begin
      Require (Item);
   ...
   function Load_U32
     (Item : Const_Ref; Offset : Natural) return Interfaces.Unsigned_32 is
   begin
      Require (Item);
   ...
   function Load_U64
     (Item : Const_Ref; Offset : Natural) return Interfaces.Unsigned_64 is
   begin
      Require (Item);
   ...
   procedure Store_U32
     (Item : in out Builder;
      Offset : Natural;
      Data : Interfaces.Unsigned_32) is
   begin
      Require (Item);
   ...
   begin
      Require (Item);
      Bytes.Copy
        (Field_Address (Item.Data'Address, Offset, 4, 1),
         Data'Address, Interfaces.C.size_t (4));
   end Store_U32;
   ...
   begin
      Require (Item);
      Bytes.Copy
        (Field_Address (Item.Data'Address, Offset, 8, 1),
         Data'Address, Interfaces.C.size_t (8));
   end Store_U64;
   ...
   begin
      ...
      Bytes.Copy
        (Result'Address, Source.Base, Interfaces.C.size_t (Byte_Size));
      return Result;
   end Copy_From;

   procedure Bind
     (Item : out Const_Ref; Source : Immutable_Storage_View) is
   begin
      Check_Contract (Source);
   ...
   procedure Bind
     (Item : out Builder; Target : Immutable_Storage_View) is
   begin
      Check_Contract (Target);
      if not Target.Writable then
