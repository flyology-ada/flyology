...
package body Flyology.Data_Structures.Dynamic.Byte_Strings is
   ...
   Capacity_Check_Mask : constant Interfaces.Unsigned_64 := 16#D71B_405E_29A6_F38C#;
   ...
   begin
      ...
      Bytes_Required := Layouts.Checked_Multiply (Byte_Count (Initial_Capacity), Byte_Count (Element_Size));
      if Bytes_Required > Byte_Count (Metadata.Usable_Capacity) then
         raise Constraint_Error with "dynamic-byte-string initial capacity does not fit the arena";
      end if;
   ...
   is
      ...
      Core     : Layouts.Local_View;
   begin
      Validate_Configuration (Arena, Initial_Capacity, Element_Size, Metadata);
      Detach (Item);
   ...
   begin
      ...
      case Claim is
         ...
         when Layouts.Existing_Ready    =>
            Attach_Core (Item, Region, Location, Arena, Initial_Capacity, Element_Size);
   ...
   begin
      ...
      then
         raise Layout_Error with "dynamic byte string is attached to another arena incarnation";
      end if;
   end Require_Arena;
   ...
   begin
      if Value = Arena_Provider.Null_Allocation then
         if Capacity /= 0 then
            raise Layout_Error with "dynamic-byte-string capacity has no allocation";
         end if;
         return;
      elsif Capacity = 0 then
         raise Layout_Error with "dynamic-byte-string allocation has zero capacity";
      end if;
      begin
         Block := Arena_Provider.Block_Capacity (Arena, Value);
      exception
         when Handle_Error =>
            raise Layout_Error with "dynamic-byte-string allocation handle is stale";
      end;
      Payload := Layouts.Checked_Multiply (Byte_Count (Capacity), Byte_Count (Item.Element_Value));
      if Payload > Block or else Block - Payload >= Byte_Count (Item.Element_Value) then
         raise Layout_Error with "dynamic-byte-string allocation capacity is inconsistent";
      end if;
   ...
   begin
      ...
      then
         raise Layout_Error with "dynamic-byte-string creation parameters do not match";
      ...
      if Retired /= Arena_Provider.Null_Allocation then
         if Retired = Current then
            raise Layout_Error with "dynamic-byte-string current and retired allocations alias";
         end if;
         declare
   ...
   exception
      when Handle_Error =>
         ...
         raise Layout_Error with "dynamic-byte-string deferred allocation handle is stale";
      when others =>
         if Item.Core.Attached then
            Detach (Item);
         end if;
   ...
   begin
      Initialize_Core (Item, Region, Location, Arena, Initial_Capacity, 1);
   end Initialize;
   ...
   begin
      Create_Or_Attach_Core (Item, Region, Location, Arena, Initial_Capacity, 1, Result);
   ...
   begin
      ...
      Requested_Bytes := Layouts.Checked_Multiply (Target, Byte_Count (Item.Element_Value));
      ...
      begin
         ...
         if New_Capacity < Byte_Count (Required) or else New_Capacity > Byte_Count (Natural'Last) then
            raise Layout_Error with "arena returned an unusable dynamic-byte-string block";
         end if;
         Copy_Length :=
           Layouts.Checked_Multiply (Byte_Count (Stored_Length (Item)), Byte_Count (Item.Element_Value));
   ...
   begin
      ...
      begin
         ...
         if New_Length = 0 then
            ...
            Ensure_Capacity (Item, Arena, Positive (New_Length), Mutated, Result);
            if Result = Completed then
               Current := Read_Handle (Item.Current_Address);
               Mutated := True;
               Arena_Provider.Write (Arena, Current, 0, Data);
               Bytes.Write_U64 (Item.Length_Address, Interfaces.Unsigned_64 (New_Length));
            end if;
         end if;
      exception
         when others =>
      ...
      begin
         ...
         if Added = 0 then
            ...
            if Required > Byte_Count (Positive'Last) then
               ...
               Ensure_Capacity (Item, Arena, Positive (Required), Mutated, Result);
               if Result = Completed then
                  Current := Read_Handle (Item.Current_Address);
                  Mutated := True;
                  Arena_Provider.Write (Arena, Current, Byte_Count (Current_Length), Data);
                  Bytes.Write_U64 (Item.Length_Address, Interfaces.Unsigned_64 (Required));
               end if;
            end if;
         end if;
      exception
   ...
   procedure Read (Item : View; Arena : Arena_Provider.View; Data : out Ada.Streams.Stream_Element_Array) is
      Current_Length : Interfaces.Unsigned_64;
      Current        : Arena_Provider.Allocation_Handle;
   begin
      Acquire (Item);
      begin
         ...
         if Byte_Count (Data'Length) /= Byte_Count (Current_Length) then
            raise Constraint_Error with "dynamic-byte-string destination has the wrong length";
         ...
         if Current /= Arena_Provider.Null_Allocation then
            if Flyology.Dynamic_Destroy_Test_Hooks.Enabled then
               ...
               if Injected_Contention then
                  raise Busy_Error with "injected dynamic-string arena contention";
               end if;
            end if;
