...
is
   ...
   procedure Drive (Item : in out Operation; Event : Driver_Event) is abstract;
   ...
   with Pre => Required <= Set.Capacity and then Completed.Capacity = Set.Capacity;
...
private
   ...
   type Source_Kind is (No_Source, Descriptor_Source, Timer_Source, Dependency_Source, Immediate_Source);
   ...
   type Completion_Set (Capacity : Operation_Capacity) is tagged limited record
   ...
   type Member_Generation_Array is array (Operation_Id) of Interfaces.Unsigned_64;
   ...
   overriding
   procedure Drive (Item : in out Gate_Operation; Event : Driver_Event);

   --  @exclude
   --  @param Item Gate to detach and terminalize
   overriding
   procedure Request_Cancellation (Item : in out Gate_Operation);
   ...
   procedure Publish_Terminal (Item : in out Operation'Class; Result : Terminal_Outcome);
