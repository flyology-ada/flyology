...
package body Flyology.Data_Structures.Dynamic.Hash_Maps is
   ...
   Capacity_Check_Mask : constant Interfaces.Unsigned_64 := 16#8E52_CA19_73B4_06DF#;

   function Capacity_Check (Capacity : Interfaces.Unsigned_64) return Interfaces.Unsigned_64
   is (Capacity xor Capacity_Check_Mask);
   ...
   function Field_At (Address : System.Address; Offset : Native.Storage_Offset) return System.Address
   is (Address + Offset);
   pragma Inline_Always (Field_At);

   function Read_Handle (Address : System.Address) return Arena_Provider.Allocation_Handle
   is ((Token => Bytes.Read_U64 (Address), Generation => Bytes.Read_U64 (Field_At (Address, 8))));

   procedure Write_Handle (Address : System.Address; Value : Arena_Provider.Allocation_Handle) is
   begin
      Bytes.Write_U64 (Address, Value.Token);
      Bytes.Write_U64 (Field_At (Address, 8), Value.Generation);
   end Write_Handle;

   procedure Slot_Geometry (Key_Offset, Value_Offset, Stride : out Byte_Count) is
   begin
   ...
   begin
      if Initial_Capacity < 2 or else not Policy.Is_Power_Of_Two (Byte_Count (Initial_Capacity)) then
         raise Constraint_Error with "dynamic-map initial capacity must be a power of two at least two";
      end if;
      ...
      if Key.Signature = 0
        ...
        or else Metadata.Minimum_Block_Size < Interfaces.Unsigned_32 (Storage_Alignment)
      then
         raise Constraint_Error with "arena cannot satisfy the immutable map element contract";
      end if;
      Slot_Geometry (Key_Offset, Value_Offset, Stride);
      Initial_Bytes := Layouts.Checked_Multiply (Byte_Count (Initial_Capacity), Stride);
      if Initial_Bytes > Byte_Count (Metadata.Usable_Capacity) then
         raise Constraint_Error with "dynamic-map initial table does not fit the arena";
      end if;
   ...
   begin
      ...
      Item.Capacity_Address := Layouts.Address_At (Core, Capacity_Offset, 8, 8);
      Item.Capacity_Check_Address := Layouts.Address_At (Core, Capacity_Check_Offset, 8, 8);
      Item.Current_Address := Layouts.Address_At (Core, Current_Offset, 16, 8);
      Item.Retired_Address := Layouts.Address_At (Core, Retired_Offset, 16, 8);
   ...
   begin
      ...
      Bytes.Write_U32 (Layouts.Address_At (Core, Arena_Epoch_Offset, 4, 4), Arena_Epoch);
      Bytes.Write_U32 (Layouts.Address_At (Core, Reserved_32_Offset, 4, 4), 0);
      Bytes.Write_U64 (Layouts.Address_At (Core, Reserved_2_Offset, 8, 8), 0);
   ...
   is
      ...
      Core                             : Layouts.Local_View;
   begin
      Validate_Configuration (Arena, Initial_Capacity, Metadata, Key_Offset, Value_Offset, Stride);
      Detach (Item);
      ...
      Finish_Initialize
        (Item,
         Core,
         Interfaces.Unsigned_32 (Initial_Capacity),
         Interfaces.Unsigned_32 (Key.Size),
         Interfaces.Unsigned_32 (Element.Size),
   ...
   begin
      ...
      case Claim is
         ...
         when Layouts.Existing_Ready    =>
            Attach (Item, Region, Location, Arena, Initial_Capacity);
   ...
   procedure Require_Arena (Item : View; Arena : Arena_Provider.View) is
      Metadata : constant Arena_Provider.Metadata := Arena_Provider.Current_Metadata (Arena);
   begin
      if Metadata.Instance_ID /= Item.Arena_ID_Value or else Metadata.Incarnation /= Item.Arena_Epoch_Value
      then
         raise Layout_Error with "dynamic map is attached to another arena incarnation";
      end if;
   end Require_Arena;

   function Table_Bytes (Item : View; Capacity : Interfaces.Unsigned_64) return Byte_Count
   is (Layouts.Checked_Multiply (Byte_Count (Capacity), Item.Stride));
   ...
   begin
      ...
      if Region.Length_Value < Required or else Region.Base = System.Null_Address then
         raise Layout_Error with "dynamic-map allocation is too small";
      end if;
      Table := (Base => Region.Base, Extent => Required, Capacity => Interfaces.Unsigned_32 (Capacity));
   exception
      when Handle_Error =>
   ...
   function Slot_Address
     (Item : View; Table : Table_View; Index : Interfaces.Unsigned_64) return System.Address
   is
      Relative : Byte_Count;
   begin
      if Table.Base = System.Null_Address or else Index >= Interfaces.Unsigned_64 (Table.Capacity) then
   ...
   function Slot_Field (Item : View; Slot : System.Address; Offset, Extent : Byte_Count) return System.Address
   is
   begin
      if Offset > Item.Stride
   ...
   function Key_Binding (Item : View; Slot : System.Address; Writable : Boolean) return Immutable_Storage_View
   is (Base      => Slot_Field (Item, Slot, Item.Key_Offset, Byte_Count (Key.Size)),
   ...
   function Value_Binding
     (Item : View; Slot : System.Address; Writable : Boolean) return Immutable_Storage_View
   is (Base      => Slot_Field (Item, Slot, Item.Value_Offset, Byte_Count (Element.Size)),
       Extent    => Byte_Count (Element.Size),
       Signature => Element.Signature,
       Version   => Element.Version,
       Writable  => Writable);

   function Stored_Key_Hash (Item : View; Slot : System.Address) return Interfaces.Unsigned_64 is
   ...
   procedure Validate_Table (Item : View; Table : Table_View; Expected_Count : Interfaces.Unsigned_64) is
      ...
      Reached                     : Boolean;
   begin
      for Index in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Table.Capacity) - 1 loop
         Slot := Slot_Address (Item, Table, Index);
         State := Bytes.Read_U32 (Slot_Field (Item, Slot, Slot_State_Offset, 4));
         if Bytes.Read_U32 (Slot_Field (Item, Slot, Slot_Reserved_Offset, 4)) /= 0 then
      ...
      for Index in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Table.Capacity) - 1 loop
         ...
         if State = Occupied_State then
            Stored_Hash := Bytes.Read_U64 (Slot_Field (Item, Slot, Hash_Offset, 8));
            if Stored_Hash /= Stored_Key_Hash (Item, Slot) then
               raise Layout_Error with "dynamic-map stored key hash is corrupt";
            end if;
            Reached := False;
            for Probe in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Table.Capacity) - 1 loop
               Candidate := Policy.Masked_Index (Stored_Hash + Probe, Policy.Positive_U32 (Table.Capacity));
               Candidate_Entry := Slot_Address (Item, Table, Candidate);
               Candidate_State := Bytes.Read_U32 (Slot_Field (Item, Candidate_Entry, Slot_State_Offset, 4));
               if Candidate_State = Empty_State then
                  raise Layout_Error with "dynamic-map entry is outside its probe chain";
               elsif Candidate = Index then
                  Reached := True;
                  exit;
               elsif Candidate_State = Occupied_State then
                  Candidate_Hash := Bytes.Read_U64 (Slot_Field (Item, Candidate_Entry, Hash_Offset, 8));
                  if Candidate_Hash = Stored_Hash then
                     declare
                        Candidate_Key : Key.Const_Ref;
                        Stored_Key    : Key.Const_Ref;
                     begin
                        Key.Bind (Candidate_Key, Key_Binding (Item, Candidate_Entry, False));
                        Key.Bind (Stored_Key, Key_Binding (Item, Slot, False));
                        if Key.Equivalent (Candidate_Key, Stored_Key) then
                           raise Layout_Error with "dynamic-map contains duplicate keys";
                        end if;
                     end;
                  end if;
               end if;
   ...
   begin
      ...
      Arena_Epoch := Bytes.Read_U32 (Layouts.Address_At (Core, Arena_Epoch_Offset, 4, 4));
      ...
      then
         raise Layout_Error with "dynamic-map creation parameters do not match";
      elsif Bytes.Read_U32 (Layouts.Address_At (Core, Reserved_32_Offset, 4, 4)) /= 0
        or else Bytes.Read_U64 (Layouts.Address_At (Core, Reserved_2_Offset, 8, 8)) /= 0
      then
      ...
      if Bytes.Read_U64 (Item.Capacity_Check_Address) /= Capacity_Check (Current_Capacity)
      ...
      if Current = Arena_Provider.Null_Allocation and then Expected_Count /= 0 then
      ...
      if Retired /= Arena_Provider.Null_Allocation then
         if Retired = Current then
            raise Layout_Error with "dynamic-map current and retired allocations alias";
         end if;
   ...
   function Stored_Capacity (Item : View) return Interfaces.Unsigned_64 is
      Value : constant Interfaces.Unsigned_64 := Bytes.Read_U64 (Item.Capacity_Address);
   begin
      if Bytes.Read_U64 (Item.Capacity_Check_Address) /= Capacity_Check (Value)
   ...
   function Stored_Count (Item : View) return Interfaces.Unsigned_64 is
      Count            : constant Interfaces.Unsigned_64 := Bytes.Read_U64 (Item.Count_Address);
      Current_Capacity : constant Interfaces.Unsigned_64 := Stored_Capacity (Item);
   begin
   ...
   function Key_Matches
     (Item : View; Slot : System.Address; Hash_Value : Interfaces.Unsigned_64; Stored_Key : Key.Value)
      return Boolean
   is
      Reference : Key.Const_Ref;
   begin
      if Bytes.Read_U64 (Slot_Field (Item, Slot, Hash_Offset, 8)) /= Hash_Value then
         return False;
      end if;
      Key.Bind (Reference, Key_Binding (Item, Slot, False));
   ...
   begin
      for Probe in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Table.Capacity) - 1 loop
         ...
         State := Bytes.Read_U32 (Slot_Field (Item, Slot, Slot_State_Offset, 4));
         if State = Occupied_State and then Key_Matches (Item, Slot, Hash_Value, Stored_Key) then
            Found := True;
            Index := Candidate;
            Insertion := Candidate;
            return;
   ...
   begin
      for Index in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Table.Capacity) - 1 loop
         Slot := Slot_Address (Item, Table, Index);
         Bytes.Write_U32 (Slot_Field (Item, Slot, Slot_State_Offset, 4), Empty_State);
         Bytes.Write_U32 (Slot_Field (Item, Slot, Slot_Reserved_Offset, 4), 0);
      end loop;
   end Initialize_Table;

   procedure Insert_Stored (Item : View; Source : System.Address; Target_Table : Table_View) is
      Hash_Value : constant Interfaces.Unsigned_64 :=
        Bytes.Read_U64 (Slot_Field (Item, Source, Hash_Offset, 8));
      Candidate  : Interfaces.Unsigned_64;
      Target     : System.Address;
   begin
      for Probe in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Target_Table.Capacity) - 1 loop
         Candidate := Policy.Masked_Index (Hash_Value + Probe, Policy.Positive_U32 (Target_Table.Capacity));
         Target := Slot_Address (Item, Target_Table, Candidate);
         if Bytes.Read_U32 (Slot_Field (Item, Target, Slot_State_Offset, 4)) = Empty_State then
            Bytes.Write_U64 (Slot_Field (Item, Target, Hash_Offset, 8), Hash_Value);
            Key.Copy (Key_Binding (Item, Source, False), Key_Binding (Item, Target, True));
            Element.Copy (Value_Binding (Item, Source, False), Value_Binding (Item, Target, True));
            Bytes.Write_U32 (Slot_Field (Item, Target, Slot_State_Offset, 4), Occupied_State);
            return;
         end if;
      end loop;
   ...
   is
      Current_Capacity     : constant Interfaces.Unsigned_64 := Stored_Capacity (Item);
      Current              : constant Arena_Provider.Allocation_Handle := Read_Handle (Item.Current_Address);
      Target               : Byte_Count;
      Requested            : Byte_Count;
      New_Handle           : Arena_Provider.Allocation_Handle := Arena_Provider.Null_Allocation;
   ...
   begin
      ...
      Target :=
        (if Current_Capacity = 0 then Byte_Count (Item.Initial_Value) else Byte_Count (Current_Capacity) * 2);
      ...
      Arena_Provider.Try_Allocate (Arena, Positive (Requested), New_Handle, Allocation);
      case Allocation is
      ...
      begin
         Attach_Table (Item, Arena, New_Handle, Interfaces.Unsigned_64 (Target), New_Table);
         Initialize_Table (Item, New_Table);
         if Current /= Arena_Provider.Null_Allocation then
            Attach_Table (Item, Arena, Current, Current_Capacity, Old_Table);
            for Index in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Old_Table.Capacity) - 1
            loop
               Slot := Slot_Address (Item, Old_Table, Index);
               if Bytes.Read_U32 (Slot_Field (Item, Slot, Slot_State_Offset, 4)) = Occupied_State then
      ...
      Bytes.Write_U64 (Item.Capacity_Address, Interfaces.Unsigned_64 (Target));
      Bytes.Write_U64 (Item.Capacity_Check_Address, Capacity_Check (Interfaces.Unsigned_64 (Target)));
      Result := Put_Inserted;
      ...
      begin
         ...
         if Current_Capacity /= 0 then
            ...
            if Found then
               ...
               begin
                  Element.Bind (Reference, Value_Binding (Item, Slot, False));
