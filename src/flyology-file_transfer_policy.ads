...
is
   ...
   type Completion_Action is
     (Return_Progress, Raise_Cancelled, Use_Buffered_Fallback, Raise_Socket_Error, Raise_Invalid_Completion);
   ...
   with
     ...
     Global         => null,
     Contract_Cases =>
       (Status = 0 and then Transferred > 0 and then Transferred <= Limit
        => Classify'Result = Return_Progress,
        Status = 0 and then Transferred > Limit
        => Classify'Result = Raise_Invalid_Completion,
        Status = 0 and then Transferred <= 0 and then Cancelled /= 0
        => Classify'Result = Raise_Cancelled,
        Status = 0 and then Transferred <= 0 and then Cancelled = 0 and then Error_Code = Not_Supported_Error
        ...
        Status = 1 and then Transferred = 0 and then Error_Code = 0 and then Cancelled = 0
        => Classify'Result = Use_Buffered_Fallback,
        others
        => Classify'Result = Raise_Invalid_Completion);
