...
package body Gnatevl.IO.Files is
   ...
   pragma Import (C, Event_File_IO, "gnatevl_runtime_file_io");
   ...
   begin
      ...
      declare
         Evented : constant Boolean := Is_Evented_Task;
      begin
         Cancellation_Source (Token, Evented, Cancel_FD);
         if Evented then
            Status :=
              Event_File_IO
                (C.int (File),
      ...
      if Status /= 0 and then Error_Code = 0 then
         raise Device_Error with "evented pread submission failed";
   ...
   begin
      ...
      if Status /= 0 and then Error_Code = 0 then
         raise Device_Error with "evented pwrite submission failed";
      elsif Cancelled /= 0 then
         raise Operation_Cancelled;
      elsif Error_Code /= 0 then
