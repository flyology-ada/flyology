...
is
   ...
   type Received_Address_Action is (Use_Endpoint, Use_No_Endpoint, Fail_Receive);

   function Family_Code (IPv6 : Boolean) return C.int
   with Global => null, Post => Family_Code'Result = (if IPv6 then 6 else 4);

   function Mode_Code (Datagram : Boolean) return C.int
   with Global => null, Post => Mode_Code'Result = (if Datagram then 2 else 1);

   function Should_Enable_Datagram_Metadata (Mode : C.int) return Boolean
   with Global => null, Post => Should_Enable_Datagram_Metadata'Result = (Mode = 2);

   function Classify_Post_Accept_Failure (Stage : Post_Accept_Failure_Stage) return Post_Accept_Failure_Action
   with
     Global         => null,
     Contract_Cases =>
       (Stage = Peer_Address_Decode      => Classify_Post_Accept_Failure'Result = Fail_Listener,
        Stage = Descriptor_Configuration => Classify_Post_Accept_Failure'Result = Discard_Accepted_Peer);
   ...
   with
     ...
     Contract_Cases =>
       (not Address_Present                                                                            =>
        ...
        Address_Present and then not Decode_Succeeded and then Decode_Error = Unsupported_Family_Error =>
          Classify_Received_Address'Result = Use_No_Endpoint,
        others                                                                                         =>
   ...
   with
     ...
     Post   =>
       (if Error_Code = 0
        then Classify_Error'Result = Success
        elsif Error_Code = Would_Block_Error
        then Classify_Error'Result = Would_Block
        elsif Error_Code = Interrupted_Error
        then Classify_Error'Result = Interrupted
        elsif Error_Code = In_Progress_Error
        then Classify_Error'Result = In_Progress
        elsif Error_Code = Already_In_Progress_Error
        then Classify_Error'Result = Already_In_Progress
        elsif Error_Code = Already_Connected_Error
        then Classify_Error'Result = Already_Connected
        elsif Error_Code = No_Buffer_Space_Error
        then Classify_Error'Result = No_Buffer_Space
        else Classify_Error'Result = Other_Error);
   ...
   with
     ...
     Post   =>
       (if Kind = Would_Block
        then Classify_IO_Error'Result = Wait_For_Ready
        elsif Kind = Interrupted
        then Classify_IO_Error'Result = Retry_Operation
        else Classify_IO_Error'Result = Fail_Operation);
   ...
   function Classify_Connect_Error (Kind : Error_Kind) return Connect_Error_Action
   with
     Global         => null,
     Contract_Cases =>
