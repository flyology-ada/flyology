...
--  Provides variable-size TLSF allocation inside a fixed caller-owned
--  relocatable region. Two-level bitmaps select one of a fixed number of
--  offset-linked free lists; physical-neighbor sizes support checked splitting
--  and coalescing. Stored metadata contains only fixed-width scalar fields.
--  Allocation and reclamation are serialized across mappings by one
...
--  recovery.
--  @exclude
package Flyology.Data_Structures.Allocation_Algorithms.TLSF_Kernel
  with Preelaborate is

   --  Eight-byte magic stored in every arena header.
   Magic : constant Interfaces.Unsigned_64 := 16#4644_5354_4C53_4631#;

   --  Schema identifier for the offset-indexed TLSF contract.
   Schema : constant Interfaces.Unsigned_64 := 16#0001_544C_5346_0001#;
   ...
   --  Immutable TLSF geometry selected when opening an arena.
   ...
   --  @return Required shared header, index, padding, and managed block bytes
   ...
   --  chain, two-level bitmaps, free-list links, and immutable configuration.
   --  @param Item View attached on success
   --  @param Region Independently attached backing region
   --  @param Location Stored arena offset
   --  @param Configuration Expected managed capacity and minimum block size
   --  @param Instance_ID Expected arena instance identity
   --  @exception Layout_Error Stored identity, geometry, or index is corrupt
   ...
   --  rounded to the arena quantum and its TLSF size class. A fitting block is
...
end Flyology.Data_Structures.Allocation_Algorithms.TLSF_Kernel;
