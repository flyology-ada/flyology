...
package body Flyology.Data_Structures.Rings.MPMC is
   ...
   begin
      ...
      Payload_Offset := Layouts.Align_Up (Sequence_Size, Byte_Count (Element.Alignment));
      Stride :=
        Layouts.Align_Up (Layouts.Checked_Add (Payload_Offset, Byte_Count (Element.Size)), Storage_Alignment);
      Extent := Layouts.Checked_Add (Slots_Offset, Layouts.Checked_Multiply (Byte_Count (Capacity), Stride));
   end Geometry;
   ...
   begin
      ...
      Item.Stride := Stride;
      Item.Enqueue_Address := Layouts.Address_At (Core, Enqueue_Offset, 8, 8);
      Item.Dequeue_Address := Layouts.Address_At (Core, Dequeue_Offset, 8, 8);
   end Set_View;

   function Slot_Relative (Item : View; Position : Interfaces.Unsigned_64) return Byte_Count is
   begin
      return
        Layouts.Checked_Add
          (Slots_Offset,
           Layouts.Checked_Multiply
             (Byte_Count (Policy.Masked_Index (Position, Policy.Positive_U32 (Item.Capacity_Value))),
              Item.Stride));
   end Slot_Relative;

   function Slot_Address (Item : View; Position : Interfaces.Unsigned_64) return System.Address is
   begin
      return Layouts.Address_At (Item.Core, Slot_Relative (Item, Position), Item.Stride, Storage_Alignment);
   end Slot_Address;

   function Payload_Address (Item : View; Slot : System.Address) return System.Address is
   begin
      ...
      return Native."+" (Slot, Native.Storage_Offset (Item.Payload_Offset));
   end Payload_Address;

   pragma Inline_Always (Slot_Relative, Slot_Address, Payload_Address);
   ...
   begin
      Set_View (Item, Core, Capacity, Payload_Offset, Stride);
      ...
      for Slot in Interfaces.Unsigned_32 range 0 .. Capacity - 1 loop
         Atomic.Store_Release_U64
           (Slot_Address (Item, Interfaces.Unsigned_64 (Slot)), Interfaces.Unsigned_64 (Slot));
      end loop;
      Layouts.Publish (Item.Core);
   end Finish_Initialize;

   procedure Validate_Sequences (Item : View; Enqueue, Dequeue : Interfaces.Unsigned_64) is
      Occupancy                   : constant Interfaces.Unsigned_64 := Enqueue - Dequeue;
      Position, Expected_Sequence : Interfaces.Unsigned_64;
   begin
      if not Policy.Within_Capacity (Enqueue, Dequeue, Item.Capacity_Value) then
         raise Layout_Error with "MPMC ring claim positions are corrupt";
      end if;
      for Offset in Interfaces.Unsigned_64 range 0 .. Interfaces.Unsigned_64 (Item.Capacity_Value) - 1 loop
         Position := Dequeue + Offset;
         Expected_Sequence := (if Offset < Occupancy then Position + 1 else Position);
         if Atomic.Load_Acquire_U64 (Slot_Address (Item, Position)) /= Expected_Sequence then
   ...
   is
      Core                           : Layouts.Local_View;
      Payload_Offset, Stride, Extent : Byte_Count;
      Capacity_32                    : constant Interfaces.Unsigned_32 := Interfaces.Unsigned_32 (Capacity);
   begin
      Detach (Item);
      Geometry (Capacity, Payload_Offset, Stride, Extent);
      Layouts.Begin_Initialize
        (Core,
         ...
         Storage_Alignment);
      Finish_Initialize (Item, Core, Capacity_32, Payload_Offset, Stride);
   exception
      when others =>
   ...
   begin
      ...
      Set_View (Item, Core, Header.Capacity, Payload_Offset, Stride);
   ...
   function Binding (Item : View; Slot : System.Address; Writable : Boolean) return Immutable_Storage_View
   ...
   procedure Try_Push (Item : in out View; Data : Element.Source; Result : out Push_Result) is
   ...
   begin
      ...
      Position := Atomic.Load_Relaxed_U64 (Item.Enqueue_Address);
      for Attempt in 1 .. Contention_Limit loop
         ...
         if Relation = Policy.Sequence_Ready then
            Expected := Position;
            if Atomic.Compare_Exchange_U64 (Item.Enqueue_Address, Expected, Position + 1) then
               Element.Copy_To (Stored, Binding (Item, Slot, True));
               Atomic.Store_Release_U64 (Slot, Position + 1);
               Result := Pushed;
               return;
            end if;
            Position := Expected;
   ...
   procedure Push (Item : in out View; Data : Element.Source; Timeout : Wait_Timeout) is
   ...
   procedure Try_Pop (Item : in out View; Data : out Element.Observed; Result : out Pop_Result) is
      Position : Interfaces.Unsigned_64;
      Sequence : Interfaces.Unsigned_64;
      Expected : Interfaces.Unsigned_64;
      Relation : Policy.Sequence_Relation;
   ...
   begin
      Layouts.Require_Ready (Item.Core);
      Position := Atomic.Load_Relaxed_U64 (Item.Dequeue_Address);
      for Attempt in 1 .. Contention_Limit loop
         pragma Unreferenced (Attempt);
         Slot := Slot_Address (Item, Position);
         Sequence := Atomic.Load_Acquire_U64 (Slot);
         ...
         if Relation = Policy.Sequence_Ready then
            Expected := Position;
            if Atomic.Compare_Exchange_U64 (Item.Dequeue_Address, Expected, Position + 1) then
   ...
   procedure Pop (Item : in out View; Data : out Element.Observed; Timeout : Wait_Timeout) is
      Wait   : Waiting.Context := Waiting.Start (Timeout);
      Result : Pop_Result;
   begin
      loop
   ...
   begin
      Layouts.Require_Ready (Item.Core);
      Enqueue := Atomic.Load_Acquire_U64 (Item.Enqueue_Address);
      Dequeue := Atomic.Load_Acquire_U64 (Item.Dequeue_Address);
