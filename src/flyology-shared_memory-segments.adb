...
package body Flyology.Shared_Memory.Segments is
   ...
   Segment_Magic : constant Interfaces.Unsigned_64 := 16#464C_594F_5345_474D#;

   function CAS_U32
     (Address : System.Address; Expected : access Interfaces.Unsigned_32; Desired : Interfaces.Unsigned_32)
      return C.int
   is
      Local  : Interfaces.Unsigned_32 := Expected.all;
      Result : constant Boolean := Atomic.Compare_Exchange_U32 (Address, Local, Desired);
   begin
      Expected.all := Local;
   ...
   function Power_Of_Two (Value : Byte_Length) return Boolean
   is (Value /= 0 and then (Value and (Value - 1)) = 0);

   function Align_Up (Value, Alignment : Byte_Length) return Byte_Length is
   ...
   begin
      ...
      return (if Remainder = 0 then Value else Add (Value, Alignment - Remainder));
   end Align_Up;
   ...
   begin
      ...
      if Alignment < 8 or else not Power_Of_Two (Alignment) then
         raise Constraint_Error
           with "segment allocation alignment must be a power of two " & "at least eight";
      end if;
      Slot_Size := Align_Up (Add (Name_Offset, Name_Max), 8);
      Registry_End := Add (Header_Size, Multiply (Capacity, Slot_Size));
      return (Slot_Size => Slot_Size, Data_Start => Align_Up (Registry_End, Alignment));
   end Stored_Geometry;

   function Required_Registry_Storage (Config : Configuration) return Byte_Length
   is (Stored_Geometry (Config).Data_Start);

   function Address_At (Item : View; Offset : Byte_Length; Size : Byte_Length) return System.Address is
   begin
      if not Item.Attached or else Item.Base = System.Null_Address then
         raise Segment_Error with "detached segment view";
      elsif Size = 0 or else Offset > Item.Extent or else Size > Item.Extent - Offset then
   ...
   function At_Base
     (Base : System.Address; Extent : Byte_Length; Offset : Byte_Length; Size : Byte_Length)
      return System.Address is
   begin
      if Base = System.Null_Address
        or else Size = 0
        or else Offset > Extent
        or else Size > Extent - Offset
        or else Offset > Byte_Length (Storage.Storage_Offset'Last)
      then
         raise Segment_Error with "segment initialization field is out of range";
      end if;
      return Base + Storage.Storage_Offset (Offset);
   end At_Base;

   function Read_U32 (Address : System.Address) return Interfaces.Unsigned_32 is
   begin
      return Atomic.Load_Acquire_U32 (Address);
   end Read_U32;

   function Read_U64 (Address : System.Address) return Interfaces.Unsigned_64 is
   begin
      return Atomic.Load_Acquire_U64 (Address);
   end Read_U64;

   procedure Write_U32 (Address : System.Address; Value : Interfaces.Unsigned_32) is
   begin
      Atomic.Store_Release_U32 (Address, Value);
   end Write_U32;

   procedure Write_U64 (Address : System.Address; Value : Interfaces.Unsigned_64) is
   begin
      Atomic.Store_Release_U64 (Address, Value);
   end Write_U64;

   function Slot_Base (Item : View; Slot : Interfaces.Unsigned_32) return Byte_Length is
   begin
      if Slot = 0 or else Slot > Item.Capacity then
         raise Segment_Error with "segment registry slot is out of range";
      end if;
      return Add (Header_Size, Multiply (Byte_Length (Slot - 1), Byte_Length (Item.Slot_Size)));
   end Slot_Base;

   function Slot_At
     (Item : View; Slot : Interfaces.Unsigned_32; Relative : Byte_Length; Size : Byte_Length)
      return System.Address is
   begin
      if Relative > Byte_Length (Item.Slot_Size) or else Size > Byte_Length (Item.Slot_Size) - Relative then
         raise Segment_Error with "segment slot field is out of range";
      end if;
      return Address_At (Item, Add (Slot_Base (Item, Slot), Relative), Size);
   end Slot_At;

   procedure Validate_Slot_Metadata (Item : View; Slot : Interfaces.Unsigned_32) is
      Generation  : constant Interfaces.Unsigned_64 :=
        Read_U64 (Slot_At (Item, Slot, Slot_Generation_Offset, 8));
      Name_Length : constant Interfaces.Unsigned_32 :=
        Read_U32 (Slot_At (Item, Slot, Slot_Name_Length_Offset, 4));
      Location    : constant Byte_Length :=
        Byte_Length (Read_U64 (Slot_At (Item, Slot, Slot_Location_Offset, 8)));
      Length      : constant Byte_Length :=
        Byte_Length (Read_U64 (Slot_At (Item, Slot, Slot_Length_Offset, 8)));
      Reserved    : constant Byte_Length :=
        Byte_Length (Read_U64 (Slot_At (Item, Slot, Slot_Reserved_Offset, 8)));
   ...
   begin
      ...
      elsif Location < Item.Data_Start or else Location mod Alignment /= 0 then
         raise Segment_Error with "segment slot location is corrupt";
      elsif Length = 0 or else Reserved = 0 or else Length > Reserved or else Reserved mod Alignment /= 0 then
         raise Segment_Error with "segment slot reservation is corrupt";
      elsif Location > Item.Extent or else Reserved > Item.Extent - Location then
   ...
   begin
      ...
      elsif Interfaces.Unsigned_64 (Name'Length) > Interfaces.Unsigned_64 (Item.Name_Limit) then
         raise Constraint_Error with "segment name exceeds configured limit";
      end if;
   end Validate_Name;

   function Same_Name
     (Item : View; Slot : Interfaces.Unsigned_32; Name : String; Hash : Interfaces.Unsigned_32) return Boolean
   is
   begin
      return
        Read_U32 (Slot_At (Item, Slot, Slot_Hash_Offset, 4)) = Hash
        and then Read_U32 (Slot_At (Item, Slot, Slot_Name_Length_Offset, 4))
                 = Interfaces.Unsigned_32 (Name'Length)
        and then Memory.Equal
                   (Slot_At (Item, Slot, Name_Offset, Byte_Length (Name'Length)),
                    Name'Address,
                    C.size_t (Name'Length));
   end Same_Name;
   ...
   function Null_Named_Handle return Named_Handle
   is ((0, 0));
   ...
   begin
      ...
      State := Atomic.Load_Acquire_U32 (Address_At (Item, Lifecycle_Offset, 4));
   ...
   begin
      Require_Ready (Item);
      if CAS_U32 (Address_At (Item, Guard_Offset, 4), Expected'Access, Guard_Locked) /= 0 then
         return True;
   ...
   begin
      Atomic.Store_Release_U32 (Address_At (Item, Guard_Offset, 4), Guard_Free);
   end Release_Guard;

   procedure Set_View (Item : in out View; Source : Mapping; Config : Configuration; Shape : Geometry) is
   begin
   ...
   begin
      ...
      if not Is_Mapped (Source) then
         raise Validation_Error with "cannot attach segment to an unmapped mapping";
      end if;
      Base := Mapping_Base (Source);
      Extent := Mapping_Length (Source);
      if Extent <= Shape.Data_Start then
         raise Constraint_Error with "mapping is too small for registry and one named extent";
      elsif Extent > Byte_Length (Storage.Storage_Offset'Last) then
         raise Constraint_Error with "segment mapping is not natively indexable";
      end if;

      State := Atomic.Load_Acquire_U32 (At_Base (Base, Extent, Lifecycle_Offset, 4));
      if State = Virgin_State and then not Mapping_May_Initialize (Source) then
         ...
         Claimed :=
           CAS_U32 (At_Base (Base, Extent, Lifecycle_Offset, 4), Expected'Access, Initializing_State) /= 0;
         if not Claimed then
            State := Expected;
         end if;
      end if;
      ...
      if Claimed then
         begin
            Memory.Zero
              (At_Base (Base, Extent, Version_Offset, Shape.Data_Start - Version_Offset),
               C.size_t (Shape.Data_Start - Version_Offset));
            Write_U32 (At_Base (Base, Extent, Version_Offset, 4), Layout_Version);
            Write_U64 (At_Base (Base, Extent, Magic_Offset, 8), Segment_Magic);
            Write_U64 (At_Base (Base, Extent, Schema_Offset, 8), Config.Schema);
            Write_U64 (At_Base (Base, Extent, Extent_Offset, 8), Interfaces.Unsigned_64 (Extent));
            Write_U32
              (At_Base (Base, Extent, Capacity_Offset, 4), Interfaces.Unsigned_32 (Config.Registry_Capacity));
            Write_U32
              (At_Base (Base, Extent, Name_Limit_Offset, 4),
               Interfaces.Unsigned_32 (Config.Maximum_Name_Length));
            Write_U32 (At_Base (Base, Extent, Slot_Size_Offset, 4), Interfaces.Unsigned_32 (Shape.Slot_Size));
            Write_U32
              (At_Base (Base, Extent, Alignment_Offset, 4),
               Interfaces.Unsigned_32 (Config.Allocation_Alignment));
            Write_U64
              (At_Base (Base, Extent, Data_Start_Offset, 8), Interfaces.Unsigned_64 (Shape.Data_Start));
            Atomic.Store_Release_U64
              (At_Base (Base, Extent, Next_Offset, 8), Interfaces.Unsigned_64 (Shape.Data_Start));
            Atomic.Store_Release_U64 (At_Base (Base, Extent, Generation_Offset, 8), 0);
            Atomic.Store_Release_U32 (At_Base (Base, Extent, Guard_Offset, 4), Guard_Free);
            Set_View (Item, Source, Config, Shape);
            Atomic.Store_Release_U32 (Address_At (Item, Lifecycle_Offset, 4), Ready_State);
            Result := Initialized_New;
            return;
         exception
            when others =>
               Atomic.Store_Release_U32 (At_Base (Base, Extent, Lifecycle_Offset, 4), Poisoned_State);
               Detach (Item);
               raise;
         end;
      end if;
      ...
      if Read_U32 (At_Base (Base, Extent, Version_Offset, 4)) /= Layout_Version
        or else Read_U64 (At_Base (Base, Extent, Magic_Offset, 8)) /= Segment_Magic
        or else Read_U64 (At_Base (Base, Extent, Schema_Offset, 8)) /= Config.Schema
        or else Read_U64 (At_Base (Base, Extent, Extent_Offset, 8)) /= Interfaces.Unsigned_64 (Extent)
      ...
      begin
         if Next < Interfaces.Unsigned_64 (Shape.Data_Start)
           ...
           or else Next mod Interfaces.Unsigned_64 (Config.Allocation_Alignment) /= 0
         then
   ...
   begin
      ...
      Frontier := Byte_Length (Atomic.Load_Acquire_U64 (Address_At (Source, Next_Offset, 8)));
      Admission :=
        Policy.Validate_Replacement
          (Source_Extent      => Interfaces.Unsigned_64 (Source.Extent),
           ...
           Alignment          => Interfaces.Unsigned_64 (Config.Allocation_Alignment),
           Native_Index_Limit => Interfaces.Unsigned_64 (Storage.Storage_Offset'Last),
           Source_Ready       =>
             Atomic.Load_Acquire_U32 (Address_At (Source, Lifecycle_Offset, 4)) = Ready_State,
           Same_Configuration =>
             Source.Schema = Config.Schema
             and then Source.Capacity = Interfaces.Unsigned_32 (Config.Registry_Capacity)
             and then Source.Name_Limit = Interfaces.Unsigned_32 (Config.Maximum_Name_Length)
             and then Source.Slot_Size = Interfaces.Unsigned_32 (Shape.Slot_Size)
             and then Source.Alignment = Interfaces.Unsigned_32 (Config.Allocation_Alignment)
             and then Source.Data_Start = Shape.Data_Start,
           Target_Mapped      => Is_Mapped (Target),
           Target_Exclusive   => Is_Mapped (Target) and then Mapping_May_Initialize (Target),
           Target_Virgin      =>
             Is_Mapped (Target)
             and then Atomic.Load_Acquire_U32 (At_Base (Target_Base, Target_Extent, Lifecycle_Offset, 4))
                      = Virgin_State,
           Distinct_Mappings  => Is_Mapped (Target) and then Target_Base /= Source.Base);
      ...
      case Admission is
         ...
         when Policy.Source_Invalid | Policy.Configuration_Mismatch =>
            raise Segment_Error with "source segment is not a valid replacement source";

         when Policy.Target_Not_Larger                              =>
            raise Constraint_Error with "replacement mapping must be strictly larger";

         when Policy.Target_Not_Indexable                           =>
            raise Constraint_Error with "replacement mapping is not natively indexable";

         when Policy.Target_Unmapped                                =>
            raise Validation_Error with "replacement mapping is unmapped";

         when Policy.Target_Not_Exclusive                           =>
            raise Validation_Error with "replacement mapping was not exclusively created";

         when Policy.Target_Not_Virgin                              =>
            raise Validation_Error with "replacement mapping lifecycle is not virgin";

         when Policy.Mapping_Aliased                                =>
            raise Validation_Error with "replacement mapping aliases the source";
      end case;

      if Read_U32 (Address_At (Source, Version_Offset, 4)) /= Layout_Version
        or else Read_U64 (Address_At (Source, Magic_Offset, 8)) /= Segment_Magic
        or else Read_U64 (Address_At (Source, Schema_Offset, 8)) /= Config.Schema
        or else Read_U64 (Address_At (Source, Extent_Offset, 8)) /= Interfaces.Unsigned_64 (Source.Extent)
        ...
        or else Read_U64 (Address_At (Source, Data_Start_Offset, 8))
                /= Interfaces.Unsigned_64 (Shape.Data_Start)
      then
         raise Segment_Error with "source segment header changed before replacement";
      end if;
      ...
      Frontier := Byte_Length (Atomic.Load_Acquire_U64 (Address_At (Source, Next_Offset, 8)));
      if Frontier < Source.Data_Start
        or else Frontier > Source.Extent
        or else Frontier mod Byte_Length (Source.Alignment) /= 0
      then
         raise Segment_Error with "source segment allocation frontier is corrupt";
      end if;

      for Slot in Interfaces.Unsigned_32 range 1 .. Source.Capacity loop
         declare
            State : constant Interfaces.Unsigned_32 :=
              Atomic.Load_Acquire_U32 (Slot_At (Source, Slot, Slot_State_Offset, 4));
         begin
            if State /= Free_Slot then
               declare
                  Slot_State  : constant Policy.Replacement_Slot_State :=
                    (if State = Initializing_Slot
                     then Policy.Slot_Initializing
                     elsif State = Ready_Slot
                     then Policy.Slot_Ready
                     elsif State = Failed_Slot
                     then Policy.Slot_Failed
                     elsif State = Removed_Slot
                     then Policy.Slot_Removed
                     else Policy.Slot_Invalid);
                  Generation  : constant Interfaces.Unsigned_64 :=
                    Read_U64 (Slot_At (Source, Slot, Slot_Generation_Offset, 8));
                  Name_Length : constant Interfaces.Unsigned_32 :=
                    Read_U32 (Slot_At (Source, Slot, Slot_Name_Length_Offset, 4));
                  Location    : constant Interfaces.Unsigned_64 :=
                    Read_U64 (Slot_At (Source, Slot, Slot_Location_Offset, 8));
                  Length      : constant Interfaces.Unsigned_64 :=
                    Read_U64 (Slot_At (Source, Slot, Slot_Length_Offset, 8));
                  Reserved    : constant Interfaces.Unsigned_64 :=
                    Read_U64 (Slot_At (Source, Slot, Slot_Reserved_Offset, 8));
                  Failure     : constant Interfaces.Unsigned_32 :=
                    Read_U32 (Slot_At (Source, Slot, Slot_Failure_Offset, 4));
               begin
                  if not Policy.Valid_Replacement_Slot
                           (State       => Slot_State,
                            ...
                            Reserved    => Reserved,
                            Failure     => Failure,
                            Data_Start  => Interfaces.Unsigned_64 (Source.Data_Start),
                            Frontier    => Interfaces.Unsigned_64 (Frontier),
                            Alignment   => Interfaces.Unsigned_64 (Source.Alignment))
                  then
                     raise Segment_Error with "source slot is invalid for replacement";
                  end if;
      ...
      Atomic.Store_Release_U32
        (At_Base (Target_Base, Target_Extent, Lifecycle_Offset, 4), Initializing_State);
      Memory.Zero
        (At_Base (Target_Base, Target_Extent, Version_Offset, Target_Extent - Version_Offset),
         C.size_t (Target_Extent - Version_Offset));
      Memory.Copy
        (At_Base (Target_Base, Target_Extent, Version_Offset, Frontier - Version_Offset),
         Address_At (Source, Version_Offset, Frontier - Version_Offset),
         C.size_t (Frontier - Version_Offset));
      Write_U64
        (At_Base (Target_Base, Target_Extent, Extent_Offset, 8), Interfaces.Unsigned_64 (Target_Extent));
      Atomic.Store_Release_U32 (At_Base (Target_Base, Target_Extent, Guard_Offset, 4), Guard_Free);
      Atomic.Store_Release_U32 (At_Base (Target_Base, Target_Extent, Lifecycle_Offset, 4), Ready_State);
   ...
   exception
      when others =>
         ...
         if Target_Touched then
            Atomic.Store_Release_U32
              (At_Base (Target_Base, Target_Extent, Lifecycle_Offset, 4), Poisoned_State);
         end if;
         raise;
   ...
   procedure Attach_Region (Item : View; Region : in out Flyology.Data_Structures.Regions.View) is
   begin
      Require_Ready (Item);
      Flyology.Data_Structures.Regions.Attach (Region, Item.Base, Item.Extent);
   end Attach_Region;

   procedure Fill_Handle (Item : View; Slot : Interfaces.Unsigned_32; Handle : out Named_Handle) is
   begin
      Handle := (Slot => Slot, Stamp => Read_U64 (Slot_At (Item, Slot, Slot_Generation_Offset, 8)));
   end Fill_Handle;
   ...
   begin
      ...
      for Slot in Interfaces.Unsigned_32 range 1 .. Item.Capacity loop
         declare
            State : constant Interfaces.Unsigned_32 :=
              Atomic.Load_Acquire_U32 (Slot_At (Item, Slot, Slot_State_Offset, 4));
         begin
            if State = Free_Slot then
               ...
               begin
                  if Reserved >= Requested_Length
                    and then (Reuse_Candidate = 0 or else Reserved < Reuse_Reserved)
                  then
                     Reuse_Candidate := Slot;
                     Reuse_Reserved := Reserved;
                  end if;
               end;
            elsif State = Initializing_Slot or else State = Ready_Slot or else State = Failed_Slot then
               Validate_Slot_Metadata (Item, Slot);
               if Same_Name (Item, Slot, Name, Hash) then
                  ...
                  if State = Initializing_Slot then
                     ...
                     Failure := Read_U32 (Slot_At (Item, Slot, Slot_Failure_Offset, 4));
                     if Failure = 0 then
                        raise Segment_Error with "failed segment entry has zero failure code";
                     end if;
                     Result := Previous_Initialization_Failed;
                  elsif Byte_Length (Read_U64 (Slot_At (Item, Slot, Slot_Length_Offset, 8)))
                    /= Requested_Length
                  then
      ...
      begin
         ...
         if Reuse_Candidate /= 0 then
            ...
            Location := Byte_Length (Read_U64 (Slot_At (Item, Slot, Slot_Location_Offset, 8)));
            ...
            Slot := Free_Candidate;
            Location := Byte_Length (Atomic.Load_Acquire_U64 (Address_At (Item, Next_Offset, 8)));
            Reserved := Align_Up (Requested_Length, Byte_Length (Item.Alignment));
            if Location < Item.Data_Start or else Location mod Byte_Length (Item.Alignment) /= 0 then
               raise Segment_Error with "segment allocation frontier is corrupt";
            elsif Location > Item.Extent or else Reserved > Item.Extent - Location then
               Release_Guard (Item);
               Acquired := False;
            ...
            Atomic.Store_Release_U64
              (Address_At (Item, Next_Offset, 8), Interfaces.Unsigned_64 (Location + Reserved));
         end if;

         Generation := Generation + 1;
         Atomic.Store_Release_U64 (Address_At (Item, Generation_Offset, 8), Generation);
         Memory.Zero
           (Slot_At (Item, Slot, Name_Offset, Byte_Length (Item.Slot_Size) - Name_Offset),
            C.size_t (Byte_Length (Item.Slot_Size) - Name_Offset));
         Write_U32 (Slot_At (Item, Slot, Slot_Name_Length_Offset, 4), Interfaces.Unsigned_32 (Name'Length));
         Write_U64 (Slot_At (Item, Slot, Slot_Generation_Offset, 8), Generation);
         Write_U32 (Slot_At (Item, Slot, Slot_Hash_Offset, 4), Hash);
         Write_U64 (Slot_At (Item, Slot, Slot_Location_Offset, 8), Interfaces.Unsigned_64 (Location));
         Write_U64 (Slot_At (Item, Slot, Slot_Length_Offset, 8), Interfaces.Unsigned_64 (Requested_Length));
         Write_U32 (Slot_At (Item, Slot, Slot_Failure_Offset, 4), 0);
         Write_U64 (Slot_At (Item, Slot, Slot_Reserved_Offset, 8), Interfaces.Unsigned_64 (Reserved));
         Memory.Copy
           (Slot_At (Item, Slot, Name_Offset, Byte_Length (Name'Length)),
            Name'Address,
            C.size_t (Name'Length));
         Atomic.Store_Release_U32 (Slot_At (Item, Slot, Slot_State_Offset, 4), Initializing_Slot);
      ...
      for Slot in Interfaces.Unsigned_32 range 1 .. Item.Capacity loop
         declare
            State : constant Interfaces.Unsigned_32 :=
              Atomic.Load_Acquire_U32 (Slot_At (Item, Slot, Slot_State_Offset, 4));
         begin
            if State = Initializing_Slot or else State = Ready_Slot or else State = Failed_Slot then
               Validate_Slot_Metadata (Item, Slot);
               if Same_Name (Item, Slot, Name, Hash) then
                  Fill_Handle (Item, Slot, Handle);
                  if State = Initializing_Slot then
   ...
   begin
      Require_Ready (Item);
      if Claim.Slot = 0
        or else Claim.Stamp = 0
        or else Claim.Location = DS.Null_Offset
        or else Claim.Length = 0
      then
         raise Segment_Error with "null segment creation claim";
      elsif Claim.Slot > Item.Capacity
        or else Read_U64 (Slot_At (Item, Claim.Slot, Slot_Generation_Offset, 8)) /= Claim.Stamp
        or else Atomic.Load_Acquire_U32 (Slot_At (Item, Claim.Slot, Slot_State_Offset, 4))
                /= Initializing_Slot
        or else Read_U64 (Slot_At (Item, Claim.Slot, Slot_Location_Offset, 8))
                /= Interfaces.Unsigned_64 (Claim.Location)
        or else Read_U64 (Slot_At (Item, Claim.Slot, Slot_Length_Offset, 8))
   ...
   procedure Claimed_Extent
     (Item : View; Claim : Creation_Claim; Location : out DS.Region_Offset; Length : out Byte_Length) is
   begin
   ...
   begin
      Validate_Claim (Item, Claim);
      if CAS_U32 (Slot_At (Item, Claim.Slot, Slot_State_Offset, 4), Expected'Access, Ready_Slot) = 0 then
         raise Segment_Error with "segment creation claim changed during publish";
      end if;
      Clear_Claim (Claim);
   end Publish;

   procedure Publish_Failure (Item : View; Claim : in out Creation_Claim; Failure : Failure_Code) is
      Expected : aliased Interfaces.Unsigned_32 := Initializing_Slot;
   begin
      Validate_Claim (Item, Claim);
      Write_U32 (Slot_At (Item, Claim.Slot, Slot_Failure_Offset, 4), Failure);
      if CAS_U32 (Slot_At (Item, Claim.Slot, Slot_State_Offset, 4), Expected'Access, Failed_Slot) = 0 then
         raise Segment_Error with "segment creation claim changed during failure publication";
      end if;
      Clear_Claim (Claim);
   end Publish_Failure;

   function State_Of (Item : View; Handle : Named_Handle) return Handle_State is
      State : Interfaces.Unsigned_32;
   begin
      Require_Ready (Item);
      if Handle.Slot = 0 and then Handle.Stamp = 0 then
         return Null_Handle;
      elsif Handle.Slot = 0 or else Handle.Stamp = 0 or else Handle.Slot > Item.Capacity then
         return Stale;
      end if;
      State := Atomic.Load_Acquire_U32 (Slot_At (Item, Handle.Slot, Slot_State_Offset, 4));
      if State = Free_Slot then
         return Stale;
      elsif Read_U64 (Slot_At (Item, Handle.Slot, Slot_Generation_Offset, 8)) /= Handle.Stamp then
         return Stale;
      end if;
      case State is
         when Initializing_Slot =>
            return Initializing;
   ...
   procedure Resolve
     (Item : View; Handle : Named_Handle; Location : out DS.Region_Offset; Length : out Byte_Length) is
   begin
      ...
      Location := DS.Region_Offset (Read_U64 (Slot_At (Item, Handle.Slot, Slot_Location_Offset, 8)));
      Length := Byte_Length (Read_U64 (Slot_At (Item, Handle.Slot, Slot_Length_Offset, 8)));
      if State_Of (Item, Handle) /= Ready then
         raise Segment_Error with "named segment handle changed during resolution";
      elsif Location = DS.Null_Offset
        or else Length = 0
   ...
   function Failure_Of (Item : View; Handle : Named_Handle) return Failure_Code is
      Value : Interfaces.Unsigned_32;
   begin
      if State_Of (Item, Handle) /= Failed then
         raise Segment_Error with "named segment handle is not failed";
      end if;
      Value := Read_U32 (Slot_At (Item, Handle.Slot, Slot_Failure_Offset, 4));
      if State_Of (Item, Handle) /= Failed then
         raise Segment_Error with "named segment handle changed during failure inspection";
      elsif Value = 0 then
         raise Segment_Error with "failed segment entry has zero failure code";
      end if;
   ...
   procedure Try_Remove (Item : View; Name : String; Result : out Remove_Result) is
      Hash     : Interfaces.Unsigned_32;
      Acquired : Boolean := False;
   begin
      Validate_Name (Item, Name);
            ...
            if State = Initializing_Slot or else State = Ready_Slot or else State = Failed_Slot then
               ...
               if Same_Name (Item, Slot, Name, Hash) then
                  if State = Initializing_Slot then
                     ...
                     Atomic.Store_Release_U32 (Slot_At (Item, Slot, Slot_State_Offset, 4), Removed_Slot);
                     Result := Removed;
                  end if;
                  Release_Guard (Item);
                  return;
