...
package body Flyology.Operations is
   ...
   function Bit (Id : Operation_Id) return Interfaces.Unsigned_32
   is (Interfaces.Shift_Left (Interfaces.Unsigned_32 (1), Natural (Id) - 1));

   function Contains (Mask : Interfaces.Unsigned_32; Id : Operation_Id) return Boolean
   is ((Mask and Bit (Id)) /= 0);

   function Population (Mask : Interfaces.Unsigned_32) return Natural is
   ...
   function Has_Readiness
     (Slot : Slot_Record; Descriptor : Interfaces.C.int; For_Write : Boolean) return Boolean is
   begin
      ...
      for Position in 1 .. Slot.Source_Count loop
         if Slot.Descriptors (Readiness_Source_Index (Position)) = Descriptor
           and then Slot.For_Write (Readiness_Source_Index (Position)) = For_Write
         then
            return True;
         end if;
      end loop;
   ...
   procedure Notify_Terminal (Set : in out Completion_Set'Class; Id : Operation_Id) is
   begin
      Set.Dirty_Dependents := Set.Dirty_Dependents or Set.Slots (Id).Dependents;
   ...
   begin
      ...
      return Duration (Now.Seconds) + Duration (Now.Nanoseconds) / 1_000_000_000;
   end Clock;
   ...
   begin
      ...
      declare
         Slot : constant Slot_Record := Item.Set.Slots (Operation_Id (Item.Slot));
      begin
   ...
   begin
      if Item.Slot /= 0 then
         if Owner.Slots (Operation_Id (Item.Slot)).State /= Idle then
            ...
            raise Operation_Error with "referenced operation cannot be restarted";
         end if;
      else
   ...
   procedure Publish_Terminal (Item : in out Operation'Class; Result : Terminal_Outcome) is
      Id : Operation_Id;
   begin
      if Item.Slot = 0 then
         raise Operation_Error with "operation has not been started";
      ...
      begin
         if Slot.Generation /= Item.Generation or else Slot.State /= Pending then
            ...
            raise Operation_Error with "operation cannot complete while a child remains attached";
         end if;
         Slot.State := Terminal;
         Clear_Source (Slot);
         Slot.Has_Deadline := False;
   ...
   function Reference (Item : Operation'Class) return Operation_Reference is
      Slot : Slot_Record;
   begin
      if Item.Slot = 0 then
         raise Operation_Error with "operation has not been started";
      ...
      if Slot.State not in Pending | Terminal then
         raise Operation_Error with "operation has no active or terminal outcome";
      elsif Slot.Internal then
         raise Operation_Error with "internal child operations cannot be referenced";
      end if;
      return (Set_Address => Item.Set.all'Address, Slot => Item.Slot, Generation => Item.Generation);
   end Reference;

   function Is_Active (Item : Operation'Class) return Boolean
   is (Item.Slot /= 0 and then Current_Slot (Item).State = Pending);

   function Is_Terminal (Item : Operation'Class) return Boolean
   is (Item.Slot /= 0 and then Current_Slot (Item).State = Terminal);

   function Outcome (Item : Operation'Class) return Terminal_Outcome
   is (Current_Slot (Item).Result);
         ...
         if Contains (Item.Members, Member_Id) then
            ...
            begin
               if Member.Generation = Item.Generations (Member_Id) then
                  Member.Dependents := Member.Dependents and not Bit (Gate_Id);
                  if Member.Dependents = 0 and then Member.Owner = null and then Member.State = Terminal then
                     declare
                        Generation : constant Interfaces.Unsigned_64 := Member.Generation;
                     begin
                        Member := (others => <>);
   ...
   begin
      for Member_Id in Operation_Id loop
         if Contains (Item.Members, Member_Id) and then not Contains (Item.Seen, Member_Id) then
            declare
               Member : Slot_Record renames Item.Set.Slots (Member_Id);
            begin
               if Member.Generation /= Item.Generations (Member_Id) then
                  raise Operation_Error with "gate member generation changed while observed";
               elsif Member.State = Terminal then
                  Item.Seen := Item.Seen or Bit (Member_Id);
                  if Member.Result = Succeeded then
                     Item.Successful := Item.Successful or Bit (Member_Id);
                  end if;
               elsif Member.State /= Pending then
                  raise Operation_Error with "gate member stopped without a terminal outcome";
               end if;
            end;
         end if;
      end loop;
      ...
      if (Item.Mode = Terminal_Members and then Terminal_Matches >= Item.Required)
        or else (Item.Mode = Successful_Members and then Success_Matches >= Item.Required)
      then
         Detach (Item);
         Publish_Terminal (Item, Succeeded);
      elsif Item.Mode = Successful_Members and then Success_Matches + Remaining < Item.Required then
         Detach (Item);
         Publish_Terminal (Item, Failed);
      else
         Item.Set.Slots (Operation_Id (Item.Slot)).Source := Dependency_Source;
      end if;
   end Evaluate;

   overriding
   procedure Drive (Item : in out Gate_Operation; Event : Driver_Event) is
   ...
   overriding
   procedure Request_Cancellation (Item : in out Gate_Operation) is
   begin
      Detach (Item);
         ...
         while Set.Dirty_Dependents /= 0 loop
            for Id in Set.Slots'Range loop
               if Contains (Set.Dirty_Dependents, Id) then
                  Set.Dirty_Dependents := Set.Dirty_Dependents and not Bit (Id);
                  if Set.Slots (Id).State = Pending then
                     if Set.Slots (Id).Owner = null then
                        raise Operation_Error with "dependent operation has no owner";
                     end if;
                     Drive (Set.Slots (Id).Owner.all, Dependency_Changed);
                  end if;
                  exit;
               end if;
            end loop;
   ...
   begin
      if Members'Length = 0 then
         ...
         raise Operation_Error with "gate threshold exceeds its member count";
      end if;
      ...
      for Position in Members'Range loop
         ...
         begin
            if Member.Set_Address /= Item.Set.all'Address then
               raise Operation_Error with "gate member belongs to another completion set";
            ...
            if Contains (Item.Members, Member_Id) then
              ...
              or else Item.Set.Slots (Member_Id).State not in Pending | Terminal
            then
               raise Operation_Error with "gate member has no active outcome";
            end if;
   ...
   function Wait_Some
     (Set      : not null access Completion_Set'Class;
      Members  : Operation_Reference_Array;
      Required : Positive := 1) return Gate_Operation is
   begin
   ...
   begin
      ...
      return Result : Gate_Operation (Set) do
         Configure_Gate (Result, Members, Terminal_Members, Members'Length);
      end return;
   end Wait_All;
   ...
   procedure Finish (Item : in out Gate_Operation; Matched : out Completion_Batch) is
   ...
   procedure Publish_Unreported (Set : in out Completion_Set; Completed : out Completion_Batch) is
   begin
      Completed.Count := 0;
   ...
   begin
      for Slot of Set.Slots loop
         if Slot.State = Terminal and then not Slot.Internal and then not Slot.Reported then
            Count := Count + 1;
         end if;
      end loop;
      return Count;
   ...
   function Unreported_Success_Count (Set : Completion_Set) return Natural is
      Count : Natural := 0;
   begin
      for Slot of Set.Slots loop
   ...
   procedure Expire_Timers
     (Set               : in out Completion_Set;
      Only              : Interfaces.Unsigned_32 := Interfaces.Unsigned_32'Last;
   ...
   begin
      for Id in Set.Slots'Range loop
         if Set.Slots (Id).State = Pending
           ...
           and then (not Preserve_Progress
                     or else Set.Slots (Id).Source not in Descriptor_Source | Immediate_Source)
         then
            ...
            Drive (Set.Slots (Id).Owner.all, Deadline_Reached);
         end if;
      end loop;
   end Expire_Timers;

   function Drive_Immediate (Set : in out Completion_Set) return Interfaces.Unsigned_32 is
      Drove : Interfaces.Unsigned_32 := 0;
   begin
      for Id in Set.Slots'Range loop
         if Set.Slots (Id).State = Pending and then Set.Slots (Id).Source = Immediate_Source then
            Drove := Drove or Bit (Id);
            Clear_Source (Set.Slots (Id));
            if Set.Slots (Id).Owner = null then
               raise Operation_Error with "pending operation has no driver";
            end if;
            Drive (Set.Slots (Id).Owner.all, Continue_Operation);
         end if;
      end loop;
      return Drove;
   end Drive_Immediate;

   procedure Wait_Some (Set : in out Completion_Set; Completed : out Completion_Batch) is
   begin
   ...
   is
      ...
      begin
         for Id in Set.Slots'Range loop
            if Set.Slots (Id).State = Pending and then Set.Slots (Id).Source = Descriptor_Source then
   ...
   begin
      loop
         ...
         declare
            Maximum_Requests : constant Natural := Active_Request_Count;
            Requests         : Flyology.IO.Wait_Request_Array (1 .. Maximum_Requests);
            Slot_Map         : array (Positive range 1 .. Maximum_Requests) of Operation_Id;
         ...
         begin
                  ...
                  case Set.Slots (Id).Source is
                     when Descriptor_Source =>
                        for Position in 1 .. Set.Slots (Id).Source_Count loop
                           ...
                           Requests (Request_Count) :=
                             (FD        => Set.Slots (Id).Descriptors (Readiness_Source_Index (Position)),
                              Condition =>
                                (if Set.Slots (Id).For_Write (Readiness_Source_Index (Position))
                     ...
                     when No_Source         =>
                        if not Set.Slots (Id).Has_Deadline then
                           raise Operation_Error with "pending operation has no wait source";
                        end if;
                  end case;
                  if Set.Slots (Id).Has_Deadline
                    and then (not Have_Timer or else Set.Slots (Id).Deadline < Earliest)
                  then
            ...
            if Request_Count = 0 then
               ...
               declare
                  ...
                  Processed : array (Set.Slots'Range) of Boolean := (others => False);
               begin
                  if Drove_Immediate then
                  ...
                  Flyology.IO.Wait_Some (Requests (1 .. Request_Count), Ready, Wait_For);
                  Begin_Propagation_Batch (Set);
                  begin
                     for Position in 1 .. Ready.Count loop
                        declare
                           Request_Index : constant Positive := Ready.Indexes (Position);
                           Id            : constant Operation_Id := Slot_Map (Request_Index);
                           Descriptor    : constant Interfaces.C.int := Requests (Request_Index).FD;
                           For_Write     : constant Boolean :=
                             Requests (Request_Index).Condition = Flyology.IO.For_Write;
                        begin
                           if not Processed (Id) then
                              if Descriptor = Flyology.Wake_Sources.Descriptor (Set.Wake) then
                              ...
                              for Candidate in Set.Slots'Range loop
                                 if not Processed (Candidate)
                                   and then Set.Slots (Candidate).State = Pending
   ...
   procedure Wait_At_Least
     (Set : in out Completion_Set; Required : Positive; Completed : out Completion_Batch) is
   begin
      Wait_For_Gate (Set, Required, Completed, Terminal_Gate);
   end Wait_At_Least;

   procedure Wait_Some (Set : in out Completion_Set; Required : Positive; Completed : out Completion_Batch) is
   begin
      Wait_At_Least (Set, Required, Completed);
   end Wait_Some;

   procedure Wait_For_Success (Set : in out Completion_Set; Completed : out Completion_Batch) is
   begin
   ...
   procedure Continue_After (Parent : in out Operation'Class; Child : in out Operation'Class) is
      Parent_Id : Operation_Id;
      Child_Id  : Operation_Id;
   begin
      if Parent.Set.all'Address /= Child.Set.all'Address then
         raise Operation_Error with "parent and child belong to different completion sets";
      elsif Parent.Slot = 0 or else Child.Slot = 0 then
         raise Operation_Error with "parent and child must both be started";
      ...
      begin
         if Parent_Slot.Generation /= Parent.Generation or else Parent_Slot.State /= Pending then
            raise Operation_Error with "parent operation is not pending";
         elsif Parent_Slot.Child /= 0 or else Parent_Slot.Source /= No_Source then
            raise Operation_Error with "parent operation already awaits a source or child";
         elsif Child_Slot.Generation /= Child.Generation or else Child_Slot.State not in Pending | Terminal
         then
            raise Operation_Error with "child operation has no active outcome";
         elsif Child_Slot.Internal or else Child_Slot.Dependents /= 0 or else Child_Slot.Reported then
            raise Operation_Error with "child operation is already observed";
         end if;
         ...
         while Parent.Set.Slots (Operation_Id (Cursor)).Child /= 0 loop
            Cursor := Parent.Set.Slots (Operation_Id (Cursor)).Child;
            if Cursor = Natural (Parent_Id) then
               raise Operation_Error with "operation continuation cycle";
            end if;
         end loop;
   ...
   begin
      ...
      declare
         Slot : Slot_Record renames Item.Set.Slots (Operation_Id (Item.Slot));
      begin
         if Slot.Generation /= Item.Generation or else Slot.State /= Terminal then
            ...
            if not Slot.Internal or else Population (Slot.Dependents) /= 1 then
               raise Operation_Error with "operation still has dependent observers";
            end if;
            declare
            ...
            begin
               ...
               declare
                  Parent_Slot : Slot_Record renames Item.Set.Slots (Parent_Id);
               begin
                  if Parent_Slot.State /= Pending
                    or else Parent_Slot.Child /= Item.Slot
                    or else Parent_Slot.Child_Generation /= Item.Generation
                  then
                     raise Operation_Error with "child continuation relation is stale";
                  end if;
                  Parent_Slot.Child := 0;
   ...
   begin
      ...
      begin
         if Slot.Generation /= Item.Generation or else Slot.State /= Idle then
            raise Operation_Error with "only a consumed operation can release its slot";
         elsif Slot.Dependents /= 0 or else Slot.Child /= 0 then
            raise Operation_Error with "operation still participates in composition";
         end if;
         Slot := (others => <>);
   ...
   overriding
   procedure Finalize (Item : in out Operation) is
   begin
      if Item.Slot /= 0 then
         declare
            Slot       : Slot_Record renames Item.Set.Slots (Operation_Id (Item.Slot));
