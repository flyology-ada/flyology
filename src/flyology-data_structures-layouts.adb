...
package body Flyology.Data_Structures.Layouts is
   ...
   procedure Write_Header
     (Item : Local_View; Identity : Layout_Identity; Extent : Byte_Count; Header : Header_Values) is
   begin
      Bytes.Write_U32 (Address_At (Item, Version_Offset, 4, 4), Identity.Version);
      Bytes.Write_U64 (Address_At (Item, Magic_Offset, 8, 8), Identity.Magic);
      Bytes.Write_U64 (Address_At (Item, Schema_Offset, 8, 8), Identity.Schema);
   ...
   begin
      if not Atomic.Supported then
         raise Program_Error with "process-capable 32/64-bit atomics are unavailable";
   ...
   begin
      ...
      if Bytes.Read_U32 (Address_At (Initial, Version_Offset, 4, 4)) /= Identity.Version then
         raise Layout_Error with "unsupported structure layout version";
      elsif Bytes.Read_U64 (Address_At (Initial, Magic_Offset, 8, 8)) /= Identity.Magic then
         raise Layout_Error with "structure magic does not match";
      elsif Bytes.Read_U64 (Address_At (Initial, Schema_Offset, 8, 8)) /= Identity.Schema then
   ...
   procedure Poison_At
     (Region : Region_View; Location : Region_Offset; Identity : Layout_Identity; Base_Alignment : Byte_Count)
   is
   ...
   begin
      ...
      elsif Bytes.Read_U32 (Address_At (Item, Version_Offset, 4, 4)) /= Identity.Version
        or else Bytes.Read_U64 (Address_At (Item, Magic_Offset, 8, 8)) /= Identity.Magic
        or else Bytes.Read_U64 (Address_At (Item, Schema_Offset, 8, 8)) /= Identity.Schema
      then
