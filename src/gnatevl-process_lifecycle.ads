package Gnatevl.Process_Lifecycle with Preelaborate is
   ...
   --  Dormant means that GNATEVL is initialized but no event-loop group has
   ...
   --  runtime that initialized GNATEVL. The underlying query takes no runtime
   ...
   --  Ada tasking and all other GNATEVL operations are unsupported.
