...
is
   ...
   is (Running and then Active < Expected);
   ...
   is (if Cancelled then Count_Cancelled elsif Failed then Count_Neither else Count_Completed);

   function Worker_Finish_Allowed
     (Running : Boolean; Workers_Done : Natural; Expected : Natural) return Boolean
   is (Running and then Workers_Done < Expected);
   ...
   is (Expected > 0 and then Workers_Done = Expected);

   function Finish_Allowed
     (Running : Boolean; Active : Natural; Workers_Done : Natural; Expected : Natural) return Boolean
   is (Running and then Active = 0 and then Expected > 0 and then Workers_Done = Expected);
