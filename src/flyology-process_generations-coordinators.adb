...
package body Flyology.Process_Generations.Coordinators is
   ...
   function Failure_Message (Item : Coordinator_Snapshot) return String is
     (if Item.Failure_Size = 0 then ""
      else String (Item.Failure (1 .. Item.Failure_Size)));
   ...
   procedure Apply
     (Item    : in out Coordinator;
      Command : Upgrade_Command) is
   begin
      if not Policy.Command_Allowed (Item.State.Phase, Command) then
         raise Invalid_Phase with
           "command " & Upgrade_Command'Image (Command) &
           " is invalid in " & Upgrade_Phase'Image (Item.State.Phase);
      end if;
   ...
   procedure Allocate_Authority
     (Item      : in out Coordinator;
      Authority : out Upgrade_Handle) is
   begin
      if Item.Upgrade_Exhausted or else Item.Generation_Exhausted then
         raise Identity_Exhausted with
           "process-generation identity space is exhausted";
      end if;
      Authority :=
        (Coordinator => Item.Identity,
         Upgrade     => Item.Next_Upgrade,
         Candidate   => Item.Next_Generation);
      if Interfaces.Unsigned_64 (Item.Next_Upgrade) =
      ...
      if Interfaces.Unsigned_64 (Item.Next_Generation) =
        Interfaces.Unsigned_64'Last
      then
   ...
   procedure Require_Authority
     (Item      : Coordinator;
      Authority : Upgrade_Handle) is
   begin
      if not Policy.Authority_Matches (Item.State.Authority, Authority) then
         raise Stale_Authority with
           "upgrade command has stale coordinator authority";
      end if;
   end Require_Authority;

   procedure Cleanup_Slot
     (Slot : in out Image_Slot) is
   begin
         ...
         begin
            for Index in Text'Range loop
               Text (Index) := Character'Val
                 (Frame.Payload (Protocol.Payload_Index (Index - 1)));
            end loop;
            return Text;
         end;
      end if;
   ...
   begin
      if not Transport.Message_Available
        (Slot.Control, Timeout => Timeout, Token => Token)
      then
         raise Flyology.IO.Timeout_Error with
           "control response deadline expired";
      end if;
      Transport.Receive
        (Slot.Control, Frame, Timeout => Remaining (Started, Timeout));
      if Frame.Kind = Protocol.Failure_Message then
         ...
         raise Upgrade_Error with
           "expected " & Protocol.Message_Kind'Image (Kind) &
           ", received " & Protocol.Message_Kind'Image (Frame.Kind);
      end if;
   end Expect;

   procedure Validate_Proof
   ...
   begin
      Messages.Decode_Topology_Proof
        (Frame.Payload, Frame.Length, Proof, Result);
      if Result /= Messages.Decoded or else
        Proof.Epoch /= Provision.Topology_Epoch or else
        Proof.Digest /= Provision.Digest
      then
         raise Upgrade_Error with
           "image topology proof does not match desired state";
      end if;
   end Validate_Proof;

   function Slot_Exited (Slot : Image_Slot) return Boolean is
     (Slot.Occupied and then Flyology.Subprocesses.Is_Open (Slot.Child)
   ...
   procedure Reconcile_Exited (Item : in out Coordinator) is
      Candidate_Dead : constant Boolean :=
        Item.State.Has_Candidate and then
        Slot_Exited (Item.Slots (Item.Candidate_Slot));
      Active_Dead : constant Boolean :=
        Item.State.Has_Active and then
        Slot_Exited (Item.Slots (Item.Active_Slot));
   begin
      if Candidate_Dead then
         if Item.State.Phase in Canary | Revoking_Admission |
           Draining_Candidate | Compensating
         then
         ...
         Item.State.Phase :=
           (if Policy.Promotion_Committed (Item.State.Phase) then
               Rollback_Required
            else Failed);
      end if;
      ...
      if Active_Dead then
         ...
         Item.State.Phase :=
           (if Item.State.Rollback_Available then Rollback_Required
            else Failed);
      end if;
   ...
   begin
      ...
      Candidate :=
        (if Item.State.Has_Active then 1 - Item.Active_Slot else 0);
      ...
      Bootstrap.Spawn
        (Command, Item.Slots (Candidate).Child, Control_Socket,
         Item.Slots (Candidate).Capabilities);
      Check_Cancelled (Token);
      ...
      Transport.Adopt
        (Item.Slots (Candidate).Control, Control_Socket, Authority);
      Apply (Item, Candidate_Started);

      Transport.Send
        (Item.Slots (Candidate).Control, Protocol.Hello,
         Timeout => Remaining (Started, Timeout));
      Expect
        (Item.Slots (Candidate), Protocol.Acknowledgment, Frame,
         Remaining (Started, Timeout), Token);
      Check_Cancelled (Token);
      Messages.Encode_Provision (Provision, Payload);
      Transport.Send
        (Item.Slots (Candidate).Control, Protocol.Provision, Payload,
         Messages.Provision_Length, Remaining (Started, Timeout));
      Transport.Send
        (Item.Slots (Candidate).Control, Protocol.Expect_Capability,
         Timeout => Remaining (Started, Timeout));
      Expect
        (Item.Slots (Candidate), Protocol.Capability_Ready, Frame,
         Remaining (Started, Timeout), Token);
      Check_Cancelled (Token);
      Handoffs.Send_Listener
        (Item.Slots (Candidate).Capabilities, Item.Listener, Handoffs.Borrow);
      Expect
        (Item.Slots (Candidate), Protocol.Capability_Adopted, Frame,
         Remaining (Started, Timeout), Token);
      Expect
        (Item.Slots (Candidate), Protocol.Prepared_Message, Frame,
         Remaining (Started, Timeout), Token);
   ...
   exception
      when Error : others =>
         ...
         if Ada.Exceptions.Exception_Identity (Error) =
           Identity_Exhausted'Identity
           or else Ada.Exceptions.Exception_Identity (Error) =
             Invalid_Phase'Identity
           or else Ada.Exceptions.Exception_Identity (Error) =
   ...
   begin
      ...
      then
         raise Flyology.IO.Timeout_Error with
           "candidate readiness deadline expired";
      end if;
      Transport.Receive
        (Item.Slots (Item.Candidate_Slot).Control, Frame,
         Timeout => Remaining (Started, Timeout));
      Check_Cancelled (Token);
      if Frame.Kind = Protocol.Ready_Message then
         Validate_Proof
           (Frame, Item.Slots (Item.Candidate_Slot).Provision);
         ...
         Expect
           (Item.Slots (Item.Candidate_Slot), Protocol.Drained_Message,
            Frame, Remaining (Started, Timeout));
         Apply (Item, Candidate_Drained);
         Expect
           (Item.Slots (Item.Candidate_Slot), Protocol.Compensation_Message,
            Frame, Remaining (Started, Timeout));
         Messages.Decode_Compensation
           (Frame.Payload, Frame.Length, Compensation, Decode);
         ...
         if Frame.Kind /= Protocol.Failure_Message then
            raise Upgrade_Error with
              "candidate recovery omitted its readiness failure";
         end if;
         Wait_And_Release
         ...
         raise Upgrade_Error with
           "candidate returned an invalid readiness response";
      end if;
   exception
      when Error : others =>
         if not Mutated then
   ...
   begin
      ...
      if Item.State.Phase not in Prepared | Canary then
         raise Invalid_Phase with
           "candidate cannot be cancelled in this phase";
      end if;
      ...
      Transport.Send
        (Item.Slots (Item.Candidate_Slot).Control, Protocol.Cancel_Message,
         Timeout => Remaining (Started, Timeout));
      if Was_Canary then
         if not Transport.Message_Available
           (Item.Slots (Item.Candidate_Slot).Control,
         ...
         then
            raise Flyology.IO.Timeout_Error with
              "candidate revocation deadline expired";
         end if;
         Transport.Receive
           (Item.Slots (Item.Candidate_Slot).Control, Frame,
            Remaining (Started, Timeout));
         if Frame.Kind = Protocol.Ready_Message then
            ...
            Expect
              (Item.Slots (Item.Candidate_Slot),
               Protocol.Admission_Revoked_Message, Frame,
               Remaining (Started, Timeout));
         elsif Frame.Kind /= Protocol.Admission_Revoked_Message then
            if Frame.Kind = Protocol.Failure_Message then
               raise Upgrade_Error with Agent_Failure_Message (Frame);
            ...
            raise Upgrade_Error with
              "candidate returned an invalid revocation response";
         end if;
         Item.State.Candidate_Admitted := False;
         Apply (Item, Admission_Revoked);
         Expect
         ...
         Expect
           (Item.Slots (Item.Candidate_Slot), Protocol.Compensation_Message,
            Frame, Remaining (Started, Timeout));
         Messages.Decode_Compensation
           (Frame.Payload, Frame.Length, Compensation, Result);
         ...
         Expect
           (Item.Slots (Item.Candidate_Slot), Protocol.Acknowledgment,
            Frame, Remaining (Started, Timeout));
         Apply (Item, Candidate_Stopped);
      end if;
      Wait_And_Release
        (Item.Slots (Item.Candidate_Slot), Remaining (Started, Timeout));
      Item.State.Has_Candidate := False;
      Item.State.Candidate_Ready := False;
      Item.State.Candidate_Admitted := False;
   exception
   ...
   begin
      Reconcile_Exited (Item);
      Require_Authority (Item, Authority);
      if Item.State.Phase /= Canary or else
        not Item.State.Candidate_Ready
      then
      ...
      Mutated := True;
      Transport.Send
        (Item.Slots (Item.Candidate_Slot).Control, Protocol.Promote_Message,
         Timeout => Remaining (Started, Timeout));
      Expect
        (Item.Slots (Item.Candidate_Slot), Protocol.Acknowledgment,
         Frame, Remaining (Started, Timeout));
      ...
      if Had_Previous then
         Transport.Send
           (Item.Slots (Previous).Control, Protocol.Drain_Message,
            Timeout => Remaining (Started, Timeout));
         Expect
           (Item.Slots (Previous), Protocol.Drained_Message,
            Frame, Remaining (Started, Timeout));
         Wait_And_Release
           (Item.Slots (Previous), Remaining (Started, Timeout));
      end if;
   ...
   begin
      Start_Upgrade
        (Item, Executable, Provision, Authority, Timeout, Token);
      Begin_Canary
        (Item, Authority, Remaining (Started, Timeout), Token);
      if Token /= null and then Token.Requested then
         Cancel
           (Item, Authority, Compensation, Remaining (Started, Timeout));
         raise Flyology.Cancellation.Operation_Cancelled;
      end if;
   ...
   is
      Started : constant Ada.Real_Time.Time := Ada.Real_Time.Clock;
      Target_Artifact : constant Flyology.Subprocesses.Command :=
        Item.Rollback_Artifact;
      Target_Provision : constant Messages.Provisioning_Data :=
        Item.Rollback_Provision;
      Forward_Available : Boolean := False;
      Forward_Artifact : Flyology.Subprocesses.Command;
      Forward_Provision : Messages.Provisioning_Data :=
        Target_Provision;
      Frame : Protocol.Frame;
   ...
   begin
      ...
      Reconcile_Exited (Item);
      Forward_Available :=
        Item.State.Has_Candidate or else Item.State.Has_Active;
      if not Item.State.Initialized then
         raise Program_Error with "process-generation coordinator is closed";
         ...
         raise Invalid_Phase with
           "rollback requires a completed or uncertain promotion";
      end if;
      Reversal_Started := True;

      if Item.State.Has_Candidate then
         Forward_Artifact :=
           Item.Slots (Item.Candidate_Slot).Artifact;
         Forward_Provision :=
           Item.Slots (Item.Candidate_Slot).Provision;
         ...
         if Item.Slots (Index).Occupied then
            begin
               if Transport.Is_Open (Item.Slots (Index).Control) then
                  Transport.Send
                    (Item.Slots (Index).Control, Protocol.Drain_Message,
                     Timeout => Remaining (Started, Timeout));
                  Expect
                    (Item.Slots (Index), Protocol.Drained_Message, Frame,
                     Remaining (Started, Timeout));
                  Wait_And_Release
                    (Item.Slots (Index), Remaining (Started, Timeout));
               else
      ...
      Start_Initial
        (Item, Target_Artifact, Target_Provision, Authority,
         Remaining (Started, Timeout));
   ...
   overriding procedure Finalize (Item : in out Coordinator) is
   begin
      for Index in Item.Slots'Range loop
         Cleanup_Slot (Item.Slots (Index));
      end loop;
