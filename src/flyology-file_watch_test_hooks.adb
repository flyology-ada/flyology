...
package body Flyology.File_Watch_Test_Hooks is
   ...
   with Import, Convention => C, External_Name => "flyology_test_file_watch_events_lost";
   function Test_Remove_Failure return Interfaces.C.int
   with Import, Convention => C, External_Name => "flyology_test_file_watch_remove_failure";
   function Test_Close_Failure return Interfaces.C.int
   with Import, Convention => C, External_Name => "flyology_test_file_watch_close_failure";

   function Consume_Events_Lost return Boolean
   is (Test_Events_Lost /= 0);
