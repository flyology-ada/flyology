...
package body Flyology.Subprocesses.Bootstrap is
   ...
   function C_Duplicate_Above
     (Descriptor : C.int; Minimum : C.int) return C.int;
   pragma Import
     (C, C_Duplicate_Above, "flyology_subprocess_duplicate_above");
   ...
   procedure Release_Above_Bootstrap
   ...
   begin
      ...
      if Descriptor <= C.int (Child_Capability_Descriptor) then
         Replacement := C_Duplicate_Above
           (Descriptor, C.int (Child_Capability_Descriptor) + 1);
         if Replacement < 0 then
            raise Spawn_Error with
              "cannot relocate subprocess bootstrap descriptor";
         end if;
         Close_Raw (Descriptor);
   ...
   begin
      Descriptor_Handoffs.Validate_Carrier
        (Descriptor_Handoffs.Socket_Descriptor (Descriptor),
         (if Trust = Socket_Handoffs.Trusted_Peer then
   ...
   begin
      if Is_Open (Child) or else Sockets.Is_Open (Control) or else
        Socket_Handoffs.Is_Open (Capabilities)
      then
         raise Program_Error with "bootstrap spawn target is already open";
      end if;
      begin
         Sockets.Create_Socket_Pair
           (Control_Parent_Socket, Control_Child_Socket);
         Sockets.Create_Socket_Pair
           (Capability_Parent_Socket, Capability_Child_Socket);
         Release_Above_Bootstrap
           (Control_Parent_Socket, Control_Pair (0));
         Release_Above_Bootstrap
           (Control_Child_Socket, Control_Pair (1));
         Release_Above_Bootstrap
           (Capability_Parent_Socket, Capability_Pair (0));
         Release_Above_Bootstrap
           (Capability_Child_Socket, Capability_Pair (1));
      exception
         when Error : others =>
            ...
            raise Spawn_Error with
              "cannot create subprocess bootstrap channels: " &
              Ada.Exceptions.Exception_Message (Error);
      end;
      if Control_Pair (0) <= C.int (Child_Capability_Descriptor) or else
        Control_Pair (1) <= C.int (Child_Capability_Descriptor) or else
        Capability_Pair (0) <= C.int (Child_Capability_Descriptor) or else
        Capability_Pair (1) <= C.int (Child_Capability_Descriptor)
      then
         Close_Pair (Control_Pair);
         Close_Pair (Capability_Pair);
         raise Spawn_Error with
           "subprocess bootstrap descriptor relocation was incomplete";
      end if;

      begin
         Spawn_Internal
           (Item, Child,
            Control_Pair (0), Control_Pair (1),
            Capability_Pair (0), Capability_Pair (1),
            Pipe_Output => False,
            Pipe_Error  => False);
         ...
         Validate_Control
           (Control_Pair (0), Socket_Handoffs.Trusted_Peer);
         ...
         Socket_Handoffs.Adopt
           (Capabilities, Capability_Socket,
   ...
   is
      ...
      Capability_Socket : Sockets.Socket_Type;
   begin
      if Sockets.Is_Open (Control) or else
        Socket_Handoffs.Is_Open (Capabilities)
      then
