...
private package Flyology.Shared_Memory_Policy
  with SPARK_Mode
is
   ...
   type Descriptor_Validation is (Descriptor_Valid, Size_Mismatch, Type_Mismatch, Security_Missing);

   function Is_Regular (Mode, Type_Mask, Regular_Type : U32) return Boolean
   is ((Mode and Type_Mask) = Regular_Type);

   function Is_Untyped (Mode, Type_Mask : U32) return Boolean
   is ((Mode and Type_Mask) = 0);

   function Owner_Only (Mode, Other_Access_Mask : U32) return Boolean
   is ((Mode and Other_Access_Mask) = 0);
   ...
   with
     Post =>
       ...
       = (Actual_Length = Expected_Length
          and then (Is_Regular (Mode, Type_Mask, Regular_Type)
   ...
   type Namespace_Classification is (Namespace_Ready, Namespace_In_Progress, Namespace_Size_Mismatch);

   function Classify_Namespace
     (Created : Boolean; Actual_Length : U64; Expected_Length : U64) return Namespace_Classification
   with
     Post =>
       (Classify_Namespace'Result = Namespace_In_Progress) = (not Created and then Actual_Length = 0)
       and then (Classify_Namespace'Result = Namespace_Ready)
                = (Actual_Length = Expected_Length and then (Created or else Actual_Length /= 0));
   ...
   function Valid_Handoff
     (Amount                 : Long_Long_Integer;
      ...
      Payload_Truncated      : Boolean) return Boolean
   is (Amount = 1
   ...
   type Replacement_Slot_State is
     (Slot_Free, Slot_Initializing, Slot_Ready, Slot_Failed, Slot_Removed, Slot_Invalid);
   ...
   with
     ...
     Post =>
       ...
       and then (if Validate_Replacement'Result = Replacement_Valid
                 then
                   Frontier >= 4
                   and then Frontier <= Source_Extent
