...
package body Flyology.Subprocesses.Capture is
   ...
   function Status (Item : Result) return Exit_Status is
     (Item.Child_Status);

   function Standard_Output (Item : Result) return String is
     (US.To_String (Item.Output_State));

   function Standard_Error (Item : Result) return String is
     (US.To_String (Item.Error_State));

   function Output_Truncated (Item : Result) return Boolean is
     (Item.Output_Was_Cut);

   function Error_Truncated (Item : Result) return Boolean is
     (Item.Error_Was_Cut);
   ...
   is
      ...
      is
         Received : constant Natural :=
           (if Last < Bytes'First
            then 0
            else Natural (Last - Bytes'First + 1));
         Available : constant Natural :=
           (if US.Length (Storage) >= Maximum
            then 0
            else Maximum - US.Length (Storage));
   ...
   begin
      ...
      while Standard_Output_Is_Open (Child)
        or else Standard_Error_Is_Open (Child)
        or else not Process_Done
      loop
         if Token /= null and then Token.Requested then
            raise Flyology.Cancellation.Operation_Cancelled;
         elsif not First_Wait
           and then Timeout >= 0.0
           and then Time_Left <= 0.0
         then
            raise Flyology.IO.Timeout_Error with
              "subprocess capture deadline expired";
         end if;
         First_Wait := False;
         declare
            ...
            procedure Add
              (Descriptor : Flyology.IO.Descriptor;
               Condition  : Flyology.IO.Wait_Kind;
               Slot       : out Natural) is
            begin
               Count := Count + 1;
               Requests (Count) :=
                 (FD => Descriptor, Condition => Condition);
               Slot := Count;
            end Add;
         ...
         begin
            for Offset in 0 .. 3 loop
               declare
                  Slot_Class : constant Natural range 0 .. 3 :=
                    (Next_First + Offset) mod 4;
               begin
                  case Slot_Class is
                     when 0 =>
                     ...
                     when 3 =>
                        if not Process_Done then
                           Add
                             (Child.Exit_State.Wait_Descriptor,
                              Flyology.IO.For_Read,
                              Exit_Slot);
                        end if;
                  end case;
            ...
            Ready_Slot := Flyology.IO.Wait_Any
              (Requests (1 .. Count), Time_Left);
            if Ready_Slot = 0 then
               ...
               begin
                  ...
                  Retain
                    (Buffer, Last, Maximum_Output, Value.Output_State,
                     Value.Output_Was_Cut);
               exception
               ...
               begin
                  ...
                  Retain
                    (Buffer, Last, Maximum_Error, Value.Error_State,
                     Value.Error_Was_Cut);
               exception
                  when Flyology.IO.Timeout_Error => null;
               end;
               ...
               declare
                  Remaining_Input : constant Natural :=
                    Standard_Input'Length - Input_Offset;
                  Length : constant Natural :=
                    Natural'Min (Chunk_Size, Remaining_Input);
               ...
               begin
                  for Index in Buffer'Range loop
                     Buffer (Index) := Ada.Streams.Stream_Element
                       (Character'Pos
                          (Standard_Input
                             (Standard_Input'First + Input_Offset
                              + Natural (Index) - 1)));
                  end loop;
                  Write_Standard_Input
                    (Child, Buffer, Last, Timeout => 0.0);
