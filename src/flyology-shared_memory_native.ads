...
private package Flyology.Shared_Memory_Native with Preelaborate is
   ...
   type Descriptor_Array is
     array (C.size_t range <>) of aliased C.int
       with Convention => C;
   ...
   function Map_Shared
     (Descriptor : C.int; Length : C.size_t) return System.Address;
   ...
   function Flush_Mapping
     (Base : System.Address;
      Length : C.size_t;
      Synchronous : Boolean) return C.int;
   ...
   function Set_Descriptor_Flags
     (Descriptor : C.int; Flags : C.int) return C.int;
   ...
   function Send_Descriptor_Once
     (Socket : C.int; Descriptor : C.int) return C.long;
