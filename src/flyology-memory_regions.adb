...
package body Flyology.Memory_Regions is
   ...
   procedure Free_Chunk is new Ada.Unchecked_Deallocation (Chunk, Chunk_Access);
   procedure Free_Region is new Ada.Unchecked_Deallocation (Region_Data, Region_Access);

   procedure Check_Owner (Pool : Task_Pool);

   function Aligned_Address
     (Address : System.Address; Alignment : Storage.Storage_Count) return System.Address;

   function Fits
     (Item : Chunk; Size : Storage.Storage_Count; Alignment : Storage.Storage_Count) return Boolean;

   procedure Check_Owner (Pool : Task_Pool) is
   begin
      if Pool.Owner /= Ada.Task_Identification.Current_Task then
         raise Ownership_Error with "memory region used by a task other than its owner";
      end if;
   end Check_Owner;
   ...
   function Fits
     (Item : Chunk; Size : Storage.Storage_Count; Alignment : Storage.Storage_Count) return Boolean
   is
      Current : constant System.Address := Item.Data'Address + Storage.Storage_Offset (Item.Used);
      Aligned : constant System.Address := Aligned_Address (Current, Alignment);
      Padding : constant Storage.Storage_Count := Storage.Storage_Count (Aligned - Current);
   begin
      return
        Padding <= Storage.Storage_Count (Item.Last) - Item.Used
        and then Size <= Storage.Storage_Count (Item.Last) - Item.Used - Padding;
   end Fits;

   function Create_Region
     (Pool : in out Task_Pool; Chunk_Storage : Storage.Storage_Count := 65_536) return not null Region_Handle
   is
   ...
   begin
      ...
      Result :=
        new Region_Data'
          (Subpools.Root_Subpool
           with First => null, Chunk_Size => Chunk_Storage, Consumed => 0, Reserved => 0);
      begin
   ...
   overriding
   function Create_Subpool (Pool : in out Task_Pool) return not null Region_Handle is
   begin
   ...
   is
      ...
      Size      : constant Storage.Storage_Count := Storage.Storage_Count'Max (1, Size_In_Storage_Elements);
   ...
   begin
      ...
      if Item.First = null or else not Fits (Item.First.all, Size, Alignment) then
      ...
      Current := Item.First.Data'Address + Storage.Storage_Offset (Item.First.Used);
   ...
   overriding
   procedure Deallocate_Subpool (Pool : in out Task_Pool; Subpool : in out Region_Handle) is
      Item       : Region_Data renames Region_Data (Subpool.all);
   ...
   begin
      ...
      if Owner = null or else not (Owner.all in Task_Pool'Class) then
         raise Ownership_Error with "region was not created by Flyology.Memory_Regions";
      end if;
      ...
      exception
         when Failure : others =>
            ...
            begin
               ...
               if Region /= null and then Subpools.Pool_Of_Subpool (Region) /= null then
                  Ada.Unchecked_Deallocate_Subpool (Region);
               end if;
