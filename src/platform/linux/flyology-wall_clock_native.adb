...
package body Flyology.Wall_Clock_Native is
   ...
   pragma Import (C, Timerfd_Create_Flags, "flyology_wall_clock_timerfd_create_flags");

   function Timerfd_Settime_Flags return C.int;
   pragma Import (C, Timerfd_Settime_Flags, "flyology_wall_clock_timerfd_settime_flags");
   ...
   function Timerfd_Settime
     (FD : C.int; Flags : C.int; New_Value : access Itimerspec; Old_Value : System.Address) return C.int;
   ...
   function To_Native (Value : Policy.Timestamp) return Timespec
   is (Seconds => C.long (Value.Seconds), Nanoseconds => C.long (Value.Nanoseconds));
   ...
   begin
      ...
      declare
         Timer      : aliased Itimerspec :=
           (Interval => (Seconds => 0, Nanoseconds => 0), Value => To_Native (Deadline));
         Result     : constant C.int :=
           Timerfd_Settime (State.Wait_FD, Timerfd_Settime_Flags, Timer'Access, System.Null_Address);
         Error_Code : constant C.int := (if Result < 0 then C.int (GNAT.OS_Lib.Errno) else 0);
      begin
         if Result = 0 then
            return Armed;
         end if;
