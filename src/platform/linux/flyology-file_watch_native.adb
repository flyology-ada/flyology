...
package body Flyology.File_Watch_Native is
   ...
   WATCH_MASK     : constant C.unsigned :=
     IN_MODIFY or IN_ATTRIB or IN_CLOSE_WRITE or IN_MOVED_FROM or IN_MOVED_TO
     or IN_CREATE or IN_DELETE or IN_DELETE_SELF or IN_MOVE_SELF;
   ...
   package Event_Conversions is new System.Address_To_Access_Conversions
     (Inotify_Event);

   Buffer_Size : constant := 16 * 1_024;
   type Byte_Array is array (Natural range <>) of aliased C.unsigned_char
     with Convention => C;

   function Inotify_Init (Flags : C.int) return C.int;
   pragma Import (C, Inotify_Init, "inotify_init1");
   function Inotify_Add
     (FD : C.int; Path : System.Address; Mask : C.unsigned) return C.int;
   ...
   begin
      ...
      if New_Source < 0 then
         raise Flyology.IO.Device_Error with
           "inotify file watcher creation failed, errno="
   ...
   begin
      ...
      if Result < 0 then
         raise Flyology.IO.Device_Error with
           "file watch registration failed, errno="
           & GNAT.OS_Lib.Errno'Image;
      end if;
      ...
      begin
         ...
         for Index in Events'First .. Events'First + Count - 1 loop
            if Events (Index).Subject = Subject then
               Events (Index).Changes.Contents :=
                 Events (Index).Changes.Contents or else Changes.Contents;
               Events (Index).Changes.Metadata :=
                 Events (Index).Changes.Metadata or else Changes.Metadata;
               Events (Index).Changes.Identity :=
                 Events (Index).Changes.Identity or else Changes.Identity;
               Events (Index).Changes.Invalidated :=
                 Events (Index).Changes.Invalidated
                 or else Changes.Invalidated;
               Events (Index).Changes.Events_Lost :=
                 Events (Index).Changes.Events_Lost
                 or else Changes.Events_Lost;
               return;
            end if;
         end loop;
         if Count = Events'Length then
   ...
   begin
      ...
      loop
         Bytes :=
           C_Read (Source, Buffer'Address, C.size_t (Buffer'Length));
      ...
      while Offset < Natural (Bytes) loop
         if Natural (Bytes) - Offset < Inotify_Event'Size / System.Storage_Unit
         then
            raise Flyology.IO.Device_Error with
         ...
         declare
            Header : constant Event_Conversions.Object_Pointer :=
              Event_Conversions.To_Pointer
                (Buffer'Address + SSE.Storage_Offset (Offset));
            Header_Size : constant Natural :=
              Inotify_Event'Size / System.Storage_Unit;
            Step : constant Natural :=
              Header_Size + Natural (Header.Name_Length);
            Changes : Raw_Changes := (others => False);
         begin
            if Step < Header_Size
              or else Step > Natural (Bytes) - Offset
            then
               raise Flyology.IO.Device_Error with
                 "invalid inotify event length";
            end if;
            ...
            if Has (Header.Mask, IN_Q_OVERFLOW) then
               ...
               Changes.Identity :=
                 Has (Header.Mask, IN_DELETE_SELF)
                 or else Has (Header.Mask, IN_MOVE_SELF);
               Changes.Invalidated :=
                 Changes.Identity
                 or else Has (Header.Mask, IN_UNMOUNT)
                 or else Has (Header.Mask, IN_IGNORED);
