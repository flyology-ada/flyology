...
package body Flyology.Shared_Memory_Native is
   ...
   function Is_Linux return Boolean is (True);
   function POSIX_Identity_Supported return Boolean is (True);
   function Untrusted_Handoff_Supported return Boolean is (True);
   ...
   function Error_Not_Socket return C.int is (88);
   function Error_Not_Connected return C.int is (107);
   ...
   function Shared_Open_Close_On_Exec return C.int is (O_CLOEXEC);
   ...
   function Message_Close_On_Exec return C.int is (16#4000_0000#);
   function Message_Control_Truncated return C.int is (16#0008#);
   function Message_Truncated return C.int is (16#0020#);
   ...
   function Flush_Mapping
     (Base : System.Address; Length : C.size_t; Synchronous : Boolean)
      return C.int is
     (C_Msync (Base, Length, (if Synchronous then 4 else 1)));
   ...
   function Enable_No_SIGPIPE (Descriptor : C.int) return C.int is (0);
