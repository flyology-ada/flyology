...
package body Flyology.Shared_Memory_Native is
   ...
   O_CREAT       : constant C.int := 16#0040#;
   O_EXCL        : constant C.int := 16#0080#;
   O_CLOEXEC     : constant C.int := 16#0008_0000#;
   ...
   pragma Import (C, C_Shared_Open, "shm_open");
   ...
   function C_Getentropy (Target : System.Address; Length : C.size_t) return C.int;
   pragma Import (C, C_Getentropy, "getentropy");
   ...
   is (C_Msync (Base, Length, (if Synchronous then 4 else 1)));
   ...
   is (C_Getentropy (Target, Length));

   function Socket_Type (Descriptor : C.int; Value : out C.int) return C.int is
      Local  : aliased C.int;
      Length : aliased C.unsigned := C.unsigned (C.int'Size / 8);
      Result : constant C.int := C_Getsockopt (Descriptor, SOL_SOCKET, SO_TYPE, Local'Address, Length'Access);
   begin
      Value := Local;
      return Result;
   end Socket_Type;
   function Socket_Accepting (Descriptor : C.int; Value : out C.int) return C.int is
      ...
      Result : constant C.int :=
        C_Getsockopt (Descriptor, SOL_SOCKET, SO_ACCEPTCONN, Local'Address, Length'Access);
   begin
      Value := Local;
      return Result;
