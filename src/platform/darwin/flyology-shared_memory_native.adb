...
package body Flyology.Shared_Memory_Native is
   ...
   function C_Shared_Open (Name : C_Strings.chars_ptr; Flags : C.int; Mode : C.unsigned) return C.int;
   ...
   function C_File_Open (Path : C_Strings.chars_ptr; Flags : C.int; Mode : C.unsigned) return C.int;
   ...
   function C_Truncate (Descriptor : C.int; Length : C.long_long) return C.int;
   ...
   function C_Munmap (Base : System.Address; Length : C.size_t) return C.int;
   pragma Import (C, C_Munmap, "munmap");
   function C_Msync (Base : System.Address; Length : C.size_t; Flags : C.int) return C.int;
   ...
   function C_Getsockopt
     (Socket, Level, Option : C.int; Value : System.Address; Length : access C.unsigned) return C.int;
   pragma Import (C, C_Getsockopt, "getsockopt");
   function C_Setsockopt
     (Socket, Level, Option : C.int; Value : System.Address; Length : C.unsigned) return C.int;
   ...
   function C_Local_Family (Descriptor : C.int; Family : access C.int) return C.int;
   pragma Import (C, C_Local_Family, "flyology_shm_getsockname_family");
   function C_Peer_Family (Descriptor : C.int; Family : access C.int) return C.int;
   pragma Import (C, C_Peer_Family, "flyology_shm_getpeername_family");
   function C_Socket_Accepting (Descriptor : C.int; Accepting : access C.int) return C.int;
   ...
   function To_Address is new Ada.Unchecked_Conversion (C.unsigned_long_long, System.Address);
   ...
   is (C_Shared_Open (Name, Flags, Mode));
   function Shared_Unlink (Name : C_Strings.chars_ptr) return C.int
   is (C_Shared_Unlink (Name));
   function File_Open (Path : C_Strings.chars_ptr; Flags : C.int; Mode : C.unsigned) return C.int
   is (C_File_Open (Path, Flags, Mode));
   function File_Unlink (Path : C_Strings.chars_ptr) return C.int
   is (C_File_Unlink (Path));
   function Truncate (Descriptor : C.int; Length : C.long_long) return C.int
   is (C_Truncate (Descriptor, Length));
   function Close (Descriptor : C.int) return C.int
   is (C_Close (Descriptor));
   function Flush_File (Descriptor : C.int) return C.int
   is (C_Fsync (Descriptor));
   function Map_Shared (Descriptor : C.int; Length : C.size_t) return System.Address
   is (C_Mmap (System.Null_Address, Length, 3, 1, Descriptor, 0));
   function Failed_Mapping return System.Address
   is (To_Address (C.unsigned_long_long'Last));
   function Unmap (Base : System.Address; Length : C.size_t) return C.int
   is (C_Munmap (Base, Length));
   function Flush_Mapping (Base : System.Address; Length : C.size_t; Synchronous : Boolean) return C.int
   is (C_Msync (Base, Length, (if Synchronous then 16#10# else 1)));

   function Descriptor_Stat (Descriptor : C.int; Fields : out Stat_Fields) return C.int is
      Size          : aliased C.long_long;
      Device, Inode : aliased C.unsigned_long_long;
      Mode          : aliased C.unsigned;
      Result        : C.int;
   begin
      Result := C_Descriptor_Stat (Descriptor, Size'Access, Device'Access, Inode'Access, Mode'Access);
      Fields := (Size, Device, Inode, Mode);
      return Result;
   end Descriptor_Stat;

   function Path_Stat (Path : C_Strings.chars_ptr; Fields : out Stat_Fields) return C.int is
      Size          : aliased C.long_long;
      Device, Inode : aliased C.unsigned_long_long;
      Mode          : aliased C.unsigned;
      Result        : C.int;
   begin
      Result := C_Path_Stat (Path, Size'Access, Device'Access, Inode'Access, Mode'Access);
      Fields := (Size, Device, Inode, Mode);
      return Result;
   end Path_Stat;

   function Get_Descriptor_Flags (Descriptor : C.int) return C.int
   is (C_Get_FD (Descriptor));
   function Set_Descriptor_Flags (Descriptor : C.int; Flags : C.int) return C.int
   is (C_Set_FD (Descriptor, Flags));
   function Get_Status_Flags (Descriptor : C.int) return C.int
   is (C_Get_FL (Descriptor));
   function Get_Seals (Descriptor : C.int) return C.int
   is (C_Get_Seals (Descriptor));
   function Add_Seals (Descriptor : C.int; Seals : C.int) return C.int
   is (C_Add_Seals (Descriptor, Seals));
   function Memfd_Create (Flags : C.unsigned) return C.int
   is (C_Memfd_Create (Flags));
   function Fill_Random (Target : System.Address; Length : C.size_t) return C.int is
   begin
      Arc4random_Buf (Target, Length);
      return 0;
   end Fill_Random;

   function Socket_Type (Descriptor : C.int; Value : out C.int) return C.int is
      Local  : aliased C.int;
      Length : aliased C.unsigned := C.unsigned (C.int'Size / 8);
      Result : constant C.int := C_Getsockopt (Descriptor, 16#FFFF#, 16#1008#, Local'Address, Length'Access);
   begin
      Value := Local;
      return Result;
   end Socket_Type;
   function Socket_Accepting (Descriptor : C.int; Value : out C.int) return C.int is
      Local  : aliased C.int;
      Result : constant C.int := C_Socket_Accepting (Descriptor, Local'Access);
   begin
      Value := Local;
      return Result;
   end Socket_Accepting;
   function Local_Socket_Family (Descriptor : C.int; Family : out C.int) return C.int is
      Local  : aliased C.int;
   ...
   function Peer_Socket_Family (Descriptor : C.int; Family : out C.int) return C.int is
      Local  : aliased C.int;
   ...
   is (1);
   function Enable_No_SIGPIPE (Descriptor : C.int) return C.int is
      Enabled : aliased C.int := 1;
   begin
      return C_Setsockopt (Descriptor, 16#FFFF#, 16#1022#, Enabled'Address, C.unsigned (C.int'Size / 8));
   end Enable_No_SIGPIPE;
   function Send_Descriptor_Once (Socket : C.int; Descriptor : C.int) return C.long
   is (C_Send_Once (Socket, Descriptor));
      ...
      Result :=
        C_Receive_Once
          (Socket,
           ...
           Descriptors'Length,
           Local_Count'Access,
           Local_Payload'Access,
           Local_Flags'Access,
           Local_Malformed'Access);
      Count := Local_Count;
