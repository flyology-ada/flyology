...
is
   ...
   with Pre => Is_Valid (Item), Post => Armed_Count'Result <= Item.Capacity;
   ...
   with Pre => Is_Valid (Item), Post => Is_Valid (Item) and then Armed_Count (Item) = 0;
   ...
   procedure Arm (Item : in out Timer_State; Id : Timer_Id; Deadline : Ada.Real_Time.Time)
   with
     Pre  =>
       Is_Valid (Item)
       and then Id <= Item.Capacity
       and then (Is_Armed (Item, Id) or else Armed_Count (Item) < Item.Capacity),
     Post =>
       Is_Valid (Item)
       and then Is_Armed (Item, Id)
       and then Deadline_Of (Item, Id) = Deadline
       and then Armed_Count (Item)
                = (if Is_Armed (Item'Old, Id) then Armed_Count (Item'Old) else Armed_Count (Item'Old) + 1)
       and then (for all Other in Timer_Id range 1 .. Item.Capacity =>
                   (if Other /= Id
                    then
                      Is_Armed (Item, Other) = Is_Armed (Item'Old, Other)
                      and then (if Is_Armed (Item, Other)
                                then Deadline_Of (Item, Other) = Deadline_Of (Item'Old, Other))));

   --  Idempotently disarm one identity.
   --  @param Item Valid state to update
   --  @param Id In-range timer identity
   procedure Cancel (Item : in out Timer_State; Id : Timer_Id)
   with
     Pre  => Is_Valid (Item) and then Id <= Item.Capacity,
     Post =>
       Is_Valid (Item)
       and then not Is_Armed (Item, Id)
       and then Armed_Count (Item)
                = (if Is_Armed (Item'Old, Id) then Armed_Count (Item'Old) - 1 else Armed_Count (Item'Old))
       and then (for all Other in Timer_Id range 1 .. Item.Capacity =>
                   (if Other /= Id
                    then
                      Is_Armed (Item, Other) = Is_Armed (Item'Old, Other)
   ...
   function Earliest_Deadline (Item : Timer_State) return Ada.Real_Time.Time
   with
     Pre  => Is_Valid (Item) and then Armed_Count (Item) > 0,
     Post =>
       (for all Id in Timer_Id range 1 .. Item.Capacity =>
          (if Is_Armed (Item, Id) then Earliest_Deadline'Result <= Deadline_Of (Item, Id)));
   ...
   procedure Collect_Due (Item : in out Timer_State; Now : Ada.Real_Time.Time; Activated : out Due_Batch)
   with
     Pre  => Is_Valid (Item) and then Activated.Capacity = Item.Capacity,
     Post =>
       Is_Valid (Item)
       and then Activated.Count <= Activated.Capacity
       and then Armed_Count (Item) = Armed_Count (Item'Old) - Activated.Count
       and then (for all Id in Timer_Id range 1 .. Item.Capacity =>
                   ((for some Position in 1 .. Activated.Count => Activated.Ids (Position) = Id)
                    = (Is_Armed (Item'Old, Id) and then Deadline_Of (Item'Old, Id) <= Now)))
       and then (for all Id in Timer_Id range 1 .. Item.Capacity =>
                   Is_Armed (Item, Id) = (Is_Armed (Item'Old, Id) and then Deadline_Of (Item'Old, Id) > Now))
       and then (for all Position in 1 .. Activated.Count =>
                   Is_Armed (Item'Old, Activated.Ids (Position))
                   and then Deadline_Of (Item'Old, Activated.Ids (Position)) <= Now
                   and then (for all Earlier in 1 .. Position - 1 =>
                               Activated.Ids (Earlier) /= Activated.Ids (Position)))
       and then (for all Id in Timer_Id range 1 .. Item.Capacity =>
                   (if Is_Armed (Item, Id) then Deadline_Of (Item, Id) = Deadline_Of (Item'Old, Id)));
