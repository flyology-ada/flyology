...
package body Flyology.Process_Generations.Transport is
   ...
   protected body Channel_Controller is
      procedure Adopt (Authority : Upgrade_Handle; Accepted : out Boolean) is
      begin
      ...
      begin
         ...
         case State is
            when Ready     =>
               State := Busy;
               Sequence := (if Way = Sending then Next_Send else Next_Receive);
               Result := Acquired;
      ...
      begin
         if State = Busy then
            if (Way = Sending and then Next_Send = Interfaces.Unsigned_64'Last)
              or else (Way = Receiving and then Next_Receive = Interfaces.Unsigned_64'Last)
            then
               State := Exhausted;
            else
               if Way = Sending then
      ...
      is (State in Poisoned | Exhausted);
   ...
   procedure Reraise_Transport (Error : Ada.Exceptions.Exception_Occurrence) is
   begin
      if Ada.Exceptions.Exception_Identity (Error) = Protocol_Error'Identity
        or else Ada.Exceptions.Exception_Identity (Error) = Sequence_Error'Identity
        or else Ada.Exceptions.Exception_Identity (Error) = Channel_Busy'Identity
        or else Ada.Exceptions.Exception_Identity (Error) = Validation_Error'Identity
        or else Ada.Exceptions.Exception_Identity (Error) = Flyology.IO.Timeout_Error'Identity
      then
   ...
   begin
      ...
      begin
         Descriptor_Handoffs.Validate_Carrier
           (Descriptor_Handoffs.Socket_Descriptor (Interfaces.C.int (Raw)), Descriptor_Handoffs.Trusted_Peer);
      exception
         when Error : others =>
            raise Validation_Error with Ada.Exceptions.Exception_Message (Error);
      end;
   ...
   is (Item.Controller.Open and then Sockets.Is_Open (Item.Socket));

   function Is_Poisoned (Item : Control_Channel) return Boolean
   is (Item.Controller.Failed);
   ...
   begin
      if not Is_Open (Item) then
         ...
         return Flyology.IO.Wait (Sockets.Native_Descriptor (Item.Socket), Flyology.IO.For_Read, Timeout);
      end if;
      ...
      declare
         Interrupts : constant Flyology.IO.Interrupt_Set (1 .. 1) := (1 => Wake_FD);
      begin
   ...
   is
      ...
      Result    : Begin_Result;
   begin
      Item.Controller.Begin_Operation (Sending, Authority, Sequence, Result);
      Raise_Begin (Result);
      begin
         declare
            Frame  : constant Protocol.Frame :=
              (Kind      => Kind,
               ...
               Length    => Length,
               Payload   => Payload);
            Octets : Protocol.Octet_Array (0 .. Protocol.Encoded_Length (Frame) - 1);
            Bytes  :
              Ada.Streams.Stream_Element_Array
                (1 .. Ada.Streams.Stream_Element_Offset (Protocol.Encoded_Length (Frame)));
         begin
            Protocol.Encode (Frame, Octets);
            for Index in Octets'Range loop
               Bytes (Bytes'First + Ada.Streams.Stream_Element_Offset (Index)) :=
   ...
   procedure Send
     (Item : in out Control_Channel; Kind : Protocol.Message_Kind; Timeout : Duration := Flyology.IO.Infinite)
   is
   begin
   ...
   is
      ...
      Result       : Begin_Result;
      Header_Bytes :
        Ada.Streams.Stream_Element_Array (1 .. Ada.Streams.Stream_Element_Offset (Protocol.Header_Length));
   ...
   begin
      Frame :=
        (Kind      => Protocol.Hello,
         ...
         Length    => 0,
         Payload   => (others => 0));
      Item.Controller.Begin_Operation (Receiving, Authority, Sequence, Result);
      Raise_Begin (Result);
      begin
         Sockets.Receive_Exactly (Item.Socket, Header_Bytes, Timeout => Remaining (Started, Timeout));
         for Index in Header'Range loop
            Header (Index) :=
              Protocol.Octet (Header_Bytes (Header_Bytes'First + Ada.Streams.Stream_Element_Offset (Index)));
         end loop;
         Protocol.Inspect_Header (Header, Length, Status);
         if Status /= Protocol.Decoded then
            raise Protocol_Error with "invalid control header: " & Protocol.Decode_Status'Image (Status);
         end if;
         declare
            Octets : Protocol.Octet_Array (0 .. Protocol.Header_Length + Length - 1);
         begin
            ...
            if Length > 0 then
               ...
               begin
                  Sockets.Receive_Exactly
                    (Item.Socket, Payload_Bytes, Timeout => Remaining (Started, Timeout));
                  for Index in 0 .. Length - 1 loop
                     Octets (Protocol.Header_Length + Index) :=
                       Protocol.Octet
                         (Payload_Bytes (Payload_Bytes'First + Ada.Streams.Stream_Element_Offset (Index)));
                  end loop;
               end;
            end if;
         ...
         if Status /= Protocol.Decoded then
            raise Protocol_Error with "invalid control frame: " & Protocol.Decode_Status'Image (Status);
         elsif not Same_Upgrade (Frame.Authority, Authority) then
            raise Protocol_Error with "control authority does not match channel";
