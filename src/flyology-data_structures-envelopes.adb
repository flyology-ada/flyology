...
package body Flyology.Data_Structures.Envelopes is
   ...
   begin
      if Contract_Signature = 0 or else Contract_Version = 0 then
         raise Constraint_Error with "envelope signature and version must be nonzero";
      elsif Nested_Identity.Magic = 0 or else Nested_Identity.Version = 0 or else Nested_Identity.Schema = 0
      then
         raise Constraint_Error with "nested layout identity fields must be nonzero";
      elsif Content_Extent < Layouts.Header_Size then
         raise Constraint_Error with "envelope content is smaller than a nested structure header";
      elsif Content_Alignment > Byte_Count (Interfaces.Unsigned_32'Last) then
         raise Constraint_Error with "envelope alignment exceeds 32 bits";
      end if;
      Content_Offset := Layouts.Align_Up (Content_Prefix, Content_Alignment);
   ...
   begin
      Geometry (Content_Extent, Content_Alignment, Content_Offset, Total_Extent);
      return Total_Extent;
   end Required_Storage;

   procedure Set_View
     (Item : out View; Core : Layouts.Local_View; Content_Offset, Content_Size, Alignment : Byte_Count) is
   begin
      Item.Core := Core;
   ...
   begin
      Set_View (Item, Core, Content_Offset, Content_Extent, Content_Alignment);
      Layouts.Invalidate_Nested (Item.Core, Item.Content_Offset);
      Bytes.Write_U64 (Layouts.Address_At (Item.Core, Nested_Magic_Offset, 8, 8), Nested_Identity.Magic);
      Bytes.Write_U32 (Layouts.Address_At (Item.Core, Nested_Version_Offset, 4, 4), Nested_Identity.Version);
      Bytes.Write_U32 (Layouts.Address_At (Item.Core, Nested_Reserved_Offset, 4, 4), 0);
      Bytes.Write_U64 (Layouts.Address_At (Item.Core, Nested_Schema_Offset, 8, 8), Nested_Identity.Schema);
   ...
   begin
      ...
      Finish_Initialize (Item, Core, Content_Offset, Content_Extent, Content_Alignment);
   exception
      when others =>
   ...
   begin
      ...
      case Claim is
         ...
         when Layouts.Existing_Ready    =>
            Attach (Item, Region, Location, Content_Extent, Content_Alignment);
   ...
   begin
      Detach (Item);
      Geometry (Content_Extent, Content_Alignment, Content_Offset, Total_Extent);
      Layouts.Attach (Core, Header, Region, Location, Identity, Byte_Count'Max (8, Content_Alignment));
      if Core.Extent /= Total_Extent then
        ...
        or else Header.Alignment /= Interfaces.Unsigned_32 (Content_Alignment)
      ...
      then
         raise Layout_Error with "envelope contract or geometry does not match";
      elsif Bytes.Read_U64 (Layouts.Address_At (Core, Nested_Magic_Offset, 8, 8)) /= Nested_Identity.Magic
        or else Bytes.Read_U32 (Layouts.Address_At (Core, Nested_Version_Offset, 4, 4))
                /= Nested_Identity.Version
        or else Bytes.Read_U32 (Layouts.Address_At (Core, Nested_Reserved_Offset, 4, 4)) /= 0
        or else Bytes.Read_U64 (Layouts.Address_At (Core, Nested_Schema_Offset, 8, 8))
                /= Nested_Identity.Schema
      then
         raise Layout_Error with "envelope nested layout identity does not match";
      end if;
      Set_View (Item, Core, Content_Offset, Content_Extent, Content_Alignment);
   exception
      when others =>
   ...
   begin
      ...
      Result := Layouts.Checked_Add (Byte_Count (Item.Core.Location), Item.Content_Offset);
