...
is
   ...
   function Classify_Acquire
     (Generation_Matches : Boolean;
      Closing            : Boolean;
      Descriptor_Open    : Boolean) return Acquire_Action
   with Post =>
     (if not Generation_Matches then
         Classify_Acquire'Result = Reject_Replaced
      elsif Closing then Classify_Acquire'Result = Reject_Closing
      elsif not Descriptor_Open then Classify_Acquire'Result = Reject_Closed
   ...
   function Finish_Close_Allowed
     (Closing            : Boolean;
      Active             : Boolean;
      Generation_Matches : Boolean) return Boolean
   with Post =>
     Finish_Close_Allowed'Result =
       (Closing and then not Active and then Generation_Matches);
   ...
   function Invalid_Progress_Sentinel
     (First : Offset;
      Last  : Offset) return Sentinel_Choice
   with Pre => First <= Last,
        Post =>
          Invalid_Progress_Sentinel'Result.Available =
            (First > Offset'First or else Last < Offset'Last)
          and then
            (if Invalid_Progress_Sentinel'Result.Available then
                Invalid_Progress_Sentinel'Result.Value < First
                or else Invalid_Progress_Sentinel'Result.Value > Last);

   function Complete_Progress_Valid
     (First    : Offset;
      Last     : Offset;
      Reported : Offset) return Boolean
   with Pre => First <= Last,
        Post => Complete_Progress_Valid'Result =
          (Reported >= First and then Reported <= Last);

   function Progress_Preserved
     (Before : Offset;
      After  : Offset) return Boolean
   with Post => Progress_Preserved'Result = (After = Before);

   function Next_Offset (Position : Offset) return Offset
   with Pre  => Position < Offset'Last,
        Post => Next_Offset'Result = Position + 1;
