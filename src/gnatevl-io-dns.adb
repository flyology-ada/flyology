...
package body Gnatevl.IO.DNS is
   ...
   procedure Encode_Name
     (Name : Name_Buffer; Packet : in out Byte_Array; Position : in out Natural)
   is
      Start : Positive := 1;
      Stop  : Natural;
   begin
         ...
         if Record_Index <= Answer_Count and then Class = Class_IN then
            if Kind = Type_A and then Data_Length = 4 then
               ...
               if Record_Count < Max_Records then
                  ...
                  Records (Record_Count).Kind := Kind;
                  Records (Record_Count).TTL := TTL;
                  Records (Record_Count).Address.Family := Sockets.Family_Inet6;
         ...
         begin
            declare
               ...
               Attempt_Time : constant Duration :=
                 (if Infinite then Per_Attempt
                  else Duration'Min (Per_Attempt, Remaining (Deadline, False)));
            ...
            begin
               ...
               begin
                  ...
                  Gnatevl.IO.Sockets.Prepare (Channels (1));
                  ...
                  Add_Request
                    (Gnatevl.IO.Sockets.Native_Descriptor (Channels (1)),
                     For_Read);
               end;
                                 ...
                                 then
                                    declare
                                       Alias_Result : constant Parse_Result :=
                                         Query_Kind
                                           (Parsed.Canonical, Kind, Name_Servers,
