...
is
   ...
   with
     ...
     Post =>
       ...
       = (Encoded_Length /= 0 and then Wire_Length (Encoded_Length) <= Total_Length - Cursor - 1)
       and then (if Inspect_Item'Result.Valid
                 then
                   Inspect_Item'Result.Value_Offset = Cursor + 1
                   and then Inspect_Item'Result.Value_Length = Wire_Length (Encoded_Length)
                   and then Inspect_Item'Result.Next_Offset = Cursor + 1 + Wire_Length (Encoded_Length)
   ...
   function Select_ALPN (Server : Wire_Bytes; Offered : Wire_Bytes) return ALPN_Selection
   with
     Post =>
       (if Select_ALPN'Result.Found
        then
          Select_ALPN'Result.Value_Length in 1 .. 255
          and then Long_Long_Integer (Select_ALPN'Result.Value_Offset) < Long_Long_Integer (Offered'Length)
   ...
   type Provider_Result is
     (Provider_Complete, Provider_Want_Read, Provider_Want_Write, Provider_Peer_Closed, Provider_Failed);

   function Classify_Provider_Result (Value : C.long) return Provider_Result
   with
     Post =>
       (if Value = 0
        then Classify_Provider_Result'Result = Provider_Complete
        elsif Value = -2
        then Classify_Provider_Result'Result = Provider_Want_Read
        elsif Value = -3
        then Classify_Provider_Result'Result = Provider_Want_Write
        elsif Value = -4
        then Classify_Provider_Result'Result = Provider_Peer_Closed
   ...
   with Export, Convention => C, External_Name => "flyology_tls_openssl_policy_classify_error";

   function Error_Is_Transport_End (Error : C.int) return C.int
   with Export, Convention => C, External_Name => "flyology_tls_openssl_policy_transport_end";
   ...
   with Export, Convention => C, External_Name => "flyology_tls_openssl_policy_versions_match";

   function Classify_Shutdown (Result : C.int) return C.int
   with Export, Convention => C, External_Name => "flyology_tls_openssl_policy_classify_shutdown";
