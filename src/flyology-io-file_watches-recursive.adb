...
package body Flyology.IO.File_Watches.Recursive is
   ...
   procedure Free_Candidates (Candidates : in out Candidate_Array);
   function Find_Path (Item : Recursive_Watcher'Class; Path : String) return Natural;
   function Has_Candidate (Candidates : Candidate_Array; Count : Natural; Path : String) return Boolean;
   procedure Reconcile (Item : in out Recursive_Watcher'Class; Result : out Recursive_Event);
   procedure Close_Internal (Item : in out Recursive_Watcher; Allow_Borrowed : Boolean);

   procedure Complete_Operation (Item : in out Next_Operation; Result : Flyology.Operations.Terminal_Outcome);
   ...
   begin
      if Event /= Flyology.Operations.Dependency_Changed or else not Item.State.Child_Live then
         Item.State.Failure := State_Failure;
         Complete_Operation (Item, Flyology.Operations.Failed);
         return;
      end if;

      Finish_Child (Item, Raw, Child_Outcome, Child_Succeeded, Child_Cancelled);
   ...
   procedure Start_Scoped_Next
     (Item : not null access Recursive_Watcher'Class; Timeout : Duration; Operation : in out Next_Operation)
   is
   begin
      ...
      File_Watches.Next (Item.Source'Unchecked_Access, Timeout, Operation.State.Child);
   ...
   exception
      when others =>
         ...
         if Flyology.Operations.Is_Terminal (Operation.State.Child) then
            ...
            begin
               begin
                  File_Watches.Finish (Operation.State.Child, Raw, Child_Outcome);
               exception
                  when others =>
                     null;
               end;
   ...
   begin
      ...
      case Terminal is
         ...
         when Flyology.Operations.Failed    =>
            case Failure is
               when State_Failure     =>
                  raise Device_Error with "recursive watcher is closed or already borrowed";

               when Child_Failure     =>
                  raise Device_Error with "recursive watcher readiness wait failed";

               when Reconcile_Failure =>
                  raise Device_Error with "recursive watcher reconciliation failed";

               when No_Failure        =>
                  raise Device_Error with "recursive watcher operation failed";
            end case;
      end case;
   end Finish;
   ...
   procedure Free_Candidates (Candidates : in out Candidate_Array) is
   begin
   ...
   function Find_Path (Item : Recursive_Watcher'Class; Path : String) return Natural is
   begin
      for Index in Item.Directories'Range loop
         if Item.Directories (Index).Path /= null and then Item.Directories (Index).Path.all = Path then
            return Index;
         end if;
      end loop;
      return 0;
   ...
   function Has_Candidate (Candidates : Candidate_Array; Count : Natural; Path : String) return Boolean is
   begin
      for Index in Candidates'First .. Candidates'First + Count - 1 loop
         if Candidates (Index) /= null and then Candidates (Index).all = Path then
            return True;
         end if;
      end loop;
      return False;
   ...
   is
      ...
      begin
         ...
         exception
            when Ada.Directories.Name_Error | Ada.Directories.Use_Error | Ada.Directories.Status_Error =>
               if Started then
                  Ada.Directories.End_Search (Search);
               end if;
               Complete := False;
   ...
   procedure Open (Item : in out Recursive_Watcher; Root : String) is
      ...
      begin
         if Is_Open (Item) then
            ...
            raise Device_Error with "recursive watcher is already open for another root";
         end if;

         Discover (Canonical, Candidates, Candidate_Count, Root_Present, Complete);
         if not Root_Present then
            ...
            Free_Candidates (Candidates);
            raise Device_Error with "recursive watcher initial discovery is incomplete";
         end if;

         begin
            Item.Source.Open;
            Item.Root := new String'(Canonical);
            for Candidate_Index in Candidates'First .. Candidates'First + Candidate_Count - 1 loop
               declare
                  Slot : Natural := 0;
               begin
                  for Index in Item.Directories'Range loop
                  ...
                  Item.Directories (Slot).Watch := Item.Source.Add (Candidates (Candidate_Index).all);
                  Item.Directories (Slot).Path := Candidates (Candidate_Index);
   ...
   begin
      if Item.Scoped_Borrowed then
         raise Device_Error with "recursive watcher is borrowed by a scoped operation";
      end if;
   ...
   procedure Reconcile (Item : in out Recursive_Watcher'Class; Result : out Recursive_Event) is
      Candidates      : Candidate_Array (1 .. Item.Capacity) := (others => null);
      Candidate_Count : Natural;
      Root_Present    : Boolean;
      Complete        : Boolean;
   ...
   begin
      Result :=
        (Changes               => No_Changes,
         ...
         Directory_Count       => Item.Count,
         Coverage_Complete     => Item.Complete_Coverage);
      Discover (Item.Root.all, Candidates, Candidate_Count, Root_Present, Complete);
      ...
      if not Root_Present then
         ...
         Close_Internal (Recursive_Watcher (Item), Allow_Borrowed => True);
      ...
      for Index in Item.Directories'Range loop
         if Item.Directories (Index).Path /= null
           and then not Has_Candidate (Candidates, Candidate_Count, Item.Directories (Index).Path.all)
         then
            begin
      ...
      if Failed then
         raise Device_Error with "recursive watcher registration reconciliation failed";
      end if;
   exception
      when others =>
         Free_Candidates (Candidates);
   ...
   procedure Refresh (Item : in out Recursive_Watcher; Result : out Recursive_Event) is
   begin
      if Item.Scoped_Borrowed then
         raise Device_Error with "recursive watcher is borrowed by a scoped operation";
   ...
   begin
      ...
      Result := Reconciled;
      for Kind in Change_Kind loop
         Result.Changes (Kind) := Result.Changes (Kind) or else Raw.Changes (Kind);
      end loop;
   end Next;

   procedure Close_Internal (Item : in out Recursive_Watcher; Allow_Borrowed : Boolean) is
      Failed : Boolean := False;
   begin
   ...
   is (Is_Open (Item) and then Item.Complete_Coverage);

   overriding
   procedure Finalize (Item : in out Recursive_Watcher) is
   begin
      begin
         Close (Item);
      exception
