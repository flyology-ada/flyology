...
is
   ...
   begin
      if Encoded_Length = 0
        or else Value_Length > Total_Length - Value_Offset
      then
         return (others => <>);
      end if;
      return
      ...
      while Cursor < Data_Length loop
         ...
         declare
            Item_Length : constant Long_Long_Integer :=
              Long_Long_Integer
                (Data (Data'First + Natural (Cursor)));
            Value_Offset : constant Long_Long_Integer := Cursor + 1;
         begin
            if Item_Length = 0
              or else Item_Length > Data_Length - Value_Offset
            then
               return False;
            end if;
   ...
   begin
      if not Valid_ALPN_List (Server)
        or else not Valid_ALPN_List (Offered)
      then
         return (others => <>);
      end if;
      ...
      while Server_Cursor < Server_Total loop
         ...
         declare
            Server_Length : constant Long_Long_Integer :=
              Long_Long_Integer
                (Server (Server'First + Natural (Server_Cursor)));
            Server_Value_Offset : constant Long_Long_Integer :=
              Server_Cursor + 1;
            Client_Cursor : Long_Long_Integer := 0;
         begin
            if Server_Length = 0
              or else Server_Length >
                Server_Total - Server_Value_Offset
            then
               return (others => <>);
            end if;
            ...
            while Client_Cursor < Offered_Total loop
               ...
               declare
                  Client_Length : constant Long_Long_Integer :=
                    Long_Long_Integer
                      (Offered
                         (Offered'First + Natural (Client_Cursor)));
                  Client_Value_Offset : constant Long_Long_Integer :=
                    Client_Cursor + 1;
               begin
                  if Client_Length = 0
                    or else Client_Length >
                      Offered_Total - Client_Value_Offset
                  then
                     return (others => <>);
                  end if;
                     ...
                     begin
                        for Offset in 0 .. Server_Length - 1 loop
                           if Server
                             (Server'First
                              + Natural
                                (Server_Value_Offset + Offset)) /=
                             Offered
                               (Offered'First
                                + Natural
                                  (Client_Value_Offset + Offset))
                           then
                              Same := False;
                           end if;
                        end loop;
   ...
   is
     (if Error = SSL_Error_Want_Read then Fly_Want_Read
      elsif Error = SSL_Error_Want_Write then Fly_Want_Write
      elsif Error = SSL_Error_Zero_Return and then Peer_Close_OK /= 0 then
         Fly_Peer_Closed
      else Fly_Failed);

   function Error_Is_Transport_End (Error : C.int) return C.int is
     (if Error = SSL_Error_Syscall then 1 else 0);
   ...
   is
     (if Symbols_Match /= 0
        and then Dependency = Explicit
        and then Major_Source / OpenSSL_Major_Shift = 3
      then 1
      else 0);

   function Classify_Shutdown (Result : C.int) return C.int is
     (if Result = 1 then Fly_Complete
      elsif Result = 0 then Fly_Want_Read
      else Fly_Failed);
