...
is
   ...
   is (if not Generation_Matches or else not Resources_Open or else Closing
       then Cancel_Lease
       elsif Active
       then Wait_For_Lease
       else Acquire_Lease);
   ...
   is (not Active and then not Closing and then Started > 0 and then not Signalled);
   ...
   is (if Active then Started - 1 else Started);
