...
is
   ...
   function Appears_Before (Plan : Order_Plan; Id : Child_Id; Position : Child_Id) return Boolean
   is (for some Earlier in Child_Id =>
         Earlier < Position and then Earlier <= Plan.Length and then Plan.Items (Earlier) = Id)
   with Ghost;
   ...
   function Valid_Start_Order
     (Count : Child_Count; Dependencies : Dependency_Matrix; Plan : Order_Plan) return Boolean
   is (Plan.Length <= Count
       and then (for all Position in Child_Id =>
                   (if Position <= Plan.Length
                    then
                      Plan.Items (Position) <= Count
                      and then (for all Earlier in Child_Id =>
                                  (if Earlier < Position then Plan.Items (Earlier) /= Plan.Items (Position)))
                      and then (for all Prerequisite in Child_Id =>
                                  (if Prerequisite <= Count
                                     and then Dependencies (Plan.Items (Position), Prerequisite)
   ...
   with
     ...
     Post   =>
       Plan.Length <= Count
       and then (if Plan.Valid
                 then Plan.Length = Count and then Valid_Start_Order (Count, Dependencies, Plan)
                 else Plan.Length < Count);

   --  Reverse a valid start order for rollback or shutdown.
   procedure Plan_Stop_Order (Start : Order_Plan; Stop : out Order_Plan)
   with
     Global => null,
     Pre    => Start.Valid,
     Post   =>
       ...
       and then (for all Position in Child_Id =>
                   (if Position <= Stop.Length
                    then Stop.Items (Position) = Start.Items (Child_Id (Start.Length - Position + 1))));
   ...
   function Dependent_Recovery_Closed
     (Count : Child_Count; Dependencies : Dependency_Matrix; Affected : Child_Set) return Boolean
   is ((for all Id in Child_Id => (if Id > Count then not Affected (Id)))
       and then (for all User in Child_Id =>
   ...
   with
     ...
     Post   =>
       (case Impact is
          when Public.Escalate           => (for all Id in Child_Id => not Affected (Id)),
          when Public.Isolate_Child      => (for all Id in Child_Id => Affected (Id) = (Id = Failed)),
          when Public.Restart_Cohort     =>
            (for all Id in Child_Id =>
               Affected (Id) = (Id = Failed or else (Id <= Count and then Cohort (Id)))),
          when Public.Restart_Dependents =>
            Affected (Failed) and then Dependent_Recovery_Closed (Count, Dependencies, Affected));
   ...
   with
     Global => null,
     Post   =>
       Recorded_Transition_Allowed'Result
       = (Before = After
   ...
   with
     ...
     Post   =>
       (if Kind
           in Public.No_Termination | Public.Supervisor_Shutdown | Public.Stuck | Public.Policy_Exhaustion
        then not Should_Restart'Result
        elsif Kind in Public.Restart_Requested
        then Should_Restart'Result = (Policy in Public.On_Failure | Public.Always)
        else
          (case Policy is
             when Public.Never      => not Should_Restart'Result,
             when Public.On_Failure => Should_Restart'Result = Is_Failure (Kind),
   ...
   function Backoff_For (Attempt : Attempt_Count; Initial_Delay : Tick; Maximum_Delay : Tick) return Tick
   with
     Global => null,
     Pre    => Attempt > 0 and then Initial_Delay <= Maximum_Delay,
     Post   =>
       Backoff_For'Result in Initial_Delay .. Maximum_Delay
       and then (if Attempt = 1 then Backoff_For'Result = Initial_Delay);
   ...
   procedure Reset_If_Stable
     (Limits : Restart_Limits; Now : Tick; Ready_Since : Tick; Account : in out Restart_Account)
   with
     Global => null,
     Pre    => Ready_Since <= Now,
     Post   =>
       (if Now - Ready_Since >= Limits.Stability_Time
        then
   ...
   with
     Global => null,
     Pre    => not Incident.Attempt_Active and then Incident.Attempt < Attempt_Count'Last,
     Post   =>
       Incident.Attempt_Active
   ...
   procedure Observe_Incident
     (Incident : Incident_Context; Observation : in out Incident_Observation; Was_New : out Boolean)
   with
     Global => null,
     Pre    => Incident.Attempt_Active and then Observation.Count < Attempt_Count'Last,
     Post   =>
       Was_New
   ...
   with
     Global => null,
     Post   =>
       Same_Incident_Attempt'Result
       = (Seen and then Previous_Id = Current_Id and then Previous_Attempt = Current_Attempt);
   ...
   with
     Global => null,
     Post   => Family_Admission_Open'Result = (Configured and then not Shutdown and then not Terminal);
   ...
   with
     Global => null,
     Post   =>
       Family_Stop_Command_Allowed'Result = (Current and then (Queued or else (Managed and then Live)));
   ...
   with
     Global => null,
     Post   =>
       Family_Intervention_Command_Allowed'Result
       = (Current
          and then Managed
          and then Live
          and then Ready
   ...
   with
     Global => null,
     Post   =>
       Family_Generation_Start_Allowed'Result
       = (Generation_Allowed
          and then Managed
          and then not Stop_Pending
          and then not Shutdown
   ...
   with
     Global => null,
     Post   =>
       Family_Replacement_Wait_Allowed'Result
       = (Managed and then Backing_Off and then not Stop_Pending and then not Shutdown and then not Terminal);

   --  Report whether an incident id can advance without reuse.
   function Incident_Can_Advance (Value : Incident_Id) return Boolean
   is (Value < Incident_Id'Last);

   --  Advance an incident id without wrapping to a stale identity.
   function Next_Incident (Value : Incident_Id) return Incident_Id
   with Global => null, Pre => Incident_Can_Advance (Value), Post => Next_Incident'Result = Value + 1;

   --  Report whether a generation can advance without making an old handle
   --  current again.
   function Generation_Can_Advance (Value : Public.Generation) return Boolean
   is (Value < Public.Generation'Last);
   ...
   with Global => null, Pre => Generation_Can_Advance (Value), Post => Next_Generation'Result = Value + 1;
   ...
   with
     Global => null,
     Post   =>
       Generation_Matches'Result
       = (Expected_Id = Supplied_Id and then Expected_Generation = Supplied_Generation);
   ...
   with
     Global => null,
     Post   =>
       Generation_Start_Allowed'Result
       = (Generation_Matches (Expected_Id, Expected_Generation, Supplied_Id, Supplied_Generation)
