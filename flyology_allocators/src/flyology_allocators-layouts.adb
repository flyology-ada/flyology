...
package body Flyology_Allocators.Layouts is
   ...
   begin
      Bytes.Write_U32
        (Address_At (Item, Reserved_32_Offset, 4, 4), 0);
      Bytes.Write_U64
        (Address_At (Item, Reserved_64_1_Offset, 8, 8), 0);
      Bytes.Write_U64
        (Address_At (Item, Reserved_64_2_Offset, 8, 8), 0);
      Bytes.Write_U64
   ...
   begin
      if not Atomic.Supported then
         raise Program_Error with
           "required 32/64-bit atomics are unavailable";
   ...
   begin
      ...
      if Bytes.Read_U32
        (Address_At (Initial, Reserved_32_Offset, 4, 4)) /= 0
        or else Bytes.Read_U64
          (Address_At (Initial, Reserved_64_1_Offset, 8, 8)) /= 0
        or else Bytes.Read_U64
          (Address_At (Initial, Reserved_64_2_Offset, 8, 8)) /= 0
      then
