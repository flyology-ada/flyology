...
package body Flyology_Allocators.Allocation_Algorithms.Slab_Span_Kernel is
   ...
   Run_Metadata_Reserved_Offset : constant Byte_Count := Run_Metadata_Offset + 4;
   Head_Table_Offset            : constant Byte_Count := Layouts.Header_Size + 8;
   Head_Entry_Size              : constant Byte_Count := 4;
   Descriptor_Table_Offset      : constant Byte_Count := Head_Table_Offset + Max_Classes * Head_Entry_Size;
   ...
   function Geometry (Usable_Capacity, Minimum_Block_Size, Run_Size : Positive) return Geometry_Values is
      Usable                                                       : constant Byte_Count :=
        Byte_Count (Usable_Capacity);
      Minimum                                                      : constant Byte_Count :=
        Byte_Count (Minimum_Block_Size);
      Run                                                          : constant Byte_Count :=
        Byte_Count (Run_Size);
   ...
   begin
      ...
      Descriptor_End :=
        Layouts.Checked_Add (Descriptor_Table_Offset, Layouts.Checked_Multiply (Runs, Descriptor_Size));
      Generation_Start := Layouts.Align_Up (Descriptor_End, 8);
      Generation_End := Layouts.Checked_Add (Generation_Start, Layouts.Checked_Multiply (Units, 8));
   ...
   is (Geometry (Configuration.Usable_Capacity, Configuration.Minimum_Block_Size, Configuration.Run_Size)
         .Extent);

   function Make_Token (Epoch, Unit : Interfaces.Unsigned_32) return Interfaces.Unsigned_64
   is (Interfaces.Shift_Left (Interfaces.Unsigned_64 (Epoch), 32) or Interfaces.Unsigned_64 (Unit));
   ...
   function Token_Unit (Value : Allocation_Handle) return Interfaces.Unsigned_32
   is (Interfaces.Unsigned_32 (Value.Token and Interfaces.Unsigned_64 (Interfaces.Unsigned_32'Last)));
   ...
   is
      Offset : Byte_Count;
   begin
      if not Item.Core.Attached or else Item.Descriptor_Address = System.Null_Address then
   ...
   function State_Address (Item : View; Run : Interfaces.Unsigned_32) return System.Address
   is (Descriptor_Address (Item, Run, State_Offset, 4, 4));
   pragma Inline_Always (State_Address);

   function U32_Field (Item : View; Run : Interfaces.Unsigned_32; Relative : Byte_Count) return System.Address
   is (Descriptor_Address (Item, Run, Relative, 4, 4));
   pragma Inline_Always (U32_Field);

   function Bitmap_Word_Address
     (Item : View; Run : Interfaces.Unsigned_32; High : Boolean) return System.Address
   is (Descriptor_Address (Item, Run, (if High then Bitmap_High_Offset else Bitmap_Offset), 4, 4));
   pragma Inline_Always (Bitmap_Word_Address);

   function Read_Bitmap (Item : View; Run : Interfaces.Unsigned_32) return Interfaces.Unsigned_64 is
      Low  : constant Interfaces.Unsigned_32 :=
        Atomic.Load_Acquire_U32 (Bitmap_Word_Address (Item, Run, High => False));
      High : constant Interfaces.Unsigned_32 :=
        Atomic.Load_Acquire_U32 (Bitmap_Word_Address (Item, Run, High => True));
   begin
      return Interfaces.Unsigned_64 (Low) or Interfaces.Shift_Left (Interfaces.Unsigned_64 (High), 32);
   end Read_Bitmap;
   pragma Inline_Always (Read_Bitmap);

   procedure Store_Bitmap (Item : View; Run : Interfaces.Unsigned_32; Value : Interfaces.Unsigned_64) is
   begin
      Atomic.Store_Release_U32
        (Bitmap_Word_Address (Item, Run, High => False),
         Interfaces.Unsigned_32 (Value and Interfaces.Unsigned_64 (Interfaces.Unsigned_32'Last)));
      Atomic.Store_Release_U32
   ...
   procedure Publish_Bitmap_Word
     (Item : View; Run, Slot : Interfaces.Unsigned_32; Value : Interfaces.Unsigned_64) is
   begin
      if Slot < 32 then
         Atomic.Store_Release_U32
           (Bitmap_Word_Address (Item, Run, High => False),
   ...
   function Bitmap_Bit_Is_Set (Item : View; Run, Slot : Interfaces.Unsigned_32) return Boolean is
   ...
   begin
      ...
      Word := Atomic.Load_Acquire_U32 (Bitmap_Word_Address (Item, Run, High => Slot >= 32));
      Bit := Interfaces.Shift_Left (Interfaces.Unsigned_32 (1), Natural (Slot mod 32));
      return (Word and Bit) /= 0;
   end Bitmap_Bit_Is_Set;
   pragma Inline_Always (Bitmap_Bit_Is_Set);

   function Class_Head_Address (Item : View; Class : Interfaces.Unsigned_32) return System.Address is
   begin
      if not Item.Core.Attached or else Item.Head_Address = System.Null_Address then
         raise Region_Error with "detached slab/span view";
      elsif Class >= Item.Class_Count then
         raise Layout_Error with "slab/span class is out of range";
      end if;
      return Item.Head_Address + Native.Storage_Offset (Byte_Count (Class) * Head_Entry_Size);
   end Class_Head_Address;
   pragma Inline_Always (Class_Head_Address);

   function Unit_Generation_Address (Item : View; Unit : Interfaces.Unsigned_32) return System.Address is
      Unit_Count : constant Interfaces.Unsigned_64 :=
        Interfaces.Unsigned_64 (Item.Run_Count) * Interfaces.Unsigned_64 (Item.Units_Per_Run);
      Offset     : Byte_Count;
   begin
      if not Item.Core.Attached or else Item.Generation_Address = System.Null_Address then
         raise Region_Error with "detached slab/span view";
   ...
   is
      Descriptor_Bytes : constant Byte_Count := Byte_Count (Values.Runs) * Descriptor_Size;
      Generation_Bytes : constant Byte_Count := Byte_Count (Values.Units) * 8;
      procedure Prove_Native_Range (Address : System.Address; Extent : Byte_Count) is
         Base : constant Native.Integer_Address := Native.To_Integer (Address);
      begin
         if Extent = 0
           or else Extent - 1 > Byte_Count (Native.Storage_Offset'Last)
           or else Extent - 1 > Byte_Count (Native.Integer_Address'Last - Base)
         then
   ...
   begin
      Item.Core := Core;
      Item.Head_Address := Layouts.Address_At (Core, Head_Table_Offset, Max_Classes * Head_Entry_Size, 4);
      Item.Descriptor_Address := Layouts.Address_At (Core, Descriptor_Table_Offset, Descriptor_Bytes, 8);
      Item.Generation_Address := Layouts.Address_At (Core, Values.Generation_Offset, Generation_Bytes, 8);
      Prove_Native_Range (Item.Head_Address, Max_Classes * Head_Entry_Size);
   ...
   procedure Initialize_Tables (Item : View) is
      Unit_Count : constant Interfaces.Unsigned_32 := Item.Run_Count * Item.Units_Per_Run;
   begin
      Bytes.Write_U32 (Layouts.Address_At (Item.Core, Run_Metadata_Offset, 4, 4), Item.Run_Value);
      Bytes.Write_U32 (Layouts.Address_At (Item.Core, Run_Metadata_Reserved_Offset, 4, 4), 0);
      ...
      for Class in Interfaces.Unsigned_32 range Item.Class_Count .. Interfaces.Unsigned_32 (Max_Classes) - 1
      loop
         Bytes.Write_U32
           (Item.Head_Address + Native.Storage_Offset (Byte_Count (Class) * Head_Entry_Size), Null_Run);
      end loop;
   ...
   is
      Values : constant Geometry_Values :=
        Geometry (Configuration.Usable_Capacity, Configuration.Minimum_Block_Size, Configuration.Run_Size);
      Core   : Layouts.Local_View;
   begin
   ...
   function Class_Capacity (Item : View; Class : Interfaces.Unsigned_32) return Byte_Count is
   ...
   function Slot_Mask (Slots : Interfaces.Unsigned_32) return Interfaces.Unsigned_64 is
   begin
      if Slots = 0 or else Slots > 64 then
         ...
         return Interfaces.Shift_Left (Interfaces.Unsigned_64 (1), Natural (Slots)) - 1;
      end if;
   end Slot_Mask;
   pragma Inline_Always (Slot_Mask);

   function Population (Value : Interfaces.Unsigned_64) return Interfaces.Unsigned_32 is
   ...
   begin
      for Class in Interfaces.Unsigned_32 range Item.Class_Count .. Interfaces.Unsigned_32 (Max_Classes) - 1
      loop
         if Bytes.Read_U32 (Item.Head_Address + Native.Storage_Offset (Byte_Count (Class) * Head_Entry_Size))
           /= Null_Run
         then
      ...
      for Run in Interfaces.Unsigned_32 range 0 .. Item.Run_Count - 1 loop
         declare
            State    : constant Interfaces.Unsigned_32 := Atomic.Load_Acquire_U32 (State_Address (Item, Run));
            Class    : constant Interfaces.Unsigned_32 :=
              Bytes.Read_U32 (U32_Field (Item, Run, Class_Offset));
            Live     : constant Interfaces.Unsigned_32 := Bytes.Read_U32 (U32_Field (Item, Run, Live_Offset));
            Span     : constant Interfaces.Unsigned_32 := Bytes.Read_U32 (U32_Field (Item, Run, Span_Offset));
            Next     : constant Interfaces.Unsigned_32 := Bytes.Read_U32 (U32_Field (Item, Run, Next_Offset));
            Reserved : constant Interfaces.Unsigned_32 :=
              Bytes.Read_U32 (U32_Field (Item, Run, Reserved_Offset));
            Bitmap   : constant Interfaces.Unsigned_64 := Read_Bitmap (Item, Run);
         begin
            if Reserved /= 0 then
            ...
            case State is
               ...
               when Small_State      =>
                  if Expected_Head /= Null_Run or else Class >= Item.Class_Count or else Span /= 0 then
                  ...
                  begin
                     ...
                     for Slot in Interfaces.Unsigned_32 range 0 .. Slots - 1 loop
                        if (Bitmap and Interfaces.Shift_Left (Interfaces.Unsigned_64 (1), Natural (Slot)))
                          /= 0
                        then
                           declare
                           ...
                           begin
                              if Generation = 0 or else Generation > Counter then
                                 raise Layout_Error with "small slab generation is corrupt";
                              end if;
                           end;
                        end if;
                     end loop;
               ...
               when Large_Head_State =>
                  if Expected_Head /= Null_Run
                    or else Class /= 0
                    or else Live /= 1
                    or else Span = 0
                    or else Span > Item.Run_Count - Run
                    or else Next /= Null_Run
                    or else Bitmap /= 0
                  then
               ...
               when Large_Tail_State =>
                  ...
                  declare
                     Head_Span : constant Interfaces.Unsigned_32 :=
                       Bytes.Read_U32 (U32_Field (Item, Expected_Head, Span_Offset));
                  begin
      ...
      for Class in Interfaces.Unsigned_32 range 0 .. Item.Class_Count - 1 loop
         declare
            Cursor : Interfaces.Unsigned_32 := Bytes.Read_U32 (Class_Head_Address (Item, Class));
            Steps  : Interfaces.Unsigned_32 := 0;
         begin
            while Cursor /= Null_Run loop
               if Cursor >= Item.Run_Count
                 or else Steps >= Item.Run_Count
                 or else Atomic.Load_Acquire_U32 (State_Address (Item, Cursor)) /= Small_State
                 or else Bytes.Read_U32 (U32_Field (Item, Cursor, Class_Offset)) /= Class
               then
               ...
               declare
                  ...
                  Live     : constant Interfaces.Unsigned_32 :=
                    Bytes.Read_U32 (U32_Field (Item, Cursor, Live_Offset));
               begin
                  if Live >= Slots then
      ...
      for Run in Interfaces.Unsigned_32 range 0 .. Item.Run_Count - 1 loop
         if Atomic.Load_Acquire_U32 (State_Address (Item, Run)) = Small_State then
            ...
            begin
               ...
               if (Live < Slots and then Occurrences /= 1) or else (Live = Slots and then Occurrences /= 0)
               then
   ...
   begin
      ...
      Set_View (Item, Core, Values, Instance_ID);
      if Bytes.Read_U32 (Layouts.Address_At (Item.Core, Run_Metadata_Offset, 4, 4)) /= Values.Run_Size
        or else Bytes.Read_U32 (Layouts.Address_At (Item.Core, Run_Metadata_Reserved_Offset, 4, 4)) /= 0
      then
   ...
   procedure Add_To_Class (Item : View; Run, Class : Interfaces.Unsigned_32) is
      Head : constant Interfaces.Unsigned_32 := Bytes.Read_U32 (Class_Head_Address (Item, Class));
   begin
   ...
   procedure Remove_Class_Head (Item : View; Run, Class : Interfaces.Unsigned_32) is
      Head : constant Interfaces.Unsigned_32 := Bytes.Read_U32 (Class_Head_Address (Item, Class));
         ...
         if Atomic.Load_Acquire_U32 (State_Address (Item, Run)) = Small_State then
            declare
               Class : constant Interfaces.Unsigned_32 :=
                 Bytes.Read_U32 (U32_Field (Item, Run, Class_Offset));
               Slots : constant Interfaces.Unsigned_32 :=
                 Interfaces.Unsigned_32 (Byte_Count (Item.Run_Value) / Class_Capacity (Item, Class));
               Live  : constant Interfaces.Unsigned_32 := Bytes.Read_U32 (U32_Field (Item, Run, Live_Offset));
            begin
               Bytes.Write_U32 (U32_Field (Item, Run, Next_Offset), Null_Run);
               if Live < Slots then
   ...
   function First_Clear_Bit (Bitmap, Mask : Interfaces.Unsigned_64) return Interfaces.Unsigned_32 is
      Free  : Interfaces.Unsigned_64 := (not Bitmap) and Mask;
      Index : Interfaces.Unsigned_32 := 0;
   begin
      if Free = 0 then
         raise Layout_Error with "slab has no free slot";
      end if;
   ...
   function Allocate_Small (Item : View; Requested : Byte_Count; Value : out Allocation_Handle) return Boolean
   is
   ...
   begin
      ...
      if Capacity >= Byte_Count (Item.Run_Value) then
         return False;
      end if;
      ...
      if Live >= Slots then
         raise Layout_Error with "indexed slab is full";
      end if;
      Units_Per_Slot := Interfaces.Unsigned_32 (Capacity / Byte_Count (Item.Minimum_Value));
      ...
      if Live + 1 = Slots then
         Remove_Class_Head (Item, Run, Class);
      end if;
      Value := (Token => Make_Token (Item.Core.Epoch_Value, Unit), Generation => Generation);
      return True;
   end Allocate_Small;

   function Find_Free_Span (Item : View; Needed : Interfaces.Unsigned_32) return Interfaces.Unsigned_32 is
      Start  : Interfaces.Unsigned_32 := 0;
      Length : Interfaces.Unsigned_32 := 0;
   begin
      if Needed = 0 or else Needed > Item.Run_Count then
         return Null_Run;
      end if;
      for Run in Interfaces.Unsigned_32 range 0 .. Item.Run_Count - 1 loop
         if Atomic.Load_Acquire_U32 (State_Address (Item, Run)) = Free_State then
   ...
   function Allocate_Large (Item : View; Requested : Byte_Count; Value : out Allocation_Handle) return Boolean
   is
      Needed     : constant Interfaces.Unsigned_32 :=
        Interfaces.Unsigned_32 ((Requested + Byte_Count (Item.Run_Value) - 1) / Byte_Count (Item.Run_Value));
      Start      : constant Interfaces.Unsigned_32 := Find_Free_Span (Item, Needed);
      Unit       : Interfaces.Unsigned_32;
      Generation : Interfaces.Unsigned_64;
   begin
      if Start = Null_Run then
         return False;
      end if;
   ...
   function Allocate_Unlocked
     (Item : View; Requested : Byte_Count; Value : out Allocation_Handle) return Boolean is
   begin
      Value := Null_Allocation;
      if Requested > Byte_Count (Item.Usable_Value) then
         return False;
      end if;
      if Requested <= Byte_Count (Item.Run_Value) / 2 and then Allocate_Small (Item, Requested, Value) then
         return True;
      elsif Requested > Byte_Count (Item.Run_Value) / 2 and then Allocate_Large (Item, Requested, Value) then
         return True;
      end if;
      declare
   ...
   procedure Try_Allocate
     (Item           : in out View;
      Requested_Size : Positive;
      Value          : out Allocation_Handle;
      Result         : out Allocation_Result) is
   begin
      Value := Null_Allocation;
      begin
         Acquire (Item);
   ...
   begin
      ...
      if State = Small_State then
         ...
         begin
            ...
            if Slot >= Slots or else not Bitmap_Bit_Is_Set (Item, Run, Slot) then
         ...
         if Bytes.Read_U32 (U32_Field (Item, Run, Class_Offset)) /= 0
           ...
           or else Bytes.Read_U32 (U32_Field (Item, Run, Next_Offset)) /= Null_Run
           or else Bytes.Read_U32 (U32_Field (Item, Run, Reserved_Offset)) /= 0
           or else Read_Bitmap (Item, Run) /= 0
         then
            raise Layout_Error with "large span head is corrupt";
         end if;
         if Span > 1 then
            for Tail in Interfaces.Unsigned_32 range Run + 1 .. Run + Span - 1 loop
               if Atomic.Load_Acquire_U32 (State_Address (Item, Tail)) /= Large_Tail_State
                 or else Bytes.Read_U32 (U32_Field (Item, Tail, Class_Offset)) /= 0
                 or else Bytes.Read_U32 (U32_Field (Item, Tail, Live_Offset)) /= 0
                 or else Bytes.Read_U32 (U32_Field (Item, Tail, Span_Offset)) /= Run
                 or else Bytes.Read_U32 (U32_Field (Item, Tail, Next_Offset)) /= Null_Run
                 or else Bytes.Read_U32 (U32_Field (Item, Tail, Reserved_Offset)) /= 0
   ...
   begin
      if Description.State = Small_State then
         declare
            Slots  : constant Interfaces.Unsigned_32 :=
              Interfaces.Unsigned_32 (Byte_Count (Item.Run_Value) / Description.Capacity);
            Live   : constant Interfaces.Unsigned_32 :=
              Bytes.Read_U32 (U32_Field (Item, Description.Run, Live_Offset));
            Bitmap : constant Interfaces.Unsigned_64 := Read_Bitmap (Item, Description.Run);
            Bit    : constant Interfaces.Unsigned_64 :=
              Interfaces.Shift_Left (Interfaces.Unsigned_64 (1), Natural (Description.Slot));
         begin
            if Live = 0 or else (Bitmap and Bit) = 0 then
               raise Handle_Error with "slab handle is reclaimed or stale";
            end if;
            Publish_Bitmap_Word (Item, Description.Run, Description.Slot, Bitmap and not Bit);
            Bytes.Write_U32 (U32_Field (Item, Description.Run, Live_Offset), Live - 1);
            if Live = Slots then
               Add_To_Class (Item, Description.Run, Description.Class);
            end if;
         end;
      else
         for Run in Interfaces.Unsigned_32 range Description.Run .. Description.Run + Description.Span - 1
         loop
            Clear_Descriptor (Item, Run);
         end loop;
      end if;
   ...
   begin
      ...
      begin
         declare
            D : constant Block_Description := Validate_Handle (Item, Value);
         begin
            Mutated := True;
            Release_Unlocked (Item, Value, D);
         end;
      exception
         when others =>
            Finish_Failure (Item, Mutated);
   ...
   begin
      ...
      Region.Base :=
        Layouts.Address_At (Item.Core, Layouts.Checked_Add (Item.Data_Offset, D.Offset), D.Capacity, 1);
      Region.Length_Value := D.Capacity;
      Region.Attached := True;
   end Attach_Allocation;

   function Payload_Address
     (Item : View; D : Block_Description; Offset, Extent : Byte_Count) return System.Address is
   begin
      ...
      return
        Layouts.Address_At
          (Item.Core,
           Layouts.Checked_Add (Item.Data_Offset, Layouts.Checked_Add (D.Offset, Offset)),
           Extent,
           1);
   end Payload_Address;
   ...
   begin
      if Source_Offset > S.Capacity
        or else Length > S.Capacity - Source_Offset
        or else Target_Offset > T.Capacity
        or else Length > T.Capacity - Target_Offset
      then
         ...
         Bytes.Copy
           (Payload_Address (Item, T, Target_Offset, Length),
            Payload_Address (Item, S, Source_Offset, Length),
            Native_Length);
      end if;
   end Copy;
