...
package body Flyology_Allocators.Allocation_Algorithms.TLSF_Kernel is
   ...
   function Count_Leading_Zeros (Value : Interfaces.C.unsigned) return Interfaces.C.int;
   pragma Import (Intrinsic, Count_Leading_Zeros, "__builtin_clz");

   function Count_Trailing_Zeros (Value : Interfaces.C.unsigned) return Interfaces.C.int;
   pragma Import (Intrinsic, Count_Trailing_Zeros, "__builtin_ctz");

   function Make_Token (Arena_Epoch : Interfaces.Unsigned_32; Block : Block_Ref) return Interfaces.Unsigned_64
   is (Interfaces.Shift_Left (Interfaces.Unsigned_64 (Arena_Epoch), 32) or Interfaces.Unsigned_64 (Block));
   pragma Inline_Always (Make_Token);

   function Token_Epoch (Value : Allocation_Handle) return Interfaces.Unsigned_32
   is (Interfaces.Unsigned_32 (Interfaces.Shift_Right (Value.Token, 32)));
   pragma Inline_Always (Token_Epoch);

   function Token_Block (Value : Allocation_Handle) return Block_Ref
   is (Block_Ref (Value.Token and Interfaces.Unsigned_64 (Interfaces.Unsigned_32'Last)));
   ...
   Heads_Offset       : constant Byte_Count := Second_Maps_Offset + First_Level_Count * 4;
   Index_Size         : constant Byte_Count :=
     16 + First_Level_Count * 4 + First_Level_Count * Second_Level_Count * 4;
   ...
   function Geometry (Usable_Capacity : Positive; Minimum_Block_Size : Positive) return Geometry_Values is
   ...
   begin
      ...
      Data_Start := Layouts.Align_Up (Layouts.Checked_Add (Index_Offset, Index_Size), Minimum);
      return
   ...
   is (Geometry (Configuration.Usable_Capacity, Configuration.Minimum_Block_Size).Extent);

   function Index_Address
     (Item : View; Relative : Byte_Count; Extent : Byte_Count; Alignment : Byte_Count) return System.Address
   is
      Local : Byte_Count;
   begin
      if not Item.Core.Attached or else Item.Index_Base = System.Null_Address then
      ...
      if Extent = 0 or else Local > Index_Size or else Extent > Index_Size - Local then
         raise Layout_Error with "arena index extent is corrupt";
      elsif Alignment = 0
        or else (Alignment and (Alignment - 1)) /= 0
        or else (Addressing.To_Integer (Item.Index_Base) + Addressing.Integer_Address (Local))
                mod Addressing.Integer_Address (Alignment)
                /= 0
      then
   ...
   function Block_Address
     (Item : View; Block : Block_Ref; Relative : Byte_Count; Extent : Byte_Count; Alignment : Byte_Count)
      return System.Address is
   begin
      if Block = Null_Block or else Block >= Item.Usable_Value / Item.Minimum_Value then
      ...
      if not Item.Core.Attached or else Item.Data_Base = System.Null_Address then
         raise Region_Error with "detached arena view";
      end if;
      declare
         Local   : constant Byte_Count :=
           Layouts.Checked_Add
             (Layouts.Checked_Multiply (Byte_Count (Block), Byte_Count (Item.Minimum_Value)), Relative);
         Address : constant System.Address := Item.Data_Base + Addressing.Storage_Offset (Local);
      begin
         if Extent = 0
         ...
         then
           ...
           or else (Alignment and (Alignment - 1)) /= 0
           or else Addressing.To_Integer (Address) mod Addressing.Integer_Address (Alignment) /= 0
         then
   ...
   function Block_Base (Item : View; Block : Block_Ref) return System.Address
   is (Block_Address (Item, Block, 0, Byte_Count (Item.Prefix_Value), Byte_Count (Item.Minimum_Value)));
   pragma Inline_Always (Block_Base);

   function Read_Field (Item : View; Block : Block_Ref; Relative : Byte_Count) return Interfaces.Unsigned_32
   is (Bytes.Read_U32 (Block_Address (Item, Block, Relative, 4, 4)));
   pragma Inline_Always (Read_Field);

   procedure Write_Field
     (Item : View; Block : Block_Ref; Relative : Byte_Count; Value : Interfaces.Unsigned_32) is
   begin
      Bytes.Write_U32 (Block_Address (Item, Block, Relative, 4, 4), Value);
   end Write_Field;
   pragma Inline_Always (Write_Field);

   function Block_Size (Item : View; Block : Block_Ref) return Interfaces.Unsigned_32
   is (Read_Field (Item, Block, Block_Size_Offset));
   function Previous_Size (Item : View; Block : Block_Ref) return Interfaces.Unsigned_32
   is (Read_Field (Item, Block, Previous_Size_Offset));
   function Block_State (Item : View; Block : Block_Ref) return Interfaces.Unsigned_32
   is (Atomic.Load_Acquire_U32 (Block_Address (Item, Block, Block_State_Offset, 4, 4)));
   function Next_Free (Item : View; Block : Block_Ref) return Block_Ref
   is (Read_Field (Item, Block, Next_Free_Offset));
   function Previous_Free (Item : View; Block : Block_Ref) return Block_Ref
   is (Read_Field (Item, Block, Previous_Free_Offset));
   pragma Inline_Always (Block_Size, Previous_Size, Block_State, Next_Free, Previous_Free);

   procedure Set_Block_Size (Item : View; Block : Block_Ref; Value : Interfaces.Unsigned_32) is
   begin
      Write_Field (Item, Block, Block_Size_Offset, Value);
   end Set_Block_Size;
   procedure Set_Previous_Size (Item : View; Block : Block_Ref; Value : Interfaces.Unsigned_32) is
   begin
      Write_Field (Item, Block, Previous_Size_Offset, Value);
   end Set_Previous_Size;
   procedure Set_State (Item : View; Block : Block_Ref; Value : Interfaces.Unsigned_32) is
   begin
      Atomic.Store_Release_U32 (Block_Address (Item, Block, Block_State_Offset, 4, 4), Value);
   end Set_State;
   procedure Set_Next_Free (Item : View; Block, Value : Block_Ref) is
   begin
      Write_Field (Item, Block, Next_Free_Offset, Value);
   end Set_Next_Free;
   procedure Set_Previous_Free (Item : View; Block, Value : Block_Ref) is
   begin
      Write_Field (Item, Block, Previous_Free_Offset, Value);
   end Set_Previous_Free;
   pragma Inline_Always (Set_Block_Size, Set_Previous_Size, Set_State, Set_Next_Free, Set_Previous_Free);

   function Live_Count (Item : View) return Interfaces.Unsigned_64
   is (Bytes.Read_U64 (Item.Live_Count_Address));
   procedure Set_Live_Count (Item : View; Value : Interfaces.Unsigned_64) is
   begin
   ...
   function Bit (Index : Natural) return Interfaces.Unsigned_32
   is (Interfaces.Shift_Left (Interfaces.Unsigned_32 (1), Index));
   pragma Inline_Always (Bit);

   function Floor_Log_2 (Value : Interfaces.Unsigned_32) return First_Level is
   begin
      if Value = 0 then
         raise Layout_Error with "TLSF cannot classify a zero size";
      end if;
      return First_Level (31 - Integer (Count_Leading_Zeros (Interfaces.C.unsigned (Value))));
   end Floor_Log_2;
   pragma Inline_Always (Floor_Log_2);

   function Class_Of (Item : View; Size : Interfaces.Unsigned_32) return Size_Class is
      Units  : Interfaces.Unsigned_32;
      First  : First_Level;
      ...
      Second :=
        Interfaces.Unsigned_32
          (Interfaces.Shift_Right
             (Interfaces.Shift_Left (Interfaces.Unsigned_64 (Units - Base), Second_Level_Log), First));
   ...
   function First_Map (Item : View) return Interfaces.Unsigned_32
   is (Bytes.Read_U32 (Item.First_Map_Address));
   procedure Set_First_Map (Item : View; Value : Interfaces.Unsigned_32) is
   begin
      Bytes.Write_U32 (Item.First_Map_Address, Value);
   end Set_First_Map;

   function Second_Map (Item : View; First : First_Level) return Interfaces.Unsigned_32
   is (Bytes.Read_U32 (Item.Second_Maps_Base + Addressing.Storage_Offset (First * 4)));
   procedure Set_Second_Map (Item : View; First : First_Level; Value : Interfaces.Unsigned_32) is
   begin
      Bytes.Write_U32 (Item.Second_Maps_Base + Addressing.Storage_Offset (First * 4), Value);
   end Set_Second_Map;

   function Head (Item : View; Class : Size_Class) return Block_Ref
   is (Bytes.Read_U32
         (Item.Heads_Base
          + Addressing.Storage_Offset ((Class.First * Second_Level_Count + Class.Second) * 4)));
   procedure Set_Head (Item : View; Class : Size_Class; Value : Block_Ref) is
   begin
      Bytes.Write_U32
        (Item.Heads_Base + Addressing.Storage_Offset ((Class.First * Second_Level_Count + Class.Second) * 4),
         Value);
   end Set_Head;
   pragma Inline_Always (First_Map, Set_First_Map, Second_Map, Set_Second_Map, Head, Set_Head);

   function Generation (Item : View; Block : Block_Ref) return Interfaces.Unsigned_64
   is (Bytes.Read_U64 (Block_Address (Item, Block, Generation_Offset, 8, 8)));
   procedure Set_Generation (Item : View; Block : Block_Ref; Value : Interfaces.Unsigned_64) is
   begin
      Bytes.Write_U64 (Block_Address (Item, Block, Generation_Offset, 8, 8), Value);
   end Set_Generation;
   ...
   begin
      ...
      Item.Index_Base := Layouts.Address_At (Core, Index_Offset, Index_Size, 8);
      Item.Data_Base :=
        Layouts.Address_At (Core, Values.Data_Start, Byte_Count (Values.Usable), Byte_Count (Values.Minimum));
      Item.First_Map_Address := Layouts.Address_At (Core, First_Map_Offset, 4, 4);
      Item.Second_Maps_Base := Layouts.Address_At (Core, Second_Maps_Offset, First_Level_Count * 4, 4);
      Item.Heads_Base :=
        Layouts.Address_At (Core, Heads_Offset, First_Level_Count * Second_Level_Count * 4, 4);
      Item.Live_Count_Address := Layouts.Address_At (Core, Live_Count_Offset, 8, 8);
   ...
   begin
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Block_Size_Offset), Size);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Previous_Size_Offset), Previous);
      Bytes.Write_U64 (Base + Addressing.Storage_Offset (Generation_Offset), Generation_ID);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Next_Free_Offset), Null_Block);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Previous_Free_Offset), Null_Block);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Block_Reserved_Offset), 0);
      Atomic.Store_Release_U32 (Base + Addressing.Storage_Offset (Block_State_Offset), State);
   ...
   procedure Initialize_Block
     (Item          : View;
      ...
      Generation_ID : Interfaces.Unsigned_64 := 0) is
   begin
      Initialize_Block_At (Block_Base (Item, Block), Size, Previous, State, Generation_ID);
   ...
   is
      Values : constant Geometry_Values :=
        Geometry (Configuration.Usable_Capacity, Configuration.Minimum_Block_Size);
      Core   : Layouts.Local_View;
   begin
   ...
   function Valid_Block_Ref (Item : View; Block : Block_Ref) return Boolean
   is (Block /= Null_Block and then Block < Item.Usable_Value / Item.Minimum_Value);
   ...
   procedure Validate_Index (Item : View) is
      ...
      Maximum_Blocks : constant Interfaces.Unsigned_32 := Item.Usable_Value / Item.Minimum_Value;
   begin
      ...
      while Covered < Interfaces.Unsigned_64 (Item.Usable_Value) loop
         ...
         declare
            Size    : constant Interfaces.Unsigned_32 := Block_Size (Item, Cursor);
            State   : constant Interfaces.Unsigned_32 := Block_State (Item, Cursor);
            Gen     : constant Interfaces.Unsigned_64 := Generation (Item, Cursor);
            Counter : constant Interfaces.Unsigned_64 := Bytes.Read_U64 (Item.Counter_Address);
         begin
            if Size < Item.Prefix_Value + Item.Minimum_Value
              ...
              or else Interfaces.Unsigned_64 (Size) > Interfaces.Unsigned_64 (Item.Usable_Value) - Covered
              or else Read_Field (Item, Cursor, Block_Reserved_Offset) /= 0
              or else Gen > Counter
            then
               raise Layout_Error with "arena physical block metadata is corrupt";
               ...
               then
                  raise Layout_Error with "arena allocated block metadata is corrupt";
               end if;
            else
      ...
      if Covered /= Interfaces.Unsigned_64 (Item.Usable_Value) or else Used_Blocks /= Live_Count (Item) then
      ...
      begin
         for First in First_Level loop
            declare
               Map                 : constant Interfaces.Unsigned_32 := Second_Map (Item, First);
               Expected_Second_Map : Interfaces.Unsigned_32 := 0;
            begin
               if (Map and 16#FFFF_0000#) /= 0 then
                  raise Layout_Error with "TLSF second-level bitmap has reserved bits";
               end if;
               for Second in Second_Level loop
                  declare
                  ...
                  begin
                     if Block /= Null_Block then
                        Expected_Second_Map := Expected_Second_Map or Bit (Second);
                     end if;
                     while Block /= Null_Block loop
                        ...
                        if Steps > Maximum_Blocks
                          ...
                          or else Class_Of (Item, Block_Size (Item, Block)) /= Class
                        then
               ...
               if Map /= Expected_Second_Map then
                  raise Layout_Error with "TLSF second-level bitmap disagrees with free lists";
               elsif Map /= 0 then
                  Expected_First_Map := Expected_First_Map or Bit (First);
               end if;
            end;
         end loop;
         if First_Map (Item) /= Expected_First_Map or else Listed_Free /= Physical_Free then
            raise Layout_Error with "TLSF first-level index or free-block count is corrupt";
         end if;
      end;
   end Validate_Index;
   ...
   procedure List_Insert
     (Item : View; Block : Block_Ref; Base : System.Address; Size : Interfaces.Unsigned_32)
   is
      Stored_Size   : constant Interfaces.Unsigned_32 :=
        Bytes.Read_U32 (Base + Addressing.Storage_Offset (Block_Size_Offset));
   ...
   begin
      ...
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Previous_Free_Offset), Null_Block);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Next_Free_Offset), Previous_Head);
      if Previous_Head /= Null_Block then
         declare
            Previous_Base : constant System.Address := Block_Base (Item, Previous_Head);
         begin
            Bytes.Write_U32 (Previous_Base + Addressing.Storage_Offset (Previous_Free_Offset), Block);
         end;
      end if;
   ...
   procedure List_Remove
     (Item : View; Block : Block_Ref; Base : System.Address; Size : Interfaces.Unsigned_32)
   is
      Previous       : constant Block_Ref :=
        Bytes.Read_U32 (Base + Addressing.Storage_Offset (Previous_Free_Offset));
      Following      : constant Block_Ref :=
        Bytes.Read_U32 (Base + Addressing.Storage_Offset (Next_Free_Offset));
      Class          : constant Size_Class := Class_Of (Item, Size);
   ...
   begin
      ...
      if (Previous = Null_Block and then Head (Item, Class) /= Block)
        or else (Previous /= Null_Block
                 and then Bytes.Read_U32 (Previous_Base + Addressing.Storage_Offset (Next_Free_Offset))
                          /= Block)
        or else (Following /= Null_Block
                 and then Bytes.Read_U32 (Following_Base + Addressing.Storage_Offset (Previous_Free_Offset))
                          /= Block)
      then
         raise Layout_Error with "TLSF free-list linkage is corrupt";
      end if;
      if Previous = Null_Block then
         ...
         Bytes.Write_U32 (Previous_Base + Addressing.Storage_Offset (Next_Free_Offset), Following);
      end if;
      if Following /= Null_Block then
         Bytes.Write_U32 (Following_Base + Addressing.Storage_Offset (Previous_Free_Offset), Previous);
      end if;
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Next_Free_Offset), Null_Block);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Previous_Free_Offset), Null_Block);
      if Previous = Null_Block and then Following = Null_Block then
         ...
         Set_Second_Map (Item, Class.First, Map);
         if Map = 0 then
            Set_First_Map (Item, First_Map (Item) and not Bit (Class.First));
         end if;
      end if;
   end List_Remove;

   function First_Set (Map : Interfaces.Unsigned_32) return Natural is
   begin
      if Map = 0 then
         raise Layout_Error with "TLSF bitmap search received zero";
      end if;
      return Natural (Count_Trailing_Zeros (Interfaces.C.unsigned (Map)));
   end First_Set;
   pragma Inline_Always (First_Set);

   function Mask_From (Index : Natural) return Interfaces.Unsigned_32
   is (if Index = 0 then Interfaces.Unsigned_32'Last else not (Bit (Index) - 1));
   ...
   function Find_TLSF
     (Item     : View;
      ...
      Size     : out Interfaces.Unsigned_32) return Block_Ref
   is
      Class           : Size_Class := Class_Of (Item, Required);
      Map             : Interfaces.Unsigned_32 := Second_Map (Item, Class.First) and Mask_From (Class.Second);
   ...
   begin
      ...
      if Map = 0 then
         ...
         First_Map_Value := First_Map (Item) and Mask_From (Class.First + 1);
      ...
      Size := Bytes.Read_U32 (Base + Addressing.Storage_Offset (Block_Size_Offset));
   ...
   function Rounded_Block_Size (Item : View; Requested : Byte_Count) return Interfaces.Unsigned_32 is
      Payload       : constant Byte_Count := Layouts.Align_Up (Requested, Byte_Count (Item.Minimum_Value));
      Total         : constant Byte_Count := Layouts.Checked_Add (Byte_Count (Item.Prefix_Value), Payload);
   ...
   begin
      if Total > Byte_Count (Interfaces.Unsigned_32'Last) then
         raise Constraint_Error with "arena allocation size exceeds stored width";
      end if;
      Units := Interfaces.Unsigned_64 (Total) / Interfaces.Unsigned_64 (Item.Minimum_Value);
      if Units <= Second_Level_Count then
         Rounded_Units := Units;
      else
         Base :=
           Interfaces.Shift_Left (Interfaces.Unsigned_64 (1), Floor_Log_2 (Interfaces.Unsigned_32 (Units)));
         Step := Base / Second_Level_Count;
         Rounded_Units := ((Units + Step - 1) / Step) * Step;
      end if;
      Rounded_Total := Rounded_Units * Interfaces.Unsigned_64 (Item.Minimum_Value);
      if Rounded_Total > Interfaces.Unsigned_64 (Interfaces.Unsigned_32'Last) then
         raise Constraint_Error with "TLSF rounded allocation exceeds stored width";
      end if;
   ...
   function Allocate_Unlocked
     (Item : View; Requested_Size : Byte_Count; Value : out Allocation_Handle) return Boolean
   is
      Required                    : Interfaces.Unsigned_32;
   ...
   begin
      ...
      if Requested_Size > Byte_Count (Item.Usable_Value - Item.Prefix_Value) then
         return False;
      end if;
      ...
      if Size - Required >= Item.Prefix_Value + Item.Minimum_Value then
         ...
         Initialize_Block_At (Remainder_Base, Remainder_Size, Required, Free_State);
         ...
         Bytes.Write_U32 (Base + Addressing.Storage_Offset (Block_Size_Offset), Required);
         List_Insert (Item, Remainder, Remainder_Base, Remainder_Size);
      end if;

      Counter := Counter + 1;
      Bytes.Write_U64 (Item.Counter_Address, Counter);
      Bytes.Write_U64 (Base + Addressing.Storage_Offset (Generation_Offset), Counter);
      Atomic.Store_Release_U32 (Base + Addressing.Storage_Offset (Block_State_Offset), Allocated_State);
      Set_Live_Count (Item, Count + 1);
      Value := (Token => Make_Token (Item.Core.Epoch_Value, Block), Generation => Counter);
      return True;
   ...
   function Validate_Handle_Metadata (Item : View; Value : Allocation_Handle) return Block_Description is
   ...
   begin
      ...
      declare
         ...
         Size : constant Interfaces.Unsigned_32 :=
           Bytes.Read_U32 (Base + Addressing.Storage_Offset (Block_Size_Offset));
      begin
         if Atomic.Load_Acquire_U32 (Base + Addressing.Storage_Offset (Block_State_Offset)) /= Allocated_State
           or else Bytes.Read_U64 (Base + Addressing.Storage_Offset (Generation_Offset)) /= Value.Generation
         then
            raise Handle_Error with "arena allocation handle is invalid or stale";
         ...
         return
           (Capacity => Byte_Count (Size - Item.Prefix_Value),
            Offset   =>
              Layouts.Checked_Add
                (Layouts.Checked_Multiply (Byte_Count (Block), Byte_Count (Item.Minimum_Value)),
   ...
   function Validate_Handle (Item : View; Value : Allocation_Handle) return Block_Description is
   begin
      Layouts.Require_Ready (Item.Core);
      return Validate_Handle_Metadata (Item, Value);
   end Validate_Handle;

   function Next_Block (Item : View; Block : Block_Ref; Size : Interfaces.Unsigned_32) return Block_Ref is
      Candidate : constant Interfaces.Unsigned_64 :=
        Interfaces.Unsigned_64 (Block) + Interfaces.Unsigned_64 (Size / Item.Minimum_Value);
   begin
      if Candidate = Interfaces.Unsigned_64 (Item.Usable_Value / Item.Minimum_Value) then
         return Null_Block;
      elsif Candidate > Interfaces.Unsigned_64 (Item.Usable_Value / Item.Minimum_Value) then
   ...
   procedure Coalesce_All_Free (Item : View) is
      ...
      Maximum_Steps     : constant Interfaces.Unsigned_64 :=
        2 * Interfaces.Unsigned_64 (Item.Usable_Value / Item.Minimum_Value);
   begin
      while Cursor /= Null_Block loop
         ...
         begin
            ...
            if Steps > Maximum_Steps then
               raise Layout_Error with "TLSF physical coalescing exceeded its block bound";
            end if;
            Base := Block_Base (Item, Cursor);
            Size := Bytes.Read_U32 (Base + Addressing.Storage_Offset (Block_Size_Offset));
            State := Atomic.Load_Acquire_U32 (Base + Addressing.Storage_Offset (Block_State_Offset));
            if Size < Item.Prefix_Value + Item.Minimum_Value
              or else Size mod Item.Minimum_Value /= 0
              or else Bytes.Read_U32 (Base + Addressing.Storage_Offset (Previous_Size_Offset)) /= Prior_Size
              or else (State /= Free_State and then State /= Allocated_State)
            then
               raise Layout_Error with "TLSF physical coalescing found corrupt block metadata";
            end if;
            Next := Next_Block (Item, Cursor, Size);
            if Next /= Null_Block then
               Next_Base := Block_Base (Item, Next);
               Next_State :=
                 Atomic.Load_Acquire_U32 (Next_Base + Addressing.Storage_Offset (Block_State_Offset));
               if Bytes.Read_U32 (Next_Base + Addressing.Storage_Offset (Previous_Size_Offset)) /= Size
                 or else (Next_State /= Free_State and then Next_State /= Allocated_State)
               then
                  raise Layout_Error with "TLSF physical coalescing found corrupt neighbors";
               elsif State = Free_State and then Next_State = Free_State then
                  Next_Size := Bytes.Read_U32 (Next_Base + Addressing.Storage_Offset (Block_Size_Offset));
                  if Next_Size < Item.Prefix_Value + Item.Minimum_Value
                    or else Next_Size mod Item.Minimum_Value /= 0
                    or else Interfaces.Unsigned_64 (Size) + Interfaces.Unsigned_64 (Next_Size)
                            > Interfaces.Unsigned_64 (Item.Usable_Value)
                  then
                     raise Layout_Error with "TLSF physical coalescing found corrupt neighbors";
                  end if;
   ...
   procedure Release_Unlocked (Item : View; Value : Allocation_Handle; Description : Block_Description) is
      Block : constant Block_Ref := Token_Block (Value);
   ...
   begin
      ...
      Atomic.Store_Release_U32 (Base + Addressing.Storage_Offset (Block_State_Offset), Free_State);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Next_Free_Offset), Null_Block);
      Bytes.Write_U32 (Base + Addressing.Storage_Offset (Previous_Free_Offset), Null_Block);
   ...
   begin
      ...
      begin
         declare
            Description : constant Block_Description := Validate_Handle_Metadata (Item, Value);
         begin
            Mutated := True;
   ...
   procedure Release (Item : in out View; Value : Allocation_Handle; Timeout : Wait_Timeout) is
      Mutated : Boolean := False;
   begin
      Acquire (Item, Timeout);
      begin
   ...
   is (Validate_Handle (Item, Value).Capacity);

   procedure Attach_Allocation (Region : in out Region_View; Item : View; Value : Allocation_Handle) is
   ...
   begin
      ...
      Region.Base :=
        Layouts.Address_At
          (Item.Core, Layouts.Checked_Add (Item.Data_Offset, Description.Offset), Description.Capacity, 1);
      Region.Length_Value := Description.Capacity;
      Region.Attached := True;
   end Attach_Allocation;

   function Payload_Address
     (Item : View; Description : Block_Description; Offset, Extent : Byte_Count; Alignment : Byte_Count := 1)
      return System.Address
   is
      Relative : Byte_Count;
   begin
      if Offset > Description.Capacity or else Extent > Description.Capacity - Offset then
      ...
      declare
         Address : constant System.Address := Item.Data_Base + Addressing.Storage_Offset (Relative);
      begin
         if Alignment = 0
           or else (Alignment and (Alignment - 1)) /= 0
           or else Addressing.To_Integer (Address) mod Addressing.Integer_Address (Alignment) /= 0
   ...
   procedure Copy
     (Item                  : View;
      ...
      Target_Offset, Length : Byte_Count)
   is
      Source_Description : constant Block_Description := Validate_Handle (Item, Source);
      Target_Description : constant Block_Description := Validate_Handle (Item, Target);
   ...
   begin
      ...
      then
         ...
         if Byte_Count (Native_Length) /= Length then
            raise Constraint_Error with "arena copy is not natively representable";
         end if;
         Bytes.Copy
           (Payload_Address (Item, Target_Description, Target_Offset, Length),
            Payload_Address (Item, Source_Description, Source_Offset, Length),
            Native_Length);
      end if;
   end Copy;
   ...
   begin
      ...
      begin
         ...
         Coalesce_All_Free (Item);
         Validate_Index (Item);
         if Head (Item, Class_Of (Item, Item.Usable_Value)) /= 0
