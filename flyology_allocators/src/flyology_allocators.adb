...
package body Flyology_Allocators is
   ...
   begin
      ...
      elsif Byte_Count (Offset) > Length or else Extent > Length - Byte_Count (Offset) then
         raise Region_Error with "backing-region slice is out of bounds";
      elsif Byte_Count (Offset) > Byte_Count (Addressing.Storage_Offset'Last) then
         raise Region_Error with "backing-region offset is not native";
      end if;

      Base_Value := Addressing.To_Integer (Base);
      if Byte_Count (Offset) > Byte_Count (Addressing.Integer_Address'Last - Base_Value) then
