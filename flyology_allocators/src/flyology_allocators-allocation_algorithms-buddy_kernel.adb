...
package body Flyology_Allocators.Allocation_Algorithms.Buddy_Kernel is
   ...
   function Make_Token
     (Arena_Epoch : Interfaces.Unsigned_32; Node : Interfaces.Unsigned_32) return Interfaces.Unsigned_64
   is (Interfaces.Shift_Left (Interfaces.Unsigned_64 (Arena_Epoch), 32) or Interfaces.Unsigned_64 (Node));
   ...
   function Token_Node (Value : Allocation_Handle) return Interfaces.Unsigned_32
   ...
   begin
      ...
      Table_End := Layouts.Checked_Add (Node_Table_Offset, Layouts.Checked_Multiply (Nodes, Node_Entry_Size));
   ...
   function Required_Storage (Configuration : Buddy_Kernel.Configuration) return Byte_Count
   ...
   is
      Offset : Byte_Count;
   begin
      if not Item.Core.Attached or else Item.Node_Table_Address = System.Null_Address then
      ...
      elsif Relative > Node_Entry_Size or else Extent > Node_Entry_Size - Relative then
         raise Layout_Error with "arena node field extent is corrupt";
      elsif Alignment = 0 or else (Alignment and (Alignment - 1)) /= 0 or else Relative mod Alignment /= 0
      then
      ...
      return Item.Node_Table_Address + Native.Storage_Offset (Offset);
   end Node_Address;
   pragma Inline_Always (Node_Address);

   function State_Address (Item : View; Node : Interfaces.Unsigned_32) return System.Address
   is (Node_Address (Item, Node, Node_State_Offset, 4, 4));
   pragma Inline_Always (State_Address);

   function Generation_Address (Item : View; Node : Interfaces.Unsigned_32) return System.Address
   is (Node_Address (Item, Node, Node_Generation_Offset, 8, 8));
   ...
   is
      Table_Size : constant Byte_Count := Byte_Count (Values.Nodes) * Node_Entry_Size;
      Table_Base : Native.Integer_Address;
   begin
      Item.Core := Core;
      Item.Node_Table_Address := Layouts.Address_At (Core, Node_Table_Offset, Table_Size, 8);
      ...
      if Table_Size - 1 > Byte_Count (Native.Integer_Address'Last - Table_Base) then
   ...
   begin
      for Node in Interfaces.Unsigned_32 range 0 .. Item.Node_Count - 1 loop
         ...
         Bytes.Write_U32 (Node_Address (Item, Node, Node_Reserved_Offset, 4, 4), 0);
   ...
   procedure Validate_Tree (Item : View) is
      ...
      procedure Validate_Node (Node : Interfaces.Unsigned_32; Active : Boolean) is
         ...
         Reserved     : constant Interfaces.Unsigned_32 :=
           Bytes.Read_U32 (Node_Address (Item, Node, Node_Reserved_Offset, 4, 4));
         Generation   : constant Interfaces.Unsigned_64 := Bytes.Read_U64 (Generation_Address (Item, Node));
         Left_64      : constant Interfaces.Unsigned_64 := Interfaces.Unsigned_64 (Node) * 2 + 1;
         Has_Children : constant Boolean := Left_64 < Interfaces.Unsigned_64 (Item.Node_Count);
      begin
         if Reserved /= 0 then
         ...
         elsif Generation > Counter or else (State = Allocated_State and then Generation = 0) then
            raise Layout_Error with "arena allocation generation is corrupt";
         end if;

         if Has_Children then
            declare
               Left            : constant Interfaces.Unsigned_32 := Interfaces.Unsigned_32 (Left_64);
               Right           : constant Interfaces.Unsigned_32 := Left + 1;
               Children_Active : constant Boolean := Active and then State = Split_State;
            begin
   ...
   function Allocate_Cached_Unlocked
     (Item : in out View; Requested_Size : Byte_Count; Value : out Allocation_Handle) return Boolean
   is
   ...
   begin
      ...
      if Node >= Item.Node_Count or else Atomic.Load_Acquire_U32 (State_Address (Item, Node)) /= Free_State
      then
         return False;
      end if;
      ...
      Atomic.Store_Release_U32 (State_Address (Item, Node), Allocated_State);
      Value := (Token => Make_Token (Item.Core.Epoch_Value, Node), Generation => Counter);
      return True;
   ...
   is
      ...
      function Visit (Node : Interfaces.Unsigned_32; Block_Size : Byte_Count) return Boolean is
         State       : constant Interfaces.Unsigned_32 :=
           Atomic.Load_Acquire_U32 (State_Address (Item, Node));
         Half        : Byte_Count;
      ...
      begin
         if Requested_Size > Block_Size then
            ...
            if Block_Size > Byte_Count (Item.Minimum_Value) and then Requested_Size <= Block_Size / 2 then
               Half := Block_Size / 2;
               ...
               Right := Left + 1;
               if Atomic.Load_Acquire_U32 (State_Address (Item, Left)) /= Inactive_State
                 or else Atomic.Load_Acquire_U32 (State_Address (Item, Right)) /= Inactive_State
               then
                  raise Layout_Error with "arena free node has an active descendant";
               end if;
               Atomic.Store_Release_U32 (State_Address (Item, Left), Free_State);
               Atomic.Store_Release_U32 (State_Address (Item, Right), Free_State);
               Atomic.Store_Release_U32 (State_Address (Item, Node), Split_State);
         ...
         case State is
            ...
            when Split_State     =>
               ...
               if Left_Free and then Right_Free then
                  Atomic.Store_Release_U32 (State_Address (Item, Left), Inactive_State);
                  Atomic.Store_Release_U32 (State_Address (Item, Right), Inactive_State);
   ...
   function Tree_Is_Free_Unlocked (Item : View) return Boolean is
      function Visit (Node : Interfaces.Unsigned_32) return Boolean is
         State   : constant Interfaces.Unsigned_32 := Atomic.Load_Acquire_U32 (State_Address (Item, Node));
         Left_64 : Interfaces.Unsigned_64;
   ...
   begin
      ...
      begin
         ...
         if Allocate_Cached_Unlocked (Item, Byte_Count (Requested_Size), Value)
           or else Allocate_Unlocked (Item, Byte_Count (Requested_Size), Value)
         then
            Result := Allocated;
   ...
   function Describe_Node (Item : View; Node : Interfaces.Unsigned_32) return Block_Description is
      Level_First : Interfaces.Unsigned_64 := 0;
      Level_Count : Interfaces.Unsigned_64 := 1;
      Index       : constant Interfaces.Unsigned_64 := Interfaces.Unsigned_64 (Node);
   ...
   begin
      ...
      return (Capacity => Capacity, Offset => Byte_Count (Index - Level_First) * Capacity);
   end Describe_Node;
   ...
   begin
      ...
      Stored_Generation := Bytes.Read_U64 (Generation_Address (Item, Node));
      if State /= Allocated_State or else Stored_Generation /= Value.Generation then
         raise Handle_Error with "arena allocation handle is reclaimed or stale";
      end if;
      return Describe_Node (Item, Node);
   end Validate_Handle;

   procedure Release_Unlocked (Item : in out View; Value : Allocation_Handle; Description : Block_Description)
   is
      Node     : constant Interfaces.Unsigned_32 := Token_Node (Value);
      Capacity : Byte_Count := Byte_Count (Item.Minimum_Value);
      Order    : Natural := 0;
   ...
   begin
      ...
      begin
         declare
            Description : constant Block_Description := Validate_Handle (Item, Value);
         begin
            Mutated := True;
   ...
   begin
      ...
      Relative := Layouts.Checked_Add (Item.Data_Offset, Layouts.Checked_Add (Description.Offset, Offset));
      return Layouts.Address_At (Item.Core, Relative, Extent, Alignment);
   end Payload_Address;
