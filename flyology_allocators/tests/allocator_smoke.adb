...
procedure Allocator_Smoke is
   ...
   package Slab_Span_Arenas is new FA.Arenas (FA.Allocation_Algorithms.Slab_Span);
   package U32_Pointers is new System.Address_To_Access_Conversions (Interfaces.Unsigned_32);
   package U8_Pointers is new System.Address_To_Access_Conversions (Interfaces.Unsigned_8);

   type Buddy_Handle_Array is array (Positive range <>) of Buddy_Arenas.Allocation_Handle;
   ...
   procedure Attach (Region : in out FA.Regions.View; Storage : aliased in out Storage_Array) is
   begin
      FA.Regions.Attach (Region, Storage (Storage'First)'Address, FA.Byte_Count (Storage'Length));
   end Attach;

   Buddy_Region, Best_Fit_Region, TLSF_Region, Slab_Span_Region     : FA.Regions.View;
   ...
   Remaining_Indices                                                : constant Index_Array :=
     [1, 2, 5, 6, 7, 8];
   ...
   type TLSF_Handle_Array is array (Natural range 0 .. 7) of TLSF_Arenas.Allocation_Handle;
   TLSF_Handles                                                     : TLSF_Handle_Array :=
     [others => TLSF_Arenas.Null_Allocation];
   Slab_Span_Handle, Slab_Span_Other_Handle, Stale_Slab_Span_Handle : Slab_Span_Arenas.Allocation_Handle;
   Result                                                           :
     FA.Allocation_Algorithms.Allocation_Result;
...
begin
   ...
   Buddy_Arenas.Initialize
     (Buddy_View, Buddy_Region, 64, (Usable_Capacity => 65_536, Minimum_Block_Size => 64), 1);
   Check
     (Buddy_Arenas.Capabilities.Search = FA.Allocation_Algorithms.Linear
   ...
   Buddy_Arenas.Attach_Allocation (Buddy_Allocation, Buddy_View, Buddy_Handle);
   for Index in FA.Byte_Count range 0 .. 15 loop
      U8_Pointers.To_Pointer (FA.Regions.Address_At (Buddy_Allocation, Index, 1)).all :=
        Interfaces.Unsigned_8 (Index + 1);
   end loop;
   Buddy_Arenas.Copy (Buddy_View, Buddy_Handle, 0, Buddy_Handle, 4, 16);
   for Index in FA.Byte_Count range 0 .. 15 loop
      Check
        (U8_Pointers.To_Pointer (FA.Regions.Address_At (Buddy_Allocation, Index + 4, 1)).all
         = Interfaces.Unsigned_8 (Index + 1),
         "backward overlapping copy failed");
   end loop;
   Buddy_Arenas.Copy (Buddy_View, Buddy_Handle, 4, Buddy_Handle, 0, 16);
   for Index in FA.Byte_Count range 0 .. 15 loop
      Check
        (U8_Pointers.To_Pointer (FA.Regions.Address_At (Buddy_Allocation, Index, 1)).all
         = Interfaces.Unsigned_8 (Index + 1),
   ...
   for Index in Buddy_Small_Handles'Range loop
      Buddy_Arenas.Try_Allocate (Buddy_View, 64, Buddy_Small_Handles (Index), Result);
      Check (Result = FA.Allocation_Algorithms.Allocated, "buddy leaf fill failed");
   end loop;
   ...
   begin
      declare
         Ignored : constant FA.Byte_Count :=
           Buddy_Arenas.Block_Capacity (Buddy_View, Buddy_Small_Handles (1));
   ...
   Buddy_Arenas.Attach
     (Buddy_Peer_View, Buddy_Region, 64, (Usable_Capacity => 65_536, Minimum_Block_Size => 64), 1);
   Buddy_Arenas.Try_Allocate (Buddy_Peer_View, 65_536, Buddy_Peer_Handle, Result);
   Check (Result = FA.Allocation_Algorithms.Allocated, "buddy peer root allocation failed");
   Buddy_Arenas.Try_Allocate (Buddy_View, 65_536, Buddy_Handle, Result);
   Check (Result = FA.Allocation_Algorithms.Exhausted, "buddy stale local hint duplicated a peer allocation");
   ...
   Check (Result = FA.Allocation_Algorithms.Allocated, "buddy live-destroy allocation failed");
   begin
   ...
   U32_Pointers.To_Pointer (Buddy_Storage (Buddy_Storage'First + 64 + 44)'Address).all := 1;
   begin
      Buddy_Arenas.Try_Allocate (Buddy_View, 64, 0.001, Buddy_Handle, Result);
   exception
      when FA.Timeout_Error =>
         Timed_Out := True;
   end;
   U32_Pointers.To_Pointer (Buddy_Storage (Buddy_Storage'First + 64 + 44)'Address).all := 0;
   ...
   Buddy_Arenas.Initialize
     (Buddy_Tiny_View, Buddy_Region, 200_000, (Usable_Capacity => 128, Minimum_Block_Size => 64), 4);
   Buddy_Arenas.Try_Allocate (Buddy_Tiny_View, 64, Buddy_Tiny_Left, Result);
   Check (Result = FA.Allocation_Algorithms.Allocated, "buddy tiny left allocation failed");
   Buddy_Arenas.Try_Allocate (Buddy_Tiny_View, 64, Buddy_Tiny_Right, Result);
   Check (Result = FA.Allocation_Algorithms.Allocated, "buddy tiny right allocation failed");
   Buddy_Arenas.Release (Buddy_Tiny_View, Buddy_Tiny_Left);
   Buddy_Arenas.Release (Buddy_Tiny_View, Buddy_Tiny_Right);
   Buddy_Arenas.Try_Allocate (Buddy_Tiny_View, 128, Buddy_Tiny_Left, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated
      and then Buddy_Arenas.Block_Capacity (Buddy_Tiny_View, Buddy_Tiny_Left) = 128,
   ...
   Best_Fit_Arenas.Initialize
     (Best_Fit_View, Best_Fit_Region, 64, (Usable_Capacity => 1_024, Minimum_Block_Size => 64), 2);
   for Index in Best_Fit_Handles'Range loop
      Best_Fit_Arenas.Try_Allocate (Best_Fit_View, 64, Best_Fit_Handles (Index), Result);
      Check (Result = FA.Allocation_Algorithms.Allocated, "best-fit fill allocation failed");
   end loop;
   ...
   Best_Fit_Arenas.Attach
     (Best_Fit_View, Best_Fit_Region, 64, (Usable_Capacity => 1_024, Minimum_Block_Size => 64), 2);
   Best_Fit_Arenas.Try_Allocate (Best_Fit_View, 160, Best_Fit_Handle, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated
      and then Best_Fit_Arenas.Block_Capacity (Best_Fit_View, Best_Fit_Handle) >= 160,
   ...
   Best_Fit_Arenas.Try_Allocate (Best_Fit_View, 160, Best_Fit_Probe, Result);
   Check
     (Result = FA.Allocation_Algorithms.Exhausted and then Best_Fit_Probe = Best_Fit_Arenas.Null_Allocation,
      "best-fit joined nonadjacent alternating gaps");
   for Index of Alternating_Indices loop
      Best_Fit_Arenas.Try_Allocate (Best_Fit_View, 64, Best_Fit_Handles (Index), Result);
      Check (Result = FA.Allocation_Algorithms.Allocated, "best-fit alternating-gap reuse failed");
   end loop;
   ...
   Check
     (TLSF_Arenas.Capabilities.Search = FA.Allocation_Algorithms.Linear
   ...
   TLSF_Arenas.Initialize
     (TLSF_View, TLSF_Region, 64, (Usable_Capacity => 65_536, Minimum_Block_Size => 64), 3);
   ...
   Slab_Span_Arenas.Initialize
     (Slab_Span_View,
      Slab_Span_Region,
      64,
      (Usable_Capacity => 65_536, Minimum_Block_Size => 64, Run_Size => 4_096),
      4);
   Check
     (Slab_Span_Arenas.Capabilities.Search = FA.Allocation_Algorithms.Linear
   ...
   Slab_Span_Arenas.Try_Allocate (Slab_Span_View, 65, Slab_Span_Handle, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated
      and then Slab_Span_Arenas.Block_Capacity (Slab_Span_View, Slab_Span_Handle) = 128,
   ...
   Slab_Span_Arenas.Attach
     (Slab_Span_Peer_View,
      ...
      (Usable_Capacity => 65_536, Minimum_Block_Size => 64, Run_Size => 4_096),
      4);
   Slab_Span_Arenas.Try_Allocate (Slab_Span_Peer_View, 4_095, Slab_Span_Handle, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated
      and then Slab_Span_Arenas.Block_Capacity (Slab_Span_Peer_View, Slab_Span_Handle) = 4_096,
   ...
   Slab_Span_Arenas.Try_Allocate (Slab_Span_Peer_View, 65_536, Slab_Span_Handle, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated
      and then Slab_Span_Arenas.Block_Capacity (Slab_Span_Peer_View, Slab_Span_Handle) = 65_536,
      "slab/span did not reclaim an empty slab for a full span");
   begin
      Slab_Span_Arenas.Release (Slab_Span_Peer_View, Stale_Slab_Span_Handle);
   ...
   Slab_Span_Arenas.Try_Allocate (Slab_Span_Peer_View, 4_095, Slab_Span_Handle, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated, "slab/span corrupt-tail setup failed for the first span");
   Slab_Span_Arenas.Try_Allocate (Slab_Span_Peer_View, 4_095, Slab_Span_Other_Handle, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated,
   ...
   declare
      ...
      First_Span_Field        : constant System.Address :=
        Slab_Span_Storage (Slab_Span_Storage'First + First_Span_Field_Offset)'Address;
   ...
   begin
      ...
      begin
         declare
            Ignored : constant FA.Byte_Count :=
              Slab_Span_Arenas.Block_Capacity (Slab_Span_Peer_View, Slab_Span_Handle);
            pragma Unreferenced (Ignored);
         begin
            null;
         end;
      ...
      Check (Capacity_Rejected, "slab/span capacity accepted a head claiming a live neighbor");
      ...
      U32_Pointers.To_Pointer (First_Span_Field).all := Saved_Span;
      Check (Release_Rejected, "slab/span release accepted a head claiming a live neighbor");
      Check
        (Slab_Span_Arenas.Block_Capacity (Slab_Span_Peer_View, Slab_Span_Other_Handle) = 4_096,
         "slab/span corrupt release damaged the neighboring allocation");
   end;
   Slab_Span_Arenas.Release (Slab_Span_Peer_View, Slab_Span_Handle);
   Slab_Span_Arenas.Release (Slab_Span_Peer_View, Slab_Span_Other_Handle);
   Slab_Span_Arenas.Destroy (Slab_Span_Peer_View);
   ...
   TLSF_Arenas.Initialize
     (TLSF_View, TLSF_Region, 64, (Usable_Capacity => 1_024, Minimum_Block_Size => 64), 3);
   for Index in TLSF_Handles'Range loop
      TLSF_Arenas.Try_Allocate (TLSF_View, 64, TLSF_Handles (Index), Result);
      Check (Result = FA.Allocation_Algorithms.Allocated, "TLSF adjacent-coalescing setup failed");
   end loop;
   TLSF_Arenas.Release (TLSF_View, TLSF_Handles (0));
   TLSF_Arenas.Release (TLSF_View, TLSF_Handles (1));
   begin
      declare
         Ignored : constant FA.Byte_Count := TLSF_Arenas.Block_Capacity (TLSF_View, TLSF_Handles (0));
      begin
   ...
   TLSF_Arenas.Attach (TLSF_View, TLSF_Region, 64, (Usable_Capacity => 1_024, Minimum_Block_Size => 64), 3);
   TLSF_Arenas.Try_Allocate (TLSF_View, 192, TLSF_Handle, Result);
   Check
     (Result = FA.Allocation_Algorithms.Allocated, "TLSF did not coalesce adjacent free blocks on demand");
   TLSF_Arenas.Release (TLSF_View, TLSF_Handle);
   ...
   for Index in TLSF_Handles'Range loop
      TLSF_Arenas.Try_Allocate (TLSF_View, 64, TLSF_Handles (Index), Result);
      Check (Result = FA.Allocation_Algorithms.Allocated, "TLSF fragmented-coalescing setup failed");
   end loop;
   for Index in TLSF_Handles'Range loop
   ...
   TLSF_Arenas.Try_Allocate (TLSF_View, 192, TLSF_Handle, Result);
   Check (Result = FA.Allocation_Algorithms.Exhausted, "TLSF coalesced across an allocated gap");
   TLSF_Arenas.Release (TLSF_View, TLSF_Handles (1));
   TLSF_Arenas.Try_Allocate (TLSF_View, 320, TLSF_Handle, Result);
   Check (Result = FA.Allocation_Algorithms.Allocated, "TLSF did not merge a mixed-size free run");
   TLSF_Arenas.Release (TLSF_View, TLSF_Handle);
   for Index in TLSF_Handles'Range loop
