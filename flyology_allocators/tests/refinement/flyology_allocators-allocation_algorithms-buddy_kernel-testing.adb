...
package body Flyology_Allocators.Allocation_Algorithms.Buddy_Kernel.Testing is
   ...
   function Node_Address
     (Item : View; Node : Interfaces.Unsigned_32; Relative : Byte_Count)
      return System.Address is
     (Item.Node_Table_Address
      + Native.Storage_Offset
          (Byte_Count (Node) * Node_Entry_Size + Relative));

   function Node_Start
     (Item : View; Node : Interfaces.Unsigned_32) return Natural
   is
      First_At_Level : Interfaces.Unsigned_32 := 0;
      Nodes_At_Level : Interfaces.Unsigned_32 := 1;
      Size           : Natural := Natural (Item.Usable_Value /
                                           Item.Minimum_Value);
   begin
   ...
   is
      ...
      procedure Visit
        (Node : Interfaces.Unsigned_32; Start : Natural; Size : Positive)
      is
      ...
      begin
         if Node >= Item.Node_Count then
            raise Program_Error with
              "buddy refinement tree is out of range";
         end if;
         State := Bytes.Read_U32
           (Node_Address (Item, Node, Node_State_Offset));
         Stored_Generation := Bytes.Read_U64
           (Node_Address (Item, Node, Node_Generation_Offset));
         if Stored_Generation > Value.Generation
           or else
             (State = Allocated_State and then Stored_Generation = 0)
         then
            raise Program_Error with
              "buddy refinement generation is invalid";
         end if;
         case State is
            when Free_State =>
            ...
            when Allocated_State =>
               Append_Block
                 (Node, Start, Size, Support.Allocated_Block,
                  Stored_Generation);
            when Split_State =>
               if Size = 1 then
                  raise Program_Error with
                    "buddy refinement leaf is split";
               end if;
               Left := Node * 2 + 1;
            ...
            when Inactive_State =>
               raise Program_Error with
                 "active buddy refinement node is inactive";
            when others =>
               raise Program_Error with
                 "buddy refinement node state is invalid";
         end case;
      end Visit;
   begin
      Value := (others => <>);
      ...
      Visit
        (0, 0,
         Positive (Item.Usable_Value / Item.Minimum_Value));
   end Capture;

   procedure Capture_Hints
     (Item  : View;
      Value : out Support.Hint_Array)
   is
      Size  : Positive := 1;
      Order : Natural := 0;
      Node  : Interfaces.Unsigned_32;
   ...
   function Handle_Start
     (Item  : View;
      Value : Allocation_Handle) return Natural is
     (Node_Start
        (Item,
