...
package body Flyology_Allocators.Allocation_Algorithms.Best_Fit_Kernel.Testing is
   ...
   is
      ...
      begin
         ...
         Value.Index (Value.Index_Count) :=
           (Start        => Natural (Block),
            Size         => Positive
              (Read_U32 (Item, Block, Block_Size_Offset) /
               Item.Minimum_Value),
            Class_First  => -1,
            Class_Second => -1);
      end Append_Index;
      ...
      begin
         if Block = Null_Block then
            return;
         elsif Block >= Item.Usable_Value / Item.Minimum_Value
           or else Index_Steps = Support.Max_Blocks
         then
            raise Program_Error with
              "best-fit refinement index is cyclic or out of range";
         end if;
   ...
   begin
      ...
      while Block < Item.Usable_Value / Item.Minimum_Value loop
         ...
         then
            raise Program_Error with
              "best-fit refinement physical chain is invalid";
         end if;
         ...
         then
            raise Program_Error with
              "best-fit refinement generation is invalid";
         end if;
           ...
           (Token      => Block,
            ...
            State      =>
              (if State = Free_State then Support.Free_Block
                 ...
                 "best-fit refinement block state is invalid"),
            Generation =>
