...
package body Flyology_Allocators.Allocation_Algorithms.TLSF_Kernel.Testing is
   ...
   begin
      ...
      for First in Value.Second_Maps'Range loop
         Value.Second_Maps (First) :=
           Bytes.Read_U32 (Item.Second_Maps_Base + Native.Storage_Offset (First * 4));
      end loop;
      while Block < Item.Usable_Value / Item.Minimum_Value loop
         ...
         then
            raise Program_Error with "TLSF refinement physical chain is invalid";
         end if;
         ...
         then
            raise Program_Error with "TLSF refinement generation is invalid";
         end if;
           ...
           (Token      => Block,
            ...
            State      =>
              (if State = Free_State
               ...
               else raise Program_Error with "TLSF refinement block state is invalid"),
      ...
      for First in 0 .. First_Level_Count - 1 loop
         for Second in 0 .. Second_Level_Count - 1 loop
            Cursor :=
              Bytes.Read_U32
                (Item.Heads_Base + Native.Storage_Offset ((First * Second_Level_Count + Second) * 4));
            while Cursor /= Null_Block loop
               if Cursor >= Item.Usable_Value / Item.Minimum_Value then
                  raise Program_Error with "TLSF refinement index is out of range";
               end if;
               ...
               Value.Index (Value.Index_Count) :=
                 (Start        => Natural (Cursor),
                  Size         => Positive (Read_U32 (Item, Cursor, Block_Size_Offset) / Item.Minimum_Value),
                  Class_First  => First,
                  Class_Second => Second);
