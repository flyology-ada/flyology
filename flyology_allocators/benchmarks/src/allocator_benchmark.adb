...
procedure Allocator_Benchmark is
   ...
   type Allocator_Case is (Native_Malloc, Buddy, Best_Fit, TLSF, Slab_Span);
   ...
   exception
      when Constraint_Error =>
         raise Program_Error with "numeric benchmark arguments must be positive integers";
   end Positive_Argument;

   function Requested_Mode return Run_Mode is
      Value : constant String :=
        (if CLI.Argument_Count < 4 then "all" else Ada.Characters.Handling.To_Lower (CLI.Argument (4)));
   begin
      if Value = "all" then
   ...
   Maximum_Iterations_Argument : constant Positive := Positive_Argument (1, 250_000);
   ...
   Churn_Sizes : constant Size_Array := [8, 24, 64, 96, 256, 1_024, 4_096];

   Buddy_Extent     : constant FA.Byte_Count :=
     Buddy_Arenas.Required_Storage ((Usable_Capacity => Arena_Capacity, Minimum_Block_Size => 64));
   Best_Fit_Extent  : constant FA.Byte_Count :=
     Best_Fit_Arenas.Required_Storage ((Usable_Capacity => Arena_Capacity, Minimum_Block_Size => 64));
   TLSF_Extent      : constant FA.Byte_Count :=
     TLSF_Arenas.Required_Storage ((Usable_Capacity => Arena_Capacity, Minimum_Block_Size => 64));
   Slab_Span_Extent : constant FA.Byte_Count :=
     Slab_Span_Arenas.Required_Storage
       ((Usable_Capacity => Arena_Capacity, Minimum_Block_Size => 64, Run_Size => 4_096));

   Buddy_Base, Best_Fit_Base, TLSF_Base, Slab_Span_Base                             : System.Address :=
     System.Null_Address;
   Buddy_Region, Best_Fit_Region, TLSF_Region, Slab_Span_Region                     : Regions.View;
   ...
   Buddy_Initialized, Best_Fit_Initialized, TLSF_Initialized, Slab_Span_Initialized : Boolean := False;

   subtype Slot_Index is Positive range 1 .. Churn_Window;
   type Native_Slot_Array is array (Slot_Index) of System.Address;
   type Handle_Array is array (Slot_Index) of FA.Allocation_Algorithms.Allocation_Handle;
   type Cursor_Array is array (Allocator_Case) of Natural;

   Native_Slots    : Native_Slot_Array := [others => System.Null_Address];
   Buddy_Slots     : Handle_Array := [others => FA.Allocation_Algorithms.Null_Allocation];
   Best_Fit_Slots  : Handle_Array := [others => FA.Allocation_Algorithms.Null_Allocation];
   TLSF_Slots      : Handle_Array := [others => FA.Allocation_Algorithms.Null_Allocation];
   Slab_Span_Slots : Handle_Array := [others => FA.Allocation_Algorithms.Null_Allocation];
   Churn_Cursors   : Cursor_Array := [others => 0];

   Active_Workload      : Workload_Kind := Fixed_Cycle;
   Active_Size          : Positive := 64;
   Completed_Operations : Bench.Iteration_Count := 0
   with Volatile;
   ...
   begin
      if FA.Byte_Count (Result) /= Value then
         raise Constraint_Error with "allocation size is not C-size representable";
      end if;
      return Result;
   end Native_Size;

   function Allocate_Native (Size : Positive) return System.Address is
      Result : constant System.Address := C_Malloc (Native_Size (FA.Byte_Count (Size)));
   begin
   ...
   procedure Prepare_Region
     (Base : in out System.Address; Region : in out Regions.View; Extent : FA.Byte_Count)
   is
      Length : constant FA.Byte_Count := Extent + FA.Byte_Count (Arena_Location);
   begin
   ...
   begin
      ...
      Prepare_Region (Slab_Span_Base, Slab_Span_Region, Slab_Span_Extent);
   ...
   begin
      case Which is
         ...
         when Best_Fit      =>
            Best_Fit_Arenas.Try_Allocate (Best_Fit_View, Size, Handle, Result);
         ...
         when Slab_Span     =>
            Slab_Span_Arenas.Try_Allocate (Slab_Span_View, Size, Handle, Result);
   ...
   function Churn_Size (Sequence : Natural) return Positive
   is (Churn_Sizes (Churn_Sizes'First + Sequence mod Churn_Sizes'Length));

   procedure Allocate_Slot (Which : Allocator_Case; Slot : Slot_Index; Size : Positive) is
      Handle : FA.Allocation_Algorithms.Allocation_Handle;
      Result : FA.Allocation_Algorithms.Allocation_Result;
   begin
      case Which is
      ...
      case Which is
         ...
         when Best_Fit      =>
            if Best_Fit_Slots (Slot) /= Best_Fit_Arenas.Null_Allocation then
               Best_Fit_Arenas.Release (Best_Fit_View, Best_Fit_Slots (Slot));
         ...
         when Slab_Span     =>
            if Slab_Span_Slots (Slot) /= Slab_Span_Arenas.Null_Allocation then
               Slab_Span_Arenas.Release (Slab_Span_View, Slab_Span_Slots (Slot));
               Slab_Span_Slots (Slot) := Slab_Span_Arenas.Null_Allocation;
            end if;
      end case;
   end Release_Slot;
   ...
   procedure Churn_Operation (Which : Allocator_Case) is
      ...
      Slot     : constant Slot_Index := Slot_Index (Sequence mod Churn_Window + 1);
   begin
   ...
   begin
      while Remaining > 0 loop
         case Active_Workload is
            when Fixed_Cycle      =>
               Fixed_Operation (Which, Active_Size);

            when Fragmented_Churn =>
               Churn_Operation (Which);
         end case;
   ...
   begin
      ...
      Active_Size := Size;
      TIO.New_Line;
      TIO.Put_Line ("Fixed-size allocation/release: requested bytes =" & Size'Image);
   ...
   begin
      ...
      TIO.New_Line;
      TIO.Put_Line
        ("Fragmented churn: replace one of" & Churn_Window'Image & " live allocations per operation");
...
begin
   ...
   TIO.Put_Line ("  shared iterations, balanced rounds, malloc reference; lower ns/op is faster");
   TIO.Put_Line
   ...
   TIO.Put_Line ("completed logical operations:" & Completed_Operations'Image);
   Finalize;
exception
   when others =>
      Finalize;
