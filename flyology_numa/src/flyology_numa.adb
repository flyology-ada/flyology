...
package body Flyology_NUMA is
   ...
   is (if Result.Available then Result.Bytes else Fallback);
   ...
   is (Bits.Has_Element (Set, Position));
   ...
   is (Facts.Nodes (Node).Processors);
   ...
   is (Facts.Nodes (From).Distances (To));
   ...
   is (Bits.Contains (Facts.Online, Node));
   ...
   is (Bits.Contains (Facts.Allowed, Node));
   ...
   is (Bits.Contains (Facts.With_Memory, Node));
   ...
   is (not Bits.Is_Empty (Facts.Nodes (Node).Processors));

   ------------
   -- To_CPU --
   ------------

   function To_CPU (Processor : Processor_Id) return System.Multiprocessors.CPU_Range is
   begin
      ...
      if not Facts.Consecutive_Processors or else not Facts.Owner (Processor).Available then
