...
package body Flyology_NUMA.Platform is
   ...
   function Query (Name : String) return Byte_Query is
      ...
      Length : aliased Interfaces.C.size_t := Interfaces.C.size_t (Value'Size / System.Storage_Unit);
      Result : constant Interfaces.C.int :=
        Sysctl_By_Name
          (Name      => C_Name'Address,
           ...
           New_Len   => 0);
   begin
      if Result /= 0 or else Value = 0 or else Value > Interfaces.C.size_t (Byte_Count'Last) then
         return No_Bytes;
      end if;
   ...
   is (Uniform.Single_Domain_Facts (Memory => Detected_Memory));
