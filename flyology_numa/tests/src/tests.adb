...
begin
   ...
   declare
      ...
      Arena : Node_Pools.Node_Pool
        (Policy => NUMA.Placement.Bound, Extent => 4096);
      ...
      Holder : constant Node_Pools.Subpool_Handle :=
        Node_Pools.On_Node (Arena, Near);

      Items : array (1 .. Count) of Sample_Access;
   begin
      Check (Node_Pools.Reserved_Bytes (Arena, Near) = 0,
             "a pool reserved memory before it was asked for any");

      for Index in Items'Range loop
         Items (Index) :=
           new (Holder) Sample'(Value => Interfaces.Unsigned_64 (Index),
                                Rest  => 0);
      end loop;

      --  The memory is real, distinct, and holds what was written to it.
      for Index in Items'Range loop
         Check (Items (Index).Value = Interfaces.Unsigned_64 (Index),
                "an allocated object did not keep its value");
         Check (System.Storage_Elements.To_Integer (Items (Index).all'Address)
                mod Sample'Alignment = 0,
                "an allocated object is not aligned for its type");

         if Index > 1 then
            Check (Items (Index).all'Address /= Items (Index - 1).all'Address,
                   "two allocations shared one address");
         end if;
      end loop;

      Check (Node_Pools.Reserved_Bytes (Arena, Near)
             >= NUMA.Byte_Count (Count) * 16,
             "a pool reserved less memory than it handed out");

      --  Asking for a node that was never used costs nothing.
      for Node of Online loop
         if Node /= Near then
            Check (Node_Pools.Reserved_Bytes (Arena, Node) = 0,
                   "an unused node reserved memory");
            Check (not Node_Pools.Placement_Reached (Arena, Node),
                   "an unused node reported its memory placed");
         end if;
      end loop;
      ...
      if NUMA.Placement.Support = NUMA.Placement.Supported then
         Check (Node_Pools.Placement_Reached (Arena, Near),
                "a placing host did not place a node pool's memory");

         for Index in 1 .. 8 loop
            declare
               Where : constant NUMA.Node_Query :=
                 NUMA.Placement.Node_Of_Address (Items (Index).all'Address);
            begin
               Check (Where.Available,
                      "an allocated object was not reported on any node");
               Check (Where.Node = Near,
                      "an object allocated for one node is held by another");
            end;
         end loop;
      else
         Check (not Node_Pools.Placement_Reached (Arena, Near),
                "a host without placement reported memory placed");
      end if;

      Ada.Text_IO.Put_Line
        ("pool node:  " & NUMA.Node_Id'Image (Near));
      Ada.Text_IO.Put_Line
        ("pool bytes: "
         & NUMA.Byte_Count'Image (Node_Pools.Reserved_Bytes (Arena, Near)));
      Ada.Text_IO.Put_Line
        ("pool placed:"
         & Boolean'Image (Node_Pools.Placement_Reached (Arena, Near)));
   end;
   ...
   declare
      ...
      overriding procedure Finalize (Item : in out Marker);

      overriding procedure Finalize (Item : in out Marker) is
   ...
   begin
      declare
         ...
         Holder : constant Node_Pools.Subpool_Handle :=
           Node_Pools.On_Node (Arena, Target);
      ...
      Check (Finalized = 5,
             "a pool going out of scope did not finalize what it held:"
             & Natural'Image (Finalized));
   end;
   ...
   declare
      ...
      type Count_Access is access Interfaces.Unsigned_64
        with Storage_Pool => Arena;

      Holder : Node_Pools.Subpool_Handle :=
        Node_Pools.On_Node (Arena, Target);
      First  : constant Count_Access :=
        new (Holder) Interfaces.Unsigned_64'(7);
   begin
      Check (First.all = 7, "an allocation did not keep its value");
      Check (Node_Pools.Reserved_Bytes (Arena, Target) > 0,
             "a used node reserved no memory");
      ...
      declare
         Again : constant Node_Pools.Subpool_Handle :=
           Node_Pools.On_Node (Arena, Target);
         Later : constant Count_Access :=
           new (Again) Interfaces.Unsigned_64'(9);
      begin
         Check (Later.all = 9,
                "a node could not be used again after being given back");
         Check (Node_Pools.Reserved_Bytes (Arena, Target) > 0,
   ...
   declare
      ...
      type Byte_Access is access Interfaces.Unsigned_8
        with Storage_Pool => Arena;
   ...
   begin
      for Index in 1 .. 8 loop
         declare
            Odd  : constant Byte_Access :=
              new (Holder) Interfaces.Unsigned_8'(1);
            Item : constant Wide_Access :=
              new (Holder) Wide'(Value => Interfaces.Unsigned_64 (Index));
         begin
            Check (Odd.all = 1, "a byte allocation did not keep its value");
            Check (Item.Value = Interfaces.Unsigned_64 (Index),
                   "a padded allocation did not keep its value");
            Check (System.Storage_Elements.To_Integer (Item.all'Address)
                   mod 16 = 0,
   ...
   if NUMA.Placement.Support = NUMA.Placement.Supported
     and then NUMA.Count (Allowed) >= 2
   then
      ...
      begin
         for Node of Allowed loop
            declare
               ...
               type Value_Access is access Interfaces.Unsigned_64
                 with Storage_Pool => Arena;

               Holder : constant Node_Pools.Subpool_Handle :=
                 Node_Pools.On_Node (Arena, Node);
               Item   : constant Value_Access :=
                 new (Holder) Interfaces.Unsigned_64'(1);
               Where  : constant NUMA.Node_Query :=
                 NUMA.Placement.Node_Of_Address (Item.all'Address);
            begin
               Check (Node_Pools.Placement_Reached (Arena, Node),
                      "a bound pool did not reach node"
                      & NUMA.Node_Id'Image (Node));
               Check (Where.Available,
                      "an allocated object was not reported on any node");
               Check (Where.Node = Node,
                      "memory bound to node" & NUMA.Node_Id'Image (Node)
                      & " is held by node" & NUMA.Node_Id'Image (Where.Node));
            end;
         end loop;
      end;
      ...
      begin
         for Node of Allowed loop
            for Other of Allowed loop
               declare
                  Near : constant NUMA.Distance_Query :=
                    NUMA.Node_Distance (Node, Node);
                  Away : constant NUMA.Distance_Query :=
                    NUMA.Node_Distance (Node, Other);
               begin
         ...
         Check (Apart,
                "no node is further from another than from itself");
      end;
      ...
      declare
         ...
         type Span is array (NUMA.Byte_Count range <>) of aliased
           Interfaces.Unsigned_8;
         type Span_Access is access Span;

         Room    : constant Span_Access :=
           new Span (1 .. (Span_Pages + 1) * Page);
         Start   : constant System.Address := Room (1)'Address;
         Skew    : constant System.Storage_Elements.Integer_Address :=
           System.Storage_Elements.To_Integer (Start)
           mod System.Storage_Elements.Integer_Address (Page);
      ...
      begin
         ...
         Check (Distinct >= 2,
                "interleaving over" & Natural'Image (NUMA.Count (Allowed))
                & " nodes put every page on" & Natural'Image (Distinct));

         Ada.Text_IO.Put_Line
           ("interleave: " & Natural'Image (Distinct) & " nodes of"
