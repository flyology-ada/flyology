...
package body Flyology_Cachelines.Linux_Testing is
   ...
   procedure Check_Core_Classes
     (Sysfs : Flyology_Cachelines.Linux.Cache_Parameters)
   is
   ...
   begin
      ...
      for Index in 1 .. Classes.Count loop
         ...
         begin
            ...
            Check
              (Class.Total_Size mod Class.Line_Size = 0,
               "a core class size is not a whole number of lines");
            Check
              (Class.Cores <= Class.CPUs,
            ...
            if Class.Total_Size = Sysfs.Total_Size
              and then Class.Line_Size = Sysfs.Line_Size
            then
               Matched := True;
            end if;
         end;
   ...
   procedure Check_Fixtures (Fixture_Root : String) is
      ...
      Hybrid   : constant Flyology_Cachelines.Linux.Core_Classes :=
        Classes ("hybrid");
      Capacity : constant Flyology_Cachelines.Linux.Core_Classes :=
        Classes ("capacity");
      SMT      : constant Flyology_Cachelines.Linux.Core_Classes :=
        Classes ("smt");
      Sparse   : constant Flyology_Cachelines.Linux.Core_Classes :=
        Classes ("sparse");
   begin
      ...
      Check
        (Hybrid.Classes (1).Total_Size = 64 * 1_024
           and then Hybrid.Classes (1).Cores = 2,
         "hybrid fixture did not order the larger class first");
      Check
        (Hybrid.Classes (2).Total_Size = 32 * 1_024,
         "hybrid fixture did not group the smaller class");
      ...
      Check
        (Sparse.Classes (1).Cores = 4,
         "sparse fixture did not enumerate across a gap in the CPU list");
      Check
        (Sparse.Classes (1).Total_Size = 48 * 1_024,
         "sparse fixture reported the wrong capacity");
   end Check_Fixtures;
