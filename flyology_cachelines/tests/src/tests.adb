...
procedure Tests is
   ...
   package Four_Integer_Groups is new
     Flyology_Cachelines.Padded_Groups (Element_Type => Integer, Group_Length => 4);
   package Fitted_Bytes is new Flyology_Cachelines.Fitted_Groups (Interfaces.Unsigned_8);
   ...
   type Padded_Array is array (Positive range <>) of aliased Padded_Integers.Padded;

   Items : Padded_Array (1 .. 3) :=
     (Padded_Integers.Create (11), Padded_Integers.Create (22), Padded_Integers.Create (33));

   type Big_Padded_Array is array (Positive range <>) of aliased Padded_Byte_Arrays.Padded;

   Big_Items : Big_Padded_Array (1 .. 2) := (others => Padded_Byte_Arrays.Create ((others => 0)));

   type Integer_Group_Array is array (Positive range <>) of aliased Four_Integer_Groups.Group;
   Integer_Groups : Integer_Group_Array (1 .. 2) := (others => Four_Integer_Groups.Create ((others => 0)));

   type Fitted_Byte_Group_Array is array (Positive range <>) of aliased Fitted_Bytes.Group;
   Fitted_Byte_Groups : Fitted_Byte_Group_Array (1 .. 2) := (others => Fitted_Bytes.Create ((others => 0)));

   Integer_Sequence          : Four_Integer_Groups.Grouped_Array (2) :=
     Four_Integer_Groups.Create (Element_Count => 8, Initial_Value => 0);
   Partial_Integer_Sequence  : aliased Four_Integer_Groups.Grouped_Array :=
     Four_Integer_Groups.Create (Element_Count => 6, Initial_Value => 0);
   Constant_Integer_Sequence : constant Four_Integer_Groups.Grouped_Array :=
     Four_Integer_Groups.Create (Element_Count => 5, Initial_Value => 7);
   Fitted_Byte_Sequence      : aliased Fitted_Bytes.Grouped_Array (2) :=
     Fitted_Bytes.Create (Element_Count => 2 * Fitted_Bytes.Elements_Per_Group, Initial_Value => 0);

   function Address_Of (Item : aliased Padded_Integers.Padded) return System.Storage_Elements.Integer_Address
   is (System.Storage_Elements.To_Integer (Item'Address));
   ...
   function Equal (Left, Right : Natural) return Boolean
   is (Left = Right);
   pragma No_Inline (Equal);

   function Less_Or_Equal (Left, Right : Natural) return Boolean
   is (Left <= Right);
   pragma No_Inline (Less_Or_Equal);

   function Greater_Than (Left, Right : Natural) return Boolean
   is (Left > Right);
   ...
   begin
      if Arch = "x86_64" or else Arch = "aarch64" or else Arch = "arm64ec" or else Arch = "powerpc64" then
         return 128;
      elsif Arch = "arm"
        or else Arch = "mips"
   ...
   function Is_Supported_OS (OS : String) return Boolean
   is (OS = "macos" or else OS = "linux");

   function Image_Or_Unavailable (Result : Flyology_Cachelines.Cache_Query_Result) return String
   is (if Result.Available then Natural'Image (Result.Value) else " unavailable");

   Alignment : constant System.Storage_Elements.Integer_Address :=
     System.Storage_Elements.Integer_Address (Flyology_Cachelines.Destructive_Interference_Size);

   Hardware_Query : constant Flyology_Cachelines.Cache_Query_Result :=
     Flyology_Cachelines.Hardware_Cache_Line_Size;
   L1_Query       : constant Flyology_Cachelines.Cache_Query_Result := Flyology_Cachelines.L1_Data_Cache_Size;
   Slots_Query    : constant Flyology_Cachelines.Cache_Query_Result :=
     Flyology_Cachelines.L1_Data_Cache_Slots;
   Class_Count    : constant Flyology_Cachelines.Cache_Query_Result := Flyology_Cachelines.Core_Class_Count;
begin
   Check
     (Padded_Integers.Padded'Alignment = Flyology_Cachelines.Destructive_Interference_Size,
      "Padded alignment does not match the compiled interference size");
   Check
     (Padded_Integers.Padded_Size_In_Storage_Elements mod Flyology_Cachelines.Destructive_Interference_Size
      = 0,
      "Padded object size is not a whole interference boundary");

   Check (Address_Of (Items (1)) mod Alignment = 0, "first padded object is misaligned");
   Check (Address_Of (Items (2)) mod Alignment = 0, "second padded object is misaligned");
   Check (Address_Of (Items (3)) mod Alignment = 0, "third padded object is misaligned");
   ...
   Check
     (Padded_Byte_Arrays.Padded_Size_In_Storage_Elements mod Flyology_Cachelines.Destructive_Interference_Size
      = 0,
   ...
   Check
     (System.Storage_Elements.To_Integer (Big_Items (2)'Address)
      ...
      = System.Storage_Elements.Integer_Address (Padded_Byte_Arrays.Padded_Size_In_Storage_Elements),
      "large padded-array stride does not match its object size");

   Items (1).Value := 42;
   Check (Padded_Integers.Unwrap (Items (1)) = 42, "wrapped value did not round-trip");
   ...
   Check
     (Four_Integer_Groups.Group_Size_In_Storage_Elements = Flyology_Cachelines.Destructive_Interference_Size,
   ...
   Check (Integer_Sequence (5) = 15, "flat explicit-group indexing did not cross a group boundary");
   Check
     (Integer_Sequence.Groups (2) (1) = 15, "flat indexing does not map to the underlying physical group");
   Check
   ...
   begin
      ...
      Check (Count = 6, "flat iteration exposed the unused tail of a partial final group");
   end;
   declare
      View  : Four_Integer_Groups.Fast_View (Partial_Integer_Sequence'Access);
      Count : Natural := 0;
   begin
      ...
      Check (Count = 6, "Fast_View exposed the unused tail of a partial final group");
      Check
        (Partial_Integer_Sequence.Groups (1) (4) = 2 and then Partial_Integer_Sequence.Groups (2) (1) = 2,
         "Fast_View did not cross padded group storage correctly");
      Check (Partial_Integer_Sequence.Groups (2) (3) = 0, "Fast_View modified an unused final-group element");
   end;
   declare
      Sum : Integer := 0;
   begin
   ...
   Check
     (System.Storage_Elements.To_Integer (Integer_Sequence.Groups (1)'Address) mod Alignment = 0,
      "first group in a flat explicit sequence is misaligned");
   Check
     (System.Storage_Elements.To_Integer (Integer_Sequence.Groups (2)'Address)
      - System.Storage_Elements.To_Integer (Integer_Sequence.Groups (1)'Address)
      = Alignment,
      "flat explicit sequence does not preserve physical group spacing");

   begin
      Integer_Sequence (Four_Integer_Groups.Length (Integer_Sequence) + 1) := 0;
   ...
   Check
     (Equal (Fitted_Bytes.Elements_Per_Group, Flyology_Cachelines.Destructive_Interference_Size),
      "automatic byte group did not fill its interference region");
   Check
     (Fitted_Bytes.Payload_Size_In_Storage_Elements = Flyology_Cachelines.Destructive_Interference_Size,
      "automatic group payload does not exactly fill its region");
   Check
     (Fitted_Bytes.Group_Size_In_Storage_Elements = Flyology_Cachelines.Destructive_Interference_Size,
   ...
   for Position in 1 .. Fitted_Bytes.Length (Fitted_Byte_Sequence) loop
      Fitted_Byte_Sequence (Position) := Interfaces.Unsigned_8 (Position mod 251);
   end loop;
   declare
      Count : Natural := 0;
   begin
   ...
   Check
     (Fitted_Byte_Sequence (Fitted_Bytes.Elements_Per_Group + 1)
      = Interfaces.Unsigned_8 ((Fitted_Bytes.Elements_Per_Group + 1) mod 251) + 1,
   ...
   Check
     (Fitted_Byte_Sequence (Fitted_Bytes.Elements_Per_Group + 1)
      = Interfaces.Unsigned_8 ((Fitted_Bytes.Elements_Per_Group + 1) mod 251) + 2,
      "fitted Fast_View did not cross a group boundary");
   Check
     (Less_Or_Equal
        (Fitted_Integers.Elements_Per_Group * Fitted_Integers.Element_Size_In_Storage_Elements,
         Flyology_Cachelines.Destructive_Interference_Size),
      "automatic integer group exceeds its interference region");
   Check
     (Greater_Than
        ((Fitted_Integers.Elements_Per_Group + 1) * Fitted_Integers.Element_Size_In_Storage_Elements,
         Flyology_Cachelines.Destructive_Interference_Size),
   ...
   if Is_Supported_OS (Tests_Config.Alire_Host_OS) then
      Check (Hardware_Query.Available, Tests_Config.Alire_Host_OS & " cache-line query failed");
      Check (L1_Query.Available, Tests_Config.Alire_Host_OS & " L1 data-cache query failed");
   end if;
   ...
   if Hardware_Query.Available then
      Check (Hardware_Query.Value > 0, "available hardware cache-line query returned zero");
      Check
   ...
   Check
     (Flyology_Cachelines.L1_Data_Cache_Size (Flyology_Cachelines.Fastest_Core_Class) = L1_Query,
   ...
   if Class_Count.Available then
      Check
        (Class_Count.Value >= 1 and then Class_Count.Value <= Flyology_Cachelines.Max_Core_Classes,
      ...
      Check
        ((Class_Count.Value = 1) = (Flyology_Cachelines.Core_Class_Ordering = Flyology_Cachelines.Unordered),
         "core-class ordering does not agree with the class count");

      for Position in 1 .. Class_Count.Value loop
         declare
            Class : constant Flyology_Cachelines.Core_Class := Flyology_Cachelines.Core_Class (Position);
         ...
         begin
            Check (Size.Available, "a counted core class reports no L1 data-cache size");
            Check
              (Size.Value >= 8 * 1_024 and then Size.Value <= 512 * 1_024,
               "a core class L1 capacity is outside the expected range");
            Check
              (not Cores.Available or else not CPUs.Available or else Cores.Value <= CPUs.Value,
            ...
            begin
               if L2.Available then
                  Check (L2.Value >= Size.Value, "a core class L2 is smaller than its L1 data cache");
                  Check
                    (not Hardware_Query.Available or else L2.Value mod Hardware_Query.Value = 0,
                     "a core class L2 is not a whole number of lines");
               end if;

               if Sharing.Available then
                  Check (L2.Available, "L2 sharing was reported without an L2 capacity");
                  Check
                    (Sharing.Value >= 1 and then (not Cores.Available or else Sharing.Value <= Cores.Value),
            ...
            if Position > 1 and then Flyology_Cachelines.Core_Class_Ordering = Flyology_Cachelines.Inferred
            then
               Check
                 (Size.Value
                  <= Flyology_Cachelines.L1_Data_Cache_Size (Flyology_Cachelines.Core_Class (Position - 1))
                       .Value,
      ...
      if Class_Count.Value < Flyology_Cachelines.Max_Core_Classes then
         Check
           (not Flyology_Cachelines.L1_Data_Cache_Size
                  (Flyology_Cachelines.Core_Class (Class_Count.Value + 1))
                  .Available,
   ...
   if L1_Query.Available then
      Check (L1_Query.Value > 0, "available L1 data-cache query returned zero");
      Check
        (L1_Query.Value >= 8 * 1_024 and then L1_Query.Value <= 512 * 1_024,
         "queried L1 data-cache capacity is outside the expected range");
      Check
        (Slots_Query.Value = L1_Query.Value / Flyology_Cachelines.Destructive_Interference_Size,
   ...
   Ada.Text_IO.Put_Line
     ("destructive interference:" & Flyology_Cachelines.Destructive_Interference_Size'Image & " bytes");
   Ada.Text_IO.Put_Line
     ("L1 data cache:" & Image_Or_Unavailable (L1_Query) & (if L1_Query.Available then " bytes" else ""));
   Ada.Text_IO.Put_Line
     ("core classes:"
      & Image_Or_Unavailable (Class_Count)
      & " ("
         ...
         begin
            Ada.Text_IO.Put_Line
              ("  class"
               & Position'Image
               & ": l1d"
               & Image_Or_Unavailable (Flyology_Cachelines.L1_Data_Cache_Size (Class))
               & " bytes, cores"
               & Image_Or_Unavailable (Flyology_Cachelines.Core_Class_Cores (Class))
               & ", cpus"
               & Image_Or_Unavailable (Flyology_Cachelines.Core_Class_CPUs (Class))
               & ", l2"
               & Image_Or_Unavailable (Flyology_Cachelines.L2_Cache_Size (Class))
               & " bytes across"
               & Image_Or_Unavailable (Flyology_Cachelines.L2_Sharing_Cores (Class))
               & " cores");
         end;
      end loop;
   end if;
