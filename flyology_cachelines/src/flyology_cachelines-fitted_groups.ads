...
package Flyology_Cachelines.Fitted_Groups is
   ...
   Element_Size_In_Storage_Elements : constant Positive :=
     Positive'Max (1, (Element_Type'Object_Size + System.Storage_Unit - 1) / System.Storage_Unit);
   ...
   Elements_Per_Group : constant Positive :=
     (if Element_Size_In_Storage_Elements > Destructive_Interference_Size
      then 1
      else Destructive_Interference_Size / Element_Size_In_Storage_Elements);
   ...
   is ((Container.Group_Count - 1) * Elements_Per_Group + Container.Last_Group_Length);
