...
--  Fits the largest safe number of same-owner values into each isolated
--  destructive-interference region.
--
--  @formal Element_Type The definite element type to store.
generic
   type Element_Type is private;
package Flyology_Cachelines.Fitted_Groups is

   --  Storage size of one element in System.Storage_Unit elements.
   --
   --  Fit the largest number of Element_Type objects that can be represented
   --  within the destructive-interference region selected for this build.
   Element_Size_In_Storage_Elements : constant Positive :=
     Positive'Max
       (1,
        (Element_Type'Object_Size + System.Storage_Unit - 1)
          / System.Storage_Unit);

   --  Maximum safe number of elements in one physical group.
   Elements_Per_Group : constant Positive :=
     (if Element_Size_In_Storage_Elements > Destructive_Interference_Size
      then 1
      else Destructive_Interference_Size /
        Element_Size_In_Storage_Elements);

   --  Valid index range within one physical group.
   subtype Index is Positive range 1 .. Elements_Per_Group;
   ...
   Payload_Size_In_Storage_Elements : constant Natural :=
     (Element_Array'Object_Size + System.Storage_Unit - 1)
       / System.Storage_Unit;

   pragma Compile_Time_Error
     (Element_Array'Object_Size
        > Destructive_Interference_Size * System.Storage_Unit,
      "automatically fitted group payload exceeds its interference region");

   pragma Warnings
     (Off, "suspiciously large alignment specified for ""Group""");
   --  One isolated group of automatically fitted, same-owner elements.
   type Group is new Element_Array
     with Alignment => Destructive_Interference_Size;
   pragma Warnings
     (On, "suspiciously large alignment specified for ""Group""");
   ...
   Group_Size_In_Storage_Elements : constant Positive :=
     Group'Object_Size / System.Storage_Unit;
   ...
   function Create (Values : Element_Array) return Group is
     (Group (Values));
   ...
   function Constant_Reference
     (Container : aliased Grouped_Array;
   ...
   function Reference
     (Container : aliased in out Grouped_Array;
   ...
   type Fast_View
     (Container : not null access Grouped_Array) is limited private
     with Iterable =>
       (First       => Fast_First,
        Next        => Fast_Next,
        Has_Element => Fast_Has_Element,
        Element     => Fast_Reference);
   ...
   function Fast_Next
     (View     : Fast_View;
      Position : Fast_Cursor) return Fast_Cursor;
   --  @exclude GNAT Iterable protocol.
   --  @param View View being traversed.
   --  @param Position Cursor to inspect.
   --  @return True when Position identifies an element.
   function Fast_Has_Element
     (View     : Fast_View;
      Position : Fast_Cursor) return Boolean;
   --  @exclude GNAT Iterable protocol.
   --  @param View View being traversed.
   --  @param Position Cursor to dereference.
   --  @return A mutable reference to the selected element.
   function Fast_Reference
     (View     : Fast_View;
      Position : Fast_Cursor) return Reference_Type;

   --  @exclude Standard iterator protocol.
   --  @param Container Container to traverse.
   --  @return A forward iterator over logical elements.
   function Iterate
     (Container : Grouped_Array)
   ...
   function Length (Container : Grouped_Array) return Positive is
     ((Container.Group_Count - 1) * Elements_Per_Group
        + Container.Last_Group_Length);
   ...
   function Create
     (Element_Count : Positive;
      Initial_Value : Element_Type) return Grouped_Array;
...
private
   ...
   type Fast_View
     (Container : not null access Grouped_Array) is limited null record;
end Flyology_Cachelines.Fitted_Groups;
