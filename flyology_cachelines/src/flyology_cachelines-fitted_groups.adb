...
package body Flyology_Cachelines.Fitted_Groups is
   ...
   function Fast_Next (View : Fast_View; Position : Fast_Cursor) return Fast_Cursor is
      ...
      Step : constant Positive :=
        (if Position.Slot = Index'Last
         then Group_Size_In_Storage_Elements - (Elements_Per_Group - 1) * Element_Stride_In_Storage_Elements
   ...
   function Create (Element_Count : Positive; Initial_Value : Element_Type) return Grouped_Array is
      Group_Count       : constant Positive := (Element_Count - 1) / Elements_Per_Group + 1;
      Last_Group_Length : constant Index := Index ((Element_Count - 1) mod Elements_Per_Group + 1);
   begin
      return
