...
procedure Suite_Smoke is
   ...
   with Volatile;
   Calls       : Natural := 0;
   Last_Config : Flyology_Bench.Configuration := Flyology_Bench.Default_Configuration;

   type Legacy_CSV_Access is
     access procedure
       (Name   : String;
        Result : Flyology_Bench.Measurement;
        File   : Ada.Text_IO.File_Type := Ada.Text_IO.Standard_Output);
   Legacy_CSV : constant Legacy_CSV_Access := Flyology_Bench.Reporters.Put_CSV'Access;
   ...
   procedure Compare_Operations is new
   ...
   procedure Compare_Multi is new Flyology_Bench.Compare_Many (Case_Id => Multi_Case, Batch => Multi_Batch);

   procedure Run_Measurement (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Measurement)
   is
   begin
      Calls := Calls + 1;
      Last_Config := Config;
   ...
   procedure Run_Comparison (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Comparison) is
   begin
      Calls := Calls + 1;
      Last_Config := Config;
   ...
   procedure Run_Multi (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Multi_Comparison)
   is
   begin
      Calls := Calls + 1;
      Last_Config := Config;
   ...
   package Multi_Registration is new Runner.Multi_Way_Registration (Case_Id => Multi_Case, Run => Run_Multi);

   procedure Fail (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Measurement) is
   ...
   function U (Value : String) return Unbounded_String
   is (To_Unbounded_String (Value));

   function Args_1 (One : String) return Runner.Argument_List
   is ([1 => U (One)]);

   function Args_2 (One, Two : String) return Runner.Argument_List
   is ([U (One), U (Two)]);

   function Args_3 (One, Two, Three : String) return Runner.Argument_List
   is ([U (One), U (Two), U (Three)]);

   procedure Expect_Option_Error (Arguments : Runner.Argument_List; Label : String) is
   begin
      declare
   ...
   Target : Runner.Suite;
begin
   Runner.Register (Target, "alpha", Run_Measurement'Access, Group => "core", Tags => "fast,smoke");
   Runner.Register (Target, "beta", Run_Measurement'Access, Group => "io", Tags => "slow,smoke");
   Runner.Register_Paired
     (Target,
      "compare",
      "reference",
      "contender",
      Run_Comparison'Access,
      Group => "core",
      Tags  => "paired,smoke");
   Runner.Register (Target, "failure", Fail'Access, Group => "core", Tags => "failure,smoke");
   ...
   Check (Runner.Kind (Target, 3) = Runner.Paired_Comparison, "paired result kind");
   ...
   begin
      begin
         Runner.Register (Target, "alpha", Run_Measurement'Access, Group => "core");
      exception
         when Runner.Registration_Error =>
            Duplicate_Rejected := True;
      end;
      begin
         Runner.Register (Target, "bad/name", Run_Measurement'Access);
      exception
         when Runner.Registration_Error =>
   ...
   declare
      ...
      Substring : constant Runner.Runner_Options := Runner.Parse (Args_2 ("--filter", "core/"), Base_Config);
      Glob      : constant Runner.Runner_Options := Runner.Parse (Args_2 ("--filter", "*/b?ta"), Base_Config);
      Group_Tag : constant Runner.Runner_Options :=
        Runner.Parse (Args_3 ("--group=core", "--tag=smoke", "--skip=*failure"), Base_Config);
   begin
   ...
   declare
      Options : constant Runner.Runner_Options :=
        Runner.Parse
          ([U ("--warmup=2ms"),
            U ("--measurement-time=4ms"),
            U ("--maximum-sampling-time=9ms"),
            U ("--samples=12"),
            U ("--minimum-sample-time=3us"),
            U ("--practical-threshold=2.5"),
            U ("--random-seed=-9"),
            U ("--output-style=json")],
           Base_Config);
      Config  : constant Flyology_Bench.Configuration := Runner.Effective_Configuration (Options);
   begin
   ...
   Expect_Option_Error (Args_1 ("--measurement-time=0.1ns"), "sub-nanosecond duration");
   ...
   Expect_Option_Error (Args_1 ("--record-baseline=baseline.dat"), "uncoordinated baseline mode");
   Expect_Option_Error (Args_2 ("--exact=core/alpha", "--filter=alpha"), "exact conflict");
   Expect_Option_Error (Args_2 ("--fail-fast", "--continue-on-error"), "error policy conflict");
   ...
   begin
      ...
      begin
         Check
           (Alpha > 0 and then Alpha < Compare and then Compare < Failure and then Failure < Beta,
   ...
   begin
      ...
      Check (Runner.Kind (Result) = Runner.Ordinary_Measurement, "execute-one ordinary kind");
      Check (Flyology_Bench.Samples (Runner.Measurement_Value (Result)) = 10, "execute-one ordinary result");
      Runner.Execute_One (Target, "core/compare", Base_Config, Result);
      Check (Runner.Kind (Result) = Runner.Paired_Comparison, "execute-one paired kind");
      Check
        (Flyology_Bench.Samples (Flyology_Bench.Reference_Measurement (Runner.Comparison_Value (Result)))
         = 10,
   ...
   declare
      ...
      Summary       : Runner.Run_Summary;
      Options       : constant Runner.Runner_Options :=
        Runner.Parse ([U ("--output-style=json"), U ("--output=" & Path)], Base_Config);
   begin
      Multi_Registration.Register (Multi_Target, "shootout", Group => "core", Tags => "multi,smoke");
      Check (Runner.Kind (Multi_Target, 1) = Runner.Multi_Way_Comparison, "multi-way registration kind");
      Runner.Execute_One_Multi (Multi_Target, "core/shootout", Base_Config, Result);
      Check (Flyology_Bench.Cases (Result) = 2, "execute-one multi-way result");
      Remove (Path);
      Remove (Progress_Path);
      Ada.Text_IO.Create (Progress, Ada.Text_IO.Out_File, Progress_Path);
      Runner.Execute (Multi_Target, "multi_suite", Options, Summary, Progress => Progress);
      ...
      begin
         Check
           (Ada.Strings.Fixed.Index (Output_Text, """result_kind"":""multi_way_comparison""") /= 0
            and then Ada.Strings.Fixed.Index (Output_Text, """type"":""multi_comparison""") /= 0
            and then Ada.Strings.Fixed.Index (Output_Text, """contenders""") /= 0,
   ...
   declare
      ...
      Summary       : Runner.Run_Summary;
      Options       : constant Runner.Runner_Options :=
        Runner.Parse ([U ("--output-style=json"), U ("--output=" & Output_Path)], Base_Config);
   begin
      ...
      Runner.Register_Gated
        (Gate_Target,
         "missing",
         Run_Measurement'Access,
         Baseline_Path,
         Group => "core",
         Tags  => "gate,smoke");
      Ada.Text_IO.Create (Progress, Ada.Text_IO.Out_File, Progress_Path);
      Runner.Execute (Gate_Target, "gate_suite", Options, Summary, Progress => Progress);
      Ada.Text_IO.Close (Progress);
      Check
        (Summary.Completed = 1
      ...
      Check
        (Ada.Strings.Fixed.Index (Read_All (Output_Path), """status"":""missing_baseline""") /= 0,
         "suite did not report the baseline gate result");
      Check (not Ada.Directories.Exists (Baseline_Path), "suite gate created a missing baseline");
      Remove (Output_Path);
      Remove (Progress_Path);
   end;
   ...
   declare
      ...
      Summary : Runner.Run_Summary;
      Options : constant Runner.Runner_Options :=
        Runner.Parse ([U ("--exact=core/alpha"), U ("--samples=12"), U ("--output-style=csv")], Base_Config);
   begin
      Remove (Path);
      Ada.Text_IO.Create (File, Ada.Text_IO.Out_File, Path);
      Runner.Execute (Target, "config_suite", Options, Summary, Output => File, Progress => File);
      Ada.Text_IO.Close (File);
      ...
      Check (Last_Config.Maximum_Iterations = 1_000, "callback configuration changed unrelated field");
      declare
         Output_Text : constant String := Read_All (Path);
      begin
         Check
           (Ada.Strings.Fixed.Index (Output_Text, "row_kind") /= 0
            and then Ada.Strings.Fixed.Index
                       (Output_Text,
                        "config_suite,core/alpha,ordinary_measurement,completed,false," & "measurement")
                     /= 0
            and then Ada.Strings.Fixed.Index (Output_Text, "clock_backend") /= 0
            and then Ada.Strings.Fixed.Index (Output_Text, ",metric,") /= 0,
   ...
   declare
      ...
      Metric_Config : constant Flyology_Bench.Configuration :=
        (Base_Config
         with delta
           Metrics         => Flyology_Bench.Time_Metrics or Flyology_Bench.Flyology_Scheduler_Metrics,
           Scheduler_Probe => null);
      Options       : constant Runner.Runner_Options :=
        Runner.Parse
          ([U ("--exact=core/alpha"), U ("--require-metrics"), U ("--output-style=csv")], Metric_Config);
   begin
      Remove (Path);
      Ada.Text_IO.Create (File, Ada.Text_IO.Out_File, Path);
      Runner.Execute (Target, "metric_suite", Options, Summary, Output => File, Progress => File);
      Ada.Text_IO.Close (File);
      Check
        (Summary.Unavailable = 1 and then Summary.Status = Runner.Requested_Metric_Unavailable,
   ...
   declare
      ...
      Options       : constant Runner.Runner_Options :=
        Runner.Parse
          ([U ("--exact=core/alpha"), U ("--output-style=human"), U ("--output=" & Path)], Base_Config);
   begin
      Remove (Path);
      Remove (Progress_Path);
      Ada.Text_IO.Create (Progress, Ada.Text_IO.Out_File, Progress_Path);
      Runner.Execute (Target, "human_suite", Options, Summary, Progress => Progress);
      Ada.Text_IO.Close (Progress);
      declare
      ...
      begin
         ...
         Check
           (Ada.Strings.Fixed.Index (Human, "case core/alpha: completed") /= 0, "human case outcome absent");
         Check (Ada.Strings.Fixed.Index (Human, "latency") /= 0, "existing human reporter was not used");
         Check (Ada.Strings.Fixed.Index (Human, "status=succeeded") /= 0, "human suite summary absent");
         Check (Ada.Strings.Fixed.Index (Human, ASCII.ESC & "[") = 0, "human output file contains ANSI");
      end;
      Remove (Path);
      Remove (Progress_Path);
   end;
   ...
   declare
      ...
      Allowed       : constant Runner.Runner_Options :=
        Runner.Parse (Args_3 ("--filter", "absent", "--allow-empty"), Base_Config);
      Summary       : Runner.Run_Summary;
      ...
      List_Options  : constant Runner.Runner_Options :=
        Runner.Parse ([U ("--list"), U ("--filter=absent")], Base_Config);
   begin
      Runner.List (Target, No_Match, Summary);
      Check (Summary.Status = Runner.No_Matching_Cases, "no-match status was successful");
      ...
      Runner.Execute
        (Target, "empty_suite", List_Options, Summary, Output => Output_File, Progress => Progress_File);
      Ada.Text_IO.Close (Output_File);
      Ada.Text_IO.Close (Progress_File);
      Check
        (Ada.Strings.Fixed.Index (Read_All (Progress_Path), "no benchmark cases matched the selection") /= 0,
   ...
   begin
      ...
      Remove (Progress_Path);
      Ada.Text_IO.Create (Progress, Ada.Text_IO.Out_File, Progress_Path);
      Runner.Execute (Target, "smoke_suite", Options, Summary, Progress => Progress);
      Ada.Text_IO.Close (Progress);
      ...
      begin
         Check (Ada.Strings.Fixed.Index (Output_Text, """dry_run"":true") /= 0, "dry JSON marker absent");
         Check
           (Ada.Strings.Fixed.Index (Output_Text, """median_ns"":null") /= 0,
            "dry JSON exposed performance number");
         Check (Ada.Strings.Fixed.Index (Output_Text, "core/failure") /= 0, "exception identity absent");
         Check (Ada.Strings.Fixed.Index (Output_Text, "running ") = 0, "progress leaked into output file");
         Check (Ada.Strings.Fixed.Index (Output_Text, ASCII.ESC & "[") = 0, "ANSI leaked into output file");
         Check (Ada.Strings.Fixed.Index (Progress_Text, "running core/alpha") /= 0, "progress stream absent");
      end;
      Remove (JSON_Path);
      Remove (Progress_Path);
   end;
   ...
   declare
      ...
      Options       : constant Runner.Runner_Options :=
        Runner.Parse
          ([U ("--exact=core/alpha"), U ("--output-style=json"), U ("--output=" & Path)], Terminal_Base);
   begin
      Remove (Path);
      Remove (Progress_Path);
      Ada.Text_IO.Create (Progress, Ada.Text_IO.Out_File, Progress_Path);
      Runner.Execute (Target, "machine_suite", Options, Summary, Progress => Progress);
      Ada.Text_IO.Close (Progress);
      Check (Runner.Successful (Summary), "machine suite did not succeed");
      Check (Last_Config.Progress = null, "machine output retained configured terminal progress");
      Check
        (Ada.Strings.Fixed.Index (Read_All (Path), """clock"":{""backend""") /= 0,
   ...
   declare
      ...
      Continue_Options  : constant Runner.Runner_Options :=
        Runner.Parse ([U ("--dry-run"), U ("--continue-on-error"), U ("--output-style=csv")], Base_Config);
      Fail_Fast_Options : constant Runner.Runner_Options :=
        Runner.Parse ([U ("--dry-run"), U ("--fail-fast"), U ("--output-style=csv")], Base_Config);
   ...
   begin
      ...
      Runner.Execute
        (Fast_Target, "policy_suite", Continue_Options, Summary, Output => Sink, Progress => Sink);
      Ada.Text_IO.Close (Sink);
      Check (Summary.Completed = 2 and then Summary.Failed = 1, "continue-on-error counts");
      Ada.Text_IO.Create (Sink, Ada.Text_IO.Out_File, Sink_Path);
      Runner.Execute
        (Fast_Target, "policy_suite", Fail_Fast_Options, Summary, Output => Sink, Progress => Sink);
      Ada.Text_IO.Close (Sink);
      Check
        (Summary.Completed = 1 and then Summary.Failed = 1 and then Summary.Skipped = 1, "fail-fast counts");
      Check
        (Ada.Strings.Fixed.Index (Read_All (Sink_Path), """quoted""") /= 0, "CSV exception escaping absent");
      Remove (Sink_Path);
   end;
