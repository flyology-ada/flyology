...
procedure Sweeps_Smoke is
   ...
   function Close (Left, Right : Long_Float; Relative : Long_Float := 1.0E-12) return Boolean
   is (abs (Left - Right) <= Relative * Long_Float'Max (1.0, abs Right));
   ...
   function Point_Work (Item : Sweeps.Parameter_Point) return Sweeps.Work_Amount
   is (Sweeps.Work (Sweeps.Value (Item), Sweeps.Items, Scaling => Sweeps.Decimal_Scaling));

   function Exact_Byte_Work (Item : Sweeps.Parameter_Point) return Sweeps.Work_Amount
   is (Sweeps.Work (Sweeps.Value (Item), Sweeps.Bytes, Scaling => Sweeps.Binary_Scaling));

   function Display_Work (Item : Sweeps.Parameter_Point) return Sweeps.Work_Amount
   is (case Sweeps.Value (Item) is
         when 1      => Sweeps.Work (1_024.0, Sweeps.Bytes, Scaling => Sweeps.Binary_Scaling),
         when 2      => Sweeps.Work (1_000.0, Sweeps.Items, Scaling => Sweeps.Decimal_Scaling),
         when others => Sweeps.Work (1_000_000.0, Sweeps.Caller_Named, "records", Sweeps.Decimal_Scaling));

   function Work_Maybe_Fail (Item : Sweeps.Parameter_Point) return Sweeps.Work_Amount is
   begin
   ...
   procedure Compare_AB is new
     Flyology_Bench.Compare (Reference_Operation => Operation_A, Contender_Operation => Operation_B);

   procedure Measure_Then_Fail
     (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Measurement) is
   begin
      Measure_A (Config, Result);
      raise Program_Error with "measurement failed after writing output";
   end Measure_Then_Fail;

   procedure Compare_Then_Fail (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Comparison)
   is
   begin
   ...
   procedure Run_Ordinary is new
     Sweeps.Measure_Sweep (Select_Point => Choose_Point, Work_For => Point_Work, Run_Point => Measure_A);

   procedure Run_Paired is new
     Sweeps.Compare_Sweep (Select_Point => Choose_Point, Work_For => Point_Work, Run_Point => Compare_AB);

   procedure Run_Display is new
     Sweeps.Measure_Sweep (Select_Point => Choose_Point, Work_For => Display_Work, Run_Point => Measure_A);

   procedure Run_Maximum_Dry is new
     Sweeps.Measure_Sweep (Select_Point => Ignore_Point, Work_For => Exact_Byte_Work, Run_Point => Measure_A);

   procedure Run_Output_Failing_Measurement is new
     Sweeps.Measure_Sweep
       (Select_Point => Choose_Point,
        Work_For     => Point_Work,
        Run_Point    => Measure_Then_Fail);

   procedure Run_Output_Failing_Comparison is new
     Sweeps.Compare_Sweep
       (Select_Point => Choose_Point,
        Work_For     => Point_Work,
   ...
   procedure Run_Failing is new
     Sweeps.Measure_Sweep (Select_Point => Select_Maybe_Fail, Work_For => Point_Work, Run_Point => Measure_A);

   procedure Run_Work_Failing is new
     Sweeps.Measure_Sweep (Select_Point => Choose_Point, Work_For => Work_Maybe_Fail, Run_Point => Measure_A);
   ...
   procedure Run_Budgeted is new
     Sweeps.Measure_Sweep (Select_Point => Select_Slow_First, Work_For => Point_Work, Run_Point => Measure_A);

   procedure Run_Paired_Budgeted is new
     Sweeps.Compare_Sweep
   ...
   procedure Expect_Constraint_Error (Action : not null access procedure; Name : String) is
...
begin
   ...
   Check (Sweeps.Value (Points.Element (2)) = 2, "explicit point ordering changed");
   ...
   declare
      ...
      procedure Invalid_Label is
         Ignored : constant Sweeps.Parameter_Point := Sweeps.Point (Sweeps.Size_Parameter, 8, "bad label");
   ...
   declare
      Items_Amount  : constant Sweeps.Work_Amount := Sweeps.Work (1_000.0, Sweeps.Items);
      Byte_Amount   : constant Sweeps.Work_Amount :=
        Sweeps.Work (1_024.0, Sweeps.Bytes, Scaling => Sweeps.Binary_Scaling);
   ...
   begin
      ...
      Check (Sweeps.Availability (Rate) = Sweeps.Throughput_Available, "synthetic throughput unavailable");
      Check (Close (Sweeps.Operations_Per_Second (Rate), 500_000_000.0), "operations/s derivation");
      Check (Close (Sweeps.Operations_Confidence_Low (Rate), 250_000_000.0), "inverted low rate bound");
      Check (Close (Sweeps.Operations_Confidence_High (Rate), 1_000_000_000.0), "inverted high rate bound");
      Check (Close (Sweeps.Work_Units_Per_Second (Rate), 2_000_000_000.0), "work/s derivation");
   end;

   declare
      procedure Zero_Work is
         Ignored : constant Sweeps.Work_Amount := Sweeps.Work (0.0, Sweeps.Items);
         pragma Unreferenced (Ignored);
      begin
         null;
      end Zero_Work;
      procedure Negative_Work is
         Ignored : constant Sweeps.Work_Amount := Sweeps.Work (-1.0, Sweeps.Items);
         pragma Unreferenced (Ignored);
      begin
         null;
      end Negative_Work;
      procedure Fractional_Work is
         Ignored : constant Sweeps.Work_Amount := Sweeps.Work (1.5, Sweeps.Items);
         pragma Unreferenced (Ignored);
      begin
         null;
      end Fractional_Work;
      procedure Incoherent_Work is
         Ignored : constant Sweeps.Work_Amount := Sweeps.Work (1.0, Sweeps.Bytes, "named");
         pragma Unreferenced (Ignored);
      begin
         null;
      end Incoherent_Work;
      procedure Invalid_Unit is
         Ignored : constant Sweeps.Work_Amount := Sweeps.Work (1.0, Sweeps.Caller_Named, "bad unit");
         pragma Unreferenced (Ignored);
      begin
         null;
      end Invalid_Unit;
      procedure Overflowed_Work is
         Ignored : constant Sweeps.Work_Amount := Sweeps.Work (Long_Float'Last, Sweeps.Items);
         pragma Unreferenced (Ignored);
      begin
         null;
      end Overflowed_Work;
      procedure Nonfinite_Work is
         Ignored : constant Sweeps.Work_Amount := Sweeps.Work (Long_Float'Value ("NaN"), Sweeps.Items);
         pragma Unreferenced (Ignored);
      begin
         null;
      end Nonfinite_Work;
   ...
   declare
      ...
      Missing  : constant Sweeps.Throughput_Summary :=
        Sweeps.Derive_Throughput (Sweeps.Work (1.0, Sweeps.Items), Empty);
   ...
   begin
      ...
      Check (Sweeps.Availability (Overflow) = Sweeps.Throughput_Overflow, "rate overflow became available");
      Check (not Sweeps.Wall_Time_Available (Missing), "missing wall time became available");
      Check
   ...
   begin
      Failure_Points.Append (Sweeps.Point (Sweeps.Count_Parameter, 1, "write-then-fail"));
      Run_Output_Failing_Measurement
        ("failures/output/measurement", Failure_Points, Config, Result => Measurement_Failure);
      Check
        (Sweeps.Status (Sweeps.Element (Measurement_Failure, 1)) = Sweeps.Point_Measurement_Failed,
         "write-then-fail measurement status");
      Check
        (not Sweeps.Collection_Available (Sweeps.Element (Measurement_Failure, 1)),
      ...
      Check
        (Flyology_Bench.Median_Nanoseconds (Sweeps.Data (Sweeps.Element (Measurement_Failure, 1))) = 0.0,
         "failed measurement retained partial output");

      Run_Output_Failing_Comparison
        ("failures/output/comparison", Failure_Points, Config, Result => Comparison_Failure);
      Check
        (Sweeps.Status (Sweeps.Element (Comparison_Failure, 1)) = Sweeps.Point_Measurement_Failed,
         "write-then-fail comparison status");
      Check
        (not Sweeps.Collection_Available (Sweeps.Element (Comparison_Failure, 1)),
         "failed comparison became collected");
      Check
        (not Sweeps.Stopped_Early (Comparison_Failure), "final paired failure reported unattempted points");
      declare
         Reset_Pair : constant Flyology_Bench.Comparison :=
           Sweeps.Data (Sweeps.Element (Comparison_Failure, 1));
      begin
         Check
           (Flyology_Bench.Median_Nanoseconds (Flyology_Bench.Reference_Measurement (Reset_Pair)) = 0.0
            and then Flyology_Bench.Median_Nanoseconds (Flyology_Bench.Contender_Measurement (Reset_Pair))
                     = 0.0,
   ...
   begin
      Maximum_Points.Append (Sweeps.Point (Sweeps.Size_Parameter, Sweeps.Exact_Value'Last, "maximum"));
      Run_Maximum_Dry
        ("limits/maximum", Maximum_Points, Config, (Mode => Sweeps.Dry_Run, others => <>), Maximum_Result);
      Check
        (Sweeps.Identity (Sweeps.Parameter (Sweeps.Element (Maximum_Result, 1)))
         = "size:18446744073709551615",
         "maximum point identity lost exactness");
      Check
        (Sweeps.Raw_Value (Sweeps.Work_Per_Operation (Sweeps.Element (Maximum_Result, 1)))
         = Sweeps.Exact_Value'Last,
         "maximum work value lost exactness");
   end;

   Run_Ordinary (Case_Name => "algorithms/mix", Points => Points, Config => Config, Result => Ordinary);
   Check (Sweeps.Length (Ordinary) = 3, "ordinary sweep length");
   Check
     (Ada.Strings.Unbounded.To_String (Last_Progress_Name) = "algorithms/mix/count:4",
      "progress lost exact case/point identity");
   for Index in 1 .. Sweeps.Length (Ordinary) loop
      declare
         Item : constant Sweeps.Ordinary_Point_Result := Sweeps.Element (Ordinary, Index);
      begin
         ...
         Check
           (Sweeps.Identity (Sweeps.Parameter (Item)) = Sweeps.Identity (Points.Element (Index)),
            "ordinary point identity");
      end;
   end loop;

   Run_Display (Case_Name => "units/display", Points => Points, Config => Config, Result => Display_Result);
   Reporters.Put_Console ("units/display", Display_Result);

   Run_Paired (Case_Name => "algorithms/mix", Points => Points, Config => Config, Result => Paired);
   Check (Sweeps.Length (Paired) = 3, "paired sweep length");
   for Index in 1 .. Sweeps.Length (Paired) loop
      declare
         Item       : constant Sweeps.Paired_Point_Result := Sweeps.Element (Paired, Index);
         Pair       : constant Flyology_Bench.Comparison := Sweeps.Data (Item);
         Pair_Count : constant Natural :=
           Flyology_Bench.Reference_First_Samples (Pair) + Flyology_Bench.Contender_First_Samples (Pair);
      begin
         ...
         Check
           (Pair_Count = Natural (Flyology_Bench.Samples (Flyology_Bench.Reference_Measurement (Pair))),
            "pairing missing at sweep point");
         Check
           (abs (Integer (Flyology_Bench.Reference_First_Samples (Pair))
                 - Integer (Flyology_Bench.Contender_First_Samples (Pair)))
            <= 1,
   ...
   begin
      ...
      Check
        (Sweeps.Status (Sweeps.Element (Failure_Result, 2)) = Sweeps.Point_Setup_Failed,
      ...
      Check
        (Sweeps.Status (Sweeps.Element (Failure_Result, 3)) = Sweeps.Point_Measured,
      ...
      Run_Work_Failing
        ("algorithms/work_failure",
         ...
         Work_Failure_Result);
      Check
        (Sweeps.Status (Sweeps.Element (Work_Failure_Result, 2)) = Sweeps.Point_Setup_Failed,
         "work failure status");
      Check
      ...
      declare
         procedure Read_Unavailable_Work is
            Ignored : constant Sweeps.Work_Amount :=
              Sweeps.Work_Per_Operation (Sweeps.Element (Work_Failure_Result, 2));
            pragma Unreferenced (Ignored);
         begin
            null;
         end Read_Unavailable_Work;
      begin
         Expect_Constraint_Error (Read_Unavailable_Work'Access, "unavailable point work accessor");
      end;
   end;
   ...
   begin
      ...
      Run_Budgeted
        ("algorithms/budget",
         ...
         Budget_Result);
      Check
        (Sweeps.Status (Sweeps.Element (Budget_Result, 2)) = Sweeps.Point_Budget_Exhausted,
      ...
      Final_Points.Append (Sweeps.Point (Sweeps.Count_Parameter, 1, "final-budget"));
      Run_Budgeted
      ...
      Check
        (Sweeps.Length (Final_Ordinary) = 1
         and then Sweeps.Status (Sweeps.Element (Final_Ordinary, 1)) = Sweeps.Point_Budget_Exhausted,
      ...
      Check
        (Sweeps.Length (Final_Paired) = 1
         and then Sweeps.Status (Sweeps.Element (Final_Paired, 1)) = Sweeps.Point_Budget_Exhausted,
      ...
      Run_Ordinary ("algorithms/dry", Points, Config, (Mode => Sweeps.Dry_Run, others => <>), Dry_Result);
      for Index in 1 .. Sweeps.Length (Dry_Result) loop
         Check
           (Sweeps.Status (Sweeps.Element (Dry_Result, Index)) = Sweeps.Point_Dry_Run,
   ...
   begin
      for Power in 0 .. 4 loop
         declare
            ...
            P : constant Sweeps.Parameter_Point := Sweeps.Point (Sweeps.Size_Parameter, N);
         begin
            Linear_Data.Append (P, 3.0 * Long_Float (N));
            Quadratic_Data.Append (P, 2.0 * Long_Float (N) * Long_Float (N));
            Constant_Data.Append (P, 7.0);
            Bad_Data.Append (P, (if Power mod 2 = 0 then 1.0 else 100.0));
            if Power < 3 then
               Too_Short.Append (P, Long_Float (N));
            end if;
            if Power < 4 then
               Invalid.Append (P, (if Power = 2 then 0.0 else Long_Float (N)));
            end if;
         end;
      end loop;
      for Offset in 0 .. 3 loop
         Degenerate.Append
           (Sweeps.Point (Sweeps.Count_Parameter, Sweeps.Exact_Value (10 + Offset)),
            Long_Float (10 + Offset));
      end loop;
      declare
         Minimum       : constant Sweeps.Exact_Value := 9_007_199_254_740_993;
         Below_Maximum : constant Sweeps.Exact_Value := 18_014_398_509_481_985;
         Exact_Maximum : constant Sweeps.Exact_Value := 18_014_398_509_481_986;
         Below_Inputs  : constant array (Positive range 1 .. 4) of Sweeps.Exact_Value :=
           [Minimum, Minimum + 1, Below_Maximum - 1, Below_Maximum];
         Exact_Inputs  : constant array (Positive range 1 .. 4) of Sweeps.Exact_Value :=
           [Minimum, Minimum + 1, Exact_Maximum - 1, Exact_Maximum];
         Upper_Inputs  : constant array (Positive range 1 .. 4) of Sweeps.Exact_Value :=
      ...
      begin
         for Index in Below_Inputs'Range loop
            Below_Twofold.Append
              (Sweeps.Point (Sweeps.Size_Parameter, Below_Inputs (Index)), Long_Float (Index));
            Exact_Twofold.Append
              (Sweeps.Point (Sweeps.Size_Parameter, Exact_Inputs (Index)), Long_Float (Index));
         end loop;
         for Index in Upper_Inputs'Range loop
            Upper_Degenerate.Append
              (Sweeps.Point (Sweeps.Size_Parameter, Upper_Inputs (Index)), Long_Float (Index));
         end loop;
      end;
         ...
         begin
            N_Log_N_Data.Append
              (Sweeps.Point (Sweeps.Size_Parameter, N),
               5.0 * Input * Ada.Numerics.Long_Elementary_Functions.Log (Input));
         end;
      end loop;
      ...
      begin
         Expect_Constraint_Error (Append_Mixed_Kind'Access, "mixed scaling parameter kinds");
      end;
      declare
         Ambiguous_Inputs : constant array (Positive range 1 .. 4) of Sweeps.Exact_Value :=
           [1_000, 1_200, 1_500, 2_000];
      begin
      ...
      declare
         Linear_Fit           : constant Scaling.Empirical_Scaling_Analysis := Scaling.Analyze (Linear_Data);
         ...
         N_Log_N_Fit          : constant Scaling.Empirical_Scaling_Analysis := Scaling.Analyze (N_Log_N_Data);
         Ambiguous_Fit        : constant Scaling.Empirical_Scaling_Analysis :=
           Scaling.Analyze (Ambiguous_Data);
         Bad_Fit              : constant Scaling.Empirical_Scaling_Analysis := Scaling.Analyze (Bad_Data);
         Empty_Fit            : constant Scaling.Empirical_Scaling_Analysis := Scaling.Analyze (Empty);
         ...
         Upper_Degenerate_Fit : constant Scaling.Empirical_Scaling_Analysis :=
           Scaling.Analyze (Upper_Degenerate);
      begin
         Check (Scaling.Available (Linear_Fit), "linear fit unavailable");
         Check (Scaling.Selected_Model (Linear_Fit) = Scaling.Linear_Model, "linear model not selected");
         Check
           (Scaling.Diagnostic (Linear_Fit, Scaling.Linear_Model).Selected,
            "available analysis lost selected diagnostic");
         Check
           (Close (Scaling.Diagnostic (Linear_Fit, Scaling.Linear_Model).Coefficient, 3.0),
            "linear coefficient");
         Check (Scaling.Available (Quadratic_Fit), "quadratic fit unavailable");
         Check
           (Scaling.Selected_Model (Quadratic_Fit) = Scaling.Quadratic_Model, "quadratic model not selected");
         Check (Scaling.Available (Constant_Fit), "constant fit unavailable");
         Check
           (Scaling.Selected_Model (Constant_Fit) = Scaling.Constant_Model, "constant model not selected");
         Check (Scaling.Available (N_Log_N_Fit), "n log n fit unavailable");
         Check (Scaling.Selected_Model (N_Log_N_Fit) = Scaling.N_Log_N_Model, "n log n model not selected");
         Check
           (Scaling.Status (Ambiguous_Fit) = Scaling.Poor_Model_Identifiability,
            "poor model identifiability was accepted");
         Check (Scaling.Status (Bad_Fit) = Scaling.No_Adequate_Model, "inadequate data accepted");
         ...
         Check
           (Scaling.Status (Scaling.Analyze (Too_Short)) = Scaling.Too_Few_Distinct_Points,
            "too few scaling points accepted");
         Check
           (Scaling.Input_Kind_Available (Scaling.Analyze (Too_Short))
            and then Scaling.Input_Kind (Scaling.Analyze (Too_Short)) = Sweeps.Size_Parameter,
         ...
         Check
           (Scaling.Diagnostic (Empty_Fit, Scaling.Constant_Model).Nominal_Exponent = 0.0
            and then Scaling.Diagnostic (Empty_Fit, Scaling.Logarithmic_Model).Nominal_Exponent = 0.0
            and then Scaling.Diagnostic (Empty_Fit, Scaling.Linear_Model).Nominal_Exponent = 1.0
            and then Scaling.Diagnostic (Empty_Fit, Scaling.N_Log_N_Model).Nominal_Exponent = 1.0
            and then Scaling.Diagnostic (Empty_Fit, Scaling.Quadratic_Model).Nominal_Exponent = 2.0
            and then Scaling.Diagnostic (Empty_Fit, Scaling.Cubic_Model).Nominal_Exponent = 3.0,
            "rejected scaling data lost invariant model exponents");
         declare
            procedure Read_Empty_Range is
               Ignored : constant Sweeps.Exact_Value := Scaling.Minimum_Input (Empty_Fit);
               pragma Unreferenced (Ignored);
            begin
               null;
            end Read_Empty_Range;
         begin
            Expect_Constraint_Error (Read_Empty_Range'Access, "empty scaling range accessor");
         end;
         Check
           (Scaling.Status (Scaling.Analyze (Degenerate)) = Scaling.Degenerate_Input_Range,
            "degenerate scaling range accepted");
         Check
           (Scaling.Status (Below_Twofold_Fit) = Scaling.Degenerate_Input_Range,
            "rounded sub-twofold scaling range accepted");
         Check
           (Scaling.Status (Exact_Twofold_Fit) /= Scaling.Degenerate_Input_Range,
            "exact twofold scaling range rejected");
         Check
           (Scaling.Status (Upper_Degenerate_Fit) = Scaling.Degenerate_Input_Range,
            "upper-bound scaling range overflowed or was accepted");
         Check
           (Scaling.Status (Scaling.Analyze (Invalid)) = Scaling.Invalid_Observation,
         ...
         Reporters.Put_Comparison_CSV ("group/case", "existing", "candidate", Paired);
         ...
         Reporters.Put_Comparison_NDJSON ("group/case", "existing", "candidate", Paired);
         Reporters.Put_Scaling_NDJSON ("group/case", Linear_Fit);
         Reporters.Put_Scaling_NDJSON ("group/inadequate", Bad_Fit);
         Reporters.Put_Scaling_NDJSON ("group/boundary", Below_Twofold_Fit);
