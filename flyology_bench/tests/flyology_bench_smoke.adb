...
procedure Flyology_Bench_Smoke is
   ...
   procedure Scheduler_Probe (Snapshot : out Flyology_Bench.Flyology_Scheduler_Snapshot) is
   begin
      Snapshot := (Available => True, Dispatches => Counter, others => 0);
   end Scheduler_Probe;
   ...
   procedure Operation_Comparison is new
     Flyology_Bench.Compare (Reference_Operation => Operation, Contender_Operation => Contender_Operation);

   Batched_Calls : Flyology_Bench.Iteration_Count := 0
   with Volatile;
   ...
   procedure Batched_Comparison is new
     Flyology_Bench.Compare_Batched (Reference_Batch => Batch, Contender_Batch => Batch);
   ...
   procedure Hooked_Benchmark is new
     Flyology_Bench.Measure_With_Hooks (Setup => Setup, Operation => Operation, Teardown => Teardown);

   procedure Result_Batch (Iterations : Flyology_Bench.Iteration_Count; Value : out Natural) is
   begin
      Value := 0;
      for Iteration in Flyology_Bench.Iteration_Count range 1 .. Iterations loop
         Value := Value + 1;
   ...
   procedure Result_Benchmark is new
     Flyology_Bench.Measure_Result_Batched (Element => Natural, Batch => Result_Batch);

   procedure Slow_Batch (Iterations : Flyology_Bench.Iteration_Count) is
      pragma Unreferenced (Iterations);
   begin
      delay 0.002;
   end Slow_Batch;

   procedure Budgeted_Benchmark is new Flyology_Bench.Measure_Batched (Slow_Batch);
   ...
   type Attribution_Results is array (Attribution_Slot) of Interfaces.Unsigned_64 with Volatile_Components;
   ...
   procedure Serial_Attribution_Batch (Iterations : Flyology_Bench.Iteration_Count) is
   begin
      Mix_Slot (1, Iterations);
   end Serial_Attribution_Batch;

   procedure Worker_Attribution_Batch (Iterations : Flyology_Bench.Iteration_Count) is
      task type Worker
   ...
   procedure Serial_Attribution is new Flyology_Bench.Measure_Batched (Serial_Attribution_Batch);
   procedure Worker_Attribution is new Flyology_Bench.Measure_Batched (Worker_Attribution_Batch);

   type Smoke_Case is (Reference_Case, Same_Case, Double_Case);

   procedure Multi_Batch (Which : Smoke_Case; Iterations : Flyology_Bench.Iteration_Count) is
   begin
      for Iteration in Flyology_Bench.Iteration_Count range 1 .. Iterations loop
         case Which is
            when Reference_Case | Same_Case =>
               Operation;
   ...
   procedure Multi_Benchmark is new Flyology_Bench.Compare_Many (Case_Id => Smoke_Case, Batch => Multi_Batch);
   procedure Put_Multi_Console is new Flyology_Bench.Reporters.Put_Multi_Comparison_Console (Smoke_Case);
   procedure Put_Multi_CSV is new Flyology_Bench.Reporters.Put_Multi_Comparison_CSV (Smoke_Case);
   procedure Put_Multi_Metrics_CSV is new
     Flyology_Bench.Reporters.Put_Multi_Comparison_Metrics_CSV (Smoke_Case);
   procedure Put_Multi_JSON is new Flyology_Bench.Reporters.Put_Multi_Comparison_JSON (Smoke_Case);
   ...
   procedure Observe_Quiescence_Progress
     (Name : String; Phase : Flyology_Bench.Progress_Phase; Completed : Natural; Total : Natural)
   is
      pragma Unreferenced (Name, Completed, Total);
   begin
      Saw_Quiescence_Progress :=
        Saw_Quiescence_Progress or else Phase = Flyology_Bench.Waiting_For_CPU_Quiescence;
   ...
   begin
      if Phase = Flyology_Bench.Sampling then
         Saw_Reference_Progress :=
           Saw_Reference_Progress or else Ada.Strings.Fixed.Index (Name, "reference case") > 0;
         Saw_Same_Progress := Saw_Same_Progress or else Ada.Strings.Fixed.Index (Name, "same case") > 0;
         Saw_Double_Progress := Saw_Double_Progress or else Ada.Strings.Fixed.Index (Name, "double case") > 0;
      end if;
   ...
   Interference_Config     : constant Flyology_Bench.Configuration :=
     (Config with delta Measurement_Time => 0.100, Minimum_Sample_Time => 0.010);
   ...
   Require_Hardware : constant Boolean :=
     Ada.Environment_Variables.Value ("FLYOLOGY_BENCH_REQUIRE_PERF", Default => "0") = "1";
   ...
   Attribution_Config : constant Flyology_Bench.Configuration :=
     (Config
      with delta
        ...
        Metrics             => Flyology_Bench.Time_Metrics or Flyology_Bench.Linux_Hardware_Metrics,
   ...
   function Spawn_Foreign_Load return GNAT.OS_Lib.Process_Id is
      Arguments : constant GNAT.OS_Lib.Argument_List :=
        (1 => new String'("-c"), 2 => new String'("i=0; while [ $i -lt 200000000 ]; do i=$((i+1)); done"));
      Burner    : constant GNAT.OS_Lib.Process_Id := GNAT.OS_Lib.Non_Blocking_Spawn ("/bin/sh", Arguments);
   begin
   ...
   Stop_Sibling_Load : Boolean := False
   with Volatile;
...
begin
   Check
     (Flyology_Bench.Default_Configuration.Confidence_Level_Percent = 95.0
      and then Flyology_Bench.Default_Configuration.Bootstrap_Resamples = 2_000,
      "default statistical settings changed");
   declare
      Too_Low  : Long_Float := 49.9
      with Volatile;
      Too_Many : Positive := 10_001
      with Volatile;
   begin
      begin
         declare
   ...
   begin
      begin
         Operation_Benchmark
           ((Config with delta Samples => 1_000, Bootstrap_Resamples => 10_000, Metrics => [others => True]),
            Discarded);
      exception
         when Constraint_Error =>
            Rejected := True;
      ...
      Check (Counter = Before, "bootstrap workload was rejected after benchmark execution");
   end;
   Operation_Benchmark
     ((Config
       with delta
   ...
   Operation_Benchmark
     ((Config
      ...
      First);
   Check (Saw_Quiescence_Progress, "CPU quiescence did not report its preflight phase");
   ...
   declare
      ...
      task body CPU_Burner is
         Deadline : constant Ada.Real_Time.Time := Ada.Real_Time.Clock + Ada.Real_Time.Milliseconds (300);
         Spin     : Natural := 0
         with Volatile;
      begin
   ...
   begin
      ...
      Check (Timed_Out, "CPU quiescence did not time out while a local task was busy");
   end;

   Check (Counter > 0, "ordinary benchmark did not run the operation");
   Check
     (Flyology_Bench.Samples (First) = Config.Samples, "ordinary benchmark returned the wrong sample count");
   Check
     (Flyology_Bench.Iterations_Per_Sample (First) > 1,
      "ordinary benchmark did not calibrate a nanosecond operation");
   Check
     (Flyology_Bench.Minimum_Nanoseconds (First) >= 0.0, "ordinary benchmark returned a negative duration");
   Check
     (Flyology_Bench.Clock_Resolution_Nanoseconds (First) > 0.0
      and then Flyology_Bench.Observed_Clock_Resolution_Nanoseconds (First) > 0.0,
      "native benchmark clock was not characterized");
   Check
     (Flyology_Bench.Median_Batch_Nanoseconds (First) >= Flyology_Bench.Clock_Resolution_Nanoseconds (First),
      "calibrated sample is shorter than the clock resolution");
   Check
     (Flyology_Bench.Mean_Confidence_Low_Nanoseconds (First) <= Flyology_Bench.Mean_Nanoseconds (First)
   ...
   Check
     (Flyology_Bench.Metric_Available (First, Flyology_Bench.Process_CPU_Time)
      and then Flyology_Bench.Metric_Available (First, Flyology_Bench.Thread_CPU_Time)
      and then Flyology_Bench.Metric_Available (First, Flyology_Bench.Process_RSS),
      "portable resource axes were not collected");
   Check
     (Flyology_Bench.Metric_Sample (First, Flyology_Bench.Wall_Time, 1)
      = Flyology_Bench.Sample_Nanoseconds (First, 1),
      "wall-time axis diverges from the retained latency sample");
   Check
     (Flyology_Bench.Metric_Statistics (First, Flyology_Bench.Process_RSS).Median > 0.0,
      "resident-memory samples are empty");
   Check
     (Flyology_Bench.Metric_Available (First, Flyology_Bench.Flyology_Dispatches),
   ...
   Check
     (not Flyology_Bench.Metric_Available (First, Flyology_Bench.CPU_Cycles)
      or else Flyology_Bench.Metric_Statistics (First, Flyology_Bench.CPU_Cycles).Median > 0.0,
      "available Linux CPU-cycle samples are empty");
   for Axis in Flyology_Bench.CPU_Cycles .. Flyology_Bench.Branch_Misses loop
      Check
        (Flyology_Bench.Metric_Status (First, Axis) /= Flyology_Bench.Metric_Not_Requested,
         "requested hardware axis has no availability status");
      Check
        (Flyology_Bench.Metric_Available (First, Axis)
         = (Flyology_Bench.Metric_Status (First, Axis) = Flyology_Bench.Metric_Collected),
         "hardware metric status disagrees with availability");
      if Flyology_Bench.Metadata.Operating_System /= "linux" then
         Check
           (Flyology_Bench.Metric_Status (First, Axis) = Flyology_Bench.Unsupported_Platform,
            "non-Linux hardware axis lacks unsupported-platform status");
      end if;
   end loop;
   Check
     (Flyology_Bench.Scope (Flyology_Bench.CPU_Cycles) = Flyology_Bench.Native_Task_Tree,
      "inherited hardware counter scope is not explicit");

   if Require_Hardware or else Flyology_Bench.Metric_Available (First, Flyology_Bench.CPU_Cycles) then
      declare
      ...
      begin
         ...
         if Require_Hardware then
            for Axis in Flyology_Bench.CPU_Cycles .. Flyology_Bench.Instructions_Per_Cycle loop
               Check
         ...
         if Flyology_Bench.Metric_Available (Serial_Result, Flyology_Bench.CPU_Cycles)
           and then Flyology_Bench.Metric_Available (Worker_Result, Flyology_Bench.CPU_Cycles)
         then
            declare
               Serial_Cycles : constant Long_Float :=
                 Flyology_Bench.Metric_Statistics (Serial_Result, Flyology_Bench.CPU_Cycles).Median;
               Worker_Cycles : constant Long_Float :=
                 Flyology_Bench.Metric_Statistics (Worker_Result, Flyology_Bench.CPU_Cycles).Median;
            begin
               ...
               Check
                 (Serial_Cycles > 0.0 and then Worker_Cycles >= 2.0 * Serial_Cycles,
                  "worker-task cycles were not counted: serial"
                  & Long_Float'Image (Serial_Cycles)
                  & " worker"
                  & Long_Float'Image (Worker_Cycles)
   ...
   Check (Flyology_Bench.Sample_Nanoseconds (Second, 1) >= 0.0, "raw sample is negative");

   Operation_Comparison (Config, Compared);
   Check
     (abs (Flyology_Bench.Compare_Metric (Compared, Flyology_Bench.Wall_Time).Change
           - Flyology_Bench.Relative_Time_Change_Percent (Compared))
      < 0.000_001,
      "wall-time metric comparison diverges from the latency comparison");
   Check
     (Flyology_Bench.Samples (Flyology_Bench.Reference_Measurement (Compared)) = Config.Samples,
      "comparison returned the wrong sample count");
   Check
     (Flyology_Bench.Median_Batch_Nanoseconds (Flyology_Bench.Reference_Measurement (Compared)) > 0.0
      and then Flyology_Bench.Median_Batch_Nanoseconds (Flyology_Bench.Contender_Measurement (Compared))
               > 0.0,
      "equal-time comparison produced a nonpositive timed slice");
   Check
     (Flyology_Bench.Reference_First_Samples (Compared) + Flyology_Bench.Contender_First_Samples (Compared)
   ...
   Check (Flyology_Bench.Sample_Speedup (Compared, 1) > 0.0, "comparison returned a nonpositive speedup");
   Check
   ...
   Operation_Comparison
     ((Config with delta Comparison_Batching => Flyology_Bench.Shared_Iterations), Shared_Compared);
   Check
     (Flyology_Bench.Iterations_Per_Sample (Flyology_Bench.Reference_Measurement (Shared_Compared))
      = Flyology_Bench.Iterations_Per_Sample (Flyology_Bench.Contender_Measurement (Shared_Compared)),
   ...
   Check
     (abs (Flyology_Bench.Metric_Statistics (Hooked, Flyology_Bench.Flyology_Dispatches).Median - 1.0)
      < 0.000_001,
      "scheduler axis included untimed setup or teardown hooks");
   Result_Benchmark (Config, Result_Produced);
   Check (Flyology_Bench.Median_Nanoseconds (Result_Produced) >= 0.0, "result-producing benchmark failed");
   ...
   Multi_Benchmark
     ((Config with delta Progress => Observe_Multi_Progress'Unrestricted_Access), Multi_Compared);
   Check (Flyology_Bench.Cases (Multi_Compared) = 3, "multi-way comparison returned the wrong case count");
   Check
     (Flyology_Bench.Geometric_Mean_Speedup (Flyology_Bench.Versus_Reference (Multi_Compared, 2)) > 0.0,
      "multi-way comparison returned an invalid speedup");
   Check
     (Saw_Reference_Progress and then Saw_Same_Progress and then Saw_Double_Progress,
      "multi-way sampling progress did not identify every case");
   Check
     (Flyology_Bench.Shootout_Schedule (Multi_Compared) = Flyology_Bench.Balanced_Rounds
      and then Flyology_Bench.Shootout_Batching (Multi_Compared) = Flyology_Bench.Equal_Time,
      "multi-way comparison did not retain its collection policies");

   Multi_Benchmark
     ((Config with delta Shootout_Scheduling => Flyology_Bench.Sequential_Cases), Sequential_Compared);
   Check
     (Flyology_Bench.Shootout_Schedule (Sequential_Compared) = Flyology_Bench.Sequential_Cases,
      "sequential shootout did not retain its schedule");

   Flyology_Bench.Baselines.Save (Baseline_Path, "volatile_increment", First, "smoke-host");
   declare
      ...
      Regression   : constant Flyology_Bench.Baselines.Regression :=
        Flyology_Bench.Baselines.Compare
          (Saved,
           First,
           Fingerprint              => "smoke-host",
           Confidence_Level_Percent => 80.0,
           Bootstrap_Resamples      => 150);
      Incompatible : constant Flyology_Bench.Baselines.Regression :=
        Flyology_Bench.Baselines.Compare (Saved, First, Fingerprint => "different-host");
   begin
      Check
        (Flyology_Bench.Baselines.Name (Saved) = "volatile_increment", "baseline name did not round trip");
      Check
   ...
   begin
      Flyology_Bench.Host_Lock.Try_Acquire (Machine_Claim, Outcome, Path => Lock_Path, Tool => "smoke tool");
      Check (Outcome = Flyology_Bench.Host_Lock.Acquired, "host CPU claim was not taken on a free path");
      Check (Flyology_Bench.Host_Lock.Held (Machine_Claim), "host CPU claim does not report itself held");

      Flyology_Bench.Host_Lock.Try_Acquire (Rival_Claim, Outcome, Path => Lock_Path);
      Check (Outcome = Flyology_Bench.Host_Lock.Busy, "a second machine-wide claim did not conflict");

      Identity := Flyology_Bench.Host_Lock.Read_Holder (Lock_Path);
      Check (Identity.Available, "claim file carried no readable content");
      Check
        (To_String (Identity.Convention_Id) = Flyology_Bench.Host_Lock.Convention,
         "claim file lost its convention identifier");
      Check (To_String (Identity.Tool) = "smoke tool", "claim file lost the holding tool identity");
      Check (To_String (Identity.Claim) = "all", "a machine-wide claim did not record itself as all");
      Check (Length (Identity.Working_Directory) > 0, "claim file recorded no working directory");
      Check (Length (Identity.Process_Id) > 0, "claim file recorded no process identifier");

      Flyology_Bench.Host_Lock.Release (Machine_Claim);
      Flyology_Bench.Host_Lock.Try_Acquire (Rival_Claim, Outcome, Path => Lock_Path);
      Check (Outcome = Flyology_Bench.Host_Lock.Acquired, "released claim did not become available");
   ...
   declare
      ...
      Outcome   : Flyology_Bench.Host_Lock.Acquisition;
   begin
      Flyology_Bench.Host_Lock.Try_Acquire (Pair, Outcome, CPUs => (0, 1), Path => Lock_Path);
      Check (Outcome = Flyology_Bench.Host_Lock.Acquired, "a CPU claim was not taken on a free path");
      Flyology_Bench.Host_Lock.Try_Acquire (Single, Outcome, CPUs => (1 => 2), Path => Lock_Path);
      Check (Outcome = Flyology_Bench.Host_Lock.Acquired, "a disjoint CPU claim was refused");
      Flyology_Bench.Host_Lock.Try_Acquire (Overlap, Outcome, CPUs => (1 => 1), Path => Lock_Path);
      Check (Outcome = Flyology_Bench.Host_Lock.Busy, "an overlapping CPU claim was allowed");
      Flyology_Bench.Host_Lock.Try_Acquire (Machine, Outcome, Path => Lock_Path);
      Check (Outcome = Flyology_Bench.Host_Lock.Busy, "a machine-wide claim ignored held CPU claims");
   ...
   begin
      ...
      Check (Report.Attribution = Flyology_Bench.Host_Wide, "unplaced run claimed core-scoped attribution");
      Check (Report.Contaminated_Samples = 0, "a 100 percent foreign limit still reported contamination");
      Check
        (Report.Retaken_Samples = 0 and then Report.Pauses = 0, "Observe discarded or suspended collection");
      Check
   ...
   begin
      ...
      Check (Report.Retaken_Samples <= 10, "Retake exceeded its configured budget");
      Check (Flyology_Bench.Samples (Retaken) = Config.Samples, "retaking samples changed the sample count");
      Check
        (not Report.Budget_Exhausted or else Report.Contaminated_Samples > 0,
   ...
   begin
      ...
      Check
        ((Report.Pauses > 0) = (Report.Paused_Nanoseconds > 0.0), "pause count and elapsed time disagree");
      Check ((Report.Pauses > 0) = (Report.Retaken_Samples > 0), "pause count and retaken samples disagree");
      Check (Flyology_Bench.Samples (Paused) = Config.Samples, "pausing changed the sample count");
      Stop_Foreign_Load (Burner);
   end;
   ...
   begin
      Operation_Benchmark
        ((Config
          with delta
            Placement    => (Enabled => True, CPU => 0, Include_Siblings => True, Require_Strict => False),
            Interference =>
              (Enabled                     => True,
               Response                    => Flyology_Bench.Observe,
               Maximum_Foreign_CPU_Percent => 100.0,
      ...
      if Report.Placement = Flyology_Bench.Placement_Strict then
         ...
         Check (Report.Watched_CPUs >= 1, "core-scoped attribution watched no CPU");
         ...
         if Sibling_List_Names_Several (0) then
            Check (Report.Watched_CPUs >= 2, "SMT siblings of the placed CPU were not watched");
         end if;
      else
         Check
   ...
   begin
      ...
      if Report.Placement = Flyology_Bench.Placement_Strict then
         Check (Report.Attribution_Diluted, "a sibling thread on the watched CPUs was not detected");
         Check
           (Report.Attribution = Flyology_Bench.Host_Wide,
            "diluted attribution did not fall back to host-wide");
         Check (Report.Watched_CPUs = 0, "host-wide attribution still reports watched CPUs");
      else
         Check (not Report.Attribution_Diluted, "attribution was diluted without ever being core-scoped");
      end if;
   end;
   ...
   begin
      begin
         Operation_Benchmark
           ((Config
             with delta
               Placement => (Enabled => True, CPU => 0, Include_Siblings => True, Require_Strict => True)),
            Discarded);
         Strict := Discarded;
      exception
      ...
      Check
        (Refused or else Flyology_Bench.Environment (Strict).Placement = Flyology_Bench.Placement_Strict,
   ...
   begin
      Operation_Benchmark
        ((Config
          with delta
            Host_Lock =>
              (Enabled               => True,
               Path                  => Ada.Strings.Unbounded.To_Unbounded_String (Lock_Path),
               Timeout               => 1.0,
               Poll_Interval         => 0.010,
      ...
      Check
        (Flyology_Bench.Environment (Claimed).Host_Lock
         in Flyology_Bench.Lock_Held .. Flyology_Bench.Lock_Namespace_Scoped,
         "a run on a free path did not take its host CPU claim");

      Flyology_Bench.Host_Lock.Try_Acquire (Rival, Outcome, Path => Lock_Path, Tool => "rival");
      Check
        (Outcome = Flyology_Bench.Host_Lock.Acquired, "the run did not release its claim when it finished");
      Operation_Benchmark
        ((Config
          with delta
            Host_Lock =>
         ...
         Blocked);
      Check
        (Flyology_Bench.Environment (Blocked).Host_Lock = Flyology_Bench.Lock_Busy,
      ...
      Check (Refused, "a required machine-wide claim proceeded without one");
      Flyology_Bench.Host_Lock.Release (Rival);
      Ada.Directories.Delete_File (Lock_Path);
   end;

   Flyology_Bench.Reporters.Put_Console ("volatile_increment", First);
   Flyology_Bench.Reporters.Put_Comparison_Console ("one_increment", "two_increments", Compared);
   ...
   Flyology_Bench.Reporters.Put_CSV ("maximum_resamples", Maximum_Resample_Result);
   ...
   Flyology_Bench.Reporters.Put_JSON ("maximum_resamples", Maximum_Resample_Result);
   Flyology_Bench.Reporters.Put_Comparison_CSV_Header;
   Flyology_Bench.Reporters.Put_Comparison_CSV ("one_increment", "two_increments", Compared);
   Flyology_Bench.Reporters.Put_Comparison_Metrics_CSV_Header;
   Flyology_Bench.Reporters.Put_Comparison_Metrics_CSV ("one_increment", "two_increments", Compared);
   Flyology_Bench.Reporters.Put_Comparison_JSON ("one_increment", "two_increments", Compared);
