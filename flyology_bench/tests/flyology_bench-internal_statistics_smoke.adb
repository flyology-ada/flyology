...
begin
   ...
   for Index in Values'First + 1 .. Values'Last loop
      Check (Values (Index - 1) <= Values (Index),
             "10,000-element statistical sort is not ordered");
   end loop;

   Check
     (Statistics.Lower_Tail (95.0) = 0.025,
      "confidence tail conversion changed");
   Check
     (Statistics.Percentile (Values, 0.5) = 4_999.5,
      "shared percentile calculation changed");
   ...
   Statistics.Add_Bootstrap_Work
     (Total     => Work,
      ...
      Resamples => 10_000,
      Intervals => 10,
      Context   => "exact-bound test");
   Check
     (Work = Statistics.Maximum_Bootstrap_Sample_Draws,
      "maximum permitted bootstrap work was rejected");
