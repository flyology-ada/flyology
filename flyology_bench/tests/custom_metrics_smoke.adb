...
procedure Custom_Metrics_Smoke is
   ...
   begin
      ...
      Snapshot := [others => (Status => Flyology_Bench.Metric_Not_Requested, others => <>)];
      Snapshot (1) :=
        (Status => Flyology_Bench.Metric_Collected, Counter_Value => Counter, Sample_Value => 0.0);
   end Counter_Probe;
   ...
   procedure Domain_Compare is new Flyology_Bench.Compare_Batched (Reference_Batch, Contender_Batch);

   type Multi_Case is (Reference_Case, Middle_Case, High_Case);
   procedure Multi_Batch (Which : Multi_Case; Iterations : Flyology_Bench.Iteration_Count) is
   begin
      for Index in 1 .. Iterations loop
         Counter := Counter + Long_Long_Integer (Multi_Case'Pos (Which) + 1);
      end loop;
   end Multi_Batch;
   procedure Domain_Multi is new Flyology_Bench.Compare_Many (Multi_Case, Multi_Batch);
   ...
   procedure Overflow_Probe (Snapshot : in out Flyology_Bench.Custom_Snapshot) is
   begin
      ...
      Snapshot (1) :=
        (Status        => Flyology_Bench.Metric_Collected,
         Counter_Value =>
           (if Overflow_Call mod 2 = 1 then Long_Long_Integer'First else Long_Long_Integer'Last),
         Sample_Value  => 0.0);
   end Overflow_Probe;

   Failed_Call : Natural := 0;
   Dummy       : Long_Long_Integer := 0
   with Volatile;
   ...
   begin
      ...
      Snapshot (1) :=
        (Status        =>
           (if Failed_Call = 2 then Flyology_Bench.Probe_Failed else Flyology_Bench.Metric_Collected),
         Counter_Value => Long_Long_Integer (Failed_Call),
         Sample_Value  => 0.0);
   end Partial_Probe;
   procedure Partial_Measure is new Flyology_Bench.Measure_Batched (Noop_Batch);

   procedure Raising_Probe (Snapshot : in out Flyology_Bench.Custom_Snapshot) is
      pragma Unreferenced (Snapshot);
   begin
      raise Program_Error with "synthetic probe failure";
   end Raising_Probe;

   procedure Composed_Probe (Snapshot : in out Flyology_Bench.Custom_Snapshot) is
   begin
      Snapshot := [others => (Status => Flyology_Bench.Metric_Not_Requested, others => <>)];
      Snapshot (1) := (Status => Flyology_Bench.Metric_Collected, Counter_Value => 0, Sample_Value => 11.0);
   end Composed_Probe;
   ...
   begin
      Snapshot := [others => (Status => Flyology_Bench.Metric_Not_Requested, others => <>)];
      Snapshot (1) :=
        (Status => Flyology_Bench.Metric_Collected, Sample_Value => Signed_Value, Counter_Value => 0);
   end Signed_Probe;
   ...
   procedure Signed_Compare is new Flyology_Bench.Compare_Batched (Zero_Batch, Negative_Batch);
   ...
   package Fake_Timer is new
     Flyology_Bench.Manual_Timing
       (Source_Name   => "deterministic_fake_ticks",
        Unit          => "quanta/op",
        Resolution    => 2.0,
        Scale_To_Unit => 0.5,
   ...
   package Invalid_Timer is new
     Flyology_Bench.Manual_Timing
       (Source_Name => "invalid_fake_clock",
        Unit        => "ticks/op",
        Resolution  => 1.0,
        Scope       => Flyology_Bench.Simulated_Clock,
        Attribution => Flyology_Bench.Unattributable,
   ...
   package Fake_Comparison is new
   ...
   procedure Ordered_Probe (Snapshot : in out Flyology_Bench.Custom_Snapshot) is
   begin
      Snapshot := [others => (Status => Flyology_Bench.Metric_Not_Requested, others => <>)];
      Snapshot (1) :=
        (Status => Flyology_Bench.Metric_Collected, Counter_Value => 0, Sample_Value => Ordered_Value);
   end Ordered_Probe;
   ...
   procedure Ordered_Measure is new Flyology_Bench.Measure_Batched (Ordered_Batch);

   Threshold_Value          : Long_Float := 100.0;
   Threshold_Contender_Call : Natural := 0;
   procedure Threshold_Probe (Snapshot : in out Flyology_Bench.Custom_Snapshot) is
   begin
      Snapshot := [others => (Status => Flyology_Bench.Metric_Not_Requested, others => <>)];
      Snapshot (1) :=
        (Status => Flyology_Bench.Metric_Collected, Counter_Value => 0, Sample_Value => Threshold_Value);
   end Threshold_Probe;
   procedure Threshold_Reference (Iterations : Flyology_Bench.Iteration_Count) is
   begin
   ...
   procedure Threshold_Contender (Iterations : Flyology_Bench.Iteration_Count) is
   begin
      Threshold_Contender_Call := Threshold_Contender_Call + 1;
      Threshold_Value := (if Threshold_Contender_Call mod 2 = 0 then 100.5 else 101.5);
      for Index in 1 .. Iterations loop
         Dummy := Dummy + Long_Long_Integer (Index);
      end loop;
   end Threshold_Contender;
   procedure Threshold_Compare is new
     Flyology_Bench.Compare_Batched (Threshold_Reference, Threshold_Contender);

   Failure_Status : Flyology_Bench.Metric_Availability := Flyology_Bench.Probe_Failed;
   procedure Failure_Probe (Snapshot : in out Flyology_Bench.Custom_Snapshot) is
   begin
      Snapshot := [others => (Status => Flyology_Bench.Metric_Not_Requested, others => <>)];
      Snapshot (1) := (Status => Failure_Status, Counter_Value => 0, Sample_Value => 0.0);
   end Failure_Probe;
   procedure Failure_Reference (Iterations : Flyology_Bench.Iteration_Count) is
   begin
   ...
   procedure Failure_Contender (Iterations : Flyology_Bench.Iteration_Count) is
   begin
   ...
   procedure Failure_Compare is new Flyology_Bench.Compare_Batched (Failure_Reference, Failure_Contender);
...
begin
   Check
     (Flyology_Bench.Metric_Availability'Pos (Flyology_Bench.Metric_Partially_Collected) = 7,
   ...
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Counter_Probe'Unrestricted_Access);
   Domain_Measure (Config, Measured);
   Check
     (Probe_Calls = 2 * Natural (Flyology_Bench.Samples (Measured)),
      "custom probe did not run exactly twice per retained batch");
   for Index in 1 .. Flyology_Bench.Samples (Measured) loop
      Check
        (Flyology_Bench.Custom_Metric_Sample (Measured, 1, Index) = 3.0,
         "cumulative delta or iteration normalization is wrong");
   end loop;
   Check
     (Flyology_Bench.Custom_Metric_Attribution (Measured, 1) = Flyology_Bench.Shared_Process_Window,
   ...
   Check
     (Item.Available and then Item.Method = Flyology_Bench.Relative_Ratio,
      "positive custom counter did not use paired relative comparison");
   Check
     (Item.Change > 99.9 and then Item.Change < 100.1,
      "paired custom metric lost reference/contender schedule");
   Check (Item.Verdict = Flyology_Bench.Reference_Better, "lower-is-better custom direction was not applied");
   Flyology_Bench.Reporters.Put_Comparison_Console
     ("reference", "contender", Compared, Style => Flyology_Bench.Reporters.Plain);

   Counter := 0;
   Domain_Multi (Config, Multi);
   Item := Flyology_Bench.Compare_Custom_Metric (Flyology_Bench.Versus_Reference (Multi, 3), 1);
   Check (Item.Available and then Item.Change > 199.9, "multi-way custom metric lost common rounds");
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      ...
      "events/op",
      Flyology_Bench.Caller_Defined_Window,
      Flyology_Bench.Unattributable,
      Flyology_Bench.Diagnostic);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Reset_Probe'Unrestricted_Access);
   Reset_Measure (Config, Measured);
   Check
     (Flyology_Bench.Custom_Metric_Status (Measured, 1) = Flyology_Bench.Counter_Reset,
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      ...
      Flyology_Bench.Unattributable,
      Flyology_Bench.Diagnostic);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Overflow_Probe'Unrestricted_Access);
   Reset_Measure (Config, Measured);
   Check
     (Flyology_Bench.Custom_Metric_Status (Measured, 1) = Flyology_Bench.Conversion_Overflow,
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      ...
      Flyology_Bench.Unattributable,
      Flyology_Bench.Diagnostic);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Partial_Probe'Unrestricted_Access);
   Partial_Measure (Config, Measured);
   Check
     (Flyology_Bench.Custom_Metric_Status (Measured, 1) = Flyology_Bench.Metric_Partially_Collected,
      "partial availability was collapsed");
   Check
     (Flyology_Bench.Custom_Metric_Statistics (Measured, 1).Samples = 9,
      "failed sample was substituted with zero");
   Check
     (Flyology_Bench.Custom_Metric_Sample_Status (Measured, 1, 1) = Flyology_Bench.Probe_Failed,
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      ...
      Flyology_Bench.Unattributable,
      Flyology_Bench.Diagnostic);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Raising_Probe'Unrestricted_Access);
   Partial_Measure (Config, Measured);
   Check
     (Flyology_Bench.Custom_Metric_Status (Measured, 1) = Flyology_Bench.Probe_Failed,
   ...
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Signed_Probe'Unrestricted_Access);
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      ...
      Normalization => Flyology_Bench.Per_Batch,
      Comparison    => Flyology_Bench.Absolute);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Ordered_Probe'Unrestricted_Access);
   Ordered_Measure (Config, Measured);
   Check
     (Flyology_Bench.Custom_Metric_Statistics (Measured, 1).Confidence_Low = 5.0
      and then Flyology_Bench.Custom_Metric_Statistics (Measured, 1).Confidence_High = 5.0,
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      ...
      Semantics     => Flyology_Bench.Absolute_Sample,
      Normalization => Flyology_Bench.Per_Batch);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Threshold_Probe'Unrestricted_Access);
   ...
   Check
     (Flyology_Bench.Custom_Metric_Sample (Measured, 1, 1) = 3.5,
      "manual timer conversion or normalization is wrong");
   Check
     (Flyology_Bench.Custom_Metric_Resolution (Measured, 1)
      = 1.0 / Long_Float (Flyology_Bench.Iterations_Per_Sample (Measured)),
      "manual timer resolution was not normalized");
   Check (Flyology_Bench.Primary_Timing_Axis (Measured) = 1, "primary timing axis was not discoverable");
   Check
     (Flyology_Bench.Custom_Metric_Timing_Source (Measured, 1) = "deterministic_fake_ticks",
      "manual timing source identity was lost");
   Check
     (Flyology_Bench.Metric_Available (Measured, Flyology_Bench.Wall_Time),
      "harness wall time disappeared under alternate timing");
   Flyology_Bench.Reporters.Put_Console
     ("manual timer", Measured, Style => Flyology_Bench.Reporters.Plain, Include_Telemetry => False);
   ...
   declare
      ...
      package Concurrent_Timer is new
        Flyology_Bench.Manual_Timing
          (Source_Name => "task_local_fake_ticks",
           Unit        => "ticks/op",
           Resolution  => 1.0,
           Scope       => Flyology_Bench.Simulated_Clock,
           Attribution => Flyology_Bench.Unattributable,
      ...
      begin
         ...
         for Index in 1 .. Flyology_Bench.Samples (Local_Result) loop
            if Flyology_Bench.Custom_Metric_Sample (Local_Result, 1, Index) /= Long_Float (Id) then
               Passed := False;
            end if;
         end loop;
   ...
   begin
      ...
      Check (All_Passed, "one manual timing instance shared mutable state across invocations");
   end;

   Invalid_Timer.Measure (Base, Measured);
   Check
     (Flyology_Bench.Custom_Metric_Status (Measured, 1) = Flyology_Bench.Invalid_Value,
   ...
   Check
     (Flyology_Bench.Custom_Metric_Status (Measured, 1) = Flyology_Bench.Probe_Failed
      and then Flyology_Bench.Custom_Metric_Status (Measured, 2) = Flyology_Bench.Metric_Collected,
   ...
   Check
     (not Item.Available
      and then Flyology_Bench.Custom_Metric_Status (Flyology_Bench.Reference_Measurement (Compared), 1)
               = Flyology_Bench.Probe_Failed
      and then Flyology_Bench.Custom_Metric_Status (Flyology_Bench.Contender_Measurement (Compared), 1)
               = Flyology_Bench.Probe_Failed,
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      "long_custom_metric_identity_over_32",
      "custom-units-per-batch",
      Flyology_Bench.Caller_Defined_Window,
      Flyology_Bench.Unattributable,
      Flyology_Bench.Diagnostic,
      Semantics     => Flyology_Bench.Absolute_Sample,
      Normalization => Flyology_Bench.Per_Batch,
      Comparison    => Flyology_Bench.Absolute);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Composed_Probe'Unrestricted_Access);
   Fake_Timer.Measure (Config, Measured);
   Check
   ...
   Fake_Comparison.Compare (Config, Compared);
   Check
     (Flyology_Bench.Primary_Timing_Axis (Flyology_Bench.Reference_Measurement (Compared)) = 2,
      "paired primary timing axis was not discoverable");
   Check
     (Flyology_Bench.Custom_Metric_Resolution (Flyology_Bench.Reference_Measurement (Compared), 2)
      = 1.0
        / Long_Float (Flyology_Bench.Iterations_Per_Sample (Flyology_Bench.Reference_Measurement (Compared)))
      and then Flyology_Bench.Custom_Metric_Resolution (Flyology_Bench.Contender_Measurement (Compared), 2)
               = 1.0
                 / Long_Float
                     (Flyology_Bench.Iterations_Per_Sample (Flyology_Bench.Contender_Measurement (Compared))),
      "paired manual timer resolutions do not match their denominators");
   Item := Flyology_Bench.Compare_Custom_Metric (Compared, 2);
   Check
     (Item.Available and then Item.Change > 59.9 and then Item.Change < 60.1,
      "manual timing paired comparison is not iteration-normalized");
   Check (Item.Verdict = Flyology_Bench.Reference_Better, "manual timing direction was not retained");
   Flyology_Bench.Reporters.Put_Comparison_Console
     ("old timer", "new timer", Compared, Style => Flyology_Bench.Reporters.Plain);
   ...
   Flyology_Bench.Register_Custom_Metric
     (Config.Custom_Metrics,
      ...
      Normalization => Flyology_Bench.Per_Batch,
      Comparison    => Flyology_Bench.Absolute);
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Failure_Probe'Unrestricted_Access);
   Failure_Compare (Config, Failed_Compared);
   Flyology_Bench.Reporters.Put_Comparison_Console
     ("failed old", "failed new", Failed_Compared, Style => Flyology_Bench.Reporters.Plain);
   ...
   Flyology_Bench.Reporters.Put_Extended_Metrics_CSV ("failed_custom", Failed_Measured);
   Flyology_Bench.Reporters.Put_Metrics_NDJSON ("failed_custom", Failed_Measured);
   Flyology_Bench.Reporters.Put_Extended_Comparison_Metrics_CSV_Header;
   Flyology_Bench.Reporters.Put_Extended_Comparison_Metrics_CSV ("old", "new", Compared);
   Flyology_Bench.Reporters.Put_Comparison_Metrics_NDJSON ("old", "new", Compared);
   Flyology_Bench.Reporters.Put_Extended_Comparison_Metrics_CSV ("failed old", "failed new", Failed_Compared);
   Flyology_Bench.Reporters.Put_Comparison_Metrics_NDJSON ("failed old", "failed new", Failed_Compared);
   ...
   begin
      ...
      begin
         Flyology_Bench.Register_Custom_Metric
           (Registry,
            "bad_timer",
            "ticks/op",
            Flyology_Bench.Simulated_Clock,
            Flyology_Bench.Unattributable,
            Flyology_Bench.Lower_Is_Better,
      ...
      exception
         when Flyology_Bench.Capacity_Error =>
            null;
      end;
   end;
