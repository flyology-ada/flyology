   ...
   procedure Measure (Config : Configuration := Default_Configuration; Result : out Measurement) is
      ...
      begin
         ...
         if Status = Metric_Collected then
            if Raw /= Raw or else abs Raw > Long_Float'Last or else Raw < 0.0 then
               Latest.Status := Invalid_Value;
            elsif Scale_To_Unit /= Scale_To_Unit
      ...
      procedure Underlying is new Flyology_Bench.Measure_Batched (Run);
   begin
      ...
      Result.Custom_Data.Data.Descriptors (Timer_Axis).Resolution_Value :=
        Source_Resolution / Long_Float (Iterations_Per_Sample (Result));
   end Measure;
