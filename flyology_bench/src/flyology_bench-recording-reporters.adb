...
package body Flyology_Bench.Recording.Reporters is
   ...
   function Image (Value : Long_Float; Aft : Natural := 3) return String is
      Buffer : String (1 .. 64);
   begin
      Float_IO.Put (Buffer, Value, Aft => Aft, Exp => (if abs Value >= 1.0E+40 then 4 else 0));
   ...
   function Duration_Image (Value : Duration) return String
   is (Image (Long_Float (Value), 3) & " s");
   ...
   function Method_Name (Value : Metric_Comparison_Method) return String
   is (if Value = Relative_Ratio then "relative_ratio" else "absolute_difference");

   function Outcome_Name (Value : Sample_Outcome) return String
   is (Ada.Characters.Handling.To_Lower (Value'Image));
   ...
   function JSON_String (Value : String) return String is
      ...
      function Hex_Digit (Number : Natural) return Character
      is (if Number < 10
   ...
   begin
      ...
      for Character of Value loop
         case Standard.Character'Pos (Character) is
            ...
            when Standard.Character'Pos ('"') | Standard.Character'Pos ('\') =>
               Append ('\');
               Append (Character);
   ...
   begin
      ...
      Ada.Text_IO.Put_Line (File, "-- " & Name (Result) & " / recorded spans --");
      if Color then
      ...
      Ada.Text_IO.Put_Line
        (File,
         "session     | "
         & Duration_Image (Session_Elapsed (Result))
      ...
      Ada.Text_IO.Put_Line
        (File,
         ...
         & " abandoned    "
         & Natural_Image
             (Outcomes (Result, Failure) + Outcomes (Result, Timeout) + Outcomes (Result, Cancelled))
         & " errors");
      ...
      Ada.Text_IO.Put_Line
        (File,
         Pad ("axis", 30) & " " & Pad ("attribution", 26) & " samples      median         p95        status");
      if Color then
         Ada.Text_IO.Put (File, Reset);
      end if;
      for Axis in Metric_Axis loop
         if Metric_Status (Result, Axis) /= Metric_Not_Requested then
            declare
               ...
               Status  : constant Metric_Availability := Metric_Status (Result, Axis);
            begin
               if Color then
                  Ada.Text_IO.Put (File, (if Status = Metric_Collected then Green else Yellow));
               end if;
               Ada.Text_IO.Put
                 (File,
                  Pad (Metric_Name (Axis), 30)
                  & " "
               ...
               if Summary.Available then
                  Ada.Text_IO.Put
                    (File, Left_Pad (Image (Summary.Median), 12) & "  " & Left_Pad (Image (Summary.P95), 12));
               else
               ...
               Ada.Text_IO.Put_Line
                 (File,
                  ...
                  & (if Scope_Changed_Samples (Result, Axis) > 0
                     then " (" & Natural_Image (Scope_Changed_Samples (Result, Axis)) & " scope changed)"
                     else ""));
               if Color then
                  Ada.Text_IO.Put (File, Reset);
               end if;
   ...
   begin
      Ada.Text_IO.Put_Line
        (File,
         "name,sample_semantics,confidence_level_percent,"
   ...
   procedure Put_CSV
     (Result : Recorded_Measurement; File : Ada.Text_IO.File_Type := Ada.Text_IO.Standard_Output) is
   begin
      for Axis in Metric_Axis loop
         if Metric_Status (Result, Axis) /= Metric_Not_Requested then
            declare
   ...
   procedure Put_Samples_CSV_Header (File : Ada.Text_IO.File_Type := Ada.Text_IO.Standard_Output) is
   begin
      Ada.Text_IO.Put_Line
        (File, "name,sample_semantics,observation,outcome,axis,scope," & "attribution,unit,status,value");
   ...
   begin
      for Index in 1 .. Retained (Result) loop
         for Axis in Metric_Axis loop
            if Metric_Status (Result, Axis) /= Metric_Not_Requested then
               declare
                  Status : constant Metric_Availability := Sample_Metric_Status (Result, Index, Axis);
               begin
                  Ada.Text_IO.Put_Line
                    (File,
                     CSV_String (Name (Result))
      ...
      for Index in 1 .. Retained (Result) loop
         ...
         Ada.Text_IO.Put
           (File,
            ...
            & Natural_Image (Observation_Id (Result, Index))
            & ",""outcome"":"
            & JSON_String (Outcome_Name (Outcome_At (Result, Index)))
            & ",""metrics"":[");
         First := True;
         for Axis in Metric_Axis loop
            if Metric_Status (Result, Axis) /= Metric_Not_Requested then
   ...
   begin
      ...
      Ada.Text_IO.Put_Line
        (File,
         "-- independent comparison: " & Reference_Name (Result) & " vs " & Contender_Name (Result) & " --");
      Ada.Text_IO.Put_Line
        (File,
         Image (Confidence_Level_Percent (Result))
         & "% confidence,"
      ...
      if Wall_Comparison_Available (Result) then
         ...
         Ada.Text_IO.Put_Line
           (File,
            "change   "
            & Image (Relative_Change_Percent (Result))
            & "%  ["
         ...
         Ada.Text_IO.Put_Line (File, "wall     unavailable; no speedup or relative change");
      end if;
      if Color then
         Ada.Text_IO.Put (File, Reset);
      end if;
      Ada.Text_IO.Put_Line
        (File, Pad ("axis", 28) & " " & Pad ("reference", 20) & " " & Pad ("contender", 20) & " result");
      for Axis in Metric_Axis loop
         declare
            Reference_Status : constant Metric_Availability := Reference_Metric_Status (Result, Axis);
            Contender_Status : constant Metric_Availability := Contender_Metric_Status (Result, Axis);
            Item             : constant Metric_Comparison_Result := Compare_Metric (Result, Axis);
         begin
            if Reference_Status /= Metric_Not_Requested or else Contender_Status /= Metric_Not_Requested then
               Ada.Text_IO.Put_Line
                 (File,
                  Pad (Metric_Name (Axis), 28)
                  & " "
                  ...
                  & " "
                  & (if Item.Available
                     then Image (Item.Change) & " " & Metric_Verdict_Name (Item.Verdict)
                     else "unavailable"));
            end if;
         end;
      end loop;
   ...
   begin
      Ada.Text_IO.Put_Line
        (File,
         "reference,contender,comparison_design,"
   ...
   procedure Put_Comparison_CSV
     (Result : Recorded_Comparison; File : Ada.Text_IO.File_Type := Ada.Text_IO.Standard_Output) is
   begin
      for Axis in Metric_Axis loop
         declare
            Item             : constant Metric_Comparison_Result := Compare_Metric (Result, Axis);
         ...
         begin
            if Reference_Status /= Metric_Not_Requested or else Contender_Status /= Metric_Not_Requested then
               Ada.Text_IO.Put_Line
                 (File,
                  CSV_String (Reference_Name (Result))
                  & ","
                  & CSV_String (Contender_Name (Result))
                  & ",independent,"
                  & Image (Confidence_Level_Percent (Result))
                  & ","
                  & Natural_Image (Bootstrap_Resamples (Result))
   ...
   begin
      Ada.Text_IO.Put
        (File,
         "{""reference"":"
         & JSON_String (Reference_Name (Result))
         & ",""contender"":"
         & JSON_String (Contender_Name (Result))
         ...
         & ",""speedup"":"
         & (if Wall_Comparison_Available (Result) then Image (Speedup (Result)) else "null")
         & ",""speedup_ci_low"":"
         & (if Wall_Comparison_Available (Result) then Image (Speedup_Confidence_Low (Result)) else "null")
         & ",""speedup_ci_high"":"
         & (if Wall_Comparison_Available (Result) then Image (Speedup_Confidence_High (Result)) else "null")
         & ",""relative_change_percent"":"
         & (if Wall_Comparison_Available (Result) then Image (Relative_Change_Percent (Result)) else "null")
         & ",""relative_change_ci_low"":"
         & (if Wall_Comparison_Available (Result)
            then Image (Relative_Change_Confidence_Low (Result))
            else "null")
         & ",""relative_change_ci_high"":"
         & (if Wall_Comparison_Available (Result)
            then Image (Relative_Change_Confidence_High (Result))
            else "null")
         & ",""verdict"":"
