...
package body Flyology_Bench.Baselines is
   ...
   function C_Write
     (Descriptor : Interfaces.C.int; Buffer : System.Address; Count : Interfaces.C.size_t)
   ...
   function C_Rename (Old_Path : System.Address; New_Path : System.Address) return Interfaces.C.int;
   ...
   procedure Validate_Text (Value : String; Field : String; Allow_Empty : Boolean := True) is
   begin
   ...
   procedure Hash_Text (State : in out Interfaces.Unsigned_64; Value : String) is
   begin
      for Character of Value loop
         State := (State xor Interfaces.Unsigned_64 (Standard.Character'Pos (Character))) * FNV_Prime;
      end loop;
   end Hash_Text;

   procedure Hash_Line (State : in out Interfaces.Unsigned_64; Value : String) is
   begin
      Hash_Text (State, Value);
      State := (State xor Interfaces.Unsigned_64 (Standard.Character'Pos (ASCII.LF))) * FNV_Prime;
   end Hash_Line;
   ...
   begin
      for Position in reverse Result'Range loop
         Result (Position) := Hex_Digits (Natural (Work and 16#F#) + Hex_Digits'First);
   ...
   begin
      if Value'Length /= 16 then
         raise Baseline_Format_Error with "baseline checksum must contain exactly 16 hexadecimal digits";
      end if;
      for Character of Value loop
         if Character in '0' .. '9' then
            Digit :=
              Interfaces.Unsigned_64 (Standard.Character'Pos (Character) - Standard.Character'Pos ('0'));
         elsif Character in 'a' .. 'f' then
            Digit :=
              Interfaces.Unsigned_64 (10 + Standard.Character'Pos (Character) - Standard.Character'Pos ('a'));
         elsif Character in 'A' .. 'F' then
            Digit :=
              Interfaces.Unsigned_64 (10 + Standard.Character'Pos (Character) - Standard.Character'Pos ('A'));
         else
            raise Baseline_Format_Error with "baseline checksum contains a non-hexadecimal digit";
         end if;
   ...
   begin
      return Safe_Math.Interpolate (Ordered (Lower), Ordered (Upper), Weight);
   end Percentile;
   ...
   procedure Publish_Atomically (Path : String; Content : String) is
      Template   : aliased Interfaces.C.char_array := Interfaces.C.To_C (Path & ".tmp.XXXXXX");
   ...
   begin
      ...
      if Descriptor < 0 then
         raise Baseline_IO_Error with "cannot create temporary baseline beside " & Path;
      end if;
      while Written < Content'Length loop
         ...
         begin
            if Amount <= 0 then
               raise Baseline_IO_Error with "cannot write temporary baseline beside " & Path;
            end if;
            Written := Written + Natural (Amount);
         end;
      end loop;
      if C_Fsync (Descriptor) /= 0 then
         raise Baseline_IO_Error with "cannot flush temporary baseline beside " & Path;
      end if;
      if C_Close (Descriptor) /= 0 then
         Descriptor := -1;
         raise Baseline_IO_Error with "cannot close temporary baseline beside " & Path;
      end if;
      Descriptor := -1;
      if C_Rename (Template'Address, Target'Address) /= 0 then
         raise Baseline_IO_Error with "cannot atomically publish baseline at " & Path;
      end if;
      Published := True;
   exception
      when others =>
   ...
   procedure Save (Path : String; Name : String; Result : Measurement; Fingerprint : String := "") is
      Effective_Fingerprint : constant String :=
        (if Fingerprint'Length = 0 then Flyology_Bench.Metadata.Fingerprint else Fingerprint);
   ...
   begin
      ...
      Validate_Text (Effective_Fingerprint, "baseline fingerprint", Allow_Empty => False);
      Validate_Text (Clock_Backend (Result), "clock backend", Allow_Empty => False);
         ...
         begin
            if not Safe_Math.Is_Supported_Sample (Value) then
               raise Constraint_Error
                 with "baseline samples must be within the supported nanosecond " & "range";
            end if;
            Append_Field
              ("sample." & Ada.Strings.Fixed.Trim (Sample_Index'Image (Index), Ada.Strings.Both),
   ...
   function Load (Path : String) return Baseline is
      ...
      begin
         if Seen then
            raise Baseline_Format_Error with "baseline contains duplicate " & Field & " field";
         end if;
         Seen := True;
      end Duplicate;
      ...
      begin
         if Ada.Text_IO.End_Of_File (File) then
            raise Baseline_Format_Error with "baseline is truncated before " & Description;
         end if;
      ...
      exception
         when Constraint_Error =>
            raise Baseline_Format_Error with "baseline sample_count is malformed or out of range";
      end Parse_Count;
      ...
      exception
         when Constraint_Error =>
            raise Baseline_Format_Error with "baseline sample index is malformed or out of range";
      end Parse_Index;

      function Parse_Sample (Value : String; Index : Sample_Index) return Long_Float is
         Parsed : Long_Float;
      begin
      ...
      begin
         Store
           (Required_Line ("legacy benchmark name"),
            Result.Name_Data,
            Result.Name_Length,
            "legacy baseline benchmark_name",
            Allow_Empty => True);
         Store
           (Required_Line ("legacy fingerprint"),
            Result.Fingerprint_Data,
            Result.Fingerprint_Length,
            "legacy baseline fingerprint",
            Allow_Empty => True);
         Store
           (Required_Line ("legacy clock backend"),
            Result.Backend_Id,
            Result.Backend_Id_Length,
            "legacy baseline clock_backend");
         Result.Sample_Total := Parse_Count (Required_Line ("legacy sample_count"));
         for Index in Sample_Index range 1 .. Sample_Index (Result.Sample_Total) loop
            Result.Values (Index) :=
              Parse_Sample (Required_Line ("legacy sample" & Sample_Index'Image (Index)), Index);
         end loop;
         if not Ada.Text_IO.End_Of_File (File) then
            raise Baseline_Format_Error with "legacy baseline contains unexpected trailing data";
         end if;
      end Parse_Legacy;
   begin
      Validate_Text (Path, "baseline path", Allow_Empty => False);
      ...
      begin
         if First = Legacy_Magic then
            ...
            raise Baseline_Format_Error with "unsupported benchmark baseline format";
         end if;
      ...
      loop
         ...
         begin
            if Equal = 0 then
               raise Baseline_Format_Error with "baseline field has no '=' separator";
            end if;
            declare
            ...
            begin
               ...
               if Key = "schema_version" then
                  ...
                  if Value /= "2" then
                     raise Baseline_Format_Error
                       with "unsupported benchmark baseline schema version " & Value;
                  end if;
               elsif Key = "benchmark_name" then
                  Duplicate ("benchmark_name", Seen_Name);
                  Store (Value, Result.Name_Data, Result.Name_Length, "baseline benchmark_name");
               elsif Key = "fingerprint" then
                  Duplicate ("fingerprint", Seen_Fingerprint);
                  Store (Value, Result.Fingerprint_Data, Result.Fingerprint_Length, "baseline fingerprint");
               elsif Key = "clock_backend" then
                  Duplicate ("clock_backend", Seen_Backend);
                  Store (Value, Result.Backend_Id, Result.Backend_Id_Length, "baseline clock_backend");
               elsif Key = "sample_unit" then
                  Duplicate ("sample_unit", Seen_Unit);
                  if Value /= "nanoseconds_per_operation" then
                     raise Baseline_Format_Error with "unsupported baseline sample_unit " & Value;
                  end if;
               elsif Key = "comparison_design" then
                  Duplicate ("comparison_design", Seen_Design);
                  if Value /= "independent_runs" then
                     raise Baseline_Format_Error with "unsupported baseline comparison_design " & Value;
                  end if;
               elsif Key = "bootstrap_method" then
                  Duplicate ("bootstrap_method", Seen_Method);
                  if Value /= Bootstrap_Method_Text then
                     raise Baseline_Format_Error with "unsupported baseline bootstrap_method " & Value;
                  end if;
               elsif Key = "sample_count" then
                  Duplicate ("sample_count", Seen_Count);
                  Result.Sample_Total := Parse_Count (Value);
               elsif Key'Length > 7 and then Key (Key'First .. Key'First + 6) = "sample." then
                  declare
                     Index : constant Sample_Index := Parse_Index (Key (Key'First + 7 .. Key'Last));
                  begin
                     if Samples_Seen (Index) then
                        raise Baseline_Format_Error
                          with "baseline contains duplicate sample" & Sample_Index'Image (Index);
                     end if;
                     Samples_Seen (Index) := True;
                     Result.Values (Index) := Parse_Sample (Value, Index);
                  end;
               else
                  raise Baseline_Format_Error with "baseline contains unknown field " & Key;
               end if;
            end;
         end;
      end loop;
      ...
      begin
         if End_Line /= "end=" & Footer then
            if Ada.Strings.Fixed.Index (End_Line, "checksum=") = 1 then
               raise Baseline_Format_Error with "baseline contains duplicate checksum field";
            end if;
            raise Baseline_Format_Error with "baseline commit footer is malformed";
         end if;
      end;
      if not Ada.Text_IO.End_Of_File (File) then
         raise Baseline_Format_Error with "baseline contains unexpected trailing data";
      end if;
      if not Seen_Schema then
         ...
         raise Baseline_Format_Error with "baseline is missing comparison_design";
      ...
      for Index in Sample_Index loop
         if Index <= Sample_Index (Result.Sample_Total) then
            if not Samples_Seen (Index) then
               raise Baseline_Format_Error with "baseline is missing sample" & Sample_Index'Image (Index);
            end if;
         elsif Samples_Seen (Index) then
            raise Baseline_Format_Error
              with "baseline contains sample beyond sample_count at" & Sample_Index'Image (Index);
         end if;
      end loop;
      Ada.Text_IO.Close (File);
      return Result;
   exception
      ...
      when Error : Constraint_Error =>
         ...
         raise Baseline_Format_Error
           with "malformed benchmark baseline: " & Ada.Exceptions.Exception_Message (Error);
   ...
   is
      ...
      Saved_Block_Length    : constant Positive :=
        Positive'Max (2, Positive (Long_Float'Ceiling (Math.Sqrt (Long_Float (Saved_Count)))));
      Current_Block_Length  : constant Positive :=
        Positive'Max (2, Positive (Long_Float'Ceiling (Math.Sqrt (Long_Float (Current_Count)))));
   begin
      if Practical_Threshold_Percent < 0.0 or else Practical_Threshold_Percent >= 100.0 then
         raise Constraint_Error with "practical threshold must be in the range 0 .. 100 percent";
      end if;
      Result.Is_Compatible :=
        Clock_Backend (Current) = Saved.Backend_Id (1 .. Saved.Backend_Id_Length)
        and then Effective_Fingerprint = Saved.Fingerprint_Data (1 .. Saved.Fingerprint_Length);
      ...
      for Index in 1 .. Saved_Count loop
         declare
            Value : constant Long_Float := Saved.Values (Sample_Index (Index));
         begin
            if not Safe_Math.Is_Supported_Sample (Value) then
               raise Baseline_Comparison_Error
                 with "saved baseline contains a sample outside the supported " & "nanosecond range";
            end if;
            if not Safe_Math.Can_Add_To_Sum (Saved_Sum, Value)
              or else not Safe_Math.Is_Supported_Sum (Saved_Sum + Value)
            then
               raise Baseline_Comparison_Error
                 with "saved baseline sample sum is outside the supported range";
            end if;
      ...
      for Index in 1 .. Current_Count loop
         declare
            Value : constant Long_Float := Sample_Nanoseconds (Current, Sample_Index (Index));
         begin
            if not Safe_Math.Is_Supported_Sample (Value) then
               raise Baseline_Comparison_Error
                 with "current measurement contains a sample outside the supported " & "nanosecond range";
            end if;
            if not Safe_Math.Can_Add_To_Sum (Current_Sum, Value)
              or else not Safe_Math.Is_Supported_Sum (Current_Sum + Value)
            then
               raise Baseline_Comparison_Error
                 with "current measurement sample sum is outside the supported " & "range";
            end if;
      ...
      then
         raise Baseline_Comparison_Error with "baseline sample sum is below the supported range";
      end if;
      ...
      then
         raise Baseline_Comparison_Error with "baseline mean is outside the supported nanosecond range";
      end if;
      Result.Speedup_Value := Safe_Math.Ratio (Saved_Mean, Current_Mean);
      ...
      for Resample in Bootstrap'Range loop
            ...
            while Drawn < Saved_Count loop
               declare
                  Start : constant Positive :=
                    Natural (Next_Random (State) mod Interfaces.Unsigned_64 (Saved_Count)) + 1;
               begin
                  for Offset in 0 .. Saved_Block_Length - 1 loop
                     exit when Drawn = Saved_Count;
                     declare
                        Value : constant Long_Float :=
                          Saved.Values (Sample_Index (((Start - 1 + Offset) mod Saved_Count) + 1));
                     begin
                        if not Safe_Math.Can_Add_To_Sum (Saved_Sum, Value)
                          or else not Safe_Math.Is_Supported_Sum (Saved_Sum + Value)
                        then
                           raise Baseline_Comparison_Error
                             with "bootstrap baseline sum is outside the supported " & "range";
                        end if;
                        Safe_Math.Add_To_Sum (Saved_Sum, Value);
                     end;
                     Drawn := Drawn + 1;
         ...
         begin
            while Drawn < Current_Count loop
               declare
                  Start : constant Positive :=
                    Natural (Next_Random (State) mod Interfaces.Unsigned_64 (Current_Count)) + 1;
               begin
                  for Offset in 0 .. Current_Block_Length - 1 loop
                     ...
                     declare
                        Value : constant Long_Float :=
                          Sample_Nanoseconds
                            (Current, Sample_Index (((Start - 1 + Offset) mod Current_Count) + 1));
                     begin
                        if not Safe_Math.Can_Add_To_Sum (Current_Sum, Value)
                          or else not Safe_Math.Is_Supported_Sum (Current_Sum + Value)
                        then
                           raise Baseline_Comparison_Error
                             with "bootstrap current sum is outside the supported " & "range";
                        end if;
                        Safe_Math.Add_To_Sum (Current_Sum, Value);
                     end;
                     Drawn := Drawn + 1;
         ...
         if Saved_Sum < Safe_Math.Minimum_Sample_Nanoseconds
           or else Current_Sum < Safe_Math.Minimum_Sample_Nanoseconds
         then
            raise Baseline_Comparison_Error with "bootstrap sample sum is below the supported range";
         end if;
         Saved_Mean := Safe_Math.Mean (Saved_Sum, Saved.Sample_Total);
         Current_Mean := Safe_Math.Mean (Current_Sum, Samples (Current));
         if not Safe_Math.Is_Supported_Sample (Saved_Mean)
           or else not Safe_Math.Is_Supported_Sample (Current_Mean)
         then
            raise Baseline_Comparison_Error with "bootstrap mean is outside the supported nanosecond range";
         end if;
         Bootstrap (Resample) := Safe_Math.Ratio (Saved_Mean, Current_Mean);
      end loop;
      Sort (Bootstrap);
      declare
         Tail : constant Long_Float := Internal_Statistics.Lower_Tail (Confidence_Level_Percent);
      begin
      ...
      then
         raise Baseline_Comparison_Error with "baseline interval is outside the supported speedup range";
      end if;
      declare
      ...
      begin
         ...
         Result.Verdict_Value :=
           Safe_Math.Classify (Change_Low, Change_High, Threshold_Percentage (Practical_Threshold_Percent));
      end;
      return Result;
   end Compare;

   function Name (Saved : Baseline) return String
   is (Saved.Name_Data (1 .. Saved.Name_Length));

   function Fingerprint (Saved : Baseline) return String
   is (Saved.Fingerprint_Data (1 .. Saved.Fingerprint_Length));

   function Clock_Backend (Saved : Baseline) return String
   is (Saved.Backend_Id (1 .. Saved.Backend_Id_Length));

   function Compatible (Result : Regression) return Boolean
   is (Result.Is_Compatible);

   function Speedup (Result : Regression) return Long_Float
   is (Result.Speedup_Value);
   ...
   is
      ...
      procedure Set_Result (Value : Gate_Status; Rejects : Boolean; Message : String) is
      begin
      ...
      procedure Set_Incompatible (Issue : Compatibility_Issue; Message : String) is
      begin
         Result.Issue_Value := Issue;
         Set_Result (Incompatible_Baseline, Policy.On_Incompatible = Reject, Message);
      end Set_Incompatible;
   begin
      Validate_Text (Path, "baseline path", Allow_Empty => False);
      Validate_Text (Current_Name, "current benchmark name", Allow_Empty => False);
      Validate_Text (Effective_Fingerprint, "current fingerprint", Allow_Empty => False);
      ...
      if not Ada.Directories.Exists (Path) then
         Set_Result
           (Missing_Baseline, Policy.On_Missing = Reject, "baseline artifact does not exist at " & Path);
         return Result;
      end if;
      ...
      begin
         Saved := Load (Path);
         Result.Baseline_Name_Data := Unbounded.To_Unbounded_String (Name (Saved));
         if Name (Saved) /= Current_Name then
            Set_Incompatible
              (Benchmark_Identity_Mismatch, "baseline benchmark identity does not match current identity");
            return Result;
         elsif Flyology_Bench.Baselines.Fingerprint (Saved) /= Effective_Fingerprint then
            Set_Incompatible
            ...
            Set_Incompatible (Clock_Backend_Mismatch, "baseline clock backend does not match current run");
            return Result;
         end if;
         ...
         Result.Regression_Data :=
           Compare
             (Saved,
              ...
              Practical_Threshold_Percent => Policy.Practical_Threshold_Percent,
         ...
         case Verdict (Result.Regression_Data) is
            when Contender_Faster       =>
               Set_Result
                 (Improvement,
                  False,
                  "confidence interval establishes an improvement beyond the " & "practical threshold");

            when Practically_Equivalent =>
               Set_Result
                 (Practical_Equivalence, False, "confidence interval is within the practical threshold");

            when Inconclusive           =>
               Set_Result
                 (Inconclusive,
                  Policy.On_Inconclusive = Reject,
                  "confidence interval establishes neither a practical result " & "nor a regression");

            when Reference_Faster       =>
               Set_Result
                 (Regressed,
                  True,
                  "confidence interval establishes a regression beyond the " & "practical threshold");
         end case;
         return Result;
      exception
         when Ada.IO_Exceptions.Name_Error =>
            Set_Result
              (Missing_Baseline,
               ...
               "baseline artifact disappeared before it could be loaded at " & Path);
            return Result;
         when Error : Baseline_Format_Error =>
            Set_Result
              (Invalid_Baseline, Policy.On_Invalid = Reject, Ada.Exceptions.Exception_Message (Error));
            return Result;
         when Error : Baseline_IO_Error =>
            Set_Result (Baseline_Error, Policy.On_Invalid = Reject, Ada.Exceptions.Exception_Message (Error));
            return Result;
   ...
   begin
      case Result.Status_Value is
         ...
         when Incompatible_Baseline =>
            return "incompatible_baseline";

         when Invalid_Baseline      =>
   ...
   function Time_Change_Confidence_Low (Result : Gate_Result) return Long_Float is
   begin
      Require_Statistics (Result);
      return Time_Change_Confidence_Low (Result.Regression_Data);
   end Time_Change_Confidence_Low;

   function Time_Change_Confidence_High (Result : Gate_Result) return Long_Float is
   begin
      Require_Statistics (Result);
