...
package body Flyology_Bench.Sweeps is
   ...
   function Finite (Value : Long_Float) return Boolean is
     (Value = Value and then abs Value <= Long_Float'Last);
   ...
   function Error_Text (Value : String) return Errors.Bounded_String is
      Last : constant Natural :=
        Natural'Min (Value'Length, Max_Error_Length);
   begin
      if Last = 0 then
         return Errors.Null_Bounded_String;
      end if;
      return Errors.To_Bounded_String
   ...
   begin
      if Label'Length > Max_Label_Length
        or else (Label'Length > 0 and then not Valid_Name (Label))
      then
      ...
      return
        (Kind_Value  => Kind,
         Exact       => Value,
         Label_Value => Labels.To_Bounded_String (Label));
   end Point;

   function Kind (Item : Parameter_Point) return Parameter_Kind is
     (Item.Kind_Value);

   function Value (Item : Parameter_Point) return Exact_Value is
     (Item.Exact);

   function Label (Item : Parameter_Point) return String is
     (Labels.To_String (Item.Label_Value));

   function Identity (Item : Parameter_Point) return String is
     ((case Item.Kind_Value is
         when Size_Parameter  => "size:",
         when Count_Parameter => "count:") & Image (Item.Exact));

   procedure Append (Set : in out Point_Set; Item : Parameter_Point) is
   begin
      for Index in 1 .. Set.Count loop
         if Set.Items (Index).Kind_Value = Item.Kind_Value
           and then Set.Items (Index).Exact = Item.Exact
         then
            raise Constraint_Error with
              "duplicate sweep point identity " & Identity (Item);
         end if;
      end loop;
   ...
   function Length (Set : Point_Set) return Natural is (Set.Count);
   ...
   is
      Exact : Exact_Value;
   begin
      if not Finite (Value) or else Value <= 0.0
        or else Value /= Long_Float'Floor (Value)
      then
         raise Constraint_Error with
           "work amount must be finite, positive, and integral";
      end if;
      begin
   ...
   begin
      if Unit = Caller_Named then
         ...
         raise Constraint_Error with
           "built-in work units cannot carry a caller name";
      end if;
      return
        (Exact      => Value,
         Kind_Value => Unit,
         Name_Value => Units.To_Bounded_String (Name),
         Scale      => Scaling);
   end Work;

   function Raw_Value (Amount : Work_Amount) return Exact_Value is
     (Amount.Exact);

   function Unit_Kind (Amount : Work_Amount) return Work_Unit_Kind is
     (Amount.Kind_Value);

   function Unit_Name (Amount : Work_Amount) return String is
     (case Amount.Kind_Value is
         when Items        => "items",
         when Bytes        => "bytes",
         when Caller_Named => Units.To_String (Amount.Name_Value));

   function Display_Scale (Amount : Work_Amount) return Display_Scaling is
     (Amount.Scale);

   function Scale_Power (Amount : Work_Amount) return Natural is
      Base  : constant Long_Float :=
        (if Amount.Scale = Decimal_Scaling then 1_000.0 else 1_024.0);
   ...
   function Prefix
     (Scale : Display_Scaling;
      Power : Natural) return String
   is
   begin
      if Scale = Decimal_Scaling then
         return
           (case Power is
               when 0 => "", when 1 => "k", when 2 => "M", when 3 => "G",
               when 4 => "T", when 5 => "P", when others => "E");
      else
         return
           (case Power is
               when 0 => "", when 1 => "Ki", when 2 => "Mi", when 3 => "Gi",
               when 4 => "Ti", when 5 => "Pi", when others => "Ei");
      end if;
   end Prefix;
   ...
   begin
      ...
      Result.Operations_Low :=
        Nanoseconds_Per_Second / Mean_Confidence_High_NS;
      Result.Operations_High :=
        Nanoseconds_Per_Second / Mean_Confidence_Low_NS;
   ...
   function Availability
     (Summary : Throughput_Summary) return Throughput_Availability is
     (Summary.State);
   ...
   function Operations_Confidence_Low
     (Summary : Throughput_Summary) return Long_Float is
     (Summary.Operations_Low);
   function Operations_Confidence_High
     (Summary : Throughput_Summary) return Long_Float is
     (Summary.Operations_High);
   ...
   function Length (Result : Ordinary_Sweep_Result) return Natural is
     (Result.Count);
   function Length (Result : Paired_Sweep_Result) return Natural is
     (Result.Count);

   function Element
     (Result : Ordinary_Sweep_Result;
      Index  : Positive) return Ordinary_Point_Result is
   begin
   ...
   function Element
     (Result : Paired_Sweep_Result;
      Index  : Positive) return Paired_Point_Result is
   begin
      if Index > Result.Count then
   ...
   function Stopped_Early (Result : Ordinary_Sweep_Result) return Boolean is
     (Result.Stopped);
   function Stopped_Early (Result : Paired_Sweep_Result) return Boolean is
     (Result.Stopped);
   function Parameter (Result : Ordinary_Point_Result) return Parameter_Point is
     (Result.Point_Value);
   ...
   function Status (Result : Paired_Point_Result) return Point_Status is
     (Result.State);
   function Failure_Message (Result : Ordinary_Point_Result) return String is
     (Errors.To_String (Result.Message));
   function Failure_Message (Result : Paired_Point_Result) return String is
     (Errors.To_String (Result.Message));
   function Data (Result : Ordinary_Point_Result) return Measurement is
     (Result.Measurement_Value);
   function Data (Result : Paired_Point_Result) return Comparison is
     (Result.Comparison_Value);
   function Throughput
     (Result : Ordinary_Point_Result) return Throughput_Summary is (Result.Rate);
   function Reference_Throughput
     (Result : Paired_Point_Result) return Throughput_Summary is
     (Result.Reference_Rate);
   function Contender_Throughput
     (Result : Paired_Point_Result) return Throughput_Summary is
     (Result.Contender_Rate);

   function Progress_Identity
     (Case_Name : String;
      Item      : Parameter_Point) return String is
     (if Case_Name'Length = 0 then Identity (Item)
      else Case_Name & "/" & Identity (Item));

   function Remaining_Budget
     (Started : Ada.Real_Time.Time;
      Limit   : Nonnegative_Duration) return Duration
   is
   ...
   is
      ...
      Whole_Limit : constant Nonnegative_Duration :=
        (if Policy.Budget = Whole_Sweep_Budget
         then Config.Maximum_Sampling_Time else 0.0);

      function Should_Stop (State : Point_Status) return Boolean is
        (State not in Point_Measured | Point_Dry_Run
         and then Policy.Failure = Stop_On_Point_Failure);
   begin
      Result.Count := 0;
         ...
         begin
            ...
            Point_Config.Progress_Name :=
              Ada.Strings.Unbounded.To_Unbounded_String
                (Progress_Identity (Case_Name, Item));
            ...
            then
               ...
               if Remaining < Duration'Small then
                  Item_Result.State := Point_Budget_Exhausted;
                  Item_Result.Message := Error_Text
                    ("whole-sweep budget exhausted during point setup");
               else
                  Point_Config.Maximum_Sampling_Time :=
                    Nonnegative_Duration (Remaining);
               end if;
            end if;
            ...
            if Item_Result.State = Point_Not_Run then
               if Policy.Mode = Dry_Run then
                  ...
                  begin
                     ...
                     Item_Result.Contender_Rate := Derive_Throughput
                       (Item_Result.Work_Value,
                        Contender_Measurement (Item_Result.Comparison_Value));
                     if Availability (Item_Result.Reference_Rate)
                       = Throughput_Overflow
                       or else Availability (Item_Result.Contender_Rate)
                         = Throughput_Overflow
                     then
