...
package body Flyology_Bench.Recording is
   ...
   function To_Fixed (Value : String) return Fixed_Name is
      Result : Fixed_Name;
      Count  : constant Natural := Natural'Min (Value'Length, Maximum_Name_Length);
   begin
      Result.Length := Count;
      if Count > 0 then
   ...
   function To_String (Value : Fixed_Name) return String
   is (if Value.Length = 0 then "" else String (Value.Data (1 .. Value.Length)));

   procedure Read_Resources
     (Values : out Resource_Values; Mask : out Interfaces.Unsigned_64; OK : out Boolean) is
   begin
            ...
            declare
               Index : constant Natural :=
                 (case Axis is
                    ...
                    when others        => 0);
            begin
               Result := Result or Interfaces.Shift_Left (Interfaces.Unsigned_64'(1), Index);
            end;
         end if;
      end loop;
   ...
   protected type Sample_Store (Capacity : Retained_Capacity) is
      ...
      procedure Finish_Span
        (Value : Raw_Sample; Outcome : Sample_Outcome; Policy : Retention_Policy; Seed : Long_Long_Integer);
      procedure Abandon_Span;
      procedure Copy_To
        (Target : in out Recorded_Measurement; Requested : Metric_Set; Elapsed_NS : Interfaces.Unsigned_64);
   ...
   function Percentile (Ordered : Float_Array; P : Long_Float) return Long_Float
   ...
   protected body Sample_Store is
            ...
            case Policy is
               ...
               when Reservoir =>
                  if not Reservoir_Ready then
                     Reservoir_State := Interfaces.Unsigned_64 (Seed) xor 16#9E37_79B9_7F4A_7C15#;
                  ...
                  Slot :=
                    Natural (Next_Random (Reservoir_State) mod Interfaces.Unsigned_64 (Observed_Total)) + 1;
      ...
      procedure Copy_To
        (Target : in out Recorded_Measurement; Requested : Metric_Set; Elapsed_NS : Interfaces.Unsigned_64) is
      begin
            ...
            if not Requested (Axis) then
               ...
               for Index in 1 .. Retained_Total loop
                  if Data (Index).Scope_Changed (Axis) then
                     Target.Scope_Changed (Axis) := Target.Scope_Changed (Axis) + 1;
                  end if;
                  if Data (Index).Valid (Axis) then
                     Target.Valid_Counts (Axis) := Target.Valid_Counts (Axis) + 1;
                     Target.Values (Axis) (Target.Valid_Counts (Axis)) := Data (Index).Values (Axis);
                  else
                     Target.Invalid_Counts (Axis) := Target.Invalid_Counts (Axis) + 1;
                     if Target.Valid_Counts (Axis) = 0
                       and then Data (Index).Status (Axis) /= Metric_Not_Requested
                     then
                        Target.Statuses (Axis) := Data (Index).Status (Axis);
                     end if;
                  end if;
               end loop;
               if Target.Valid_Counts (Axis) = Retained_Total and then Retained_Total > 0 then
      ...
      begin
         ...
         Data.Failures := Outcome_Totals (Failure) + Outcome_Totals (Timeout) + Outcome_Totals (Cancelled);
         ...
         for Index in First .. Retained_Total loop
            ...
            then
               ...
               CPU_Sum :=
                 CPU_Sum
                 + 100.0
                   * Sample_Store.Data (Index).Values (Process_CPU_Time)
   ...
   type Store_Array is array (Benchmark_Capacity range <>) of Sample_Store_Access;
   ...
   protected type Session_Control is
      ...
      procedure Begin_Span (Allowed : out Boolean; Overlapped : out Boolean);
      procedure End_Span;
      procedure Claim_Perf_Close (Claimed : out Boolean);
      function Running return Boolean;
      procedure Times
        (Started : out Interfaces.Unsigned_64; Ended : out Interfaces.Unsigned_64; Active : out Boolean);
   private
   ...
   protected body Session_Control is
      ...
      begin
         Allowed := not Is_Running and then not Is_Starting and then not Ever_Started;
      end Register;
      ...
      procedure Begin_Span (Allowed : out Boolean; Overlapped : out Boolean) is
      begin
         Allowed := Is_Running;
      ...
      begin
         Claimed :=
           Ever_Started and then not Is_Running and then Active_Total = 0 and then not Perf_Close_Claimed;
         if Claimed then
      ...
      procedure Times
        (Started : out Interfaces.Unsigned_64; Ended : out Interfaces.Unsigned_64; Active : out Boolean) is
      begin
         Started := Started_At;
         Ended := Ended_At;
         Active := Is_Running;
   ...
   is limited new Recorder_Backend with record
   ...
   function Backend (Object : Recorder) return Concrete_Backend_Access
   is (Concrete_Backend_Access (Object.Guard.Backend));
   ...
   procedure Free_Backend is new Ada.Unchecked_Deallocation (Concrete_Backend, Concrete_Backend_Access);
   procedure Free_Store is new Ada.Unchecked_Deallocation (Sample_Store, Sample_Store_Access);
   procedure Free_Values is new Ada.Unchecked_Deallocation (Metric_Value_Array, Metric_Value_Array_Access);
   procedure Free_Samples is new
     Ada.Unchecked_Deallocation (Recorded_Sample_Array, Recorded_Sample_Array_Access);
         ...
         if Result.Valid_Counts (Axis) > 0 then
            declare
               ...
               Random  : Interfaces.Unsigned_64 :=
                 16#243F_6A88_85A3_08D3# xor Interfaces.Unsigned_64 (Metric_Axis'Pos (Axis) + 1);
               ...
               for Resample in Means'Range loop
                  ...
                  for Draw in 1 .. Count loop
                     Sum :=
                       Sum
                       + Result.Values (Axis)
                           (Natural (Next_Random (Random) mod Interfaces.Unsigned_64 (Count)) + 1);
                  end loop;
   ...
   overriding
   procedure Initialize (Object : in out Recorder_Guard) is
   begin
      Object.Backend := new Concrete_Backend (Object.Maximum_Benchmarks, Object.Retained_Samples);
   end Initialize;

   overriding
   procedure Finalize (Object : in out Recorder_Guard) is
   ...
   procedure Register (Object : in out Recorder; Name : String; Item : out Benchmark) is
   ...
   begin
      ...
      Data.Stores (Benchmark_Capacity (Data.Count)) := new Sample_Store (Data.Capacity);
      Item :=
   ...
   procedure Start (Object : in out Recorder; Config : Configuration := Default_Configuration) is
      Data    : constant Concrete_Backend_Access := Backend (Object);
      Allowed : Boolean;
   begin
      ...
      begin
         ...
         Data.Perf_Session := 0;
         declare
            Requested : constant Interfaces.Unsigned_64 := Hardware_Mask (Config.Metrics);
         begin
   ...
   procedure Begin_Sample (Object : in out Recorder; Item : Benchmark; Value : in out Span) is
      Data           : constant Concrete_Backend_Access := Backend (Object);
   ...
   begin
      ...
      if Resource_Metrics_Requested (Data.Config.Metrics) then
         Read_Resources (Value.Resource_Before, Value.Resource_Before_Mask, Resource_OK);
         if not Resource_OK then
            Value.Resource_Before_Mask := 0;
         end if;
      end if;
      if Scheduler_Metrics_Requested (Data.Config.Metrics) and then Data.Config.Scheduler_Probe /= null then
   ...
   procedure Finish (Value : in out Span; Outcome : Sample_Outcome := Success) is
      ...
      procedure Set_Delta (Axis : Metric_Axis; Index : Natural) is
      begin
         ...
         then
            ...
            Sample.Values (Axis) := Long_Float (After (Index) - Value.Resource_Before (Index));
         end if;
      end Set_Delta;

      procedure Set_Scheduler
        (Axis : Metric_Axis; Before : Interfaces.Unsigned_64; After_Value : Interfaces.Unsigned_64) is
      begin
         Sample.Status (Axis) := Probe_Failed;
         if Value.Scheduler_Before.Available and then Scheduler_After.Available and then After_Value >= Before
         then
            Sample.Valid (Axis) := True;
            Sample.Status (Axis) := Metric_Collected;
      ...
      begin
         ...
         if Perf_After (Index) < Value.Perf_Before (Index)
           or else Perf_Enabled_After (Index) < Value.Perf_Enabled_Before (Index)
           or else Perf_Running_After (Index) < Value.Perf_Running_Before (Index)
         then
            Sample.Status (Axis) := Probe_Failed;
            return;
         end if;
         Counted_Delta := Perf_After (Index) - Value.Perf_Before (Index);
         Enabled_Delta := Perf_Enabled_After (Index) - Value.Perf_Enabled_Before (Index);
         Running_Delta := Perf_Running_After (Index) - Value.Perf_Running_Before (Index);
         ...
         Sample.Status (Axis) := Metric_Collected;
         Sample.Values (Axis) :=
           Long_Float (Counted_Delta) * Long_Float (Enabled_Delta) / Long_Float (Running_Delta);
      end Set_Perf;
   begin
      ...
      if Data.Config.Metrics (Wall_Time) then
         ...
         Sample.Values (Wall_Time) := Long_Float (Finished_At - Value.Started_At);
      end if;
      ...
      if Data.Config.Metrics (Thread_CPU_Time) then
         if Finished_Thread /= 0 and then Finished_Thread = Value.Native_Thread then
      ...
      if Data.Config.Metrics (Process_RSS) then
         if Mask_Has (After_Mask, Resident_Bytes_Index) then
            ...
            Sample.Values (Process_RSS) := Long_Float (After (Resident_Bytes_Index));
         end if;
      end if;
      ...
      if Hardware_Mask (Data.Config.Metrics) /= 0 then
         if Finished_Thread = 0 or else Finished_Thread /= Value.Native_Thread then
            for Axis in CPU_Cycles .. Branch_Misses loop
               if Data.Config.Metrics (Axis) then
            ...
            if Data.Config.Metrics (CPU_Cycles) or else Data.Config.Metrics (Instructions_Per_Cycle) then
               Set_Perf (CPU_Cycles, Counters.Cycles_Index);
            end if;
            if Data.Config.Metrics (Instructions) or else Data.Config.Metrics (Instructions_Per_Cycle) then
            ...
            if Data.Config.Metrics (Instructions_Per_Cycle) then
               Sample.Status (Instructions_Per_Cycle) := Sample.Status (CPU_Cycles);
               ...
               then
                  ...
                  Sample.Status (Instructions_Per_Cycle) := Sample.Status (Instructions);
               end if;
            end if;
         end if;
      end if;
      ...
      if Scheduler_Metrics_Requested (Data.Config.Metrics) then
         Set_Scheduler (Flyology_Dispatches, Value.Scheduler_Before.Dispatches, Scheduler_After.Dispatches);
         Set_Scheduler
           (Flyology_Poll_Batches, Value.Scheduler_Before.Poll_Batches, Scheduler_After.Poll_Batches);
         Set_Scheduler
           (Flyology_Poll_Events, Value.Scheduler_Before.Poll_Events, Scheduler_After.Poll_Events);
         Set_Scheduler (Flyology_Wakeups, Value.Scheduler_Before.Wakeups, Scheduler_After.Wakeups);
         Set_Scheduler
           (Flyology_Migrations,
            Value.Scheduler_Before.Migrations_In + Value.Scheduler_Before.Migrations_Out,
   ...
   overriding
   procedure Finalize (Object : in out Span) is
      Data : Concrete_Backend_Access;
   begin
   ...
   is (To_String (Item.Label));

   procedure Snapshot (Object : Recorder; Item : Benchmark; Result : out Recorded_Measurement) is
      Data       : constant Concrete_Backend_Access := Backend (Object);
   ...
   begin
      ...
      Result.Samples := new Recorded_Sample_Array (1 .. Natural (Data.Capacity));
      for Axis in Metric_Axis loop
         if Data.Config.Metrics (Axis) then
            Result.Values (Axis) := new Metric_Value_Array (1 .. Natural (Data.Capacity));
         end if;
      end loop;
      ...
      Data.Stores (Benchmark_Capacity (Item.Identifier)).Copy_To (Result, Data.Config.Metrics, Elapsed);
         ...
         if Data.Config.Metrics (Axis) then
            Result.Attributions (Axis) :=
              (case Axis is
                 ...
                 when Process_CPU_Time | Process_RSS .. Filesystem_Output_Operations => Shared_Process_Window,
   ...
   overriding
   procedure Adjust (Object : in out Recorded_Measurement) is
   begin
      ...
      for Axis in Metric_Axis loop
         if Object.Values (Axis) /= null then
            Object.Values (Axis) := new Metric_Value_Array'(Object.Values (Axis).all);
         end if;
      end loop;
   end Adjust;

   overriding
   procedure Finalize (Object : in out Recorded_Measurement) is
   begin
   ...
   function Observation_Id (Result : Recorded_Measurement; Index : Positive) return Natural is
   begin
      Check_Sample_Index (Result, Index);
      return Result.Samples (Index).Observation;
   end Observation_Id;

   function Outcome_At (Result : Recorded_Measurement; Index : Positive) return Sample_Outcome is
   begin
      Check_Sample_Index (Result, Index);
   ...
   function Sample_Metric_Status
     (Result : Recorded_Measurement; Index : Positive; Axis : Metric_Axis) return Metric_Availability is
   begin
      Check_Sample_Index (Result, Index);
   ...
   function Sample_Metric_Value
     (Result : Recorded_Measurement; Index : Positive; Axis : Metric_Axis) return Long_Float is
   begin
      Check_Sample_Index (Result, Index);
   ...
   begin
      ...
      declare
         ...
         Random         : Interfaces.Unsigned_64 :=
           ...
           xor Interfaces.Unsigned_64 (Seed)
      ...
      begin
         ...
         Relative := Item.Reference_Median > 0.0 and then Item.Contender_Median > 0.0;
         for Index in 1 .. Reference_Count loop
            Relative := Relative and then Reference.Values (Axis) (Index) >= 0.0;
         end loop;
         for Index in 1 .. Contender_Count loop
            Relative := Relative and then Contender.Values (Axis) (Index) >= 0.0;
         end loop;

         if Relative then
            Item.Method := Relative_Ratio;
            Item.Change := 100.0 * (Item.Contender_Median / Item.Reference_Median - 1.0);
         else
            ...
            begin
               ...
               loop
                  for Index in Reference_Draw'Range loop
                     Reference_Draw (Index) :=
                       Reference.Values (Axis)
                         (Natural (Next_Random (Random) mod Interfaces.Unsigned_64 (Reference_Count)) + 1);
                  end loop;
                  Sort (Reference_Draw);
                  Reference_Median := Percentile (Reference_Draw, 0.5);
                  exit when not Relative or else Reference_Median > 0.0 or else Attempts = 31;
                  Attempts := Attempts + 1;
               end loop;
               for Index in Contender_Draw'Range loop
                  Contender_Draw (Index) :=
                    Contender.Values (Axis)
                      (Natural (Next_Random (Random) mod Interfaces.Unsigned_64 (Contender_Count)) + 1);
               end loop;
               Sort (Contender_Draw);
               Contender_Median := Percentile (Contender_Draw, 0.5);
               if Relative and then Reference_Median > 0.0 then
                  Bootstrap (Draw) := 100.0 * (Contender_Median / Reference_Median - 1.0);
               elsif Relative then
         ...
         Sort (Bootstrap);
         Item.Confidence_Low := Percentile (Bootstrap, Lower_Tail (Confidence));
         Item.Confidence_High := Percentile (Bootstrap, 1.0 - Lower_Tail (Confidence));
         if Direction (Axis) = Diagnostic then
            Item.Verdict := Metric_Diagnostic;
         elsif Relative and then Item.Confidence_Low >= -Threshold and then Item.Confidence_High <= Threshold
         then
            Item.Verdict := Metric_Practically_Equivalent;
         elsif Item.Confidence_High < 0.0 then
            Item.Verdict :=
              (if Direction (Axis) = Lower_Is_Better then Contender_Better else Reference_Better);
         elsif Item.Confidence_Low > 0.0 then
            Item.Verdict :=
              (if Direction (Axis) = Lower_Is_Better then Reference_Better else Contender_Better);
         end if;
      end;
   ...
   begin
      if Practical_Threshold_Percent < 0.0 then
         raise Constraint_Error with "practical comparison threshold must not be negative";
      end if;
      for Axis in Metric_Axis loop
         if Reference.Statuses (Axis) = Metric_Collected
         ...
         then
            Internal_Statistics.Add_Bootstrap_Work
              (Total     => Work,
               Samples   => Reference.Valid_Counts (Axis) + Contender.Valid_Counts (Axis),
      ...
      if Wall.Available and then Wall.Reference_Median > 0.0 and then Wall.Contender_Median > 0.0 then
         Result.Wall_Available := True;
         Result.Speedup_Value := Wall.Reference_Median / Wall.Contender_Median;
         Result.Change_Value := Wall.Change;
         Result.Change_Low := Wall.Confidence_Low;
         Result.Change_High := Wall.Confidence_High;
         Result.Speedup_Low :=
           (if Result.Change_High <= -100.0
            then Long_Float'Last
            else 1.0 / (1.0 + Result.Change_High / 100.0));
         Result.Speedup_High :=
           (if Result.Change_Low <= -100.0 then Long_Float'Last else 1.0 / (1.0 + Result.Change_Low / 100.0));
   ...
   is (To_String (Result.Reference_Label));
   function Contender_Name (Result : Recorded_Comparison) return String
   is (To_String (Result.Contender_Label));
   ...
   function Speedup_Confidence_Low (Result : Recorded_Comparison) return Long_Float is
   begin
      Require_Wall_Comparison (Result);
      return Result.Speedup_Low;
   end Speedup_Confidence_Low;
   function Speedup_Confidence_High (Result : Recorded_Comparison) return Long_Float is
   begin
      Require_Wall_Comparison (Result);
      return Result.Speedup_High;
   end Speedup_Confidence_High;
   function Relative_Change_Percent (Result : Recorded_Comparison) return Long_Float is
   begin
      Require_Wall_Comparison (Result);
      return Result.Change_Value;
   end Relative_Change_Percent;
   function Relative_Change_Confidence_Low (Result : Recorded_Comparison) return Long_Float is
   begin
      Require_Wall_Comparison (Result);
      return Result.Change_Low;
   end Relative_Change_Confidence_Low;
   function Relative_Change_Confidence_High (Result : Recorded_Comparison) return Long_Float is
   begin
      Require_Wall_Comparison (Result);
   ...
   function Format_Number (Value : Long_Float; Aft : Natural := 1) return String is
      package Local_Float_IO is new Ada.Text_IO.Float_IO (Long_Float);
      Buffer : String (1 .. 32);
   begin
      Local_Float_IO.Put (Buffer, Value, Aft => Aft, Exp => 0);
   ...
   begin
      if Count > 0 then
         Result (Width - Count + 1 .. Width) := Value (Value'Last - Count + 1 .. Value'Last);
      end if;
      return Result;
   end Left_Pad;
   ...
   begin
      Result (Width - Count + 1 .. Width) := Image (Image'Last - Count + 1 .. Image'Last);
      return Result;
   end Number;
   ...
   begin
      ...
      if Use_ANSI and then Previous > 0 then
         Ada.Text_IO.Put
           (Escape & "[" & Ada.Strings.Fixed.Trim (Natural'Image (Previous), Ada.Strings.Both) & "A");
      end if;
      Read_Resources (Resources, Resource_Mask, Resource_OK);
      if Resource_OK and then Mask_Has (Resource_Mask, Process_CPU_Index) then
         Current_CPU := Resources (Process_CPU_Index);
         if Previous_Wall > 0 and then Current_Wall > Previous_Wall and then Current_CPU >= Previous_CPU then
            CPU_Percent :=
              100.0 * Long_Float (Current_CPU - Previous_CPU) / Long_Float (Current_Wall - Previous_Wall);
         end if;
         Previous_CPU := Current_CPU;
         Previous_Wall := Current_Wall;
      end if;
      if Resource_OK and then Mask_Has (Resource_Mask, Resident_Bytes_Index) then
      ...
      Ada.Text_IO.Put_Line
        ("fly recorder  elapsed "
         ...
         & "   cpu "
         & Left_Pad (Format_Number (CPU_Percent), 6)
         & "% / "
         & Left_Pad (Format_Number (CPU_Percent / 100.0), 5)
         & " cores"
         & "   rss "
         & Left_Pad (Format_Memory (RSS), 10));
      if Use_ANSI then
         Ada.Text_IO.Put (Escape & "[0m" & Escape & "[2K");
      end if;
      Ada.Text_IO.Put_Line (Pad ("benchmark", 24) & "     done  live  errors   median      p95");
      for Index in 1 .. Data.Count loop
   ...
   begin
      loop
         ...
         or
            ...
            Render_Dashboard (Owner, Use_ANSI, Previous, Previous_Wall, Previous_CPU);
         end select;
      end loop;
      Render_Dashboard (Owner, Use_ANSI, Previous, Previous_Wall, Previous_CPU);
   ...
   procedure Free_Dashboard is new Ada.Unchecked_Deallocation (Concrete_Dashboard, Concrete_Dashboard_Access);
   procedure Free_Task is new Ada.Unchecked_Deallocation (Dashboard_Task, Dashboard_Task_Access);
   ...
   begin
      ...
      Milliseconds :=
        Positive'Max (1, Positive (Long_Float'Rounding (Long_Float (Refresh_Interval) * 1_000.0)));
      Handle := new Concrete_Dashboard;
      Handle.Worker := new Dashboard_Task (Backend (Object), Milliseconds, ANSI);
