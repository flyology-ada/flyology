...
package body Flyology_Bench.Scaling is
      ...
      for Index in 1 .. Set.Count loop
         if Sweeps.Value (Set.Data (Index).Point) = Sweeps.Value (Point) then
            raise Constraint_Error with
              "duplicate empirical-scaling numeric input "
              & Sweeps.Identity (Point);
         end if;
      end loop;
   ...
   function Length (Set : Observation_Set) return Natural is (Set.Count);

   function Log_Basis
     (Model : Scaling_Model;
      Input : Long_Float;
      Valid : out Boolean) return Long_Float
   is
      Log_N : Long_Float;
   begin
      ...
      case Model is
         ...
         when Logarithmic_Model =>
            ...
            return
              (if Valid
               then Ada.Numerics.Long_Elementary_Functions.Log (Log_N)
               else 0.0);
         when Linear_Model =>
            return Log_N;
         when N_Log_N_Model =>
            ...
            return
              (if Valid
               then Log_N
                 + Ada.Numerics.Long_Elementary_Functions.Log (Log_N)
               else 0.0);
   ...
   function Nominal_Exponent (Model : Scaling_Model) return Long_Float is
     (case Model is
   ...
   begin
      ...
      for Model in Scaling_Model loop
         declare
            Diagnostic : Model_Diagnostic :=
              (Model => Model,
               Nominal_Exponent => Nominal_Exponent (Model),
               others => <>);
         ...
         begin
            for Index in 1 .. Set.Count loop
               declare
                  Basis_Valid : Boolean;
                  Log_F : constant Long_Float := Log_Basis
                    (Model,
                     Long_Float (Sweeps.Value (Set.Data (Index).Point)),
                     Basis_Valid);
                  Log_Y : constant Long_Float :=
                    Ada.Numerics.Long_Elementary_Functions.Log
                      (Set.Data (Index).Value);
            ...
            if Model_Valid then
               ...
               Diagnostic.Coefficient :=
                 Ada.Numerics.Long_Elementary_Functions.Exp (Intercept);
               if not Finite (Diagnostic.Coefficient)
                 or else Diagnostic.Coefficient <= 0.0
               then
                  Model_Valid := False;
               end if;
            end if;
      ...
      elsif Available_Models > 1
        and then Second_RMS - Best_RMS <= Identifiability_Margin
      then
   ...
   function Status
     (Result : Empirical_Scaling_Analysis) return Scaling_Status is
     (Result.State);
   function Available
     (Result : Empirical_Scaling_Analysis) return Boolean is
     (Result.State = Scaling_Available);
   function Selected_Model
     (Result : Empirical_Scaling_Analysis) return Scaling_Model is
     (Result.Selected);
   function Diagnostic
     (Result : Empirical_Scaling_Analysis;
