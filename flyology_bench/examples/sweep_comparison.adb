...
procedure Sweep_Comparison is
   ...
   begin
      ...
      for Index in 2 .. Current_Size loop
         ...
         Position := Index;
         while Position > 1
           and then Reference_Data (Position - 1) > Value
         loop
   ...
   begin
      ...
      while Gap > 0 loop
         for Index in Gap + 1 .. Current_Size loop
            ...
            Position := Index;
            while Position > Gap
              and then Contender_Data (Position - Gap) > Value
            loop
   ...
   function Work_For (Item : Sweeps.Parameter_Point) return Sweeps.Work_Amount is
     (Sweeps.Work
        (Sweeps.Value (Item),
         Sweeps.Items,
         Scaling => Sweeps.Decimal_Scaling));

   procedure Compare_Sorts is new Flyology_Bench.Compare
     (Reference_Operation => Insertion_Sort,
      Contender_Operation => Shell_Sort);

   procedure Run is new Sweeps.Compare_Sweep
     (Select_Point => Choose_Size,
      Work_For     => Work_For,
      Run_Point    => Compare_Sorts);
      ...
      begin
         if Sweeps.Status (Item) = Sweeps.Point_Measured then
            ...
            Shell_Observations.Append
              (Sweeps.Parameter (Item),
               Flyology_Bench.Median_Nanoseconds
                 (Flyology_Bench.Contender_Measurement (Pair)));
         end if;
      end;
   end loop;

   Reporters.Put_Scaling_Console
     ("sorting/in_place/insertion",
      Scaling.Analyze (Insertion_Observations));
   Reporters.Put_Scaling_Console
     ("sorting/in_place/shell",
      Scaling.Analyze (Shell_Observations));
