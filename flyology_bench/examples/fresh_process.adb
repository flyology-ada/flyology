...
begin
   ...
   begin
      ...
      Ada.Text_IO.Put_Line
        ("in-process median ns:" & Long_Float'Image (Flyology_Bench.Median_Nanoseconds (In_Process)));

      W.Run (Ada.Command_Line.Command_Name, "increment", W.Ordinary_Measurement, Config, Launch, Env, Fresh);
      for Result of Fresh loop
         if W.Outcome (Result) = W.Normal_Result then
            Ada.Text_IO.Put_Line
              ("fresh worker"
               & Positive'Image (W.Repetition (Result))
               & " seed"
               & Long_Long_Integer'Image (W.Seed (Result))
               & " median ns:"
               & Long_Float'Image (Flyology_Bench.Median_Nanoseconds (W.Measurement_Value (Result)))
               & " spawn ns:"
               & Long_Float'Image (W.Spawn_Nanoseconds (Result))
               & " setup ns:"
               & Long_Float'Image (W.Setup_Nanoseconds (Result)));
         else
            Ada.Text_IO.Put_Line ("fresh worker failed: " & W.Reason (Result));
