...
procedure Suite_Runner is
   ...
   procedure Compare_Mixes is new
     Flyology_Bench.Compare (Reference_Operation => Mix, Contender_Operation => Double_Mix);
   ...
   procedure Run_Mix (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Measurement) is
   begin
      Measure_Mix (Config, Result);
   end Run_Mix;

   procedure Run_Branch (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Measurement) is
   begin
   ...
   procedure Run_Shootout
     (Config : Flyology_Bench.Configuration; Result : out Flyology_Bench.Multi_Comparison) is
   begin
      Compare_All (Config, Result);
   end Run_Shootout;

   package Shootout_Registration is new
     Benchmarks.Multi_Way_Registration (Case_Id => Mix_Case, Run => Run_Shootout);
   ...
   Base    : constant Flyology_Bench.Configuration :=
     (Flyology_Bench.Default_Configuration
      with delta
        ...
        Random_Seed           => 42);
begin
   Benchmarks.Register (Target, "mix", Run_Mix'Access, Group => "integer", Tags => "arithmetic,smoke");
   Benchmarks.Register (Target, "branch", Run_Branch'Access, Group => "integer", Tags => "branch,smoke");
   Benchmarks.Register_Paired
     (Target,
      ...
      Group => "integer",
      Tags  => "arithmetic,paired");
   Shootout_Registration.Register (Target, "shootout", Group => "integer", Tags => "arithmetic,multi");

   declare
      Options : constant Benchmarks.Runner_Options := Benchmarks.Parse_Command_Line (Base);
   begin
...
exception
   when Error : Benchmarks.Option_Error =>
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error, "suite option error: " & Ada.Exceptions.Exception_Message (Error));
