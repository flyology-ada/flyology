...
procedure Baseline_Gate is
   ...
   Usage             : constant String := "usage: baseline_gate (record|check) BASELINE_PATH ENVIRONMENT_ID";
   Benchmark_Name    : constant String := "baseline_gate_example";
   Value             : Interfaces.Unsigned_64 := 1
   with Volatile;
   Inject_Regression : constant Boolean :=
     Ada.Environment_Variables.Value ("FLYOLOGY_BENCH_GATE_REGRESSION", Default => "0") = "1";
   ...
   CI_Gate_Policy : constant Baselines.Gate_Policy :=
     (Baselines.Fail_Closed_Gate_Policy with delta On_Inconclusive => Baselines.Report_Only);
   Result         : Flyology_Bench.Measurement;
begin
   if Ada.Command_Line.Argument_Count /= 3
     or else (Ada.Command_Line.Argument (1) /= "record" and then Ada.Command_Line.Argument (1) /= "check")
   ...
   begin
      ...
      if Ada.Command_Line.Argument (1) = "record" then
         Baselines.Save
           (Ada.Command_Line.Argument (2), Benchmark_Name, Result, Fingerprint => Full_Fingerprint);
         Ada.Text_IO.Put_Line ("recorded baseline " & Ada.Command_Line.Argument (2));
      else
         declare
