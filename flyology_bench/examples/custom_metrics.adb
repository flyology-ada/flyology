...
procedure Custom_Metrics is
   ...
   begin
      ...
      Snapshot (1) :=
        (Status => Flyology_Bench.Metric_Collected, Counter_Value => Requests, Sample_Value => 0.0);
   end Probe;
   ...
   procedure Measure_Requests is new Flyology_Bench.Measure_Batched (Request_Batch);
   ...
   package Fake_Timer is new
     Flyology_Bench.Manual_Timing
       (Source_Name => "example_simulated_clock",
        Unit        => "ticks/op",
        Resolution  => 1.0,
...
begin
   ...
   Flyology_Bench.Set_Custom_Probe (Config.Custom_Metrics, Probe'Unrestricted_Access);
   Measure_Requests (Config, Result);
   Flyology_Bench.Reporters.Put_Console ("domain_counter", Result, Style => Flyology_Bench.Reporters.Plain);
   Flyology_Bench.Reporters.Put_Extended_Metrics_CSV_Header;
   Flyology_Bench.Reporters.Put_Extended_Metrics_CSV ("domain_counter", Result);
   ...
   Flyology_Bench.Reporters.Put_Metrics_NDJSON ("manual_simulated_timer", Result);
end Custom_Metrics;
