...
package body System.Flyology.Scheduler is
   ...
   procedure Queue_Descriptor_Cancel_Locked (Group : not null Loop_Group_Access; Item : not null Fiber_Access)
   is
   begin
      ...
      while Group.Timer_Count > 0 loop
         ...
         case Scheduling.Classify_Deadline (Item.Deadline, Now) is
            ...
            when Scheduling.Expired         =>
               ...
               if Item.Descriptor_Cancel_Queued then
                  if Faults.Enabled and then Faults.Fail (Faults.Descriptor_Cancel_Timer_Pause) then
                     Faults.Pause_Descriptor_Cancel_Timer;
                  end if;
                  return 0.0;
               end if;
               Remove_Timer_Locked (Group, Item);
   ...
   begin
      ...
      loop
         ...
         Process_File_Cancellations_Locked (Group);
         Timeout := No_Deadline;
         if Faults.Enabled and then Faults.Fail (Faults.Timer_Maintenance_Due) then
            Dispatches_Until_Timer_Check := 0;
         end if;
