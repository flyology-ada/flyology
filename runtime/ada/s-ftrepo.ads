...
is
   ...
   type Event is (Complete_Normally, Complete_With_Exception, Complete_Abnormally);

   function Next (State : Phase; Occurrence : Event) return Phase
   with Pre => State = Running, Post => Next'Result = Terminal;
   ...
   type Request_Outcome is (Runtime_Failure, Malformed_Request, Unknown_Task, Not_Terminal, Terminal);
   ...
   with
     ...
     Post           => (Outcome_Code'Result in 0 .. 1) = (Outcome in Not_Terminal | Terminal);

   subtype Reference_Count is Interfaces.Unsigned_32;

   function Retain_Allowed (References : Reference_Count) return Boolean
   is (References in 1 .. Reference_Count'Last - 1);

   function After_Retain (References : Reference_Count) return Reference_Count
   with
     Pre  => Retain_Allowed (References),
     Post => After_Retain'Result = References + 1 and then After_Retain'Result > References;

   function Release_Allowed (References : Reference_Count) return Boolean
   is (References > 0);

   function After_Release (References : Reference_Count) return Reference_Count
   with
     Pre  => Release_Allowed (References),
     Post => After_Release'Result = References - 1 and then After_Release'Result < References;
