...
package body System.Flyology.Task_Results is
   ...
   package Subscription_Addresses is new System.Address_To_Access_Conversions (Subscription_Node);
   ...
   begin
      loop
         ...
         exit when Written >= 0 or else C.int (OSI.errno) /= C.int (OSI.EINTR);
      end loop;
   end Signal;
   ...
   protected type Completion_Gate is
      procedure Publish;
      procedure Subscribe
        (Node : System.Address; Node_Size : C.size_t; Signal_Descriptor : C.int; Result : out C.int);
      procedure Unsubscribe (Node : System.Address; Node_Size : C.size_t; Result : out C.int);
      entry Wait;
   private
   ...
   protected body Completion_Gate is
      ...
      procedure Subscribe
        (Node : System.Address; Node_Size : C.size_t; Signal_Descriptor : C.int; Result : out C.int)
      is
         Value : constant Subscription_Access := Subscription_Addresses.To_Pointer (Node);
      begin
         if Value = null
      ...
      procedure Unsubscribe (Node : System.Address; Node_Size : C.size_t; Result : out C.int) is
         Target   : constant Subscription_Access := Subscription_Addresses.To_Pointer (Node);
         Previous : Subscription_Access := null;
         Cursor   : Subscription_Access := Subscription_Addresses.To_Pointer (Subscribers);
      begin
         if Target = null
           or else Node_Size /= Subscription_Node'Size / 8
   ...
   type Owned_Result is limited record
      ...
      State      : aliased Atomics.uint32 := Atomics.uint32 (Policy.Phase'Pos (Policy.Running));
      Item       : Runtime_Result := Empty_Result;
   end record;

   package Owned_Addresses is new System.Address_To_Access_Conversions (Owned_Result);
   subtype Owned_Access is Owned_Addresses.Object_Pointer;
   use type Owned_Access;
   procedure Free_Owned is new Ada.Unchecked_Deallocation (Owned_Result, Owned_Access);

   package Result_Addresses is new System.Address_To_Access_Conversions (Runtime_Result);
   use type Result_Addresses.Object_Pointer;

   Result_Attribute_Index : constant Integer := Task_Attributes.Next_Index (Require_Finalization => False);
   ...
   procedure Release_Store
     (Target : System.Address; Value : Atomics.uint32; Model : Atomics.Mem_Model := Atomics.Release);
   pragma Import (C, Release_Store, "flyology_atomic_store_u32");

   function To_Task_Id is new Ada.Unchecked_Conversion (System.Address, System.Tasking.Task_Id);
   ...
   function Code (Outcome : Policy.Request_Outcome) return C.int
   is (C.int (Policy.Outcome_Code (Outcome)));
   ...
   function Observe_Owned (Value : Owned_Access; Item : System.Address; Item_Size : C.size_t) return C.int;

   function Wait_Owned (Value : Owned_Access; Timeout_Nanoseconds : C.long_long) return C.int;
   ...
   begin
      ...
      Expected := Atomics.Atomic_Load_32 (Value.References'Address, Atomics.Acquire);
      loop
         ...
         exit when
           Atomics.Atomic_Compare_Exchange_32
             (Value.References'Address,
              Expected'Address,
              Atomics.uint32 (Policy.After_Retain (Policy.Reference_Count (Expected))),
              Weak          => False,
   ...
   begin
      ...
      Expected := Atomics.Atomic_Load_32 (Value.References'Address, Atomics.Acquire);
      loop
         pragma Assert (Policy.Release_Allowed (Policy.Reference_Count (Expected)));
         Next := Atomics.uint32 (Policy.After_Release (Policy.Reference_Count (Expected)));
         exit when
           Atomics.Atomic_Compare_Exchange_32
             (Value.References'Address,
              Expected'Address,
   ...
   procedure Copy_Bounded
     (Source : String; Target : out C.char_array; Length : out C.unsigned; Truncated : out C.int);
   ...
   procedure Attach_Task_Result (T : System.Tasking.Task_Id; Storage : System.Address) is
   begin
   ...
   function Detach_Task_Result (T : System.Tasking.Task_Id) return System.Address is
   ...
   is
      ...
      Occurrence : constant Policy.Event :=
        (case Cause is
           ...
           when System.Tasking.Unhandled_Exception => Policy.Complete_With_Exception);
      Next_State : constant Policy.Phase := Policy.Next (Policy.Running, Occurrence);
   begin
      ...
      if Cause = System.Tasking.Unhandled_Exception then
         ...
         Copy_Bounded
           (Ada.Exceptions.Exception_Message (X), Item.Message, Item.Message_Length, Item.Message_Truncated);
      end if;

      Value.Item := Item;
      Release_Store (Value.State'Address, Atomics.uint32 (Policy.Phase'Pos (Next_State)));
      Value.Gate.Publish;
   end Publish;

   function Observe_Owned (Value : Owned_Access; Item : System.Address; Item_Size : C.size_t) return C.int is
      Target : constant Result_Addresses.Object_Pointer := Result_Addresses.To_Pointer (Item);
   ...
   function Observe_Task (T : System.Address; Item : System.Address; Item_Size : C.size_t) return C.int is
   ...
   function Wait_Owned (Value : Owned_Access; Timeout_Nanoseconds : C.long_long) return C.int is
      State : Atomics.uint32;
   begin
      if Value = null then
         return Code (Policy.Unknown_Task);
      ...
      if State = Atomics.uint32 (Policy.Phase'Pos (Policy.Terminal)) then
         ...
         or
            delay
              ...
              + Duration (Timeout_Nanoseconds rem C.long_long (1_000_000_000)) * 0.000_000_001;
            return Code (Policy.Not_Terminal);
         end select;
      end if;
   exception
   ...
   function Wait_Task (T : System.Address; Timeout_Nanoseconds : C.long_long) return C.int is
      ID : constant System.Tasking.Task_Id := To_Task_Id (T);
   begin
      if ID = null then
         return Code (Policy.Unknown_Task);
   ...
   function Observe_Monitor
     (Storage : System.Address; Item : System.Address; Item_Size : C.size_t) return C.int is
   begin
      return Observe_Owned (Owned_Addresses.To_Pointer (Storage), Item, Item_Size);
   exception
      when others =>
         return Code (Policy.Runtime_Failure);
   end Observe_Monitor;

   function Wait_Monitor (Storage : System.Address; Timeout_Nanoseconds : C.long_long) return C.int is
   begin
      return Wait_Owned (Owned_Addresses.To_Pointer (Storage), Timeout_Nanoseconds);
   exception
      when others =>
         return Code (Policy.Runtime_Failure);
   end Wait_Monitor;
   ...
   begin
      ...
      Value.Gate.Subscribe (Subscription_Node, Node_Size, Signal_Descriptor, Result);
      return Result;
   exception
      when others =>
         return Code (Policy.Runtime_Failure);
