...
is
   ...
   begin
      if Phase = Waiting_For_Main and then Kind = Main_Completion then
         return
           (Valid                      => True,
            Phase_After                => (if More then Waiting_For_Notification else Terminal),
            Store_Main_Result          => True,
            Emit_Terminal_Result       => not More,
            Release_Unused_Reservation => not More,
            Note_Copy_Fallback         => False);
      elsif Phase = Waiting_For_Notification and then Kind = Notification_Completion and then not More then
         return
           (Valid                      => True,
            ...
            Release_Unused_Reservation => False,
            Note_Copy_Fallback         => Usage_Reporting and then Kernel_Used_Copy);
      else
         return Empty;
      end if;
   end Plan_Completion;
