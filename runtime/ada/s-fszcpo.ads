...
is
   type Completion_Phase is
     (Waiting_For_Main, Waiting_For_Notification, Terminal);
   ...
   with Global => null,
        Post =>
          (if Phase = Waiting_For_Main and then Kind = Main_Completion then
              ...
              and then
                (if More then
                    Plan_Completion'Result.Phase_After =
                      Waiting_For_Notification
                    and then
                      not Plan_Completion'Result.Emit_Terminal_Result
                    and then
                      not Plan_Completion'Result.Release_Unused_Reservation
                 else
                    Plan_Completion'Result.Phase_After = Terminal
                    and then Plan_Completion'Result.Emit_Terminal_Result
                    and then
                      Plan_Completion'Result.Release_Unused_Reservation)
           ...
           then
              ...
              and then
                Plan_Completion'Result.Note_Copy_Fallback =
                  (Usage_Reporting and then Kernel_Used_Copy)
           else
              not Plan_Completion'Result.Valid);
