package body System.Gnatevl.ASan is
   ...
   is
   begin
      if Source_Context = System.Null_Address
        or else Switching_From /= System.Null_Address
      then
         raise Program_Error with "nested GNATEVL ASan fiber switch";
      end if;
      Switching_From := Source_Context;
      --  A fake-stack handle is intentionally not saved. ASan fake stacks
      --  belong to an OS thread, while GNATEVL permits a task context to move
      ...
      Sanitizer_Start_Switch_Fiber
        (System.Null_Address, Destination_Bottom, Destination_Size);
   end Start_Switch;
   ...
   begin
      if Switching_From = System.Null_Address then
         raise Program_Error with "GNATEVL ASan switch has no source context";
      end if;
      Sanitizer_Finish_Switch_Fiber
        (System.Null_Address, Bottom'Access, Size'Access);
