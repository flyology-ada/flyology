...
by that same task; the intended use is a task-local lexical scope as above.
Finalization by another task raises `Program_Error` in that task and leaves the
acquiring task pinned. A pin stabilizes the event-loop pthread; it does not give
the task exclusive use of that pthread, so unrelated fibers in the group still
share its pthread-local state. Use a dedicated group as well when a foreign
...
to its own pthread. `Is_Thread_Pinned` reports both explicit lightweight pins
and this inherent native binding.
