...
`Gnatevl.Cancellation.Token` is the canonical one-shot token shared by
...
file operation, and either legacy qualified exception name catches cancellation
...
| Darwin POSIX AIO submitted | Call `aio_cancel`; distinguish `AIO_CANCELED`, `AIO_NOTCANCELED`, and `AIO_ALLDONE` | The normal `EVFILT_AIO` terminal event, after `aio_return`/XNU event reaping |
