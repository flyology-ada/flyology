...
for hook in $hook_names; do
  combination_count=$((combination_count * 2))
done
...
check_project_selection () {
  ...
  for hook in $hook_names; do
    ...
    for member in $(hook_members "$hook"); do
      expected="/src/test_hooks/$member/$state"
      unexpected="/src/test_hooks/$member/$opposite"
...
compile_probe () {
  ...
  for hook in $hook_names; do
    state=$(hook_state "$combination" "$hook_index")
    for member in $(hook_members "$hook"); do
      set -- "$@" -I"$project_root/src/test_hooks/$member/$state"
    done
    hook_index=$((hook_index * 2))
  done
  ...
  for hook in $hook_names; do
    state=$(hook_state "$combination" "$hook_index")
    for member in $(hook_members "$hook"); do
      symbol=$(hook_symbol "$member")
      if [ "$state" = enabled ]; then
        if ! printf '%s\n' "$symbols" | grep -F "$symbol" >/dev/null; then
          printf '%s\n' \
            "enabled $member probe call disappeared in $mode combination $combination" >&2
          exit 1
        fi
      elif printf '%s\n' "$symbols" | grep -F "$symbol" >/dev/null; then
        printf '%s\n' \
          "disabled $member probe call survived in $mode combination $combination" >&2
        exit 1
      fi
    done
    hook_index=$((hook_index * 2))
...
printf 'test-hook-elision: all %s project selections and strict -O0 probe combinations\n' \
  "$combination_count"
combination=0
while [ "$combination" -lt "$combination_count" ]; do
...
if ! ar -t "$runtime_archive" | grep -Fx s-flypol.o >/dev/null; then
  printf '%s\n' "production runtime archive omitted s-flypol.o: $runtime_archive" >&2
  exit 1
fi
for artifact in \
  "$project_root/build/rts/obj/s-flypol.o" \
