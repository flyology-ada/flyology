...
expect_counterexample \
  SupervisionLifecycle SupervisionLifecycle_overlap_escalation.cfg \
  IndependentFailurePreservesPolicy supervision-overlap-escalation
expect_counterexample \
...
if ! grep -Fq 'All 106 obligations proved' "$poller_proof_log"; then
   cat "$poller_proof_log" >&2
   printf '%s\n' 'poller-registration proof did not discharge all 106 obligations' >&2
   exit 1
fi
printf '%s\n' 'TLAPS proved       PollerRegistrationOwnershipProof 106 obligations'
