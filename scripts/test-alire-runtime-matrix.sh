#!/bin/sh
set -eu

project_root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)

for toolchain in \
  gnat_native:13.2.2:25.0.1 \
  gnat_native:14.1.3:25.0.1 \
  gnat_native:14.2.1:25.0.1 \
  gnat_native:15.1.2:25.0.1 \
  gnat_native:15.3.1:25.0.1 \
  gnat_native:16.1.0:26.0.1 \
  gnat_flyology_native:16.2.0-patchset.1.1.0:26.0.1
do
  gnat_provider=${toolchain%%:*}
  versions=${toolchain#*:}
  gnat_version=${versions%%:*}
  gprbuild_version=${versions#*:}
  printf '\nTesting Linux %s %s with GPRbuild %s\n' \
    "$gnat_provider" "$gnat_version" "$gprbuild_version"
  FLYOLOGY_LINUX_IMAGE="flyology-linux-test-$gnat_version" \
  FLYOLOGY_GNAT_PROVIDER="$gnat_provider" \
  FLYOLOGY_GNAT_VERSION="$gnat_version" \
  FLYOLOGY_GPRBUILD_VERSION="$gprbuild_version" \
    "$project_root/scripts/test-linux-docker.sh"
done
